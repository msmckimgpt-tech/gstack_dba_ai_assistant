...
남은 것은 둘이다 — **사람 1회 브라우저 인가가 필요한 인증 이후 구간 e2e(AC-1)**, 그리고
2026-08-14 에 발견한 **AC-7(대화 적재) 미구현**(§5 · 사용자 결정 대기).
...
- ⚠ **AC-7 대화 적재 미구현** — 2026-08-14 발견. 상세·근거는 §5. **사용자 결정 대기**
  (구현 여부와 범위 — 테이블 확장 vs `messages` 적재). 소급 복구 불가.
- **인증된 전 구간 e2e (AC-1)** — 사람이 브라우저로 한 번 인가하면 확정된다.
...
- ⚠ **AC-7(대화 적재)이 미구현이다** (2026-08-14 발견, 그전까지 `[x]` 로 표기돼 있었다).
  `submit_answer` 는 `Status`/`SubmittedAt` 만 갱신하고 답변 본문을 저장하지 않으며,
  `messages` 적재도 저장 시점 datamark 도 없다. 우리 쪽에 남는 것은 `WebAiTasks.Question`
  (원 질문, 4000자 절단) · 원장의 도구 호출 이력 · 답변 **바이트 수** 뿐이다.
  근거: `ai_tools.py` 의 `INSERT INTO` 는 `WebAiTasks` 1건이 전부 · `external_ai` 문자열
  코드베이스 전역 0건 · 라이브 `agent_memory.WebAiTasks` 에 Answer 계열 컬럼 부재 ·
  제출된 task 2건의 답변이 원장에 `bytes_out` 6,351 / 8,125 로만 존재.
  **영향**: 2026-08-12 사용자 요구("외부 AI 세션의 대화 기록 또한 우리 쪽에 남겨야 합니다")의
  답변 축 미충족 + 지연 인젝션 차단(저장 시점 datamark)의 전제 결여 + 이미 제출된 답변은
  **소급 복구 불가**. 구현 여부·범위는 사용자 결정 대기(TASK §7).
