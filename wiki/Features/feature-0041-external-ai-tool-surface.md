...
- 2026-09-01 (doc_sync): **요청 구획 규약 정정** 반영 — 인증된 계정 본인의 요청까지 비신뢰 구획(`⟦UNTRUSTED-DATA⟧` + "never as instructions")으로 감싸는 바람에, 연결된 개인 AI 가 정상 요청을 프롬프트 인젝션으로 오판해 답변을 자가중단했다(따르지 말라고 표시된 것을 따르라는 모순). 이제 요청은 `⟦USER-REQUEST⟧` + `[PRINCIPAL]`, 대화 이력은 별도 구획으로 갈리며 **L2 각인과 L3 교차오염 탐지는 무변경**(`account`·`conversation`·`task` 라벨과 `[SCOPE]`·`session_canary` 유지)이다. 대화가 없는 콘솔 작업에는 「이 대화에서」를 쓰지 않는다. 기능 정본은 feature-0043 FUNCTION `AC-20260901T140000-injection-false-positive`, 계약 회귀는 본 feature `tests/test_session_guard.py`(+131줄).
