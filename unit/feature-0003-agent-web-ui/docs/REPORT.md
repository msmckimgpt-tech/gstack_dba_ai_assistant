...
## TASK-20260910T064200-folder-newconv — 폴더 행 '···' → '📝'(이 폴더에서 새 대화)

사용자 요청(2026-09-10): 좌측 대화목록에서 폴더 요소의 '...' 버튼이 옵션을 펼치는 대신 **그 폴더를
대상으로 새 대화를 시작**하게 대체. 사용자 결정 2건(AskUserQuestion): ①폴더 관리 메뉴는 제거가 아니라
**헤더 우클릭으로 이관** ②버튼 표시는 **노트·펜 이모지 `📝`**.

상태: 구현·검증 완료, 배포 및 라이브 시각검증은 후속(같은 cycle). 백엔드·스키마·권한·엔드포인트 변경 0 —
기존 `PATCH /api/conversations/{cid}/folder` 를 재사용한다. 상세는 feature-0024 `TASK/MODIFY`,
검증은 `docs/test-runs.d/TASK-20260910T064200-folder-newconv.md`, 패널은
`REV-20260910T064200-folder-newconv`.

### §8 개선 제안 (기록만 — 본 변경 범위 밖의 선행 결함)

검증 패널(ux·design)이 이번 diff 를 보다가 **그 전부터 있던** 결함 4건을 짚었다. 본 cycle 에서 고치지
않았고, 고치려면 각각 별도 범위가 필요하다.

1. **`openFloatingMenu` 의 키보드 메뉴 내비게이션 부재** — 메뉴를 열어도 포커스가 첫 항목으로 가지
   않고(트리거에 남는다), roving tabindex·Tab 트랩이 없어 항목이 사이드바·본문·컴포저를 다 지난 뒤에야
   탭 순서에 온다. 게다가 capture `scroll` 리스너가 탭 이동 중 발생한 스크롤로 메뉴를 닫는다. `role="menu"`
   를 이미 선언하고 있으므로 APG 계약(첫 항목 포커스 · ↑↓/Home/End · Esc 복귀)을 채우는 것이 맞다.
   종전 '···' 경로도 같았던 선행 결함이지만, 폴더 메뉴가 우클릭 전용이 되면서 우선순위가 올랐다.
2. **`--text-1` 토큰이 정의 없이 8곳에서 사용된다** — 정의 0건. `color: var(--text-1)` 은
   invalid-at-computed-value-time 으로 `inherit` 에 떨어져 «우연히» 의도한 색을 얻고 있었다.
   `base.css :root` 에 `--text-1: #26251e;` 를 추가하면 8곳이 한 번에 의도대로 돌아온다.
3. **`forced-colors: active`(Windows 고대비) 규칙이 CSS 전체에 0건** — 이모지·비트맵 아이콘은
   시스템 색 강제 대상이 아니라 고대비 바탕 위에 원래 색으로 남는다.
4. **사이드바 이모지 프레젠테이션 혼재** — `🗂`(U+1F5C2, VS16 없음)은 유니코드 기본이 텍스트(흑백)
   프레젠테이션이고 `📝`(U+1F4DD)는 항상 컬러라, Windows 기본 폰트 조합에서 한 행에 «흑백 폴더 +
   컬러 메모» 가 섞일 수 있다. macOS/리눅스에서는 드러나지 않아 검수 환경에 따라 보이지 않는다.
   `🗂️`(VS16)로 통일하거나 사이드바 아이콘 어휘를 라인아트 SVG 로 일원화하는 축.
