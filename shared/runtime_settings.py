...
_TIMEOUT_SPECS: tuple[dict[str, Any], ...] = (
    # ── 쿼리·에이전트 실행 ──
    {
        ...
        "category": "쿼리·에이전트 실행",
        "label": "무응답 대기 상한 (에이전트/쿼리)",
        "description": "이 값은 AI 추론 시간을 제한하지 않습니다. 스트리밍 응답에서는 \"응답이 한 조각도 오지 않는 구간\"의 상한으로만 동작하므로, 추론이 몇 시간이 걸려도 응답이 흐르는 동안에는 중단되지 않습니다. 즉 실질적으로 \"연결이 끊겼거나 상대가 멈춘 것\"을 판정하는 값입니다. 쿼리 실행·저수준 DB 연결 설정에도 재사용되며(그 경로는 재배포 시 반영), LLM 요청 경로는 즉시 반영됩니다. 올리면 장애 감지가 그만큼 늦어지고, 내리면 정상 대기가 실패로 잘릴 수 있습니다.",
        "unit": "초",
        "default": 60,
        "minimum": 5,
        "maximum": 3600,
...
_SCRATCH_SPECS: tuple[dict[str, Any], ...] = (
    ...
    {
        "key": "AGENT_SCRATCH_FORK_CARRYOVER",
        "category": "작업공간(스크래치)",
        "label": "대화 분기 시 작업공간 이월",
        "description": "대화를 분기(사본 만들기·여기까지 분기·공유 링크 복제)할 때 원본 대화의 작업공간 테이블을 분기본으로 복사합니다 (1=이월, 0=빈 작업공간으로 시작). 이월하지 않으면 assistant 가 복사된 대화 문맥에 남은 테이블 기록을 참조하려다 실패해 답변 품질이 떨어집니다. 복사본은 원본과 독립이며 이후 원본 변경·삭제의 영향을 받지 않습니다.",
        "unit": "0/1",
        "default": 1,
        "minimum": 0,
        "maximum": 1,
    ...
    {
        "key": "AGENT_SCRATCH_MAX_CLONE_ROWS",
        "category": "작업공간(스크래치)",
        "label": "분기 이월 최대 행수",
        "description": "대화 분기 1회에 이월할 수 있는 작업공간 총 행수 상한(디스크 폭주 방지). 초과분은 이월되지 않으며 그 사실이 분기 결과에 표시됩니다.",
        "unit": "행",
        "default": 200000,
        "minimum": 0,
        "maximum": 5000000,
    ...
    {
        "key": "AGENT_SCRATCH_FORK_BUDGET_MS",
        "category": "작업공간(스크래치)",
        "label": "분기 이월 시간 예산",
        "description": "대화 분기 시 작업공간 이월에 쓸 수 있는 전체 시간 상한(밀리초). 분기 응답이 지연되지 않도록 이 예산을 넘으면 남은 테이블은 이월하지 않습니다.",
        "unit": "ms",
        "default": 10000,
        "minimum": 1000,
        "maximum": 120000,
...
_PERF_SPECS: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "category": "자원 격리·관측",
        "label": "중요 테이블 자동 분석",
        "description": "아직 분석되지 않은 테이블 중 **중요한 것부터** 자동으로 분석 대기열에 넣습니다. 중요도는 다른 테이블과 얼마나 연결돼 있는지, 실제 대화에서 조인해 쓴 적이 있는지로 정합니다. 지금까지는 사람이 그래프에서 클릭한 것만 분석돼 커버리지가 한쪽으로 치우쳐 있었습니다. 0 으로 내리면 자동 시드가 멈춥니다.",
        "unit": "",
        "default": 1,
        "minimum": 0,
        "maximum": 1,
    ...
    {
        "key": "AGENT_ANALYSIS_COVERAGE_SEED_CAP",
        "category": "자원 격리·관측",
        "label": "자동 분석 시드 수(사이클당)",
        "description": "한 점검 사이클에 자동으로 대기열에 넣을 테이블 수입니다. 작게 두면 비용이 천천히 오르고, 올리면 커버리지가 빨리 채워집니다. 시드된 테이블은 기존 분석 경로를 그대로 타므로 이후 처리량·비용 상한이 함께 적용됩니다.",
        "unit": "개",
        "default": 3,
        "minimum": 0,
        "maximum": 50,
    ...
    {
        "key": "AGENT_ANALYSIS_COVERAGE_CYCLE_CAP",
        "category": "자원 격리·관측",
        "label": "자동 분석 시드 수(점검 1회 전체)",
        "description": "한 번의 점검 사이클 전체에서 자동으로 대기열에 넣을 테이블 수의 상한입니다. 위의 '사이클당' 값은 DB 하나에 적용되므로, DB가 많으면 그만큼 곱해집니다 — 이 값이 그 총량을 묶습니다. 0 으로 두면 총량 제한 없음(DB별 상한만 적용).",
        "unit": "개",
        "default": 9,
        "minimum": 0,
        "maximum": 200,
