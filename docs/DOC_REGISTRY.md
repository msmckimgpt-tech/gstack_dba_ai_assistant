...
- **stale leftover**(2026-09-03 재실측): ① 등록 worktree 중 `ahead == 0` 방치분 **7건** — `ai/claude/feature-0007-llm-timeout-align`(1352 behind) · `ai/claude/feature-0041-external-ai-tool-surface`(566 behind) · `ai/claude/feature-0006-winbrowser-tab-survival`(394 behind) · `ai/claude/feature-0043-tool-permission-friction`(362 behind) · `ai/claude/feature-0043-ai-assisted-setup`(356 behind) · `ai/claude/feature-0043-caps-trust-gate`(120 behind) · `ai/claude/ai-jobs-auto-run-delegation`(11 behind, 디렉토리는 `.worktrees/ai-jobs-rewire-external`). 뒤 2건은 **2026-09-02 창의 PR 착륙(#1539 등)으로 새로 ahead 0 이 된** 방치분이고, 직전 재실측이 지목한 `ai/claude/conv-status-dot-wiring` 은 정리 완료(등록·디스크 모두 부재) → 나머지는 `bin/cycle-finalize.sh` 또는 `git worktree remove` 대상(본 run 의 `ai/claude/doc-sync-20260903-010309` 는 실행 중이라 제외). ② worktree 등록 없이 `.worktrees/` 에 디렉토리만 남은 잔재 **8건**(`base-check` · `datasource-accessible-db` · `feature-0003-attach-version-diff` · `feature-0003-attach-diff-colgroup` · `feature-0016-cluster-label-target` · `feature-0043-ai-jobs-perm-gate`(신규) · `task0215-insight-toggle` · `task0216-rag-ds-aware`) → `git worktree prune` + 디렉토리 삭제 대상. 어느 쪽도 흡수 대상 아님.
