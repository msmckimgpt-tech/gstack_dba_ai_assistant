...
## 3. Recent Changes
- (최신) gc-unread-read-idspace-fix: 읽음 커서·unread 집계는 `core_messages.id` 공간인데 FE 가 보내던 `last_read_message_id` 는 `/api/history` 가 채운 표시 store `messages.id` 공간이라 두 id 가 disjoint(예: 750~845 vs 3369~3655) → `set_last_read` GREATEST 가 전진 영구 거부(read 200·updated=1 이나 커서 불변). 핸들러가 requested 를 무시하고 항상 `MAX(core_messages.id)` 로 전진하도록 수정 + id-space 계약 회귀 테스트. 4-dim 조사 워크플로 + §18.8 패널로 근본원인 확정. unread 미감소·전환 시 회귀의 최종 근본원인. (CHG-20260625T225851)
