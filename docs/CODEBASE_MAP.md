...
├── shared/                 # 공통 모듈 패키지 (feature-0002·0003 공유, `ls shared/*.py` 실측 19): __init__·model_catalog·config·db·conn_health·datasources·runtime_settings·perf_counters·hangul_qwerty·llm_budget·llm_gate·resource_budget·share_window·bridge_tasks·attachment_write·self_review·bridge_consent·bridge_caps·dqa_identity (뒤 4종 2026-09-01~09-03 신설) (feature-0011 추출 + 0018/0026 외 후속 feature 편입)
...
| `shared/dqa_identity.py` | 개인 머신 클라이언트 **명칭 단일 정본** — `APP_NAME`(사람이 읽는 제품명) · `SCHEME`(URL 스킴 = 머신 전역 네임스페이스 = 레지스트리 키) · `APP_ID`(역-DNS) 3축 + `MCP_SERVER_KEY` + 딥링크 조립 `scheme_url()`. 문자열만 갖고 import 부작용이 없다(러너·설치 스크립트는 stdlib-only 계약이라 import 대신 `feature-0043/tests/test_name_ssot.py` 가 대조). 옛 이름 `mysql-ai-bridge` 는 `LEGACY_*` 로 정리 대상만 고정 (feature-0043, 2026-09-03) | feature-0003 (`routers/oauth_as.py`·`routers/ai_tools.py`)·feature-0046 클라이언트·러너/설치 스크립트(대조) |
...
| `feature-0046-native-client` | **Windows 네이티브 연결 클라이언트** — 「AI 연결」의 터미널 의존 제거(GUI 껍데기 + 설치 마법사) | `src/dqa_connect.py`(진입점 — PyInstaller 가 최상위 스크립트로 돌리므로 **패키지 밖·절대 임포트**, `client/__main__.py` 는 `python -m client` 전용) · `src/client/core.py`(감지·로그인 대행·무결성 대조·러너 기동, GUI 무의존) · `src/client/gui.py`(tkinter 껍데기 + `tell()` 안내) · `src/scripts/build_client.py`(PyInstaller onedir/windowed, 비-Windows 기본 차단) · `src/installer/DQAConnect.iss`(Inno Setup per-user + 스킴 HKCU 등록/제거) + 의존 정본: feature-0043 러너·`shared/dqa_identity.py`, feature-0003 웹 연결 퍼널 |
| `feature-0046-native-client` | **Windows 네이티브 클라이언트** — 터미널 없이 자기 AI 를 연결(설치 마법사 + 런타임 동봉 · 로그인 대행) | `src/dqa_connect.py`(진입점 — 패키지 밖·절대 임포트: PyInstaller 가 진입 파일을 `__package__` 없는 최상위 스크립트로 돌린다) · `src/client/core.py`(감지·로그인 대행·무결성 대조·러너 기동, GUI 무의존) · `src/client/gui.py`(tkinter 껍데기 — `print()` 금지, `tell()` messagebox; [X]=트레이로 숨김·연결 단일 실행 게이트·선택은 GUI 스레드에서 확정) · `src/client/tray.py`(**알림 영역 상주** — stdlib `ctypes`+`Shell_NotifyIconW`, 순수 로직 `Tray` / Win32 `Win32Backend` 2층. 서드파티 0, 아이콘은 `ExtractIconW` 로 exe 내부 것 사용. 실 Windows 검증은 `tests/windows/`) · `src/scripts/build_client.py`(PyInstaller `--onedir --windowed` + 임베더블 CPython 동봉 + `verify_runtime_runs_runner()`) · `src/installer/DQAConnect.iss`(Inno Setup per-user · 시작메뉴 · 제거 · `dqa-connect://` 스킴 등록) + 명칭 정본 `shared/dqa_identity.py` |
