#!/usr/bin/env python3
# `_template:daily-report` persona 사전 집계기 (deterministic pre-aggregation).
#
...
# 특정 프로젝트 경로를 하드코딩하지 않고 항상 `--repo` (또는 cwd 의 git toplevel) 로
# 대상 저장소를 받는다.
#
...
#   python3 .claude/commands/_template/daily-report-collect.py [YYYY-MM-DD] --repo "$(git rev-parse --show-toplevel)"
#   python3 daily-report-collect.py 2026-06-09
#   python3 daily-report-collect.py --json [YYYY-MM-DD]      # 기계용 JSON
#   python3 daily-report-collect.py --repo /path/to/repo 2026-06-09
#
...
def render_digest(p: dict) -> str:
    ...
    L.append("4. 같은 시간대 안에 유사/같은 scope TASK 가 2건 이상이면 §4.3 으로 카테고리 하위헤더 묶기(시간대 넘나들며 묶지 말 것, 1건뿐이면 헤더 없이 단독 유지).")
    L.append("5. §6 형식으로 출력. 총계 한 줄 + git 로그 재구성 각주. 19:00 이후 이월분 있으면 각주 명시.")
    return "\n".join(L)
