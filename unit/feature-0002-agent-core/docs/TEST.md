...
- **뮤테이션 19/19 KILLED**(초판 12 + 임시파일 가드 2 + 패널 흡수 5). 초판 테스트 2건은 판별력이 부족해 강화했다 — 임시파일 검사가 예외 핸들러 호출로 대신 통과했고(범위를 `except` 앞까지로 좁힘), 그 상태에서 '정상 fall-through 누수' 뮤턴트가 살아남았다: 재시도 상한 0 · 전송층 패턴 제거 · `_TAG_PAT` 오염(배너 켜기) · 히스토리 `ORDER BY id ASC LIMIT` 복귀 · compose grace 120s 복귀 · permanent 집합을 노드분석 수준으로 완화 · headroom 게이트 제거 · SDK 재시도 knob 복귀 · finalize 확인 제거 · backoff 후 취소 확인 제거 · slow-failure 게이트 제거 · 게이트웨이 타임아웃 어휘 제거. **초판은 m9(finalize 확인 1곳 삭제)가 생존** — 확인 지점 개수·순서를 세는 테스트로 교체해 KILLED.
...
### 20260812T180000-llm-transient-resume 일시 장애 작업 보존 + 추론 재개 (Major §12.3, 2026-08-12, primary feature-0002) — **Environment: unit + 라이브 배포본 실측(진단) (백엔드 전용, 웹/UI 자산 변경 0 → CHECK#13 미해당)**
- **자동 검증 — PASS**: `tests/test_llm_transient_resume.py` **30 passed**(§18.8 흡수 후) + `tests/test_llm_transient_retry.py` 계약 1건 갱신(총 두 파일 **64 passed / 1 skipped**). 축: ①값싼/비싼 예산 분리 8(값싼 상한 확대·**총 대기 76.5초 > 실측 공백 48초**·예산 유계·backoff cap 비교·비싼 예산 유지·`_slow` 오분류 차단·permanent 불재시도) ②재큐 SQL 계약 5(True/False 반환·pending·claim흔적·`finished_at` 리셋·`lease_epoch++`·**self-lease**·**attempts<cap**·`resume_hint` payload **병합**·attempts 미증가) ③워커 배선 4(**재큐가 KV terminal 앞**·`_deferred_terminal` pop·재큐 후 `return`·`resume_allowed` attempts 게이팅) ④run 계약 5(재개 경로 core 오류 미기록·KV 미기록·화면 안내 유지·**역검증** 정상 오류 경로는 종전대로 기록+terminal·`resume_hint` 지시 내용·히스토리/사용자 turn **뒤** 배치·transient 한정 resumable·누적 대기 계상) ⑤출하 상수 계약 1.
- **뮤테이션 12/12 KILLED**: 값싼 상한 6→2(사고 당시 값) · backoff cap 30→8 · 누적 대기 게이트 제거 · `_slow`→cheap 오분류 · 재큐 `attempts<cap` 제거 · self-lease 가드 제거 · payload 병합→덮어쓰기 · **재큐/terminal 순서 역전** · 재개 경로가 core 오류 기록 · `resume_hint` 블록 제거 · permanent 까지 resumable · `AGENT_ASK_RESUME_ON_TRANSIENT` 기본 false.
- **선행 cycle 계약 테스트 갱신(중요)**: `test_connection_failure_retries_up_to_cap_then_stops` 는 "연결 실패도 2회 상한" 을 고정하고 있었다 — **그 계약이 곧 이 사고**다. `…_up_to_cheap_cap_…` 으로 갱신해 옛 계약으로의 회귀를 차단했다(테스트가 버그를 지키고 있던 사례).
- **회귀**: 전 testpaths 실패 **선재 1건뿐**(`test_oauth_exhaustion_gate::test_write_failure_…` = 컨테이너 `chattr` 부재, pristine main 대조 동일). ruff clean · `py_compile` 4모듈 OK.
