...
## REV-20260710T013315-ai-claude-feature-0016-graph-edge-midpan [SUBAGENT: PASS] — §62 관계선 위 중간버튼 카메라 팬 무반응 수정 적대 리뷰
- 대상: CHG-20260710T013315-graph-edge-midpan (admin.js `_metaGraphEnsure` baseCfg 에 `edge: { style: { draggable: true } }` 1줄 + admin.html 캐시버스터).
- 방법: §18.8 적대 리뷰(general-purpose, 통과 아닌 결함 적발 목적). 근본원인 6-링크를 vendor `g6.min.js` 바이트오프셋으로 확증 + 회귀 시나리오(엣지 클릭/우클릭 메뉴/hover·drag-element 엣지 픽업·config 병합 clobber·엣지 생성 경로 커버리지·G6 shape 적용 위치·perf) 능동 탐색.
- 판정: **PASS** — 6-링크 전부 CONFIRMED. (1) drag-canvas 는 global `dragstart`(`Rv.DRAG_START`)에서 `enable`=`_metaCanvasDragEnable` 검증 (class `lE`). (2) dragndrop 은 pointerdown `closest("[draggable=true]")` 로 소스 해소, null 이면 무이벤트(@1296566, main 캔버스 `isDocumentDraggable:true`·threshold 10px/100ms). (3) 번들 전체 `draggable:!0` 2곳뿐=노드`Nw`·콤보`Wb`, 엣지 base 부재. (4) `getElementComputedStyle` 병합 `Object.assign({},theme,palette,datum.style,defaultStyle,themeState,state)` — `options.edge.style` 가 datum 뒤라 항상 반영(@1327030), `lw` 는 정적 boolean 그대로. (5) drag-element `uE.enableElements=["node","combo"]` 하드코딩 → `edge:dragstart` 미바인딩 → 엣지 무이동. (6) `draggable` 은 root-container 프롭(@1095154)→엣지 루트 그룹 적용→`closest` 발견.
...
## REV-20260710T233000-ai-claude-feature-0016-minimap-reuse [SUBAGENT: PASS-WITH-FIXES] — §70 미니맵 전체-이미지 재사용 diff 적대 리뷰
- 대상: CHG-20260710T230000-minimap-reuse (admin.js `_metaMinimapGeomSig`·`_metaPatchMinimapReuse` + `_metaG6ApplyOnce` 서명 배선 + minimap `key:"minimap"` + admin.html 버스터 + 신규 test_g6build_minimap_reuse.js).
- 방법: §18.8 적대 리뷰(general-purpose). 초기 2렌즈(정확성/geometry-sig 완전성 · UX/시각/perf)는 세션 중 user-interrupt 로 중단됐으나 렌즈1 이 **linchpin 결함(H1) 을 번들 역공학으로 적발** 후 중단. 수정본에 대해 집중 확인 리뷰(H1~H5, 도구 5회, 번들 사실 주입) 재수행 → **H2 신규 BLOCK 추가 적발**. 두 결함 모두 수정·테스트 커버 후 잔여 가설 클리어.
- **BLOCK 2건 적발 → 전부 수정**:
  - **H1 (patch no-op, 렌즈1)**: G6 v5 Graph 는 `context.plugin` 을 생성자가 아니라 첫 `draw()` 의 `prepare()→initRuntime()`(`this.context.plugin||(this.context.plugin=new dJ(...))`) 에서 lazy 생성한다. 최초 구현이 `_metaGraph.graph=graph` **직후**(init) `_metaPatchMinimapReuse` 를 호출 → `getPluginInstance("minimap")` 가 `context.plugin` 부재로 throw(→ catch → mm=null → no-op) → **패치가 영영 안 걸려 최적화 사멸**(dead). **수정**: `_metaG6ApplyOnce` 의 `await g.draw()` **직후** 멱등 호출로 이동(첫 draw 후 plugin 존재).
  - **H2 (네이티브 드래그 stale, 확인 리뷰)**: 노드/콤보 드래그는 `_metaG6Apply`(setData+draw)를 안 거치고 G6 가 `translateElementTo`→`element.draw({stage:"translate"})`(콤보는 native drag-element)로 요소를 직접 이동한다. 이 draw 도 `AFTER_DRAW`(payload `{…,stage:"translate",render:false}`, 번들 emit 실측)를 발생시켜 minimap onRender→renderMinimap 을 부르는데, `_miniGeomSig` 는 마지막 build 기준(**stale**) === `__lastGeomSig` → 게이트가 옛 배치로 **skip** → 미니맵이 드래그를 반영 못 하고 얼어붙는다(원본 minimap 은 드래그마다 재복제했으므로 **회귀** + 문서화한 "드래그→서명 변경" 불변식 위반). **수정**: `graph.on("afterdraw")` 리스너가 `e.data.stage==="translate"` 시 `_miniGeomSig=null` 무효화 → 다음 renderMinimap 재복제 폴백. apply-driven data draw(`graph.draw()`→`element.draw()`, stage 미지정)는 서명 유지(게이트 정상). node·combo·기타 translate 단일 지점 커버. 신규 D 섹션 10 테스트로 lock.
- **클리어(수정 후 재검)**: H1 타이밍(패치는 draw 직후 동기 설치→debounce onRender 발화 전 발효; `mm.canvas` null·`__lastGeomSig` undefined 로 첫 렌더 항상 보장·blank 없음). H3 서명 필드 정합(build 노드 `{id,combo,style:{x,y,size}}`·rect [w,h]/circle 스칼라 분기·엣지 source/target·combo.id — 오필드 "항상 동일 해시" 없음). H4 멱등(`__reusePatched`)·안전(getPluginInstance throw/null·비함수 no-op). H5 renderMask(뷰포트 사각형)는 renderMinimap 다음 별도 호출이라 skip 시에도 갱신.
- 트레이드오프(수용): 역할 칩 색·선택 하이라이트·dim(시각상태)은 미니맵에 즉시 반영 안 되고 다음 기하변경 때 반영 — 168×112px 스케일서 무의미, 사용자 요구("구성 불변 시 전체-이미지 재사용")와 정합.
- 검증: 수정 후 신규 `test_g6build_minimap_reuse.js` **35 PASS**(A11·B4·C10·D10) + 회귀 **150 PASS**(collod 20·agglod 8·category 26·edge 71·viewport-cull 6·vpack 19) · `node --check` PASS. POST-DEPLOY PB-0008 육안(feature-0003 TEST §70 — 드래그 반영 실증 포함).
