...
- **정책**: 미머지 worktree 의 작업 현황 정본 = 그 worktree 의 `unit/<feature>/docs/{TASK,REPORT}.md`.
  STATUS.md 인덱스는 **main 머지 기준** 상태를 반영하고, 미머지 활성 worktree 는 STATUS 표의 행으로 승격하지 않는다.
  머지 시 STATUS 행이 갱신된다. STATUS 인덱스는 셀 누적이 없어 worktree 머지 시 conflict 표면이 작다(인덱스화의 부수 효과).
- **현 상태 확인 경로**(2026-09-08 정리 이후): 활성 worktree 목록·ahead 수치는 문서에 적재하지 않는다 — `docs/STATUS.md` §1 의 「Worktree 확인」이 저장소 루트에서 `git worktree list --porcelain` + `git log <base>..<branch>` 로 매 작업 시작 시 실측하도록 규정하며, 2026-09-04 관측 스냅샷은 [정리 전 STATUS](./archive/delegation-friction-20260908/STATUS-before.md)에 보존됐다(SSOT 계약 2조 — 재서술 금지). 각 worktree 의 작업 현황 정본은 그 worktree 의 `unit/<feature>/docs/{TASK,REPORT}.md` 이며, 머지 시 STATUS 행이 갱신된다.
- **stale leftover**(2026-09-04 스냅샷 — 2026-09-08 정리 이후의 «현재 활성 목록» 이 아니다. 현행 판정은 위 「현 상태 확인 경로」의 실측으로 한다): ① 등록 worktree 중 `ahead == 0` 방치분 **7건** — `ai/claude/feature-0007-llm-timeout-align`(1416 behind) · `ai/claude/feature-0041-external-ai-tool-surface`(630 behind) · `ai/claude/feature-0006-winbrowser-tab-survival`(458 behind) · `ai/claude/feature-0043-tool-permission-friction`(426 behind) · `ai/claude/feature-0043-ai-assisted-setup`(420 behind) · `ai/claude/feature-0043-caps-trust-gate`(184 behind) · `ai/claude/ai-jobs-auto-run-delegation`(75 behind, 디렉토리는 `.worktrees/ai-jobs-rewire-external`). 뒤 2건은 **2026-09-02 창의 PR 착륙(#1539 등)으로 새로 ahead 0 이 된** 방치분이고, 2026-09-02 재실측이 지목한 `ai/claude/conv-status-dot-wiring` 은 정리 완료(등록·디스크 모두 부재)이고 `ai/claude/doc-sync-20260903-010309` 도 등록·디스크 모두 부재로 정리 완료 → 나머지는 `bin/cycle-finalize.sh` 또는 `git worktree remove` 대상(본 run 의 `ai/claude/doc-sync-20260904-010301` 는 실행 중이라 제외). ② worktree 등록 없이 `.worktrees/` 에 디렉토리만 남은 잔재 **8건**(`base-check` · `datasource-accessible-db` · `feature-0003-attach-version-diff` · `feature-0003-attach-diff-colgroup` · `feature-0016-cluster-label-target` · `feature-0043-ai-jobs-perm-gate`(신규) · `task0215-insight-toggle` · `task0216-rag-ds-aware`) → `git worktree prune` + 디렉토리 삭제 대상. 어느 쪽도 흡수 대상 아님.
