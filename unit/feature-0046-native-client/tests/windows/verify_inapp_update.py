"""Verify an installed DQA updates through ZIP only, preserving its live WebView2 and runner.

Use isolated AppId/scheme for --first. --second is the actual next-version update ZIP.
The service page and runner are fixtures. Existing user apps are never stopped.
"""
...
def main():
    ...
    assert "dqa-inapp" in str(root).lower() and not root.exists()
    ...
    state = {"page_loads": 0, "ticks": [], "bridge": None, "downloads": []}
    processes = []
...
'''
    ...
    target = args.second.stem.removeprefix("DQAConnect-Update-")
    manifest = {"version": target, "update": {"filename": args.second.name,
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if url.path == "/client/" + args.second.name:
                state["downloads"].append(args.second.name)
                return self.send(payload, "application/octet-stream")
            if args.notes and url.path.lstrip("/") in ("release-notes.js", "release-notes-data.js", "release-notes.css"):
                mime = "text/css" if url.path.endswith(".css") else "application/javascript"
                return self.send((args.notes / url.path.lstrip("/")).read_bytes(), mime)
            query = urllib.parse.parse_qs(url.query)
            if "client_port" in query and "client_nonce" in query:
                state["bridge"] = (int(query["client_port"][0]), query["client_nonce"][0])
                state["page_loads"] += 1
            self.send(b'''<!doctype html><meta charset="utf-8"><title>DQA Update Verification</title>
<h1>Update continuity</h1><textarea id="draft">DRAFT_CONTINUITY_42</textarea>
<p id="progress"></p><div id="notes"></div>
<link rel="stylesheet" href="/release-notes.css">
<script src="/release-notes-data.js"></script><script src="/release-notes.js"></script><script>
if(window.ReleaseNotes) ReleaseNotes.render(document.getElementById('notes'),{areas:['work','common']});
...
fetch('/ui-tick',{method:'POST',body:JSON.stringify({instance,n,hadCookie,notes:document.querySelector("#notes").innerText,draft:document.querySelector('#draft').value})})},100);
    ...
    try:
        ...
        if args.notes:
            text = page_before["notes"]
            record("rendered release notes text", text=text[:900])
            assert "앱에서 업데이트합니다 (1.4.0)" in text
            assert "1.2.x에서 전환하면 앱이 재시작" in text
            record("real release notes renderer in installed WebView2 includes first-transition caveat")
        ...
        # The shipped app shares its exclusion with the production installer.
        mutex = kernel.CreateMutexW(None, False, "Global\\DQAConnectSetup")
        assert mutex
        try:
            failed = apply_update()
            assert failed["error"] == "install_failed", failed
            assert (install / "active-slot.txt").read_text().strip() == first_slot
            record("competing setup mutex blocks ZIP activation and keeps the old pointer")
        finally:
            kernel.CloseHandle(wintypes.HANDLE(mutex))

        good_digest = manifest["update"]["sha256"]
        manifest["update"]["sha256"] = "0" * 64
        failed = apply_update()
        assert failed["error"] == "download_failed", failed
        assert (install / "active-slot.txt").read_text().strip() == first_slot
        manifest["update"]["sha256"] = good_digest
        record("download digest failure keeps current app and pointer")

        pointer = install / "active-slot.txt"
        held = kernel.CreateFileW(str(pointer), 0x80000000, 1, None, 3, 0, None)
        assert held != wintypes.HANDLE(-1).value
        try:
            failed = apply_update()
            assert failed["error"] == "install_failed", failed
            assert pointer.read_text().strip() == first_slot
            record("activation sharing violation rolls back the unpublished slot")
        finally:
            kernel.CloseHandle(wintypes.HANDLE(held))
        assert list((install / "versions").iterdir()) == [install / "versions" / first_slot]
        ...
        third_slot = second_slot
        assert state["downloads"] and set(state["downloads"]) == {args.second.name}
        assert not list((home / "update").glob("*.exe"))
        record("update server serves only ZIP; no installer downloaded", downloads=state["downloads"])
        quit_app(app.pid)
        app.wait(timeout=20)
