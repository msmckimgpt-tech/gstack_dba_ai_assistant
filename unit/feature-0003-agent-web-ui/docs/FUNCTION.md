...
- REQ-20260824-sidebar-reorder-anim (**Minor** §12.3): 좌측 대화목록에서 **요소의 명칭을 바꾸면
  ...
  - AC-20260824T164437-sidebar-reorder-anim-1: 명칭 변경 후 목록이 재구성될 때, 자리가 바뀐 행은
    **이전 자리에서 새 자리로 트윈**한다(FLIP — 재구성 직전/직후 좌표를 비교해 역이동 후
    ...
    곡선은 **easeInOutBack**(`cubic-bezier(.68,-.6,.32,1.6)`, 420ms — 사용자 요청 2026-08-24):
    시작에서 진행 방향의 반대로 살짝 당겼다가 끝에서 목표를 조금 지나쳐 되돌아온다. 제어점 y 가
    [0,1] 밖이라 트윈 도중 행이 원래 자리보다 뒤 / 목표보다 앞으로 나가며, 그 폭은 이동 거리의
    약 10%다(실측: 29px 이동 → ±3px, 176px 이동 → ±18px). 상한(2400px)이 과장의 안전판이다.
