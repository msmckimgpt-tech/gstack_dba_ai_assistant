...
#:
#: - `catalog`  — 그 CLI 가 **스스로 출력한 모델 카탈로그**(`codex debug models` 류).
#:                LLM 답이 아니라 기계적 조회다 (사용자 요청 2026-09-07).
#: - `baseline` — 서버 원장 **그대로**. 아래 완화 참조 (사용자 결정 2026-09-07).
#:
#: ⚠ **`builtin` 을 넣지 마라 (불변).** 러너 소스에 적혀 있던 표가 그 이름이고, 그것이
#: `gpt-5.1-codex` 가 사용자 화면에 뜬 경로다 (사용자 제보 4회, 2026-08-31~09-01).
#: `catalog` 는 그 반대다 — 값을 우리가 쓴 것이 아니라 **그 CLI 가 출력한 것**이다.
#:
#: ## `baseline` 허용 = 「확인-후-표시」의 **완화** (사용자 결정 2026-09-07)
#:
#: 2026-09-02 결정은 「서버 보관 목록은 라이브 확인을 통과해야 화면에 오른다」였고, 이 자리에
#: 「`baseline` 같은 «확인 전» 출처를 넣지 마라」가 적혀 있었다. 그 규칙이 라이브에서 낸
#: 결과는 **목록이 영영 비는 것**이었다 — codex 는 확인 질의가 600초에도 끝나지 않고
#: 열린 질의에는 `models: []` 로 답한다(실측 2026-09-07). 사용자는 그 상태를 결함으로
#: 신고했고(「AI가 연결되었음에도 모델 목록이 나타나지 않고 있습니다」), 완화를 결정했다.
#:
#: 화석과 다른 점: 원장에 들어가는 값은 이 함수를 이미 통과한 `probe`/`cache`/`verified`
#: 뿐이라 **그 계정의 그 AI 가 직접 답했던 목록**이다. 러너는 이 폴백을 캐시하지 않고
#: (`_UNCACHEABLE_SOURCES`) 다음 기동에 다시 실조회한다 — 임시 표시이지 결론이 아니다.
#: 낡은 값을 골랐을 때의 backstop 은 `handler.py` 의 「반영하지 못한 지정을 답변에 적는다」다.
_SANITIZE_SOURCE_ALLOW: frozenset = frozenset(
