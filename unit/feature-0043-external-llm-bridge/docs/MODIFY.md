...
## CHG-20260902T183000-ai-claude-feature-0043-l10-harness-fix — CI 가 잡은 하네스 오류

- **날짜**: 2026-09-02
- **REQ**: 직전 항(`CHG-20260902T170000-…-launcher-ca-pin`)의 후속 수정
- **위험도**: Minor (§12.3 — 테스트 하네스 전용. 제품 코드 변경 없음)

직전 cycle 이 넣은 `test_l10` 이 `chmod 500` 으로 «갱신 임시 파일을 못 쓰는» 상황을 만들려
했는데, 그 조작은 같은 디렉토리의 `bridge.log` 도 함께 막는다. 런처는 러너 stderr 를 그 파일로
잇기 때문에, **러너가 뜰 수 없는 조건을 만들어 놓고 「러너가 떠야 한다」고 검사**하는 꼴이었다.

로컬은 root 라 `chmod` 자체가 무효였고(권한 검사 우회), 내가 넣은 `skipif(root)` 가 그 모순을
통째로 가렸다 — **skip 은 「이 환경에선 검사 안 함」이지 「계약이 지켜짐」이 아닌데, 초록은 그
둘을 구별해 보여 주지 않는다.** CI(비-root)에서 red 로 드러났다(`cannot create …/bridge.log`,
rc=4).

### 변경 내용

- `test_l10` 은 **정적 계약만** 잠근다 — `cat > "$_dl" <<'DLEOF' || true` 존재 + 받지 못한
  다운로더를 실행하지 않음(`[ -s "$_dl" ]`). 환경에 무관하다.
- 「갱신이 실패해도 러너는 뜬다」는 **L3 이 실물로** 본다 — 잘린 응답으로 갱신을 실패시키고
  런처가 `rc=0` 으로 끝나는지 확인(검사 추가). 실패 주입이 «갱신 단계에만» 걸리는 자리다.

### 교훈

실패를 주입할 때는 그 주입이 **검사하려는 것만** 막는지 확인한다. 더 많이 막으면 제품이 지킬
수 없는 조건이 되고, 그때의 red 는 제품이 아니라 하네스를 가리킨다. 그리고 환경 의존 `skip` 은
그 판정을 **로컬에서 보이지 않게** 만든다.

## CHG-20260902T170000-ai-claude-feature-0043-launcher-ca-pin — 런처의 러너 갱신이 신뢰 앵커를 지킨다

- **날짜**: 2026-09-02
- **REQ**: 사용자 제보 2026-09-02 (「자동 연결 시, 러너 파일도 스스로 갱신될 수 있도록」)
- **위험도**: Major (§12.3 — 다음 순간 **실행될** 파일을 받는 경로다. 방향은 강화:
  종전에 풀려 있던 pin 을 채운다. 실패는 종전 동작(있던 파일 유지)으로 떨어지므로
  갱신 실패가 러너를 죽이지 않는다)
- **승인**: 사용자 요청에 따른 직접 수정 (신뢰 앵커를 **좁히는** 변경이라 새 권한·새 통로 없음)

### 배경 — 기능은 있었고, 그 사용자에게서만 100% 실패했다

두 cycle 이 이미 갱신을 넣었다(`T100000` 런처 갱신 · `T140000` 러너 자기 갱신). 그런데
화면은 여전히 「다시 실행했지만 러너 파일이 그대로입니다」였다. **없는 것이 아니라 동작하지
않는 것**이고, 그 둘은 코드를 읽는 것만으로 구별되지 않는다.

`launch.ps1` 의 갱신만 `Invoke-WebRequest` 였다. IWR 은 `rootCA.crt` 를 보지 않고 **OS 신뢰
저장소**로 검증하는데, 이 설치기는 CA 를 거기 넣지 않는다 — `bridge_setup.ps1` 자신이 그렇게
적어 두고(363행 「Windows 는 CA 를 시스템 저장소에 넣지 않고도…」, 454행 「⚠ **IWR 을 폴백으로
두지 않는다**」) 설치 다운로드는 curl `--cacert` · 파이썬 `cafile` 로만 한다. **그 스크립트가
굽는 런처만 금지된 것을 썼다.** 실패는 `catch { }` 가 삼켰고, 창은 `-WindowStyle Hidden` 이라
증거가 남을 곳도 없었다.

반대 방향도 결함이다: 사내 CA 가 OS 저장소에 **있는** 머신에서는 IWR 이 pin 과 **무관하게**
통과한다(실측 2026-08-31). 조용한 미갱신과 조용한 pin 우회가 같은 한 줄에서 나온다.

| 축 | 받는 수단 | pin |
|---|---|---|
| `launch.sh` (POSIX) | `curl --cacert` | ✔ |
| `agent/selfupdate.py` | `ssl.create_default_context(cafile=ca)` | ✔ |
| **`launch.ps1` (Windows)** | **`Invoke-WebRequest`** | **✘** |

### 변경 내용

**1. `bridge_setup.ps1` — 런처가 파이썬으로 받는다**

굽는 `launch.ps1` 의 갱신 블록에서 IWR 을 걷어내고, 임시 파이썬 다운로더를 ASCII 로 떨어뜨려
`ssl.create_default_context(cafile=rootCA.crt)` 로 받는다. 크기 바닥 20000B · `ast.parse` 는
`agent/selfupdate.py` 와 **같은 값·같은 검사**. 파이썬은 폴백이 아니라 러너가 `--ca` 로 쓰는
것과 **같은 신뢰 평가기**다 — 다운로드만 다른 평가기(Schannel)로 하면 신뢰 경로가 둘로 갈린다.

**2. `bridge_setup.sh` — curl 부재에서도 받는다**

종전에는 `command -v curl` 이 실패하면 갱신을 **통째로 건너뛰었다**. Windows 가 IWR 로 겪은
「조용한 미갱신」의 약한 판이다. 같은 파이썬 다운로더를 두 번째 손으로 둔다.

**3. 실패가 흔적을 남긴다**

Windows 는 `$Home_\launch.log` 에, POSIX 는 stderr 에 사유를 남긴다. 이 침묵이 결함을 여러
cycle 동안 살려 뒀다.

**4. heredoc 백틱 이스케이프 (기존 결함)**

`launch.sh` 를 굽는 unquoted heredoc 안에서 백틱이 **명령 치환**되어, 생성된 파일의 주석에서
`--cacert` 가 통째로 지워져 있었다(실측). 이 cycle 이 그 블록을 다시 쓰므로 함께 고친다.

### 검증

`test_launcher_ca_pin.py` 17건 — 자체서명 CA 2벌 + 실 TLS 서버로 **동작**을 본다:
정본 CA 는 실제 교체(curl·파이썬 **두 손 모두**) / **엉뚱한 CA 는 거부** / 잘린 응답 거부 /
**리다이렉트·평문 거부** / 두 축 다운로더 **본문 동등** / 호출부 **배선**(인자 순서·앵커
경로·설치 대상) / 쓰기 불가 홈에서도 러너는 뜬다.

⚠ **처음 보고한 「뮤테이션 7종 전건 KILL」은 성립하지 않았다.** 적대 패널이 같은 스위트에
17종을 돌려 **15종 생존**을 실증했다 — 특히 `Invoke-WebRequest` 를 기본 별칭 `iwr` 로 바꾸면
**원래 결함 그 자체**가 전건 통과했고, curl 손은 `--cacert` 를 `-k` 로 바꿔도 통과했다(그 손은
테스트가 한 번도 돌리지 않았다). 표본이 좁으면 「전건 KILL」은 방어 범위가 아니라 **표본의
크기**를 말한다. 그 지적을 반영해 스위트를 7 → 17건으로 넓힌 뒤 재측정한 수치는 아래 §검증
재측정에 적는다.

### 적대 패널이 찾은 것 (2026-09-02, security · qa)

이 cycle 이 **새로 넣은** 결함:

| # | 무엇 | 결과 |
|---|---|---|
| 1 | `cat > "$_dl"` 이 `set -e` 아래 — 디스크·쿼터·읽기전용 홈에서 **런처가 죽는다** | 수정(`|| true` + `[ -s ]` 게이트) |
| 2 | heredoc 안 주석의 백틱이 `` `r ``→CR · `` `a ``→BEL 로 해석되어 굽힌 `launch.ps1` 에 **제어문자 주입** | 수정(주석 백틱 전면 제거, 잔여 0 검증) |
| 3 | `try { } catch { }` 를 걷어내 terminating 예외가 기동을 막을 수 있게 됨 | 복원 (형제 스위트가 적발) |

**원래부터 있었고 이 cycle 이 드러낸** 것:

| # | 무엇 | 결과 |
|---|---|---|
| 4 | `urlopen` 이 `302 Location: http://…` 를 따라가 **pin 이 무력화** — 오리진 응답을 바꿀 수 있으면 러너가 **실행할** 파일을 지목할 수 있다(`selfupdate.py` 규율 2 가 막겠다고 선언한 형태) | 수정(`NoRedirect` opener) |
| 5 | `BRIDGE_BASE` 가 `http://` 면 CA 를 넘겨도 **무시**된다 | 수정(스킴 강제 — `selfupdate.py:92` 와 같은 가드) |
| 6 | curl 손(POSIX **기본 경로**)에 크기 바닥이 없어 31바이트 응답이 설치됨 | 수정(`_floor_ok` 를 두 손 모두에) |
| 7 | Windows 축 `$LASTEXITCODE` stale — 파이썬 실행 실패 시 직전 값(반드시 0=성공)이 남는다 | 수정(127 초기화 + 독립 `ast.parse` 재검증) |
| 8 | HTTP status 미확인 | 수정(200 아니면 거부) |

이월(이번 범위 밖, 별도 축): 임시 다운로더 파일의 **배타 생성**(`selfupdate.py` 는
`mkstemp` 를 쓰는데 런처는 예측 가능한 경로에 평범한 쓰기) · `launch.log` 의 **상한·회전·
마스킹** · `mv` 가 러너 권한을 600→644 로 넓히는 것 · `$Base` 무검증 보간(기존 결함) ·
POSIX 축은 실사용 진입점(`.app`/`.desktop`)에서 stderr 가 사용자에게 닿지 않아 「흔적을
남긴다」가 Windows 만큼 성립하지 않는 것.

### 남는 것

**이미 설치된 낡은 런처는 원격으로 고칠 수 없다** — 사용자 컴퓨터의 파일이고 서버가 그것을
바꿀 통로가 없다. 이 수정도 **1회 재설치**를 요구한다(지금 화면이 이미 요구하는 그 한 번).
달라지는 것은 그 뒤다: 종전에는 재설치해도 Windows 에서 갱신이 계속 실패해 같은 자리로
돌아왔고, 이제는 한 번이면 러너 자기 갱신에 도달해 배포를 스스로 따라간다.

이월: **런처 스크립트 자체의 자동 갱신**. `launch.*` 는 설치 시점 값(파이썬 경로·`$Base`·
조사된 인자)이 구워지므로 서버 정적 파일로 덮어쓸 수 없고, 템플릿 + 파라미터 재조립이 필요하다.
...
```python
_na_rep = _node_analysis.process_pending() if _llm_open else {}   # ← 이것
```

종전 주석은 이랬다: *"차단 중에는 claim 해서 실패시키지 않는다 — 적재 자체도 게이트로 막혀
있으므로 정상 운영에서 대기 잡은 생기지 않는다."* 전환 이전에는 **옳은 판단**이었다.

이번 cycle 이 그 전제를 깼다. 이제 게이트가 닫혀 있어도 적재된다(연결된 개인 AI 가 처리하므로).
그런데 호출부가 그대로여서:

- 그래프 능동 분석이 적재돼도 **아무도 처리하지 않는다** — 사용자가 제보한 "막혀 있다" 가
  형태만 바꿔 되돌아온다(202 는 뜨는데 결과가 영영 안 온다)
- stale `running` 회수가 **영원히 돌지 않는다** — 러너가 끝내 답하지 않은 잡이 pending 으로
  복귀하지 못하고 run 이 굳어 재트리거를 막는다

**실측**: lease 900초를 **1424초**까지 넘겼는데 회수되지 않았다(20분 관찰).

### 왜 라이브 검증이 통과했나 — 내가 손으로 불렀기 때문이다

AC-1·AC-2 를 확인할 때 `process_pending` 을 **직접 호출**했다. 그 경로는 옳게 동작했고,
그래서 "배선 완료" 로 보였다. 헬퍼가 옳은 것과 **진입점이 그것을 부르는 것**은 다른 사실이다
— 이 feature 가 P0-U 에서 이미 겪은 형태이고, 나는 그 교훈을 알고도 같은 자리를 놓쳤다.
관찰 기반 AC(회수·타임아웃)를 넣어 두지 않았다면 배포된 채 조용히 남았을 것이다.

### 수정

호출부의 게이트를 **없앤다**. 판정은 잡 단위로 함수 안에서 한다 — 게이트가 열렸으면 직접
...
## CHG-20260902T183000 — caps-live-sync POST-DEPLOY 라이브 실측 (증적 기록)

배포 `e894cd86` 에 대한 PB-0008 라이브 시각검증 결과를 기록한다. 코드 변경 없음(문서 전용).

정본: `unit/feature-0003-agent-web-ui/docs/test-runs.d/TASK-20260902T140200-caps-live-sync.md`
(웹 자산 소유가 feature-0003 이므로 시각검증 기록은 그쪽 — check #13 의 대상 판정 규약).

### 핵심 실측 3건

1. **AC-1** — 빈 상태를 화면에 얹고 **관찰자 무장 후** 러너를 띄웠다.
   `itemHidden true→false` · `menuItems 0→4` · 라벨 `…→Opus`, 그 사이 **`navCount` 1 유지**
   (새로고침 0회). 러너 기동 0.8초에 질문 처리 개시 → 협상 25초 → 화면 반영 약 14초 후.
   ⚠ 제가 적어 둔 문턱은 ≤10초였고 **실측 약 14초** — 충족했다고 적지 않고 측정값을 남겼다.
2. **중간 신고(3차 제보)** — claude 목록이 `t=+15s` 에 서버에 도달했고 **그때 codex 는 아직
   협상 중**이었다(codex 는 `t≈+270s` 에 종료). 종전 동작이면 서버는 둘 다 끝날 때까지
   아무것도 받지 못하므로, 이 한 관측이 직접 증거다.
3. **AC-5** — `caps_settling` 이 마지막 신고 +150초에 내려갔다(`CAPS_SETTLING_SEC` 와 일치).
   AC-3 의 초판 표현 「정상 상태에서 폴링 0」을 「**마지막 신고 후 150초 경과** 상태에서 0」
   으로 **정정**한다 — 그 150초는 두 번째 플랫폼을 관측하는 의도된 창이다.

### 미관측 1건 (숨기지 않는다)

**AC-4 후반**(두 번째 플랫폼이 이어서 나타나는 장면). 이 머신 `codex` 가 `TimeoutExpired`
로 240초 예산을 전부 태우고 답을 내지 못해 **도착 사건 자체가 발생하지 않았다**. 전제 조건
둘(첫 플랫폼 도착 · 그 구간에 관측 창 개방)은 라이브에서 성립했고, 판정 로직은 node 실행
검증으로 잠겨 있으나 그것은 코드이고 라이브가 아니다.

부수 확인: 그 타임아웃은 REPORT §11-B(제 기록)·codex P2-7(독립 확인)의 「행(hang) 시 재시도
불가」가 **라이브에서 재현**된 것이다(112.3×2 + 60 = 285 > 240). 총예산 상향은 기동 체감과
직접 교환이라 사용자 결정 사안.
...
## CHG-20260902T190000-caps-probe-fast — 능력 질의 응답 시간의 근본 원인 해소

사용자 제보 4차: 「모델 목록 호출은 어려운 작업이 아니므로 신속해야 한다. 응답 시간이 오래
소요되는 근본적인 원인을 해소해 달라 (모델, effort 조정 등)」.

### 근본 원인 — 사용자가 짚은 축이 아니었다

「모델·effort 조정」을 먼저 실측했으나 그것만으로 설명되지 않았다(claude 는 `--model haiku`
가 오히려 **39초로 더 느렸다**). `codex exec --json` 이벤트 스트림을 뜯어 실체를 봤다:

```
thread.started 1 · turn.started 1 · item.started 14 · item.completed 15
'exec_command' 18회 · shell 관련 94회 · web_search 29회   ← 100초 구간
```

**자기소개 질문을 받은 에이전트가 작업 디렉토리를 뒤지고 웹을 검색하고 있었다.** 능력 질의는
「너는 어떤 모델을 쓸 수 있나」인데, 요즘 CLI 는 전부 에이전트라 질문을 받으면 도구를 쓴다.
그것이 240초 예산을 태우고 `TimeoutExpired` 로 끝난 실체다(POST-DEPLOY 라이브에서도 재현).

### 실측 (같은 머신·같은 프롬프트, **러너와 동일한 추출기**로 판정)

| 조합 | 시간 | 결과 |
|---|---|---|
| codex 기본 | **>300초** | 타임아웃 |
| codex + 도구금지 가드 | 47.0초 | 정상 |
| codex + 가드 + `effort=low` | **10.8초** | 정상 |
| codex + 가드 + `effort=minimal` | 2.9초 | **rc=1 거부** |
| codex + `--output-schema` (가드 없음) | >300초 | 타임아웃 — 형태 문제가 아니다 |
| claude 기본 | **66.3초** | 정상 |
| claude + 가드 | **5.7초** | 정상 |
| claude + `--model haiku` | 39.2초 | 정상 — 모델 강제는 **역효과** |

⚠ **측정 함정 1건**: 초판 측정이 `head -c 2000` 으로 파이프를 조기에 닫았고 codex 는
**프롬프트를 되울려 출력**하므로 `"models"` 가 그 되울림에 매칭됐다 — 「답변 시간」이 아니라
「되울림 2KB 시간」을 재고 17.8초라고 적었다. 러너의 `_extract_json` + `_coerce_options` 로
판정을 바꾸자 같은 명령이 **>300초**로 드러났다. 측정기가 거짓 초록을 내던 형태다.

### 무엇을 고쳤나

**① 도구 금지 가드** (`_CAPS_NO_TOOLS_GUARD`) — probe·verify·effort **세 프롬프트 전부**
맨 앞에. 하나라도 빠지면 그 경로만 옛 지연을 유지하고, 하필 잘 안 타는 경로라 회귀가 오래
숨는다. 계약 위반이 아닌 근거: 가드는 **어떻게** 답할지만 제약하고 **무엇을** 답할지는
건드리지 않는다 — 파일시스템을 뒤져 만든 추측보다 자기 지식으로 답한 것이 P0-Z4 에 더 가깝다.

**② 질의 전용 추가 인자** (`_RUNTIME_SPECS["codex"]["probe_extra"]`) —
`-c model_reasoning_effort=low --ephemeral`. 능력 질의에만 붙고 실제 질문 처리에는 붙지
않는다(근거가 다르다: 자기소개는 추론이 필요 없지만 사용자 질문은 그 반대다).

**③ 두 겹의 가드 — ⭐ 자체 적발**

- **첫 시도에만**: 통하지 않는 CLI 버전에서 재시도가 순수 형태로 다시 묻는다. 없으면 인자
  하나 때문에 그 런타임이 화면에서 통째로 사라진다(속도를 얻고 가용성을 잃는 교환).
- **캐시에 남기지 않는다**: `got["argv"]` 는 「어느 호출 형태가 통했는가」로 저장되어 **실제
  사용자 질문에 재사용**된다. 여기 `model_reasoning_effort=low` 가 섞이면 그 계정의 **모든
  질문이 낮은 추론으로** 돌고, 사용자가 화면에서 고른 강도가 조용히 무시된다. 질의용 argv
  (`_probe_argvs`)와 캐시용 argv(`_pure`)를 분리해서 들고 다닌다.
- 삽입 위치는 **프롬프트 위치 인자 앞** — 뒤에 붙이면 CLI 가 플래그를 프롬프트로 읽는다.
- 표에 없는 CLI 는 **무변경** — 모르는 CLI 에 우리 플래그를 넣지 않는다.

### 최종 실측 (조립본의 실제 프롬프트·실제 argv)
...
| 파일 | 변경 |
|---|---|
| `agent/caps.py` | `_CAPS_NO_TOOLS_GUARD` + 세 프롬프트 적용 · `_with_probe_extra` · 질의/캐시 argv 분리 |
| `agent/runtimes.py` | codex `probe_extra` |
| `tests/test_caps_live_sync.py` | 신규 3건(가드 3프롬프트 · 첫시도만+캐시무누출 · 삽입 위치) |
| `tests/test_runtime_model_selector.py` | 경계 결합 단정을 AST 의도 검사로 + 새 계약 잠금 |

신규 권한 코드 0 · 신규 route path 0 · 파괴적 변경 0 · 스키마 변경 0.

### 검증

뮤테이션 **8종 KILL / 생존 0**(가드 부분 제거 3종 · 가드 위치 이동 · 전 시도 적용 ·
⭐캐시 누출 · 프롬프트 뒤 삽입 · 표 밖 CLI 오염). 컨테이너(py3.11) CI 전 경로 rc=0 ·
FAILED 0 · ruff F821 기준선 동일(1건).

### 알려진 대가

가드는 한 회차의 응답을 **짧게** 만들 수 있다(실측 claude 6종 → 3종). 계정 원장의 합집합
누적이 그것을 흡수하고, codex 는 종전에 **아무것도** 신고하지 못했으므로(타임아웃) 1종이라도
받는 것이 엄격히 개선이다.
...
## CHG-20260902T203000-caps-effort-axis-refute — 가드가 죽인 추론축 복구

`CHG-20260902T190000-caps-probe-fast` 를 배포(`64168d5b`)한 직후 실측에서 **회귀 1건**을
자체 적발했다. 같은 cycle 안의 후속 수정이다.

### 무엇이 깨졌나

도구 금지 가드를 넣자 claude 가 `{"efforts": [], "effort_flag": []}` 로 **자신 있게**
「추론 수준을 지정할 수 없다」고 답했다. 그런데 claude 는 `--effort` 를 실제로 지원한다
(우리 표에 있고 `--help` 에도 있다) — **가드 이전에는 도구로 자기 도움말을 읽어 5단계를
답했던 것이다.** 즉 가드가 「못 답함」을 「확신 있는 부정」으로 바꿨다.

그리고 `_settle_effort_axis` 의 «명시적 부정» 분기가 그 부정을 **확정**으로 굳혀
`--help` 검증 경로(②)를 건너뛰었다. 라이브 실측: **추론 5단계 → 0단계** — 출하된
추론 강도 선택기가 화면에서 사라진다.

**속도를 얻고 기능 하나를 잃는 교환이었고, 그것을 실측 없이는 몰랐다.**

### 무엇을 고쳤나

그 부정을 `--help` **관측으로 반증**하게 순서를 바꿨다. 표에 짝이 있는 런타임이면 부정을
곧바로 채택하지 않고 ②로 흘린다.

근거: **우리가 그 AI 에게 도구를 쓰지 말라고 요구했다.** 그러니 그 「없다」는 관측이 아니라
**기억**이다. 반면 `--help` 는 그 바이너리에 직접 물은 **관측**이고, 「이 CLI 가 `--effort`
를 받는가」는 의견이 아니라 **기계적 사실**이다. 기계적 사실에서는 관측이 기억을 이긴다.

표에 짝이 **없는** CLI 는 반증 근거가 없으므로 그 부정이 유일한 정보다 — 그대로 존중한다.

### ⚠ 종전 계약을 의도적으로 뒤집었다

`test_reask_answer_of_no_support_is_respected`(codex P1-2 시기)는 「AI 의 명시적 부정이 우리
표를 이긴다」를 잠그고 **도움말을 보지 말라**(`calls == []`)고 단정했다. 그 계약은 가드가
없던 전제에서 옳았다. 이제 전제가 바뀌었으므로 테스트를 `…_is_checked_against_help` 로
바꾸고 **왜 뒤집었는지·무엇이 안 바뀌었는지**를 docstring 에 적었다:

- P0-Z4(「목록의 출처는 연결된 AI」)는 **모델 값**에 대한 계약이다 — 그 계정이 어떤 모델을
  쓸 수 있는지는 관측할 수 없다. 여기서 뒤집는 것은 **플래그의 존재 여부**뿐이다.
- 종전 계약이 지키려던 「그 CLI 버전에는 없을 수 있다」는 **`--help` 검사가 그대로 보장**한다
  (도움말에 없으면 표를 보고도 채택하지 않는다).

### 복구 확인 (배포된 러너 실물)

```
claude  10.9s  모델 3종 ['opus','sonnet','haiku'] · 추론 5단계 ['low','medium','high','xhigh','max']
        effort_flag = ['--effort','{effort}']  effort_probed = True
        로그: "claude: 추론 수준을 답하지 않아 내장 표로 보완했다 (--help 로 실재 확인)."
```

### 검증

뮤테이션 **4종 KILL / 생존 0** — L1(부정을 무조건 확정 = 배포된 회귀 형태) · L2(도움말 검증
없이 표 채택) · L3(도움말 미판독을 확정으로 기록) · L4(표 밖 CLI 에 다른 런타임 짝 적용).
⚠ L4 는 초판에서 **생존**했다: 케이스가 ①의 이른 반환에 가려져 ②의 표 조회를 검사하지
못했다 — 추론축 키를 **아예 뺀** 응답으로 ①을 지나가게 만들어 닫았다.

### 미해결로 남긴 것 (정직 표기)

전체 CI 경로 순서에서 **`feature-0014 test_g3b_observed_isolation_that_never_clears_reports_failure`
가 간헐 실패**한다(내 트리 전체경로 2/3 실패 · 1회 통과, feature-0014 단독 0/5 실패,
...
됩니다. … 근본적인 원인을 해소해주세요."* **지적이 맞고, 앞선 두 cycle 의 범위 설정이 틀렸다**
— `claude` 인증은 사용자 환경이지만 **그 실패가 화면에 도달하지 않는 것은 제품 결함**이다.

**근본 원인(설계상 종결 부재)**: ① 협상 실패는 선택기만 숨김 → 러너가 질문을 정상 점유
② 러너 AI 상한 기본 **0(무제한)**·무진전 감시 0건 ③ 서버 `BRIDGE_NO_PROGRESS_SEC=900` 은
주석대로 「표시 경계이지 종료 조건이 아니다」 ④ lease 30분 → **재배달**, 시도 상한·종결 상태
없음. 결과: 15분 안내 → 30분 재배달 → 반복.

**변경**: `state.py` AI 건강 원장(관측 기반·성공 1회로 즉시 복귀) · `caps.py` 협상 실패를
질문 전 첫 관측으로 승격 · `invoke.py` 종결 지점 전부에 결과 반영 + **아픈 러너에만** 유한
상한(150초) + 건강 상태별 안내 · `api.py` 하트비트 `ai_ready`+사유 신고.
`tests/test_ai_unusable_surfaced.py` **신규 13건**.

**되돌린 판단**: 초판은 「무출력 N초면 끊는다」였는데 틀렸다 — `claude -p` 는 답을 끝에 한 번에
내므로 무출력이 「생각 중」과 「멈춤」을 못 가르고, 고정 상한은 27단계 조사를 죽인 이력이 있다.
시간이 아니라 **관측된 건강 상태**로 갈랐다(건강하면 종전 무제한 = 회귀 0).

**뮤테이션 7종 전건 KILL** — 1차에 G3·G8(배선 공백) 생존. 판정 함수의 계산만 단정하면 그
함수를 부르는 자리가 사라져도 통과한다. 행위 단정 2건 추가로 닫음.

## CHG-20260903T160000 — 아픈 러너는 기다리지 않고 즉시 답한다

**계기**: 사용자 지적 — *"150는 너무 깁니다. (10초 이상 소요되는 부분도 길다고 체감됩니다.)"*

**직전 cycle 의 뒤바뀐 설계**: `CHG-20260903T140000` 의 150초 상한은 「이미 못 쓴다는 것을 아는
상태에서 더 기다리게」 했다. 그 150초는 사용자를 위한 시간이 아니라 **회복 확인이라는 우리
편의**였고, 그 비용을 사용자 대기 시간으로 지불했다.

**변경**: 얽힌 두 목적을 분리 — ① 사용자 답변은 **AI 호출 없이 즉시**(`invoke.ask_local_ai`
진입부 게이트 + `_unhealthy_notice`) ② 회복 감지는 **배경**
(`caps.schedule_health_recheck`, 쿨다운 60초, 협상 경로 재사용). 150초 상한(`timeout_sec`
경로)은 제거하지 않고 남겼다 — 명시 지정(`--ai-timeout`) 사용자를 위한 경로이므로.

건강한 러너는 이 경로를 타지 않는다(무제한 유지·테스트로 잠금). 뮤테이션 **8종 전건 KILL**
(H5「건강한 러너도 즉시실패」포함 — 정상 질문이 안내문으로 바뀌는 회귀를 잠근다).
...
## CHG-20260903T170000 — 「자동 갱신 미발동」은 오판이었다 (문서 정정, 코드 변경 0)

`CHG-20260903T160000` 이 별개 결함으로 적은 「자동 갱신(`try_self_update`) 미발동」은
**틀렸다.** 실측 정정:
...
서버 하트비트도 `RunnerBuild=4669f0192526` 로 갱신됐다.

**왜 틀렸나 — 부재를 근거로 결론 냈다.** ① `stale_build` 로그 침묵은 **설계된 것**이다
(self-update 가능 시 신호만 세우고 기록하지 않음 — 2026-09-02 사용자 결정). ② 11:35 스냅샷
으로 영구 상태를 단정했으나 실제 발동은 11:48 이었다(자기갱신은 유휴+협상종료+최소간격을
모두 만족할 때 일어난다).

따라서 **수동 재기동은 불필요**하고, 이월 항목 「자동 갱신 조사」도 취소한다. 원 서술은
§5.4 에 따라 폐기 표시로 남겼다(`TASK-20260903T160000-…` §5).
...
## CHG-20260904T190000-ai-claude-runner-wsl-merge — origin/main 병합
- Timestamp: 2026-09-04T19:00:00+09:00
- 병렬 세션이 먼저 랜딩해 PR DIRTY. append-doc 드라이버가 MODIFY·REVIEW 2건을 자동
...
## CHG-20260908T010000-ai-claude-live-record — 배포 후 라이브 실측 기록 (문서 전용)
- Timestamp: 2026-09-08T01:00:00+09:00
- 코드 변경 **없음**. `docs/test-runs.d/20260908T010000-live-model-list-reached.md` 신설 +
...
## CHG-20260910T134000-effort-contract
