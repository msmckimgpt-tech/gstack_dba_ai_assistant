...
- 2026-09-09 (doc_sync): 09-09 `TASK-20260909T143400-caddy-probe-reaping` + `TASK-20260909T140000-deploy-refresh` 반영 — 정본 FUNCTION 에 `REQ-20260909-caddy-probe` · `REQ-20260909-edge-probe-process-lifecycle` · `REQ-20260909-deploy-refresh` 3절이 신설됐다. ⓐ **배포 관측 경로가 스스로를 막았다**: 장수 Caddy 안에서 HTTPS 점검 자식(BusyBox `ssl_client`)을 만들던 `edge_peer_live` 경로가 좀비 99 + live threads 24 로 `pids.current 123 / max 128` 을 채워 커널 fork 가 거절됐고(제한 도달 180회), 그 결과 로그인 화면 수정 PR #1652 의 첫 배포가 web 교체 **전에** 안전 중단했다(Caddyfile 읽기 exec 실패 `exit 128` · UI manifest pending 유지). 조치는 세 갈래다 — 파일은 **Docker archive** 로 읽고, admin HTTP 는 **같은 network namespace + 실행 중 이미지 ID 의 `--init` sidecar** 로 격리하며, replica TLS 는 PR #1656 의 **호스트 `nsenter`/`dig`/`curl`** 경로(실제 resolver·마운트 CA·공개 Host/SNI·정확히 200)를 보존한다. 조회 실패는 게이트 실패이지 미기동이 아니므로 **프록시를 재생성하지 않는다**(HTTP fallback 없음). ⚠ compose 의 `init: true`·`pids_limit 256` 은 **다음 생성 계약**으로 기록만 했다 — 실행 중 Caddy 는 재생성하지 않았고 기존 좀비 99 는 다음 정상 재생성까지 남으며, 런타임 한도 확대는 병행 세션의 기여라 중복 적용하지 않았다. 「한도를 올려 해결했다」로 적지 않는다. 검증 = 관련 49 PASS · 실제 probe 5회 후 좀비 증가 0 · 잘못된 SNI 차단 · PR #1656 / `cf55c659` 배포 `exit 0` · 90초 soak PASS · 배포 전후 Caddy PID 동일. ⓑ **배포 완료 게시**: `bin/lib/ui-release.sh` 가 롤링 시작 **전** pending 을 원자 게시하고, 배포 scope 에 필요한 readiness·edge·soak·smoke 를 마친 두 replica 의 revision·stamp 가 일치할 때만 complete 를 게시한다(전용 `artifacts/deploy/ui-release` → web `/srv/ui-release:ro` · 같은 revision 재시도도 완료 marker 확인 후 미완료면 soak 재검증 · workers-only·dry-run 미게시). rollback 은 pending 기록 실패에도 서비스 복원을 계속하고 복원 이미지의 current 태그를 먼저 되돌린 뒤 완료를 게시하며, worker 복원 실패·완료 기록 실패를 성공으로 보고하지 않는다. 실측 = PR #1653/#1655/#1658 반영 후 `4f570829` 배포에서 완료 게시 뒤 **1.242초** 문서 변경 · **4.438초** 적용 알림(설치 앱 프로세스 유지). 소비 계약은 feature-0003 의 동명 REQ. 위 §2 상태·`maturity` 는 이번 창 근거만으로 재판정하지 않았다(정본 `feature_status` 소관). 요지+포인터만(SSOT).
