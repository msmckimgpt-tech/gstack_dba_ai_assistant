...
def _bridge_phase(status: str, claimed_by: Any, submitted: bool, connected: bool) -> str:
    ...
    | `not_connected` | 연결된 AI 가 없다 — 기다리게 두지 않고 알린다 |
    ...
    if claimed_by is not None:
        return "working"
    return "waiting" if connected else "not_connected"
...
def _bridge_stream_snapshot(conn, task_id: str, account_id: int) -> dict[str, Any] | None:
    ...
    try:
        ...
        claimed_by = row[1]
        # 연결 여부는 **`waiting` 국면에서만** 의미가 있다(그 판정만 `not_connected` 로 갈린다).
        # 이미 누가 집었거나 끝난 뒤에는 물어볼 이유가 없다 — tick 마다 도는 조회라 값이 싸지 않다.
        connected = True
        ...
        return {
            "phase": _bridge_phase(status, claimed_by, bool(row[2]), connected),
            "answered": submitted,
    ...
    **웹 세션 인증**이다(외부 OAuth 토큰이 아니라). 자기 계정이 연 web task 만 보이며,
    ...
    try:
        ...
        return JSONResponse({
            ...
            "connected": connected,
            "phase": _bridge_phase(status, row[1], bool(row[2]), connected),
