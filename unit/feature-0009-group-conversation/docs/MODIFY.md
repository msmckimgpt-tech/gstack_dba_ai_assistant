...
- Reason: 라이브 그룹대화(`20260625063340-4220125d` "로그 데이터 집계 요청", product 110 마이크로볼츠-개발 / 데이터소스 `mysql-mv-dev`=MySQL)에서 두 사용자(mckim·admin)가 @assistant 사용 중 마찰을 직접 표명("명시적으로 정해줘야 찾을수있나보네요"/"능동적으로는 찾기 힘드네요"/"@assistant 대화 맥락에서 확인해주세요"/"갑자기 또 다른 DB에서 가져오네요"). 근본 원인 2건:
  - **RC-1 (SQL dialect thrashing)**: MySQL 데이터소스인데 LLM 이 T-SQL(`SELECT TOP`/`CONVERT(VARCHAR..)`/`[brackets]`/`DATEADD`/`UNION`/2-인자 `ISNULL`)을 반복 생성 → sql_guard·엔진이 15회+ 거부("got Union"/"SELECT TOP"/"Cast 'to' missing"/"1582 ISNULL param count"/"1046 No database selected"). 원인: product 전용 system prompt(`websystemprompts` Id 32, AI 자동생성 추정)가 `compose_system_prompt`에서 base 뒤에 append되며 "대용량 결과는 TOP N"·"`UNION ALL` 활용"·"`DESCRIBE [테이블명]`" 로 **T-SQL 을 적극 유도** → base 의 MySQL 규칙·MySQL 가드와 모순.
  - **RC-2 (그룹 맥락 미활용)**: 히스토리 로드(PG `_PG_LOAD_CORE_MESSAGES` + MySQL SELECT)가 `sender_account_id` 를 누락 → LLM 이 다자 대화를 "누가 말했는지 표시 없이" 받음. feature 자체 명세 REQ-GC-R5("발신자 라벨과 함께") 위반. + 그룹 전용 맥락 지침 부재 + product 프롬프트의 "명확하지 않으면 질문하세요" 가 과도 재질문 유발.
- 변경(feature-0002, cross-feature):
  - **RC-1** `src/agent_core.py`: `_MYSQL_DIALECT_GUIDANCE` 상수 신설 + system prompt 조립부의 엔진 분기에 `else`(mysql/기본) 추가 → compose_system_prompt(=base+product context) **뒤**에 권위적 주입(last-writer-wins, T-SQL 유도 product 프롬프트 교정). 멀티 데이터소스 안전을 위해 "활성(primary) MySQL 데이터소스" 로 스코프(secondary 엔진은 multi-ds grounding 위임). `src/modules/tools.py`: `_dialect_correction_hint(sql)` 신설 → sql_guard 거부 메시지에 활성 엔진 기준 교정 힌트(TOP→LIMIT, `[..]`→백틱, UNION 분리, CONVERT/ISNULL 교정) 추가(맹목 재시도 thrashing 차단).
  - **RC-2** `src/modules/runtime_backend.py`: `_PG_LOAD_CORE_MESSAGES` 에 `sender_account_id` 추가. `src/agent_core.py`: `_load_conversation_messages`/`_assemble_core_messages`/`_format_core_messages` 에 `sender_labels` 파라미터 플러밍(PG tuple idx 5·MySQL SELECT 컬럼 추가) → 그룹대화일 때 user content 앞에 `[발신자]: ` 라벨 부착(병합 **전** → 연속 user 병합 후에도 발신자 보존). `_resolve_group_sender_labels(mem_conn, cid)` 신설(멤버≥2 일 때만 PG conversation_members→MySQL `WebAccounts.Username` 매핑, 아니면 None=무회귀). `_GROUP_CONVERSATION_GUIDANCE` 상수 신설 + 그룹일 때만 주입(다자 대화 발신자 구분·멘션 직전 맥락 능동 해석·과도 재질문 억제·데이터소스 드리프트 방지·중단 후 맥락 이어가기). 현재 라이브 멘션 메시지도 그룹이면 라벨(저장·UI 는 원문 유지, LLM 전달용 in-memory 만).
  - `unit/feature-0009-group-conversation/docs/*`: MODIFY/TASK/TEST/REVIEW.
- Impact: 비파괴. RC-1 은 MySQL 단일 DS·MSSQL-only 무회귀(엔진 분기 else 신설, base 가 이미 MySQL 가정). RC-2 는 1:1·비그룹(`sender_labels=None`) 바이트 동일 무회귀, 라벨은 LLM 컨텍스트에만(저장/UI 미반영). sql_guard 로직 무변경(거부 메시지 문자열만 append). 발신자명 소스 `WebAccounts.Username` 은 `[A-Za-z0-9_.-]{3,64}` allowlist(전 가입 경로 sanitize) → 라벨 prompt-injection/spoofing 불가(보안 패널 SHIP).
- Rollback Notes: agent_core 의 2 상수 + 엔진 else 분기 + sender_labels 플러밍 + `_resolve_group_sender_labels`/live-label, tools `_dialect_correction_hint` + 호출, runtime_backend SELECT 컬럼 1줄 revert. 완전 가역, 데이터 무손실(스키마 변경 0 — sender_account_id 는 기존 컬럼).
- 검증: `py_compile`+`ruff` PASS. 신규 단위 테스트 6(`tests/test_gc_dialect_context.py`: 그룹 라벨 부착·병합 보존 / 1:1 무라벨 회귀 / 미상 발신자 skip / dialect 힌트 mysql·mssql·clean) + 관련 회귀 42(`test_db_query_ux`·`test_runtime_read_backend`) = **48 PASS**. §18.8 적대 패널 2렌즈(회귀·정확성 / 보안·prompt-injection) **SHIP-WITH-FIXES**(BLOCKING 0; 멀티DS dialect 스코프 + 테스트 부재 2건 반영 완료) — REV-20260625T202843 참조. **PB-0008 미실측**(배포 후 라이브 그룹 대화 권장 — MySQL 제품에서 @assistant 가 TOP/UNION 없이 LIMIT/단일SELECT 생성하는지, 다자 대화 맥락 재질문 감소 확인).

## CHG-20260625T121352-gc-join-ambiguous-param-fix (cross-feature → feature-0002, Minor §12.3 — 공유 대화 join 불가: PG named-param 타입 모순 수정)
- Date: 2026-06-25. 코드 정본=feature-0002-agent-core(`src/modules/group_members.py`) + 진단 로깅 feature-0003(`src/app.py`) — feature-0009 cross-feature 추적(FUNCTION.md §13 사전 승인). 사용자 보고(`/_template:entry`): "공유 대화 기능으로 다른 사용자의 대화에 참여 시 '대화 참여에 실패했습니다' 메시지와 함께 참여 불가."
- Related: gc-unread-baseline(CHG-20260625T165320 — `_PG_ADD_MEMBER` 에 last_read 서브쿼리 추가)의 **회귀**. join 경로(`POST /api/share/{token}/join` → `group_members.add_member`)와 `_ensure_owner_membership` 양쪽이 동일 INSERT 사용 → 둘 다 실패.
- **근본 원인(서버 PG)**: `_PG_ADD_MEMBER` SQL 이 named param `%(conversation_id)s` 를 ① INSERT VALUES 의 `conversation_members.conversation_id`(varchar(128)) 컬럼과 ② 신규 서브쿼리 `(SELECT MAX(id) FROM core_messages WHERE conversation_id = %(conversation_id)s)` 의 `=` 비교 — **두 곳에서 재사용**. psycopg3 는 동일 이름 param 을 같은 `$1` 로 합쳐 보내는데, INSERT 대상은 varchar 로, 서브쿼리 `=` 비교는 (varchar 비교가 text 연산자를 거쳐) text 로 추론되어 `AmbiguousParameter: inconsistent types deduced for parameter $1 — text versus character varying` → INSERT 전체 실패 → add_member 예외 → join 핸들러 `except: return _json_error("대화 참여에 실패했습니다.", 500)`. (라이브 web 로그: `_ensure_owner_membership failed` + 동일 AmbiguousParameter 6건 실증. 컨테이너 재현 PASS.)
- 수정:
  - **(서버 PG)** `group_members.py`: 서브쿼리 비교에 **별도 named param** `%(conversation_id_lookup)s` 사용(값은 동일 conversation_id, 다른 $N → 각 위치 독립 타입 추론 → 충돌 소거). `add_member()` 파라미터 dict 에 `conversation_id_lookup` 키 추가.
  - **(진단 가시성, 2차 결함)** `app.py` join 핸들러: add_member 실패 시 silent 500 이던 `except` 에 `logging.warning(..., exc_info=True)` 추가(바로 위 ban 체크 블록과 동일 정책). 근본원인 진단 지연 방지.
- Impact: 비파괴 — SQL 의미·인가(멤버십 게이트)·스키마·엔드포인트 계약 무변경, 순수 param 바인딩 수정. `_ensure_owner_membership`(owner 자가치유)도 동일 INSERT 라 동반 복구. 1:1·비그룹·기존 멤버 무영향. 신규 권한 0.
- Rollback Notes: 서브쿼리 param 을 `%(conversation_id)s` 로 환원 + add_member dict 키 제거 + app.py 로깅 1블록 제거. 완전 가역, 데이터 무손실(스키마 변경 0).
- 검증: 컨테이너(`repo-web-1`) 재현 — 수정 전 `add_member` → AmbiguousParameter, 수정 SQL → INSERT 성공(last_read_message_id baseline 정상 set) + cleanup. 배포(이미지 재빌드) 후 라이브 공유 링크 join 200 + web 로그 `_ensure_owner_membership failed` 소거 확인 필요.

## CHG-20260625T225851-gc-unread-read-idspace-fix (cross-feature → feature-0003, Major §12.3 — 읽음 커서 id-space 불일치: 읽어도 unread 배지 미감소·전환 시 회귀)
- Date: 2026-06-25. 코드 정본=feature-0003-agent-web-ui(`src/app.py`). 사용자 4차 재보고(resume): "대화 진입 직후엔 회색 배지가 사라지지만, 다른 대화로 전환하면(새로고침 없이도) 곧바로 회귀. 읽음 처리가 진행 안 됨." (gc-unread-read-500-fix 배포 후에도 잔존.)
- Related: gc-unread-badge(읽음 API 신설)·baseline·read-fix·read-500-fix 의 **최종 미해결 근본 원인**. 앞선 5건이 frontend 호출/서버 import/baseline 만 건드려 이 id-space 분리를 못 봄.
- **근본 원인(다각 조사 워크플로 + 라이브 DB 확정)**: 읽음 커서(`conversation_members.last_read_message_id`)와 unread 집계는 **`agent_runtime.core_messages.id` 공간**(대화 `20260625063340-4220125d` 는 3369~3655)을 기준으로 비교한다. 그러나 frontend `_latestLoadedMessageId()` 가 읽음 처리로 보내던 `last_read_message_id` 는 `/api/history`(`AGENT_RUNTIME_READ_BACKEND=postgres` → `_get_history`)가 채운 **표시 store `agent_runtime.messages`**(동일 대화 750~845)의 id 다. 두 테이블은 별개 base table 로 id 공간이 **완전히 분리(disjoint)** — FE 가 보내는 값(≤845)이 항상 커서(backfill 3466)보다 작다. 핸들러가 그 값을 그대로 `set_last_read` 에 넘기면 `GREATEST(COALESCE(3466,0), 845)=3466` → **영구 no-op**. 멤버 행은 매치되어 updated=1·200 OK 처럼 보이지만 커서 값은 불변(읽어도 미전진). 핸들러의 정답 폴백(`MAX(core_messages.id)=3655` 전진)은 `requested<=0` 일 때만 실행 → FE 가 항상 양수를 보내 발동 안 함. (라이브: cursor 3466 → unread 28, cursor 3655 → 0. 화면엔 진입 시 클라 로컬 0, 전환 후 7s 폴링이 서버 unread 28 로 회귀.)
- 수정(서버 `src/app.py` `mark_conversation_read`):
  - 요청 body 의 `last_read_message_id` 파싱(`data=await request.json()` + `requested=int(...)`) **제거**.
  - `target_id = requested; if target_id is None or <=0:` 조건부 폴백을 제거하고 **항상** `SELECT COALESCE(MAX(id),0) FROM agent_runtime.core_messages WHERE conversation_id=%s` 로 target_id 산출 후 `set_last_read` 호출. 즉 대화 열람=현재까지 전부 읽음. docstring 갱신.
  - `group_members.set_last_read` 의 GREATEST(되돌림 방지)는 **변경 안 함**(폴링/재진입 경합 단조 보장 — 의도된 정상).
- 회귀 가드: `tests/test_read_endpoint_pg_import.py` 에 `test_read_handler_ignores_client_id_uses_core_messages_max` 추가 — 핸들러 소스에 `target_id = requested`/`data.get("last_read_message_id")` 부재 + `MAX(id)` core_messages 무조건 사용 + set_last_read 호출을 정적 보장(client id 신뢰 회귀 차단).
- Impact: 서버 읽음 커서 전진 복구. 인가(멤버십 게이트)·스키마·엔드포인트 계약·frontend·unread SQL 무변경. 1:1·비그룹은 FE `isGroupConversation` 게이트 + 서버 멤버 행 게이트로 호출 자체 없음/no-op. 커서 MAX(id)(필터 없음) vs unread 필터(role IN user/assistant·tool_calls null·content<>'') 비대칭은 안전 — MAX 보다 큰 counted 메시지는 정의상 없어 over-count 불가(커서가 약간 앞설 뿐 절대 뒤처지지 않음). 기존 고착 커서는 일괄 backfill 하지 않음(안 읽은 대화 오처리 위험) — 사용자가 각 대화 열람 시 lazy 전진.
- Rollback Notes: 핸들러를 requested 신뢰 분기로 환원 + 테스트 제거. 완전 가역, 데이터 무손실.
- 검증: `py_compile`(app.py·test) PASS + 핸들러 소스 계약 4/4 PASS + 라이브 DB 재현(cursor 3655 → unread 0). §18.8 적대 패널 1렌즈(general-purpose) **SHIP, BLOCKING 0**(Q1 증상제거·Q2 1:1안전·Q3 부분읽음 구조적 불가·Q4 경합정상·Q5 MySQL write경로 선행부재·Q6 backfill미실행 타당·Q7 MAX/필터 비대칭 안전 — 전수 라이브 검증). **배포 후 web 로그 read 200 + 커서 conv_max 전진 + 전환·폴링 후 미회귀 실증 필요**. NIT(REVIEW 기록): MySQL backend 의 last_read write 경로 부재(선행 결함, 현 PG 배포 무관).

## CHG-20260703T182740-gc-join-notice (cross-feature → feature-0003 + feature-0002, Major §12.3 — 공유 링크 참여 시 대화 내 '참여 알림' 이벤트)
- Date: 2026-07-03. worktree `ai/claude/feature-0009-join-notice`(중단 세션 7d92a878 resume — session limit 으로 문서 갱신 직전 중단). 코드 정본=feature-0003-agent-web-ui(`src/app.py`·`src/routers/share.py`·`src/static/{app.js,index.html,styles.css}`) + feature-0002-agent-core(`src/agent_core.py`·`src/modules/runtime_backend.py`) — cross-feature(FUNCTION.md §13 사전 승인).
- Reason: 공유 링크로 새 멤버가 참여해도 대화 안에 아무 표식이 없어 기존 멤버가 참여 사실을 인지하지 못함. Slack/Discord "X joined" 패턴의 참여 알림을 신설(ANCHOR §3 "이씨는 나중에 들어와 unread 로 못 본 부분만 따라잡는다" 시나리오 보강).
- 변경:
  - (feature-0003 `routers/share.py`) `join_conversation_via_share` 의 `if not already:` 블록에서 add_member·audit 직후 `app._save_group_join_event_pg(cid, actor_id, account.username)` **best-effort** 호출 — 실패해도 join(멤버 이미 추가됨) 무효화 없음, `if not already:` 안이라 재참여 시 스킵→중복 알림 없음.
  - (feature-0003 `app.py`) `_save_group_join_event_pg` 신설 — **이중 기록**: ① core_messages(role=user, `name=EVENT_MESSAGE_NAME`, sender=가입자) → unread 배지 집계(`role IN ('user','assistant')` + `sender IS DISTINCT FROM self`)에서 **가입자 제외·기존 멤버 +1**, ② 표시 store messages(role=system, meta `event_type='member_joined'`) → 프론트 가운데 정렬 pill 렌더. + `_share_load_messages` 에 event_type 가드(**anonymous 공유 스냅샷에는 멤버 username 비노출**).
  - (feature-0002 `runtime_backend.py`) `EVENT_MESSAGE_NAME='__event__'` sentinel 상수 신설 — writer(feature-0003)·filter(feature-0002)가 공유하는 단일 소스(magic-string drift 방지).
  - (feature-0002 `agent_core.py`) `_normalize_history_rows` 최상단에서 `name==EVENT_MESSAGE_NAME` 행 배제 → 이벤트가 LLM 대화 히스토리(윈도우·kept_users·발신자 라벨·연속 user 병합) 어디에도 주입되지 않음.
  - (feature-0003 static) `app.js` `renderMessages` event pill 분기(`textContent` XSS-safe) + `styles.css` `.message-event(-pill/-time)`(다크모드 포함) + `index.html` 캐시버스터 `styles.css`/`app.js` → `20260703-gc-join-notice`.
- **§18.8 BLOCKING 수정(resume 재검증)**: 최초 구현은 join 이벤트를 core_messages(role=user)로만 기록해 LLM 히스토리에 `[Alice]: Alice님이 대화에 참여했습니다.` user 턴으로 주입 → assistant 오응답(환영 인사)/맥락 오염(적대 패널 BLOCKING #1). `name` sentinel + `_normalize_history_rows` 배제로 **unread 집계는 유지(name 미참조)하면서 LLM 오염만 차단**. 나머지 6모드는 clean/NIT(#2 unread 정확·#5 best-effort·#6 XSS·#7 멱등 clean; #3 anon-leak 가드됨·#4 core-fallback 희귀 표현·#7 동시 double-submit NIT 수용).
- Impact: 비파괴 additive. unread SQL·인가·스키마 무변경(`name` 은 기존 core_messages 컬럼). 1:1·비그룹 무회귀(이벤트는 그룹 공유 join 경로에서만 생성, 히스토리 배제도 sentinel 한정). anonymous 공유뷰 멤버 username 비노출. 현 배포 PG 기준(이벤트는 PG-only write).
- Rollback Notes: share.py 호출 1블록 + app.py `_save_group_join_event_pg`·guard + runtime_backend 상수 + agent_core 배제 블록 + static(app.js/css/html 캐시버스터) + 테스트 revert. 완전 가역, 데이터 무손실(스키마 0).
- 검증: `py_compile`(agent_core·runtime_backend·app.py·share.py) + `node --check`(app.js) + agent 컨테이너 pytest **36 PASS**(신규 `tests/test_gc_join_event_history.py` 5: normalize 배제 / 그룹 파이프라인 미주입 / content 아닌 name-sentinel 매칭(무고한 동일 문장 보존) / kept_users 슬롯 미잠식 — + 회귀 `test_gc_dialect_context`·`test_history_tooluse_sanitize`·`test_group_history_merge`·`test_group_members`). §18.8 적대 패널 1렌즈(general-purpose) — BLOCKING #1 적발→수정 재검증 통과. **PB-0008 미실측**(배포 후 라이브 그룹 공유 join → pill·기존 멤버 unread +1·@assistant 오응답 없음 실측 권장).

## CHG-20260704T130000-share-visibility-window (cross-feature → feature-0002 + feature-0003, Critical §12.3 — [from,to] 공유창 격리 + 말풍선 ☰ 통합)
- Date: 2026-07-04. worktree `ai/claude/share-visibility-window`. 코드 정본 = feature-0002-agent-core(`agent_core.py`·`modules/{group_members,runtime_backend}.py`·`alembic/versions/20260704_0036_*`·`scripts/agent_runtime_schema.sql`) + feature-0003-agent-web-ui(`app.py`·`routers/{conversations,share}.py`·`static/{app.js,styles.css}`) — cross-feature(FUNCTION.md §13 사전 승인). 사용자 요청 `/_template:entry`.
...
- 변경:
  ...
  - (feature-0003 `app.py`) `_conv_message_created_at`(join bridge)·`_member_visibility_window`·`_conversation_has_restricted_members`·`_resolve_display_window`·`_msg_outside_window`·`_resolve_copy_window`·`_attachment_outside_window`; `_conv_load_messages_raw(from_id)`·`_conv_load_core_messages_raw(from_created_at)` 하단경계; `_get_history(window)`(양 loader + core-fallback skip + DENY); `_fork_conversation_impl(share_floor_id)` 3-store 교집합 clip; `_share_load_messages(floor_message_id)`; `WebConversationShares.FloorMessageId` 컬럼+helper.
  - (feature-0003 `routers/conversations.py`) `/api/history` window 해석+DENY; `create_conversation_share` scope_mode='windowed'+floor_message_id+검증+widen-guard(bounded 재공유 403)+INSERT FloorMessageId; sample-feedback pre-floor probe 게이트(404).
  - (feature-0003 `routers/share.py`) join 이 공유 window 해석(경계 created_at 미해석 시 join 거부)→stamp_member_visibility(신규/재참여); public_share_fork share_floor_id 전달; public_share_view floor 전달.
  - (feature-0003 static) `app.js` kebab 일반화(closeFloatingMenus/makeMenuItem/openFloatingMenu)+말풍선 ☰(openMessageBubbleMenu)+여기부터 범위모드(beginShareFloor/onShareCeiling/renderShareRangeBanner)+`_issueConversationShare` windowed body; `styles.css` `.message-menu-trigger`·`.is-share-floor`·`.share-range-banner`(라이트·다크).
- **AR-1/AR-2 반전(windowed 한정)**: joinable windowed/anchored 링크는 참여 멤버 열람도 `[from,to]` 제약(AR-1 반전), bounded/windowed fork 는 `[from,to]` 만 복제(AR-2/F3 반전). full 공유는 종전 전체 열람/복제 그대로(무회귀). SECURITY.md §21 + ANCHOR §4 주석 정본.
