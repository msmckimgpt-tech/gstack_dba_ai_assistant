...
## 18. 구조 변동 감지 시 AI 자동 재귀 분석 (2026-07-27, TASK change-reanalysis)

**동작**: 사용자가 그래프 뷰에서 **'DB 전체 AI 능동 분석'을 한 번 이상 완료한 DB(스키마)** 에 한해,
insight-worker 가 그 DB 의 구조 변동 — 테이블 신규, 테이블의 **컬럼 구성 변경**(컬럼 지문), 프로시저·함수
신규 또는 **정의 변경**(definition_hash) — 을 감지하면, 사용자가 그래프 뷰에서 분석을 다시 실행하지
않아도 **변경 노드를 시드로 하는 AI 재귀 분석 run 이 자동 생성**된다. 시드 규약은 수동 DB 전체 분석과
동일(§55: `depth=0` + `anchor_key`=자기 자신 = per-seed 앵커)이라 변경 노드에서 **관련 노드까지 재귀
전개**되고, 기존 분석문은 refine-not-override 로 갱신된다. 노드 마커(분석중/분석됨)는 기존 집계 경로를
그대로 타므로 그래프 뷰에서 자동 분석 진행도 눈에 보인다.

**자동 run 의 식별**: `node_analysis_runs.root_key = <schema_key>#auto` · `root_label='SchemaAuto'` ·
`requested_by='auto:insight-change'`. 사용자의 수동 run(`root_key=<schema_key>`, `root_label='Schema'`)과
분리돼, 수동 '전체 분석'의 진행-중 dedup·planned 수치에 간섭하지 않는다.

**비용 경계 (자동 LLM 호출 — 사람 confirm 게이트 없음)**:

| 가드 | 내용 |
|---|---|
| 자격 | 그 스키마에 사용자 run(`root_label='Schema'`) 이 `status='done'` **이면서 성공이 과반**(`done*2 >= enqueued`)인 이력이 있어야만 발동. 자동 run 은 자격 판정에서 제외(순환 차단), scope·root_key 모두 원형·소문자 매칭(.env 레거시 라벨). 미분석 DB 는 종전대로 사용자가 명시 실행. 미자격 스키마는 스냅샷도 적재하지 않는다. |
| 동일 구조 샤드 | 날짜/번호 샤드(`daily_..._20260727`)처럼 **직전 샤드와 구조가 같은** 신규 테이블은 자동 분석 대상이 아니다 — 이 제품은 2026-07-03 사용자 결정으로 "동일 구조 샤드는 대표 1개만 LLM, 형제는 상속"을 이미 확정했고, 자동 경로가 이름만 보고 이를 되돌리면 샤드가 매일 생기므로 수렴하지 않는다. 그룹 서명 `(base_stem, 컬럼지문)`이 기존 형제와 같으면 스냅샷에만 반영하고 시드하지 않는다(기존 테이블의 실제 구조 변경은 그룹과 무관하게 항상 분석). |
| 진행 중 분석 | 같은 DB 에 **사용자 수동 run 또는 자동 run 이 돌고 있으면** 그 사이클은 아무것도 하지 않는다 — 사용자가 방금 승인해 도는 분석 위에 승인 없는 두 번째 분석을 겹치면 같은 노드가 이중 지출되고 refine 세대가 경합한다. |
| 시드 상한 / 정지 스위치 | `AGENT_NODE_ANALYSIS_AUTO_CHANGE_CAP` — 관리 콘솔에서 **재배포 없이** 바꾸는 3-state. `>0`(기본 50) = 1회 시드 상한, 초과분은 스냅샷 미갱신 → 다음 사이클 이어서 처리. **`0` = 완전 정지** — 신규 트리거 차단 + **이미 큐잉된 자동 잡의 소비도 보류**(삭제가 아니라 보류라 값을 되돌리면 그대로 재개). `-1` = shadow(계측만). |
| 재귀 예산 | `AGENT_NODE_ANALYSIS_AUTO_EXPAND_FACTOR`(기본 4 — 수동 12 보다 보수적) × 시드, `SCHEMA_RUN_BUDGET_MAX` 캡. `enqueued` 는 run INSERT 시점 확정값(증분 회계와 경합 없음). |
| 쿨다운 | `AGENT_NODE_ANALYSIS_AUTO_CHANGE_COOLDOWN_SEC`(기본 1800). 진행 중 자동 run 이 있으면 **그 사이클은 아무것도 하지 않는다**(append 금지 — run 이 계속 'running' 이면 쿨다운이 영구 미발동하고 사이클마다 cap 만큼 시드가 유입돼 예산이 ratchet 된다). 변경 누락은 스냅샷 미갱신으로 다음 사이클에 재시도되므로 0. |
| 구조 스냅샷 | 스키마당 KV 1건(`na_struct_snap:<hash>`)에 `{테이블: 컬럼지문, 루틴: 정의해시}` 전량을 담아 대조한다. insight 지문(`table_fp`)은 artifact 발행 성공분만·스캔당 12개씩 채워져 커버리지가 희소하므로, 그 부재를 '신규'로 읽으면 이미 분석된 DB 의 테이블 대부분이 오탐된다. **시드된 노드만** 전진하므로 AGE 미투영·cap 절단·쿨다운으로 빠진 노드는 자연 재시도. |
| baseline / 관측 축 | 스냅샷이 없는 첫 관측은 **baseline 확립만**(후보 0). 이번 사이클에 관측하지 못한 축(루틴 introspect 미발화·cap 절단·MSSQL·테이블 지문 조회 실패)은 대조도 갱신도 하지 않고 보존하며, 그 축이 처음 켜지는 사이클도 축 baseline 확립만 한다 — "관측 공백 → 전량 신규 오탐" 봉인. 스냅샷 키는 실 스키마까지 분리해 MSSQL 의 DB명 라벨 공유로 인한 전량 진동을 막는다. |
| 관측 실패 방어 | 축이 켜져 있어도 **직전 스냅샷 대비 전량 소실**(또는 5건 이상 & 과반 소실)이면 그 사이클은 '관측 실패'로 보고 축을 보존한다. `information_schema` 는 권한 필터 결과라 계정 교체·GRANT 축소·복원(DROP→CREATE→import) 창에서 예외 없이 0행을 돌려주는데, 그 한 사이클을 '전부 삭제'로 읽으면 스냅샷이 비고 복귀 사이클에 **무변경 테이블 전량이 신규로 폭발**한다. 소규모 삭제는 정상 반영된다. |
| 스위치 | `AGENT_NODE_ANALYSIS_AUTO_ON_CHANGE`(기본 1 = ON, 사용자 결정 2026-07-27). `0` = 완전 비활성, `shadow` = 후보 산출·계측만 하고 enqueue·스냅샷 전진 없음(배포 직후 실제 규모를 **비용 0** 으로 관측). |
| 관측성 | insight-worker 로그 payload 에 `auto_reanalysis_{candidates,seeded,runs,blocked}` 카운터. 무발동(ineligible·cooldown·busy·미투영)도 `blocked` 로 집계되고, 스키마별 status 변화 시 `auto_reanalysis schema=… status=…` info 로그가 남아 "왜 안 도는가"를 진단할 수 있다. |

**Out of scope / 알려진 한계**:
- 삭제된 테이블·루틴은 분석 대상 노드가 사라지므로 트리거하지 않는다(그래프 prune·재클러스터가 담당 —
  스냅샷에서는 즉시 제거). AGE 그래프에 아직 투영되지 않은 신규 객체는 그 사이클에 시드하지 않고,
  30분 sync 후 다음 감지 사이클에 자연 편입된다.
- **MSSQL 은 루틴 축을 사용하지 않는다** — 저장 라벨이 DB(catalog)명이라 한 라벨에 복수 실 스키마의
  루틴이 섞이고 prune 도 꺼져 있어 삭제 반영이 불가하다. 테이블 축은 동작하지만, 같은 이유로 **노드 key
  가 다대일로 접힌다**(`dbo.orders` 와 `staging.orders` 가 모두 `ds:salesdb.orders`) — 이는 그래프 투영
  자체의 기존 규약이며(ADR-016 계열), 자동 재분석도 그 규약을 따르므로 한쪽 스키마의 변경이 접힌 노드를
  재분석할 수 있다. 대조 축(스냅샷)은 실 스키마로 분리돼 있어 매 사이클 진동은 없다.
- **자격은 "전체 분석 완료"가 아니라 "과반 성공한 사용자 run 1건 이상"이다** — 수동 경로가
  `only_missing`+`SCHEMA_CAP` 으로 일부만 분석한 run 도 자격을 만들 수 있다(예: 2,000 테이블 DB 에서
  200개만 분석). 커버리지 비율 기준(`get_scope_analysis_status().done_keys`)으로의 강화와 스키마별
  opt-out 은 후속(TASK.md TCR.12).
- 자동 run 의 목록·취소 관리 엔드포인트는 아직 없다. 급할 때의 정지 수단은 위 CAP=0(신규 차단 + 대기
  잡 보류)이며, 이미 **실행 중인** 잡 1건은 끝까지 진행된다.

## 19. '보기 옵션 > 이웃 깊이' 투영 예산 (2026-07-28, TASK graph-hop-budget)

**요구**: 이웃 깊이(1/2/3-hop) 선택이 **실제로 다른 결과**를 내야 하고, 조회 상한 경고가 **잘리지 않은 목록에 붙지 않아야** 한다. 사용자 리포트: "1-hop 을 초과한 모든 항목에서 '이웃 조회 상한 - 일부만 불러옴' 이 출력된다." 정본 명세·AC 는 코드가 거주하는 feature-0003 `FUNCTION.md` REQ-20260728-graph-hop-budget / AC-HB-1~8 이며, 아래는 그래프 투영 계약 요약이다.

| 축 | 계약 |
|---|---|
| hop 별 확장 범위 | hop 1 = 전 엣지 라벨(앵커의 컬럼·소속 스키마·직결 루틴 = 상세 패널 원천). **hop ≥ 2 = 관계 엣지만**(`REFERENCES`·`ROUTINE_USES`·`RELATED_TERM`·`USES`·`DESCRIBES`). 계층 엣지(`HAS_*`)는 "같은 컨테이너에 있다" 는 관계라 2-hop 확장에서 제외 — 형제 수백 개가 노드 상한(300)을 소진해 정작 참조로 이어지는 노드를 탈락시켰다. 형제 목록이 필요한 화면은 `scope_schemas`/`schema_tables` 경로가 담당한다. |
| 절단 우선순위 | ① 관계 엣지로 도달한 이웃 → ② 라벨 의미 우선순위(Table > Column > Routine > GlossaryTerm > Schema > Datasource > Product) → ③ graphid tie-break. 같은 앵커·같은 depth 는 **항상 같은 부분집합**을 낸다. 종전엔 vertex 라벨 알파벳 순 fetch 순서라 Table 이 가장 먼저 탈락했다. |
| 렌더 정합 | `REFERENCES` 는 Column↔Column 이므로, hop ≥ 2 에서 들어온 관계 Column 의 **소속 Table 을 `HAS_COLUMN` 역참조로 보강**한다(프론트 `colsByTable` 이 부모 없는 컬럼을 드롭). 보강된 부모는 **다음 프론티어에 넣지 않는다**(형제 재폭발 차단). 보강 몫은 예약 예산으로 확보한다 — 예약이 없으면 상한 상황에서 부모가 탈락해 이 보강 자체가 무효화된다(§18.8 적대검증 R1-P1). |
| 파단(broken) 엣지 | 표시되지 않는 엣지는 **관계 우선권도, 부모 보강 자격도 갖지 않고**, SQL 정렬에서 상한 적용 *이전* 에 후순위로 밀린다 — sync 창 사이의 stale 행이 예산을 먹고 유효 이웃을 가리는 것을 막는다. |
| 절단 신고 | `truncated`(기존 계약) + `truncated_hop`(1-based) + `omitted_nodes`(하한). 노드 상한 도달뿐 아니라 **엣지 fetch 상한 포화**도 신고한다(정확히 상한 개수면 절단 아님 — `cap+1` fetch 로 판별). 프론트는 hop 1 절단이면 "아래 목록도 일부만", hop ≥ 2 면 "아래 직접 연결 목록은 전량" 으로 분기하고, hop 미상(구 응답)이면 완전성을 주장하지 않는다. |
| 깊이 실효 보고 | `expanded_hops`(hop 별 신규 노드 수) · `expanded_hop_edges`(hop 별 신규 엣지 수)를 additive 로 싣는다. 프론트는 2-hop 이후 실적이 **노드·엣지 모두 0 이고 절단도 없을 때만** "확장할 관계가 없어 1-hop 과 동일" 을 알린다. 두 필드가 없는 구 응답은 종전 판정으로 폴백(롤링 배포 무회귀). |

**Out of scope / 알려진 한계**:
- 노드 상한(300) 자체는 그대로다. 테이블 700개 규모 스키마 노드는 여전히 1-hop 부터 상한에 걸리며, 본 변경은 그 사실을 정직하게 표기하도록만 바꿨다(페이지네이션·클러스터 경로 유도는 별건 — REPORT §8).
- 3-hop 이 2-hop 과 같아지는 경우(실측 표본 20%)는 **관계 체인이 2단계에서 끝나는 데이터의 성질**이며 결함이 아니다 — 상태줄이 그 사실을 알린다.
- 부모 보강 예약량은 관계 Column *후보 수* 상한이라, 부모가 이미 모델에 있는 경우까지 한 칸을 잡는다(예산이 정확히 소진되는 경계에서 관계 끝점 1개 손실 가능, `truncated`/`omitted_nodes` 로 보고됨 — REPORT §8 후속).

## 그래프 sync 는 «회수»한다 — MERGE-only 를 보완 (2026-09-02)

**REQ-20260902-graph-sync-retract (Major §12.3)**: AGE 는 MERGE-only 라 원본이 사라져도 정점이
남는다. 2026-09-01 소급 정리로 `kb_glossary` 105행을 지웠는데 그 용어들이 그래프에 그대로
살아 있었다(`트랜잭션`·`복합 인덱스`·`정규화`·`멱등성`). 누적분 7,242+2,132 정점을 수동 회수해
78,362 → 68,993 으로 줄였지만, **회수 단계가 없으면 다음 삭제에서 그대로 재발한다** —
청소는 원인 제거가 아니다.

### AC

- **AC-20260902T140000-retract-1** — 전건 sync 는 투영 **뒤에** 회수 단계를 돈다.
  `kb_glossary` 에 없는 `GlossaryTerm` 정점을 `DETACH DELETE` 한다.
- **AC-20260902T140000-retract-2** — **증분(`since`) 실행에서는 회수하지 않는다.** 증분은
  「그 시각 이후 변경분」만 조회하므로, 그 결과에 없다는 것이 「DB 에 없다」를 뜻하지 않는다.
  증분에서 회수를 돌리면 **멀쩡한 용어를 전부 지운다**. 건너뛴 사실은
  `glossary_retract_skipped` 로 남긴다(무음 금지).
- **AC-20260902T140000-retract-3** — scope 를 지정한 실행은 **그 scope 안에서만** 회수한다.
  다른 scope 는 조회 범위 밖이라 판단 근거가 없다.
- **AC-20260902T140000-retract-4** — 살아있는 키는 투영과 **같은 생성기**(`_vkey`)로 만든다.
  규칙이 갈리면 전건이 stale 로 보여 그래프를 통째로 비운다.
- **AC-20260902T140000-retract-5** — cypher 리터럴은 `_cq` 로 이스케이프한다(용어에 `'` 포함 가능).
- **AC-20260902T140000-retract-6** — `glossary_retracted` 를 **0 으로 초기화**해 「회수 0건」과
  「단계가 안 돌았다」를 구별한다. 이 값이 0 인 채 그래프 정점 수가 `kb_glossary` 를 넘어서면
  회수가 다시 멈춘 것이다.

### 남은 범위 (정직)

용어 축만 회수한다. Table·Column·Routine 등 물리 스키마 축의 회수는 **원본 판정이 다르다**
(운영 DB 를 실제로 조회해 「지금도 있는가」를 물어야 하고, 접속 실패 시 「없다」로 오판하면
멀쩡한 정점을 지운다). 그 축은 별 설계가 필요하다 — 이번 수동 회수분(A·B·C·D')은 각각
**중복이거나 원본 자체가 없는** 경우라 판정이 안전했다.
