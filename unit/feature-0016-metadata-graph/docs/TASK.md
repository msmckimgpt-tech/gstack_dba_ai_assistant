...
- [~] T78.3 Phase B — 어댑터 통합 (진행 중).
  ...
  - [ ] T78.3b **B-late: graph-core.js 배선** — `_metaInitGraph` 의 `new G6.Graph` 를 렌더러 seam(`_META_RENDERER`)으로 분기해 `PixiGraphAdapter` 선택 + `_metaG6Apply` 의 `setData`+`draw`→`setScene` 치환 + admin.html vendor(pixi.min.js)·cache-buster. 착수 직전 `git fetch` 로 그래프 변경 재확인 후 rebase(R6 원칙). 배선 후 279 스위트 회귀 재확인.
  - [ ] T78.3c 미니맵(RenderTexture)·드래그 자유배치 persistence·scene diff 오브젝트 풀·BitmapText 한글 atlas — 후속 §(GPU 상주로 §61/65/67 LOD·컬링 단순화 여지 실측 포함).
