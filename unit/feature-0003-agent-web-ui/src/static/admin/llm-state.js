...
//
// 구현은 `static/console-job-poll.js` 로 옮겼다 (TASK-20260909T000000-prompt-autogen-delivery).
// 이 모듈은 `admin.js` 의 `adminState` 에 의존하므로, 여기 두면 **관리 콘솔 밖에서는 쓸 수
// 없다** — 프로필의 '내 프롬프트 > 자동 작성' 도 같은 위임 응답을 받는데 그것을 해석할 수단이
// 없어 결과가 화면에 도달하지 않았다. 폴링 자체는 admin 상태를 하나도 읽지 않으므로 옮기는
// 데 제약이 없었다.
//
// 기존 import 경로(`admin/llm-state.js` 에서 가져가던 호출부)를 깨지 않도록 여기서 그대로
// 다시 내보낸다 — 이름·시그니처 동일.
export { awaitDelegatedResult, jobPhaseLabel, looksDelegatedEnvelope } from "../console-job-poll.js?v=dev";
