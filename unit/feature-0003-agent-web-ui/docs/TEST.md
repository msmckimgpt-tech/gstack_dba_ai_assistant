...
- **POST-DEPLOY 잔여**: 격리 주입본이 아닌 **실제 배포 자산**에서 ②~⑥ 1-probe 재확인 + P2 수정이 새로 만든 두 표면(⑦ 배지-단독 억제 구간 마커 노출 ⑧ 반포인트 밴드 경계 rebuild 로 10.5px 헤더 실제 소거) 확인(TL.10). **PRE-LANDING 실촬은 P2 수정 이전 코드** — 억제 판정은 무변경이라 ②③⑤⑥ 은 계약상 성립하나 실촬 재확인은 POST-DEPLOY 몫이다. deploy_scope: included — merge 후 자동 배포.
