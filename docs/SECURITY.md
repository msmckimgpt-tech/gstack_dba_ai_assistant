...
| L2 | 반환 블록마다 `⟦UNTRUSTED-DATA account=… task=…⟧` + `[SCOPE]` 1줄 재진술 | 유도 + 탐지 기반 |
...
1. **각인 방향이 반대다.** 나가는 도구 결과는 `wrap_tool_output` 으로 *외부 AI 에게* "계정 X 전용
   데이터" 라 표시하고, 들어오는 답변은 `wrap_external_answer` 로 *우리 LLM 에게* "**통제 밖 런타임이
...
| 나가는 질문 본문 | `wrap_tool_output` 각인(§44 L2 와 동일 규약). 우리 사용자가 쓴 텍스트라도 **외부 AI 컨텍스트로 나가는 데이터**이므로 구획을 유지한다 |
| 들어오는 답변 | 기존 `submit_answer` 계약 그대로 — 인젝션 3단 판정 → 각인 → 저장 → 원장. 저장본은 각인본(지연 인젝션 방어), 화면 전달본은 원문 |
...
| 대화 문맥 | `claim_request` 가 함께 주는 이전 turn(최근 6, 4000자)도 나가는 데이터이므로 같은 각인 규약을 탄다. 절단 시 그 사실을 본문에 명시한다(조용한 절단 금지) |
...
- **답변 품질·검증 통제권 상실**: 추론이 통제 밖 런타임에서 일어나므로 red-team 자가리뷰(feature-0021)
  ...
  보증 대상이 아니다**.
- **미처리 방치**: `claim` 후 제출하지 않아도 **30분 lease 만료 시 대기열로 복귀**한다. 다만 그 30분
  동안은 사용자에게 아무 신호가 없다 — 진행 알림은 후속 항목이다.
