...
안전한 상태에서는 현재 대화·모델·저장된 첨부 선택·메시지 위치·계보 비교 쌍과 줄 위치를 복원하며 화면을 자동 갱신한다. 초안, 새 첨부/업로드, 다른 대화를 포함한 진행 요청, 미저장 편집, IME 조합 및 드래그 중에는 안내 후 안전해질 때까지 기다린다. 본문·파일 내용·자격증명은 복원 저장소에 적재하지 않는다. 자동 재로드는 같은 대상에 5분 내 2회로 제한한다.
