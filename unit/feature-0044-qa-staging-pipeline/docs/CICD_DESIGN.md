...
# QA 머신 CI/CD 설계 제안서 (rev.2 — 확정 전제 반영)
...
#### 3.4 ⚠ SVN 에 이미지 tar 를 넣을 때의 용량 문제 (반드시 결정 필요)

**SVN 은 파일을 삭제해도 리비전 이력에 blob 이 남아 저장소 용량이 줄지 않는다.** 이미지
tar 가 릴리즈당 수백 MB~수 GB 라면 저장소가 단조 증가하고, `svn checkout` 이 갈수록
무거워진다. `svnadmin dump | svndumpfilter` 로 잘라낼 수는 있으나 저장소 재구축(모든
클라이언트 재체크아웃)이 필요한 파괴적 작업이다.

두 안:

| 안 | 구성 | 장점 | 단점 |
|---|---|---|---|
| **A (권장)** | SVN 에는 **manifest·env 암호문·스크립트(텍스트)만**. 이미지 tar 는 **사내 파일서버 경로**에 두고 매니페스트가 `url` + `sha256` 으로 가리킴 | SVN 저장소가 가볍게 유지됨. 오래된 tar 는 파일서버에서 그냥 지우면 됨 | 파일서버 경로 규약이 하나 더 필요 |
| **B** | SVN 에 tar 까지 전부 | 전달 채널이 하나 | 저장소 단조 증가. 보관 정책이 "지워도 안 줄어든다" 는 제약과 싸움 |

안 B 를 택한다면 **릴리즈 주기 × tar 크기 × 보관 기간**으로 저장소 증가량을 미리 계산하고
디스크를 그만큼 확보해야 한다(예: 주 1회 × 2GB × 1년 ≈ 100GB, 삭제해도 유지).

안 A 를 권장하되, 어느 쪽이든 **매니페스트가 신원의 정본**이라는 계약은 동일하므로 나중에
바꿔도 러너 로직은 그대로다.
...
| 항목 | 최소 | **권장** | 근거 |
|---|---|---|---|
| CPU | 8 vCPU | **16 vCPU** | 상시 소비 주체는 PG(AGE 그래프 쿼리)·MySQL·임베딩·워커 다중. 워커는 LLM 대기가 대부분이라 CPU 낮으나, 그래프 sync·분석 배치가 겹치면 순간 부하가 크다 |
| RAM | 32GB | **48GB** | `mem_limit` 명시 17.5GB + 미명시 9개(~6~10GB) ≈ 24~28GB. 여기에 **MySQL 65GB 데이터의 페이지 캐시** 여유를 더한다. 32GB 는 동작하지만 캐시 여유가 없어 조회 응답이 라이브와 달라진다 |
| 디스크 | 300GB | **500GB NVMe** | 데이터 90GB + 이미지·tar 보관(last-good 3세대 + 릴리즈 tar) 50~100GB + 로컬 백업 50GB + 증가 여유. `mysql-data` 가 단조 증가 중이므로 여유를 크게 |
| **GPU** | — | **6GB 이상 (권장 8GB+)** | **§6.3 참조 — 결정 변수** |
| 네트워크 | 1Gbps | 1Gbps | 이미지 tar(수 GB) 전송 · 데이터 이전 90GB |

### 6.3 ⚠ GPU — 스펙의 결정 변수

현행은 임베딩(`bge-m3`)이 **GPU 에 상주**한다(`OLLAMA_KEEP_ALIVE: -1` — 축출 없는 warm
보장이 그 컨테이너의 존재 이유로 명시돼 있다). QA/라이브 머신에 GPU 가 없으면 임베딩이
CPU 로 떨어지며, 다음이 발생한다:

- 임베딩 지연이 수 배~수십 배 증가한다.
- **타임아웃 cliff 위험** — 임베딩 타임아웃이 정상 지연 근처로 설정돼 있으면, 느려짐이
  "처리량 0"으로 바뀐다(완료 직전 폐기 → 재시도 폭주). 이 저장소에서 실제로 관측된 실패
  양상이다.
- KB 검색·분석 파이프라인 전반의 체감이 라이브와 달라져 **QA 결과를 신뢰할 수 없게 된다.**

따라서 세 선택지 중 하나를 정해야 한다:

| 선택 | 결과 |
|---|---|
| **A. QA·라이브 모두 GPU 탑재 (권장)** | 라이브 동등성 확보. GTX 1660 SUPER(6GB) 급이면 현행과 동일. 신규 구매라면 8GB+ 권장 |
| **B. 라이브만 GPU, QA 는 CPU** | 비용 절감. 단 **QA 에서 성능·타임아웃 관련 검증은 무효**임을 명시해야 하고, CPU 환경용 타임아웃 override 가 필요하다 |
| **C. 둘 다 CPU** | 임베딩 타임아웃 설정을 CPU 기준으로 전면 재조정해야 한다. KB 기능 체감이 현행과 달라진다 |

**권장은 A** — GPU 1장(6~8GB)은 하드웨어 비용 중 작은 항목인데, 없을 때 잃는 것(QA 신뢰도,
타임아웃 재조정 공수)이 크다.
...
추가로 **§3.4 SVN 용량 정책(안 A/B)** 과 **§6.3 GPU 선택(A/B/C)** 은 설계 판단이 아니라
비용·인프라 결정이므로 사용자 확인이 필요하다.
