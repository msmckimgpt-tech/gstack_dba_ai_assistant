...
- 2026-06-18 (TASK-20260618T044611-ai-claude-release-notes — 릴리즈 노트 작업 화면 탭 + 관리 콘솔 카테고리, **Major §12.3** · additive frontend):
  ...
  - **Environment: Windows-browser** — 잔여(배포 후 기록): 작업 화면 프로필 > 릴리즈 노트 탭 + 관리 콘솔 > 릴리즈 노트 카테고리 양쪽에서 일자 그룹·기본 접힘(최신만 펼침)·머리 클릭 토글·영역 필터 칩·모두 펼치기/접기 실측. 배포(main 머지 → `docker compose build web` + `up -d --no-deps web`) 후 본 항목을 PASS 로 갱신.
