...
화면 4축(서브탭·KPI·러너 명부·배너) 확인.

**이번 실측의 핵심**: `자가 검증` KPI 가 **배포 후에 나타났다**. 사전검증(격리 인스턴스,
마이그 0057 미적용)에서는 `available:false` 라 타일을 그리지 않았고, 배포로 컬럼이 생기자
나타났다 — 「원장 미준비」와 「0건」을 가르는 설계가 **두 환경에서 각각 다르게, 의도대로**
동작했다는 실증이다.

**⚠ 러너 중단은 이 세션이 만들었다**: 배포 후 러너 갱신 중 **토큰 보존 전에 프로세스를 먼저
종료**해 `mat_` 토큰이 소실됐다(서버는 해시만 보관 · `config.json` 미저장 · 프로세스 환경변수가
유일한 사본). 파일 교체는 성공(지문 일치, 구본 백업)했으나 재기동 불가 — 사용자 재연결이
필요하다. 영향 1건은 lease 30분 규약으로 자동 복귀하며 답변 유실은 없다.

절차 교훈: **토큰 확보 → 종료 → 재기동은 한 명령 안에서** 해야 한다(Bash 호출마다 셸이 새로
...
## CHG-20260904T060000-ai-claude-visual-verified — 미룬 PB-0008 시각검증 수행·기록
- Timestamp: 2026-09-04T06:00:00+09:00
- 코드 변경 없음. TASK-20260904T033000 이 「배포 후 수행」으로 미뤄 둔 시각검증을 실제로
  하고 결과를 `test-runs.d` fragment 에 덧붙였다(5항목 전부 확인).
...
## CHG-20260907T030000-ai-claude-session-cookie-lifetime — 로그인이 앱 재실행을 견딘다
- Timestamp: 2026-09-07T03:00:00+09:00
- **사용자 제보의 진짜 원인**(실측 확정): 세션 쿠키가 비영속이라 앱 창을 닫으면 사라진다.
  영구 쿠키 저장소 0건 · 새 창에서 `logged_in: false` · 칩 `hidden`.
- `_set_session_cookie` 에 `max_age = AUTH_SESSION_MAX_DAYS(기본 90일)`.
  ⚠ 슬라이딩 창(14일)이 아니라 **절대 상한** — 슬라이딩을 주면 쿠키가 제한 요인이 되어
  매일 쓰는 사용자가 14일째 로그아웃된다(서버가 이미 고친 증상의 재발).
- 권한은 서버가 그대로 강제한다. 다른 방어는 불변이고 테스트가 잠근다.
- 뮤테이션 **5/5 KILL**(수명 제거 · 수명=슬라이딩 · HttpOnly 제거 · Secure 끔 · SameSite=none).


## CHG-20260907T040000-ai-claude-cookie-postdeploy — 세션 쿠키 수명 배포 후 실측 (문서 전용)
- Timestamp: 2026-09-07T04:00:00+09:00
- 코드 변경 없음. 배포 `ee45a260` 에서 고리를 둘로 갈라 각각 쟀다.
  ① 서버가 영구 쿠키를 보낸다(`persistent=1`, 만료 오늘+90일) ② WebView2 가 재실행 뒤에도
  들고 있다(`SURVIVED`). 하나만 보면 「고쳤다」와 「도달했다」가 갈린다.
- `weblb`(LB 고정)는 여전히 비영속임을 함께 확인 — 수명 부여가 다른 쿠키로 번지지 않았다.


## CHG-20260907T050000-ai-claude-client-live-verify — 실제 클라이언트 종단 실측 (문서 전용)
- Timestamp: 2026-09-07T05:00:00+09:00
- 코드 변경 없음. 설치된 `DQAConnect.exe` 를 그대로 띄우고 그 WebView2 에 CDP 를 열어
  **사용자 계정(`admin`)** 으로 측정했다.
- 결과: 칩 표시·계정 옆·잘림 없음 · 토큰 요청 0건(승인창 없음) · **종료 후 재실행에도
  로그인 유지**(쿠키 `persistent=1`, 만료 오늘+90일).
...
## CHG-20260907T160000-asset-stamp-reach (스탬프 없는 모듈 참조 → 배포 미도달 차단, Minor §12.3)

**변경**:

- `src/static/app/connect-modal.js` — `import … from "./client-bridge.js"` 에 `?v=dev` 부여.
  전체 정적 트리에서 **유일하게** 스탬프 토큰이 없던 참조였다.
- **신규** `tests/test_static_module_stamp_census.py` — 「모든 로컬 ES module 참조는 `?v=` 를
  갖는다」를 전수 고정. 추출 자체가 깨지면 전건 통과가 되므로 **양성 대조군**(스탬프 없는
  참조를 실제로 집어내는지)과 **표본 하한**(15건 미만이면 실패)을 함께 둔다.

**왜 파일 하나가 아니라 계약인가**: `inject_asset_stamp.py` 의 docstring 은 이 간극을 이미
설명하고 있었다(「하위 모듈이 무버전으로 import 하면 … 캐시버스터가 전파되지 않는 gap」).
아는 것과 지켜지는 것은 다르다 — 다음에 추가되는 모듈이 같은 함정에 빠지지 않게 게이트를 둔다.

**영향**: 프런트 전용, 비파괴. 이미 `connect-modal.js?v=<구 스탬프>` 를 immutable 로 캐시한
...
## CHG-20260908T100000-remove-terminal-path-live (전면 제거 라이브 실측 기록, 문서 전용)

**변경**: `docs/test-runs.d/20260907T190000-remove-terminal-path.md` 에 배포 `577b5ea1` 이후
실 Windows 설치본 실측을 추가. 코드 변경 없음.

**남긴 경계**: 브라우저(비-앱 창)의 노출 규칙은 **라이브로 재지 않았다** — 그 화면은 로그인
