    ...
    try:
        ...
        for _hop in range(depth):
            ...
            for r in erows:
                ...
                # weight/status(feature-0016): REFERENCES 엣지만 보유 — UI 신뢰/추정 구분용.
                # broken 은 sync 가 그래프에서 삭제하지만, 감쇠~다음 sync 사이 창을 방어해 여기서도 제외.
                e_status = _unwrap(r[11])
                if ekey not in seen_edges and b_key in seen_nodes and e_status != "broken":
                    seen_edges.add(ekey)
                    result["edges"].append({"source": a_key, "target": b_key, "type": etype,
                                            "cardinality": _unwrap(r[8]), "edge_source": _unwrap(r[9]),
                                            "weight": _unwrap(r[10]), "status": e_status})
