...
## 포트프록시 5분 주기 절단 해소 (2026-09-02)

**상태**: 코드 수정 + PS 5.1 실측 검증 완료. 라이브 효과 검증은 Windows 배포본 갱신 후 관측.

### 한 줄 요약

Windows 예약작업이 5분마다 포트프록시를 **조건 없이 재설정**해 80/443 의 기존 연결을 전부 끊고
있었다. 이미 맞는 매핑은 건드리지 않도록 바꿔 그 절단을 없앴다.

### 사용자에게 보이던 증상

개인 AI 러너 로그에 `WARN` 이 규칙적으로 반복 출력 — `대기 실패 — 잠시 뒤 다시 연결합니다`
(`conn.retry`) · `서버에 닿지 못했다`(`api.fail`). 19시간 원장에서 498건, 안정 러너 기준
**시간당 12건 = 5분당 1건**.
...
| 후보 | 판정 | 근거 |
|---|---|---|
| 클라이언트 타임아웃이 엄격 | **아니다** | 상한 90s > 서버 보류 55s. 성공 961건 소요 중앙값 55.3s 로 설계대로 |
| 서버 지연 / 과부하 | **아니다** | 실패 예외형이 `TimeoutError` 가 아니라 `RemoteDisconnected` |
| 실제 연결 절단 | **맞다** | Caddy `status:0`(응답 미전송) + 러너 `RemoteDisconnected` — 양쪽이 "상대가 끊었다" = 중간 단 절단 |

절단 주체는 `mysql_ai_web_portproxy_sync` (Windows 예약작업, 5분 반복)이며, 그 스크립트는 이
저장소가 소유한다(`src/windows/sync_mysql_ai_web_portproxy.ps1`).

### 기능 영향도

**기능 파손은 아니었다** — 러너가 자동 재연결(backoff 1초)하고 질문 유실은 0건이다. 실제 비용은
① 재연결 창(1~15초)에 도착한 질문의 인지 지연 ② 진짜 장애를 가리는 로그 소음이었다.

### 완료 (2026-09-02)

- [x] Windows 배포본(`C:\ProgramData\mysql_ai_web_portproxy\`) 갱신 — **관리자 권한 필요**
      (WSL 에서는 기존 파일 수정·삭제가 ACL 로 거부된다. `IsReadOnly` 해제도 비-상승
      PowerShell 에선 거부). 사용자가 관리자 PowerShell 로 `Copy-Item ... -Force` 실행,
      sha256 대조 일치 확인.
- [x] 라이브 관측 (13:33:30~13:55:45, sync 5회분) — **목표 지표 0건**.
      배포 전 동일 길이 구간은 8·8·8·12건이었다. 예약작업은 같은 구간에 5회 정상 실행
      (`LastTaskResult=0`)했으므로 «작업이 멈춰서 0» 이 아니라 «돌면서도 끊지 않은» 것이다.
      완주 wait 22건 중 21건이 55초 보류 완주. 상세는 `TEST.md` Run 기록.

## 8. 개선 제안 (§8.1 — 기록만, 지시 없이 실행하지 않음)

- **방화벽 규칙도 같은 결함 클래스**: `-SkipFirewall` 없이 실행하면 `Remove-NetFirewallRule` →
  `New-NetFirewallRule` 을 매 실행마다 무조건 수행한다. 현재 배포는 `-SkipFirewall` 이라 발동하지
  않아 **라이브 검증이 불가능**하므로 이번 cycle 범위에서 제외했다. 이 경로를 쓰게 되면 portproxy
  와 동일하게 «이미 원하는 규칙이면 건드리지 않기» 를 적용해야 한다.
- **`Test-NetConnection` 프로브 비용**: 매 실행마다 4회(포트 2 × listen/connect) 수행한다. 절단
