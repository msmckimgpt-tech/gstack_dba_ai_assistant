...
## 1. Current Status
- State: **TASK-0230 (멀티 datasource 1:N — 제품 1개 ↔ 여러 datasource 참조) 완료** — 제품이 여러 datasource 에 바인딩될 수 있고, assistant 가 한 질문에서 tool 의 `datasource` 인자로 대상을 골라 datasource 별 (연결·allowlist·dialect) 격리 컨텍스트로 조회한다 + 제품 프롬프트 자동작성이 모든 바인딩 datasource 의 DB 를 인지 (cycle: TASK-0230)
