...
## 2. Goal
- REQ-20260826-attach-version-bump-lineage (TASK-20260826T140000-attachment-version-bump-forks-new-root, **Major §12.3** — 코어 LLM 프롬프트·도구 설명·컨텍스트 라벨, conversation_audit `FR-attachment-version-bump-forks-new-root`): 사용자가 **assistant 가 전달한** 첨부에 대해 "동일한 명칭으로 버전만 상승" 을 요청하면 그 계보가 v+1 로 연장되어야 한다. **배경(라이브 실측)**: 능력은 이미 있었다 — `_materialize_assistant_attachment_edits` 는 source 가 assistant 계보면 그 체인을 연장한다(REQ-20260814-attach-version-branching). 그러나 그 계약이 `SYSTEM_PROMPT` **본문에만** 있었고 `compose_system_prompt` 은 운영자 `WebSystemPrompts` global row 가 존재하면 본문을 **통째로 대체**한다. 라이브 row 실측(2026-08-26) **9,294자 / UpdatedAt 2026-08-03** vs 코드 상수 23,060자 — `continues YOUR chain`·`FILE VERSION LINEAGES`·`attachment-new` 전부 부재. 프로덕션에 남은 권위 표면(코드-append 지침 2종 + `update_attachment` 설명)은 셋 다 "the user attached" 프레이밍이라 **assistant 전달본의 버전 상향을 아무도 claim 하지 않았고**, 유일하게 claim 하는 지침이 `attachment-new` 였다 → 모델이 명시 요청에 동명 **새 root(v1)** 를 재생성(대화 …945b2aca, 동명 v1 4건 · 답변은 "버전을 상향했다" 고 선언). **동작**: (A) `_ATTACHMENT_DELIVERY_DIRECTIVE` 발동 조건을 "이 대화에 이미 존재하는 파일(사용자 첨부 **OR** 내가 전달한 것)" 로 확장하고 `VERSION LINEAGES` 절로 계보 계약을 코드 권위선에 이식(AUTH-1a) — 동명 `attachment-new` 는 버전 상향이 **아니며** 무관한 v1 을 fork 한다는 사실을 명시. (B) `_ATTACHMENT_NEW_DELIVERY_DIRECTIVE` 가 스스로 경계를 긋는다 — **아직 존재하지 않는 파일 전용**. (C) `update_attachment` 도구 description·`filename`/`attachment_id` 인자 설명을 assistant 전달본 포함으로 교정("버전만 올려줘" → 이 도구). (D) `_build_attachment_context_section` 이 assistant 전달본에 **v1 이어도** 계보 라벨 + `update_attachment(attachment_id=N)` 진입점을 싣는다 — 종전엔 version>1 에서만 표식이 붙고 `## FILE VERSION LINEAGES` 도 동명 계보 2개 이상에서만 렌더돼 갓 만든 v1 전달본이 **무표식**이었다. 갱신-가능 판정은 저장 경로와 **같은 술어**(`AccountId` 일치)를 쓰며 불일치 시 진입점 대신 `READ-ONLY` 를 표기한다(권하면 조용한 거부 뒤 "갱신했다" 만 남는다 — 라벨이 가드보다 넓어지지 않는다). (E) census 등록 — 계보 계약을 다시 `SYSTEM_PROMPT` 본문에만 넣으면 `test_grounding_authority_directive` 가 FAIL 한다(이번 결함의 기전 자체를 봉인). 스코프 해소·소유권·kind·용량·확장자 가드 **불변**(프롬프트/도구 설명 문자열 + 표시 라벨 한정 — 보안 회귀 0). (검증 `tests/test_attach_version_lineage_prompt.py` + `tests/test_grounding_authority_directive.py` 39 PASS · 뮤테이션 3/3 KILL · §18.8 패널 `REV-20260826T140000-attach-version-bump-lineage`.)
...
- REQ-20260824-attach-original-baseline (TASK-20260824T0733-attach-original-baseline, **Major §12.3** — 사용자 요청 2026-08-24): assistant 가 첨부 파일을 비교할 때 **계보 최초 원본**을 조회할 수 있으면 `_v0` 형태로 능동 포함한다. **배경(측정)**: 첨부 버전 체인의 구버전 행은 보존되지만(`SupersededAt` 스탬프만·`DeletedAt IS NULL`) assistant 가 볼 경로가 **한 곳도 없었다** — LLM 스코프(`_resolve_conversation_attachment_scope`)도 `read_attachment`(`_attachment_scope_ids`)도 최신본만 보고, `## FILE UPDATES` diff 는 **직전 버전 대비**이며 재업로드가 일어난 그 턴에만 렌더된다. 그래서 "처음 올린 것과 지금이 뭐가 다른가" 는 v3 이상 체인에서 원리적으로 답할 수 없었고, 모델은 답할 수 없다는 사실조차 몰라 최신본만 보고 서술했다. 라이브 실측(2026-08-24): 첨부 1,088건 중 `VersionNumber>1` **245건(22.5%)** · 다중버전 체인 **171개**(길이 2=130·3=29·4=3·5=5·6·7·8·11 각 1) · 구버전 245건 = text 243(평균 **3.0KB**·최대 12.4KB) + xlsx 2 — 본문 주입 비용이 작다는 이 수치가 "항상 인라인" 결정의 근거다. **동작**: (1) 버전>1 인 스코프 첨부의 계보 최초본을 1회 조회(`_load_original_versions` — 현재본과 **같은 스코프 술어**, 삭제·삭제대기 제외)해 `## ORIGINAL VERSIONS (_v0)` 전용 섹션으로 렌더하고, 파일명은 `<stem>_v0<ext>` 로 표기한다(표기 규약일 뿐 `VersionNumber` 는 1-base 불변·웹 UI 버전 표기 무변경). (2) 트리거는 **버전 사실**이지 질문 판정이 아니다 — "비교 요청인가" 를 키워드로 가르지 않는다(취약). (3) 본문은 현재본과 같은 규약(줄번호 prefix + datamark sentinel)으로 싣고, 건수 5·파일당 40,000자 상한 초과분과 비-text·판독불가는 **존재 사실을 명시**해 "원본이 없다" 는 반대 방향 단정을 차단한다(무음 절단 금지). (4) 타 계정 원본의 **본문이 실제로 렌더된** 순간에만 `_UNTRUSTED_ATTACH_BODY_CTX` 를 세운다(REQ-20260814-attach-provenance-gate 동형 — 이 자리가 빠지면 원본 인라인이 쓰기 게이트 우회로가 된다). (5) `read_attachment(attachment_id=...)` 가 **같은 대화·같은 계보·미삭제** 3겹을 만족하는 조상까지 열린다(`_load_ancestor_attachment_row`); `filename` 경로는 최신본만 유지해 동명 되묻기(REQ-20260814) 회귀를 막는다. **스코프(`_resolve_conversation_attachment_scope`)는 무변경** — 거기에 원본 id 를 더하면 `ATTACHED FILES` 에 동명 파일이 2건 떠 `_lineages` 가 "계보 2개" 로 오인하고 `## FILE VERSION LINEAGES` 가 거짓을 말한다. 인가 확대 아님(같은 대화·같은 계보·미삭제 행, 열람권은 최신본과 동일). web 측 변경 0 · 신규 권한 코드·테이블·마이그레이션 0.
...
## 예약 작업(SQL Server Agent 작업) 조회 경로의 도달 가능성 — 정제기 비대칭 + 거부 안내 파생

**계약**: 구조화 도구의 이름 인자는 **두 문맥**으로 나뉘고, 문맥마다 정제기가 다르다.
- **식별자 문맥**(뷰·트리거·시퀀스·MySQL EVENT·스키마·테이블) → `_safe_ident`. 인용 구분자
  (`` ` `` `"` `'` `;` `[` `]` `\`)를 **제거**한다. SQL 이 `[{schema}].[{table}]` 로 인용하므로 `]`
  를 남기면 인용을 닫고 2차 SQLi 가 된다(REV-0201 B1 라이브 실증). **완화 금지 영역.**
- **리터럴 문맥**(SQL Server Agent 작업명 — `msdb.dbo.sysjobs.name` 의 임의 문자열, 질의에서
  `WHERE j.name = '<값>'` 로 비교) → `_safe_literal_value`. 문자를 **보존**하고 역슬래시·제어문자만
  제거한다. 여기에 식별자 정제기를 쓰면 `[DK] Ranking Update` 가 `DK Ranking Update` 로 훼손돼
  **영구 미매칭**이 된다(관측 서버 작업 83개 중 73개가 `[` 접두 = 88% 조회 불가).

**이스케이프 책임은 삽입 지점**: 따옴표 이중화는 `dialects._agent_jobs_sql` 이 1회만 수행한다
(`_sql_str_list` 와 같은 계약). caller 가 함께 이중화하면 따옴표가 두 번 늘어나 다시 미매칭이 된다.
파서 기준 봉쇄는 `tests/test_scheduler_discovery_reachability.py` 가 적대 입력 13종에 대해
① 리터럴 노드 갇힘 ② 문장 수 1 ③ 고정 조인 4뷰 밖 테이블 0 으로 잠근다.

**거부 안내는 도구 카탈로그에서 파생**: 차단 메시지가 대체 도구를 지목하지 않으면 모델은 대안이
없다고 판단해 우회를 자작하다 "확인 불가" 로 단정한다. 안내 목록을 `_DISCOVERY_TOOL_NAMES`
단일 출처에 두고 **상시 노출 세트 `TOOL_DEFINITIONS` 와의 정합을 테스트로 강제**한다 — 새
`search_*`/`describe_*` 도구가 추가되면 스위트가 깨져 안내 갱신을 강요한다(옛 4개 고정으로
feature-0040 의 두 도구가 누락돼 있던 drift 의 재발 봉인). 확장 전용(`TOOL_DEFINITIONS_FULL`)
도구는 **광고하지 않는다** — 모델이 부를 수 없는 이름을 권하면 새 마찰이 된다.

**보안 경계 불변**: freeform 의 msdb 하드 차단, 허용 DB fail-closed(`IN ('')`), 작업 단계의
