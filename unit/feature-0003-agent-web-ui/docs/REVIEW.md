...
---
## REV-20260630T110910-metadata-ds-single-ui [SUBAGENT:adversarial-frontend] (TASK-20260630T110910-metadata-ds-single-ui — 메타데이터 패널 '데이터소스' 선택 UI 단일화, Major §12.3 — 프론트 단독)
- Trigger(§18.8): UI/state-machine 변경(부트스트랩 데이터소스 selector 제거 → 헤더 스코프 상속) → frontend state-machine + correctness 렌즈. 대상: `static/admin.js`(`_metaSyncBootstrapVisibility`/`_metaBootstrapSyncToScopeDs`/`_metaBindControls`/`_metaBindBootstrap`), `static/admin.html`, `static/styles.css`. backend/route/RBAC/스키마 0.
- 적대 7가설 검증(코드 정독 + 시나리오 trace + 테스트 3종 실행 + main 베이스라인 대조) → **1차 VERDICT FIX-THEN-SHIP(BLOCKING 1) → 수정 → 2차 SHIP(BLOCKING 0)**.
  1. **H1 상태 누수(DS-A→공용→DS-A) — REFUTED**: 공용 진입 시 `_metaSyncBootstrapVisibility` 가 empty 노출·head/body 숨김 후 early-return(syncToScope 미호출) → `bs.datasource/tables/결과 DOM` 보존(비가시). DS-A 복귀 시 dedup 가드(`ds===bootstrap.datasource`)가 골격 보존 + 동일 DS 재렌더. stale 표시 없음(보존 골격은 정당히 DS-A).
  2. **H2 DS 간 전환 리셋 — REFUTED**: 라벨 상이 시 가드 통과 실패 → `schema=""`/`tables=[]` 리셋 + schemaSel 비움 + 결과 DOM 정리 + 스키마 재로드. `_metaScopeDatasourceKey`/sync 모두 `.trim().toLowerCase()` 정규화. 동일 DS 무의미 재전환은 change 발화조건 + dedup 가드 이중 차단(불필요 네트워크/리셋 없음).
  3. **H3 가시성/토글 desync — REFUTED**: `bootstrap.open` 유일 mutator=토글 click 핸들러(text ▾/▴·aria-expanded 원자 갱신). sync 는 `body.display` 를 `open` 에서만 읽어 항상 일치. 공용 round-trip(head 숨김)에서도 open 불변 → 복귀 시 토글 indicator 일관. HTML 초기값(aria-expanded=false·▾)도 open:false 기본과 정합.
  4. **H4 저장 footgun 잔존 — REFUTED**: 저장 버튼이 `#metadataBootstrapBody` 내부 → 공용 진입 시 body `display:none` 으로 비가시·tab-order 제외 → "골격 가져온 뒤 공용 전환 후 저장" race 구조적 차단. `_metaBootstrapSave` 는 호출 시점 scopeKey 사용, 부트스트랩은 구체 DS 에서만 노출 → 저장 스코프=골격 소스 항상 일치.
  5. **H5 초기화/바인딩/hoisting — REFUTED**: 변경 함수 전부 top-level `function` 선언(hoisted). 모든 getElementById null-guard. 초기 subTab=glossary 라 init 시 applicable=false early-return.
  6. **H6 제거 잔재 — REFUTED(src) + CONFIRMED(테스트, B1)**: `src/` 내 `#metadataBootstrapDs`/`_metaBootstrapPopulateDs` dangling 0, `bootstrap.datasource` 소비자 정합. **단** 기존 회귀 테스트 `tests/verify_metadata_bs_inline_desc.mjs` [B4] 가 깨짐 → BLOCKING B1.
  7. **H7 접근성/엣지 — REFUTED**: empty-state 가 `.admin-detail-empty` 컨벤션 정합(기존 8개 사용처와 동일 — role/aria-live 미부여는 프로젝트 자체 기준). 부트스트랩은 `kb.ingest.manual` 독립 게이트 → 무권한자 정상 숨김.
- **BLOCKING B1 (적발→수정→재검증 RESOLVED)**: `_metaSyncBootstrapVisibility` 가 의존성을 `_metaBootstrapPopulateDs` → `_metaScopeDatasourceKey`+`_metaBootstrapSyncToScopeDs` 로 바꿨는데, 이 함수를 `new Function` 으로 격리 실행하는 `verify_metadata_bs_inline_desc.mjs` [B4] 가 미갱신 → jsdom 환경에서 `ReferenceError: _metaScopeDatasourceKey is not defined`, exit 1(main 베이스라인은 통과). CI(`make test`)는 .mjs 미포함이라 녹색이나 명시한 수동 회귀 가드가 실제 깨진 채였음. **수정**: [B4] 주입 인자 갱신(`_metaBootstrapPopulateDs` 제거, `_metaScopeDatasourceKey`=()=>"x"·`_metaBootstrapSyncToScopeDs`=()=>{} 추가) + `bootstrap.open:false` 명시 + **[B4-common] 단언 신설**(공용 스코프 ""→empty-state early-return 으로 재렌더 안 함). 재실행 **30/30 PASS**.
- **NIT N1 (수용→가드 추가)**: `_metaBootstrapLoadSchemas` 가 `await` 후 `bs.datasource` 재확인 없어 DS 초고속 전환 시 늦은 응답이 다른 DS 드롭다운 덮을 수 있음(main 의 구 DS-dropdown 핸들러에도 동형 존재한 선재 결함). 본 변경이 스코프 변경마다 로드를 더 자주 유발하므로 **stale-response 가드 추가**(`if (bootstrap.datasource !== ds) return;` await 직후).
...
## REV-20260630T230501-doc-sync-rn-2305 [SKIPPED:non-policy-doc] — 릴리즈노트 06-30 블록 신규(+10) + 캐시버스터 bump (TASK-20260630T230501-doc-sync-rn-2305, 비-정책 doc-only)
- Panel skip 사유(§18.8): 변경은 사용자 노출 릴리즈노트 콘텐츠 데이터(`static/release-notes-data.js`) + cache-buster(`index/admin.html`) 뿐 — 비-정책 doc-only. 렌더 로직(`release-notes.js`)·백엔드·스키마·RBAC·엔드포인트 0 → 코드 적대 검증 대상 아님(§18.8 표 첫 행 `[SKIPPED:non-policy-doc]`). 단 콘텐츠 정합·평이화는 doc_sync ULTRACODE 5-stream 적대 패널(wf_86043cdc)이 정본 대비 직접 검증.
- 타깃별 실질 검증(doc_sync Phase 4): `node --check release-notes-data.js` PASS + vm 로드 `generated`=2026-06-30·신규 '2026-06-30' 블록 10항목·항목 스키마(type/area/title/detail) 정합. 적대 사실검증 — 사용자향 평이화·내부 비노출(feature-id/테이블/AGE/Cypher/마이그/cache-buster 슬러그 0)·과대표현 2건(그래프 엣지=0 '관계 따라가기' / 무중단 '주요 기능 멈추지 않음') 완화.
- Cross-ref: CHG/TASK/FUNCTION-20260630T230501-doc-sync-rn-2305 / 원천 머지 feature-0016 metadata-graph(PR#477/#480/#482/#484)·feature-0003 metadata-list-detail/ds-single-ui/bs-inline-desc·align/bs-prefill·feature-0002 active-interp·무중단군 0014/0015/0016-zd/0017. META(STATUS·wiki·SECURITY·RELEASE_NOTES)는 별도 commit/META mode(REV-20260630T230501-META-0016-doc-sync-2305).

## REV-20260701T163000-graphview-render [SUBAGENT:adversarial-correctness] (TASK-20260701T163000-graphview-render — 그래프 뷰 3건: 마커 렌더-타임·클러스터 선택 상세·클러스터명 좌정렬/무잘림, Major §12.3 — feature-0003 프론트 + feature-0002 백엔드 cross-cut)
- Trigger(§18.8): 신규 API 엔드포인트(backend/qa 렌즈) + Cytoscape UI/DOM-오버레이 상태·이벤트(frontend/ux 렌즈). 대상: `node_analysis.py`·`admin_metadata.py`·`static/admin.{js,html,css}`. 스키마/마이그 0, RBAC=기존 권한 재사용.
- 적대 정합성 리뷰(staged diff 정독 + 주변 함수 trace, 앱/브라우저 미실행 정적) → **VERDICT SHIP(BLOCKING 0, MAJOR 0, MINOR 1, NIT 1)**.
  1. **백엔드 SQL/psycopg2 — REFUTED**: `bool_or(...) GROUP BY node_key` 유효 PG 집계, `node_key = ANY(%s)`(list) 정규 배열 어댑테이션(주입 없음), 인덱스 `(scope_key,node_key,status)` 커버. `_rw_conn` owned 플래그 + finally close(sibling 동형), autocommit RO. 컬럼 3종 존재.
  2. **FastAPI 라우팅 — REFUTED**: `/graph/analyze/status` vs `/graph/analyze` vs `/graph/analyze/node` 전부 리터럴(비-parameterized) → Starlette exact-match, 순서 무관 shadow 없음.
  3. **마커/폴 상호작용 — REFUTED**: `_metaGraphSyncAnalysisMarkers` 는 `_metaGraph.running` 에 `.add()` 만(additive) — 폴의 `_metaGraphMarkRunning` 이 여전히 clear 소유. sync-추가 키가 다음 tick `st.running_keys` 에 없으면 정상 정리(폴 미clobber).
  4. **오버레이 DOM 수명 — REFUTED**: `_metaInitGraph` 가 `cy` 존재 시 early-return → `render` 리스너 1회 바인딩, rAF `_lblRaf` 가드 self-throttle, live `_metaGraph.*` 참조(stale closure 없음). `labelDivs` Map 재사용 + unseen id prune(`div.remove()`) → 그래프 교체 커버, cy 미파괴로 orphan 없음.
  5. **tap 핸들러/클러스터 상세 — REFUTED**: `label==='Schema'||isCat` 가드가 스키마 클러스터만 라우팅, Table parent 는 기존 노드 경로로 통과(회귀 없음). `_metaEnsureCat` 가 `isCat:1`+`label:"Schema"` 설정 → style/tap/label 선택자 정합. null-safety(node/apiFetch try/data null-check/HTML escape) 확인.
...
## REV-20260702T230501-doc-sync-rn-0702 [SKIPPED:non-policy-doc] — 릴리즈노트 07-02 블록 신규(+8) + 캐시버스터 bump (TASK-20260702T230501-doc-sync-rn-0702, 비-정책 doc-only)
- Panel skip 사유(§18.8): 변경은 사용자 노출 릴리즈노트 콘텐츠 데이터(`static/release-notes-data.js`) + cache-buster(`index/admin.html`) 뿐 — 비-정책 doc-only. 렌더 로직(`release-notes.js`)·백엔드·스키마·RBAC·엔드포인트 0 → 코드 적대 검증 대상 아님(§18.8 표 첫 행 `[SKIPPED:non-policy-doc]`). 콘텐츠 정합·평이화·과대표현·중복회피는 doc_sync ULTRACODE 적대 워크플로(wf_61d9648b — 타깃별 finder + 적대 verifier; 3 delta 오류 적발·교정: docs/RELEASE_NOTES append 방향 오류(top→bottom)·SECURITY §20 conversation_id 노출 사실오류·hot.md PB-0008 검증 과장)가 정본 대비 직접 검증.
- 타깃별 실질 검증(doc_sync Phase 4): `node --check release-notes-data.js` PASS + jsdom DOM 테스트 33 PASS(`generated`=2026-07-02·신규 '2026-07-02' 블록 8항목·스키마(type/area/title/detail) 정합·07-01 블록 7항목 보존·관리 62/작업 85/그룹 20 동적 카운트·XSS 제목/summary esc)/1 FAIL 은 pre-existing admin pane `overflow-y:auto` CSS 규칙(data-only 변경 무관·baseline 동일). 적대 사실검증 — 사용자향 평이화·내부 비노출(feature-id/G6/Cytoscape/WebGL/config/alembic/엔드포인트/권한키/모델명/ADR 0)·추정관계 정직 결함 공개(재발표 아님)·feature-0012/0017 behavior-neutral user-facing 제외(정직 분류).
- Cross-ref: CHG/TASK/FUNCTION-20260702T230501-doc-sync-rn-0702 / 원천 머지 feature-0016 그래프 뷰 07-02 진화(G6 v5 교체 ADR-004·ctxmenu·initview·search-badge·perf ADR-005/006·rel-selfheal ADR-007·dblclick-cam ADR-008)·feature-0003 aiops-panel(REV-20260702T140000)·audit-nav-ux·metadata-perm-hier·attach-count-scope. 원천 UI PB-0008 07-02: graph-ctxmenu·initview·search-badge PASS; G6 교체·rel-selfheal·카메라팬 배포 후 잔여. META(STATUS·wiki·SECURITY·ARCHITECTURE·RELEASE_NOTES)는 별도 commit/META mode(REV-20260702T230501-META-0019-doc-sync-0702).

## REV-20260703T085511-ds-avg-latency [SUBAGENT:adversarial-correctness+threadsafety+noleak] (TASK-20260703T085511-ds-avg-latency — 데이터소스 상세 패널 평균 연결 응답 시간, Major §12.3 — feature-0003 web/UI·API + cross-unit shared/conn_health·config) — VERDICT: SHIP (BLOCKING 0 / MINOR 1 FIXED / NIT 2)
- Trigger(§18.8): backend(멀티스레드 conn_health 이동평균 정확성·스레드안전) + security(snapshot 좌표 비노출 불변식) + API/frontend(null-guard·회귀) 다중 렌즈. shared/ 모듈 변경(feature-0002/0003 공용 소비) + 보안 민감 불변식 → 적대 리뷰 정당.
- 적대 코드리뷰(작업트리 diff 정독 + shared/conn_health.py 전문 + 테스트 실측, general-purpose subagent, 앱 미실행 정적 + pytest 실행):
  1. **CORRECTNESS — clean**: 산술평균 `round(sum(dq)/len(dq),1)` div-0 불가(append 직후 항상 ≥1). deque maxlen 초과 시 최오래 표본 evict 정확(`test_avg_window_is_bounded`). window 크기 변경 시 deque 재생성 — `deque(old, maxlen)` 이 최근 N개 보존(좌측 truncate)·`dq or ()` 로 None/빈-deque 안전, 표본 손실/오염 0. 표본 게이트(`source=="probe-db" and ok and elapsed>0`)가 실패 probe/foreground(0.0)/TCP·SSRF 실패 정확 제외 + 느린 성공(unstable) 포함(status 무관, `test_avg_includes_slow_success`).
  2. **THREAD SAFETY — clean**: `_SAMPLES` 5개 접근처(_sample_avg_elapsed·_prune_state·snapshot·_reset_state) 전부 `_LOCK` 안(RLock reentrancy 준수). grep 으로 stray 접근 0 확인. snapshot 의 `len(dq)` 읽기와 _sample_avg_elapsed 의 deque 교체가 동일 락으로 직렬화.
  3. **SECURITY / NO-LEAK — clean**: `snapshot()` 은 timing aggregate(label/engine/status/last_elapsed_ms/avg_elapsed_ms/sample_count/checked_at/last_error/fails)만 — host/port/user/password 비노출. `sample_count`=len(count), `avg_elapsed_ms`=스칼라. `test_snapshot_hides_coordinates` 키셋 갱신·비노출 재단언. `_attach_product_conn_status`(제품 dropup)는 3키만 → C5 `set(cs.keys())` 회귀 없음.
  4. **MEMORY — clean**: `_prune_state` 가 `_SAMPLES[k]` 를 `_STATE[k]` 와 lockstep pop. 불변식 `_SAMPLES keys ⊆ _STATE keys`(표본은 _ensure_entry 후에만 생성) → _STATE 키 기준 prune 이 전 deque 커버(`test_prune_removes_samples`).
  5. **API/FRONTEND**: admin_datasources `_h` 유/무 null-safe(fallback avg:None/sample_count:0). admin.js avg 행 `!= null` 가드로 "null ms" 방지·`Number(sample_count||0)`·checked_at `*1000` 초→ms 변환(formatDateTime ms 계약)·_dsKvRow textContent(무주입).
  6. **REGRESSION — none**: classify/should_fast_fail/last_elapsed_ms 무변경. 성공 분기 내 avg 필드 write 만 추가. _ensure_entry 가 avg_elapsed_ms:None seed → KeyError 없음. 전체 스위트 PASS.
- **흡수한 MINOR (frontend) — FIXED**: `admin.js` "최근 응답 시간" 행이 `elapsed_ms != null` 만 가드·`> 0` 미가드 → `record_foreground_result` 의 0.0-coerce(매 사용자 질의 발생, background probe 는 ~30s 주기라 마지막 이벤트가 자주 foreground 0.0)로 "최근 응답 시간: 0 ms" 가 "평균 45ms" 옆에 모순 표시 가능. → **수정**: 평균 행과 동일하게 `cs.elapsed_ms > 0` 가드 추가(0/음수 순간값 표시 생략). `node --check` 재PASS.
- **NIT (무수정 확인)**: (a) <0.05ms probe 는 round→0.0→제외 — 실 DB connect+SELECT1 에서 물리적 불가, 무해. (b) down 시 avg 미클리어 → "45ms 최근 3회 평균" 이 상태 "끊김" 옆 잔류 가능 — **의도된 동작**(평균은 성공 probe 의 상시 연결 품질 히스토리 대표값, 상태 행이 끊김을 별도 표시, 라벨 "최근 N회 평균" 이 성공 표본임을 전달). FUNCTION.md 계약에 명시.
- 실측: `test_conn_health.py` 32(신규 5 포함) + `test_product_conn_status.py` 등 대상 회귀 PASS + feature-0002/0003 전량 PASS(컨테이너 전용 `test_share_redaction_invariant.py` 제외 — `import web.app` 환경 아티팩트). py_compile(config/conn_health/admin_datasources) + node --check(admin.js). PB-0008 Windows-browser = 배포 후 라이브(TEST.md §3).

## REV-20260703T085511-ds-avg-latency-postverify [SKIPPED:post-deploy-verification-record] (TASK-20260703T085511-ds-avg-latency-postverify — POST-DEPLOY PB-0008 실측 기록, 비-정책 doc-only)
- Panel skip 사유(§18.8): 본 cycle 은 선행 ds-avg-latency(REV-20260703T085511-ds-avg-latency [SUBAGENT] SHIP)의 **배포 후 라이브 검증 결과를 문서에 기록**할 뿐 코드/자산 무변경(doc-only, TASK/MODIFY/TEST/REVIEW). 신규 로직 경로 0·백엔드/RBAC/스키마/엔드포인트 무변경 → 코드 적대검증 대상 아님(§18.8 표 `[SKIPPED:*]`). 검증 자체(실 Windows Chrome PB-0008)가 이 기록의 정본 증적.
- POST-DEPLOY 실측(배포 7ad4378b, 실 Windows Chrome/149 win-browser relay @ 172.26.144.1:9223, `https://localhost/admin`): 새 admin.js(`?v=20260703-ds-avg-latency`, `_dsConnStatusLabel`=function) 로드 → 상세 패널 섹션 [연결 좌표·**연결 상태**·출처·보안] → `mssql-dk-dev` **연결 응답 시간(평균) "11.7 ms · 최근 6회 평균"**·`mssql-qa-idc` "133.4 ms · 최근 6회 평균"(healthy)·`mssql-web-qa`/`mssql_local`(끊김) "측정 중 (연결 성공 시 집계)"(성공 표본 0 — stale 미표시). MINOR(0ms 순간값 가드) 실효(표시 최근 응답 시간 전부 >0). pageerror 0. 스크린샷 `scratchpad/ds-avg-latency-verify.png` 레이아웃 무붕괴.
- Cross-ref: REV-20260703T085511-ds-avg-latency(선행 코드 cycle) / CHG-20260703T085511-ds-avg-latency-postverify.

## REV-20260706T013532-reasoning-effort [SUBAGENT:adversarial-correctness+runtime-override+persistence+frontend] (TASK-20260706T013532-reasoning-effort — 대화 화면 사용자 지정 추론 강도 선택기, Major §12.3 — feature-0003 web/UI·API + cross-unit feature-0002·shared) — VERDICT: SHIP (BLOCKING 2 FIXED/VERIFIED · NIT 3 [1 FIXED, 1 검증-무결, 1 noted])
- Panel(§18.8): `general-purpose` 서브에이전트 1인이 전체 working-tree diff 를 9개 결함 클래스(제약 1·2, extra_body override, scope leak, persistence race, worker parity, frontend, validation, null/default)로 적대 검토(refute 지향). 결과 BLOCKING 2 + NIT 3, PASS 6클래스.
- **BLOCKING B1 (correctness 회귀) — FIXED**: `/api/ask` 가 `normalize(...) or DEFAULT('normal')` 로 강제 대입 + 프론트가 매 요청 'normal' 전송 + 매핑 normal=5000 고정 → 선택기 미상호작용·구 클라이언트의 **claude-sonnet-4 가 config 기본 thinking 16000 → 5000 으로 조용히 강등**되는 회귀. **수정**: (a) `model_catalog` 매핑에서 'normal' 제거 → `_REASONING_BUDGETS`={low2000/high10000/max16000}, `thinking_budget_for_level('normal')`=None(=override 없음), (b) `/api/ask` 강제 대입 제거(부재=None), (c) 명시 레벨일 때만 KV 저장. '일반'=각 모델 config 기본 유지 → 무회귀. 회귀 가드 테스트 2건(`test_normal_is_no_override_b1_regression_guard`·`test_reasoning_level_normal_leaves_config_default`) 봉인.
- **BLOCKING B2 (extra_body override 미검증, RUNTIME) — VERIFIED**: 기능 전제(request-level `extra_body.thinking` 이 litellm_config.yaml alias 고정 thinking 을 override)를 정적으로 증명 불가 → **라이브 게이트웨이 프로브로 실증**. `repo-bedrock-gateway-1`(healthy)에 web-a 컨테이너 내 openai SDK 직접 호출, 동일 5-house 논리 퍼즐에 budget 만 달리: **budget=1024 → completion 1377토큰·reasoning 2073자 vs budget=16000 → completion 5935토큰·reasoning 6914자**. override 확실 동작 + 실제 추론량 budget 비례 스케일(CONFIRMED). 부수 확인: budget>max_tokens(16000>8000)도 400 없음 = litellm 이 max_tokens 내부 보정(코드측 제약 관리 불요). TEST.md §3 프로브 기록.
- **NIT N1 (mid-flight hydration clobber) — FIXED**: `loadHistory` 의 `/api/history` await 동안 사용자가 강도를 새로 고르면 resolving hydration 이 덮어씀. 로드 시작 시각(`_histLoadStartedAt`) vs 픽 시각(`state._reasoningPickedAt`) seq 가드 추가 — 픽이 로드 뒤면 hydration skip(픽 보존, localStorage 미러에도 기록됨).
- **NIT N2 (모델 체인 divergence) — 검증-무결**: `_composerModelSupportsThinking`→`_composerCurrentModel` 와 `askBody.model` 의 fallback 체인이 실제로는 **동일**(selectedModel→session→modelCatalog→apiVaultOptions→"claude-sonnet-4"). 불일치 없음(패널 추정 정정). 백엔드도 `model_supports_thinking` 재검증하므로 최악도 cosmetic.
- **NIT N3 (KV 읽기 backend 의존) — noted**: `/api/history` hydration 은 `AGENT_RUNTIME_READ_BACKEND=postgres` 일 때만 KV 조회(save 는 항상 PG write). 비-postgres 읽기 구성이면 hydration 이 ""→localStorage/기본 폴백(graceful, 무크래시). 기존 KV 소비(last_status)와 동일 패턴 — 신규 결함 아님, 배포 의존만.
...
## REV-20260706T094937-runtime-settings [SUBAGENT:backend·security·qa 3lens + 자기-적대 보강] (TASK-20260706T094937-runtime-settings — 관리 콘솔 `시스템 > 설정` 실행 타임아웃 + 모델별 thinking budget 조정·저장·사용, Major §12.3 — feature-0003 web/UI·API + cross-unit feature-0002·shared)
- 요청: assistant 참조 운영 값(TIME_OUT 현행 전량·모델별 thinking budget)을 관리 콘솔에 추가해 조정·저장·사용. 반영 방식=하이브리드(AskUserQuestion 확정: 안전 live·저수준 restart).
- **패널 집행 경위(정직 기록)**: §18.8 dispatch(API/endpoint/RBAC/backend → backend·security·qa 3렌즈) 로 적대 subagent 를 기동. **1차 라운드는 3개 모두 세션 한도로 조기 종료**(backend 렌즈가 종료 직전 "live 키 get_int 가 배포 env 무시" 를 지목 → BLOCKING-1 로 완전 진단·수정). 한도 리셋 후 **2차(재검증) 라운드는 3렌즈 전부 완주** — 수정된 코드(env-fallback 반영) 기준 재검토. **3렌즈 최종 VERDICT 모두 SHIP**(신규 blocking 0). 아래 자기검증 항목은 2차 패널이 독립 확인.
- **BLOCKING-1 (backend, 적발→수정 완료)**: `get_int(key)` 의 no-override 폴백이 스펙 리터럴 default(AGENT_TIMEOUT_SEC=60)만 반환하고 배포 `.env`(운영 `AGENT_TIMEOUT_SEC=300`)를 무시 → live 배선된 llm.py LLM 요청 timeout 이 override 미설정 시 **300→60 회귀**(쿼리/에이전트 실행 상한 급감). 근본: resolver 가 config.py 의 `int(os.getenv(key, literal))` 폴백 체인을 재현하지 않음. **수정**: `_baseline_int(key, spec)` 도입 — override → **env(operator 배포값, unclamped=config raw 동치)** → 리터럴 순. `_resolve_int`·`serialize_registry`(effective/default) 반영. 회귀 가드 `test_get_int_env_fallback_when_no_override`(env 300 → get_int 300) + `test_serialize_default_reflects_env_baseline` 추가. MCP_TIMEOUT_SEC 는 .env=20=리터럴20 이라 무증상이었으나 동일 결함 → 함께 해소.
- **backend 자기검증(그 외)**: ① byte-동치 — override 없으면 전 timeout 소비처가 기존과 동치(config startup_int(env_default)·get_int(env-fallback), 서브프로세스+단위 테스트 실증). ② B1 무회귀 — `_call_llm` 은 명시 reasoning_level 우선, 없을 때만 모델 override(설정 시에만 정수, 아니면 None→미주입→모델 config 기본 thinking 유지). ③ budget<max_tokens — 주입 경로는 claude(max_tokens=20000)에서만(model_supports_thinking) 발동, `min(budget, max_tokens-1024)` clamp → ≤16000<20000 항상 만족(기존 reasoning-effort 값엔 no-op). ④ CONN_PROBE `max()` 불변식(MAX≥BASE≥10) 보존(override 50<BASE → 100 유지, 테스트 실증). ⑤ import 안전 — config→runtime_settings→model_catalog 단방향 DAG(순환 0), 방어적 try/except import, 스냅샷은 **파일**(DB 아님) 1회 lazy read(가드), litellm proxy 컨테이너는 shared.config 미import.
- **security 자기검증**: ① RBAC — GET=console.access+system.runtime.read, PUT/DELETE=+system.runtime.write(조회 종속, quota 정합), 3핸들러 전부 require_permission(403/401 테스트 12건). ② 입력검증 conn 사용 前 실행(validate_value: 등록키+정수(bool 거부)+[min,max]), 미등록/범위밖 → 400 → DB·스냅샷 오염 차단. ③ SQL %s 바인딩(f-string 사용자입력 0). ④ audit 동일-tx, 실패 시 rollback+500(fail-closed). ⑤ 스냅샷 신뢰경계 — `_load_runtime_setting_overrides` 가 validate 로 필터, resolver/startup_int 가 미등록키 불신·override clamp(변조 스냅샷 방어). ⑥ 신규 권한 admin-only auto-grant+catchup(operator/sales/dba 미부여 — 종료 직전 security 렌즈 확인). ⑦ 범위 하한 sane(operator self-harm 한정, restart 값은 재배포 인지).
- **qa 자기검증**: ① 프론트/백 계약 — admin.js 가 serialize_registry 필드(key/label/unit/default/minimum/maximum/apply_mode/effective/has_override/model/default_known) 1:1 소비. ② 패널 lifecycle — mounter 1회, save/reset 후 재fetch·재렌더, 검색 keyword·count(3행) 정합. ③ save/reset UX — disabled/error toast/재활성/범위·정수·공백 가드/초기화 disabled(무override). ④ read-only(write 없음) 입력·버튼 비활성+read 렌더. ⑤ 모델 목록 카탈로그 자동확장(0행/미지 default_known 처리). ⑥ 기존 pane 무회귀(head 라벨만 중립화, global-prompt 기본 활성 유지). ⑦ cache-buster bump. ⑧ 미커버=DB 왕복(라이브 QA)·잔여 restart timeout 소비처 동작(POST-DEPLOY 관측).
- **검증 산출**: test_runtime_settings.py **25 PASS**(+env-fallback 3) · test_runtime_settings_api.py **12 PASS** · config 서브프로세스 restart-apply PASS · 전체 로컬 스위트 회귀 0(share_redaction 은 로컬 `import web` 경로 아티팩트 — 컨테이너 `make test` 해소) · route_snapshot golden 재생성(내 3 라우트 + 기존 staleness `analyze-schema POST` 1건 흡수). PB-0008 는 정적 자산 baked → POST-DEPLOY(TEST.md §3).
- Cross-ref: CHG-20260706T094937-runtime-settings(feature-0003/0002/shared) / feature-0002·shared MODIFY 동일 slug / TEST.md §3 Run.
- **2차 재검증 라운드 findings 처리(3렌즈 완주, 전원 SHIP)**:
  - backend: BLOCKING-1 fix **VERIFIED 완전**(22키 env-default byte-동치, clamp 는 override 에만). 7개 카테고리 CLEAN. MINOR 3: ① audit 원자성(autocommit) — 아래 security#1 과 동일, **수정 완료** ② worker-before-web reconcile 부팅순서(운영 노트, 아래) ③ hot-path 스펙 재빌드 — `list_specs`/`_spec_index` **메모이즈 완료**(정적 레지스트리). NIT 2(budget floor by-construction·temp path PID)=구조상 안전, 유지.
  - security: VERDICT SHIP. SQL/RBAC/입력검증/CSRF(SameSite=lax) CLEAN. **#1 MEDIUM(수정 완료)**: `_connect_memory` autocommit=True 라 save 즉시 커밋→audit 실패 시 rollback 무효(미감사 변경 잔존). gold-standard(app.py auto-prompt) 정합으로 PUT/DELETE 를 `conn.autocommit=False`+finally 복원으로 감싸 **원자화**(quota 선례의 잠재 결함을 본 feature 는 해소). **#2 LOW(수정 완료)**: `_resolve_int` 미등록키 분기가 override verbatim 반환 → startup_int 정합으로 **override 불신**(default 만) 변경 + 가드 테스트. **#3 LOW(수정 완료)**: 컨트롤플레인 연결 timeout min 1→**3초**(breaker 부재 foot-gun) + 가드 테스트.
  - qa: VERDICT SHIP. 프론트/백 계약·lifecycle·RBAC·검색·count CLEAN. **#2 MEDIUM(수정 완료)**: 모델예산 input 이 effective pre-fill → override 없는 모델에 무변경 '저장'이 값을 명시 고정하는 트랩(미주입≠effective). `buildRuntimeSettingControl(opts.emptyWhenNoOverride)` 로 모델예산은 override 없을 때 **input 비움+placeholder**(빈 저장은 "정수 입력" 거부). reset/status 문구도 default_known=false 면 "모델 기본값" 표기. **#1 MEDIUM(수정 완료)**: serialize row-shape 테스트 부재 → timeout/model 행 필드 assert 테스트 추가. NIT(styles.css 미변경 `?v=` bump)=**되돌림**. 소수점 truncation=NIT 유지.
- **운영 노트(backend MINOR#2)**: restart-mode 값은 web 컨테이너 기동 시 `_ensure_web_tables`→`_reconcile_runtime_settings_snapshot` 가 DB→`/shared` 스냅샷을 재작성한 뒤에야 전 프로세스에 반영된다. worker(agent_core)가 web 최초 reconcile 前 기동하거나 `/shared` 초기화 직후면 그 worker 는 재시작 전까지 env 기본값 사용(restart semantics 와 정합). 배포 시 web 기동·reconcile 이 worker 의존보다 선행하도록 권장(무중단 롤링은 web 우선).
- **재검증 후 산출**: test_runtime_settings.py **29 PASS**(+shape·미등록·min-floor 가드) · test_runtime_settings_api.py **12 PASS** · 컨테이너 전체 스위트 재확인 예정(fix 반영분). 최종 3렌즈 SHIP + BLOCKING 0.

## REV-20260707T110000-runtime-settings-auditfix [SUBAGENT:security 1lens] (TASK-20260707T110000-runtime-settings-auditfix — 런타임 설정 audit action 등록, PB-0008 라이브 적발 hotfix, Minor §12.3, feature-0003 backend-only)
- 발견 경로: feature-0018 배포 후 **PB-0008 실 Windows 브라우저 write-path 검증**(Chrome/149 relay)에서 설정 저장 시 toast "저장 실패: … unknown audit action" 실측. 유닛/API 테스트(TestClient conn=None → audit 도달 前 500)와 3렌즈 패널이 놓친 결함을 라이브 검증이 적발 — §15.4.1 PB-0008 의 가치 재확인.
- 근본: `build_audit_change_json`(ActionCode allowlist)이 endpoint 의 `system.runtime.update`/`system.runtime.reset` 미등록 → `raise ValueError` → 라우터의 autocommit=False save+audit+commit 이 audit 실패로 rollback → PUT/DELETE 500. 원자화(security#1)가 fail-closed 로 미감사 변경 0(DB 무변경) — 원자성 설계는 정상, 기능만 막힘.
- 수정: 두 action builder 등록(update={setting_key, value, previous_value}, reset={setting_key, previous_value}; masked_fields=[] — 비민감 정수 튜닝 파라미터). raise 前 삽입, 기존 action·동작 불변(순수 additive).
