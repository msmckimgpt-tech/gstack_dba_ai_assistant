...
## 4. Open Issues
- **M2-a dual-write 준비 (TASK-0019) ✓ 본 cycle 변경 마감**: KbBackend ABC + skeleton + dialect notes + ADR-0024 + invariant test catalog + Blocker 1-4 해소 + outside-voice NEEDS-TWEAK Critical 4 + Blocker 3건 본 cycle 내 반영. **M2-b 진입 게이트 (사용자 별 turn 책임)**: (1) `.env` 의 `AGENT_KB_PG_REQUIRED=1` 전환 + `KB_DUAL_WRITE_START_TS` 명시 (M2 진입 timestamp), (2) `make start` 재기동 + memory-init 가 `_ensure_pg_schema()` 자동 호출, (3) `grants_present` dict 검증 (모든 role × USAGE / SELECT / mutate / sequence / TRUNCATE_denied 필드), (4) `bin/kb-dual-write-verify.sh --counts --since $KB_DUAL_WRITE_START_TS` 정합 확인. **M2-b cycle 산출 (별 cycle)**: KbBackend 의 5 method × 2 backend = 10 method body 구현 + `_dual_write_kb()` wrapper + caller 수정 (knowledge.py:598/633 + utils.py:957/1179/1230 5 위치) + test fixture/assertion 실 구현 + verify/stress 의 docker exec 본문. 약 800-1000 LOC + integration test 1-2 일.
- **Risk log (outside-voice REV-20260520-0005 / 0007 / 0008 누적, M2-c 진입 게이트로 명시)**:
  0. **Latency baseline 미측정 (outside-voice REV-20260520-0008 Critical)**: M2-b 의 `_DualWriteMirror` 가 매 mirror 호출마다 새 Postgres connection (`_get_pg_conn()` → `psycopg.connect()`). 1 fact write × 4-5 mirror call = `psycopg.connect()` 4-5회. agent 의 1 turn 당 100+ fact write 시 누적 0.5-5s latency 추가 추정. **M2-c cycle 책임**: (a) production-like 환경에서 baseline 측정 (with/without REQUIRED=1), (b) process-level connection pool 의 M3 cycle 도입 priority decision, (c) `bin/kb-measure-baseline.sh --latency` 의 7-day SLA 결과에 본 측정 포함.
