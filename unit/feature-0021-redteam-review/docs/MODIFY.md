...
## CHG-20260729-0007
- Date: 2026-07-29
- Related Requirement: TASK-20260729T173000-reasoning-hint-trim
- Summary: '감사 > AI 운영 현황 > 추론' 의 자가 적대 리뷰 활동 안내 문단을 한 문장(57자)으로
  축약했다. CHG-20260729-0004 에서 회차 구조를 설명한다며 조작법("대화를 펼치면 …", "각
  항목은 접기/펼치기 됩니다")과 다른 화면 경로(설정 > AI 자가 리뷰 / 설정 > 프롬프트)를
  덧붙였는데, 둘 다 **화면을 보면 아는 것**이거나 이 화면에서 할 일과 무관한 내비게이션이라
  매번 읽히는 벽이 됐다(사용자 지적). 상세 계약은 FUNCTION.md §7.5 가 정본이므로 UI 안내가
  그것을 복제할 이유가 없다.
- Files: feature-0003 `src/static/admin.js`(안내 1문장 + 의도 주석);
  feature-0021 `docs/{TASK,MODIFY,REVIEW,TEST}.md`
