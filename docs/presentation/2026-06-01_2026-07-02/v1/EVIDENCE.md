...
- **kind**: git
- **ref**: `git log origin/main --since=2026-06-01 --until=2026-07-02` — 약 1,084~1,089 커밋(수집 시점차), TASK-0128~0307 + TASK-20260619~. 활동 feature: 핵심 9개(0002/0003/0006/0009/0010/0012/0014/0015/0016) + 소규모(0007/0008/0011/0013/0017), git 상 총 ~14.
- **status**: 개발 이력 집계 (단, "머지 완료" = 개발 완료. "라이브 배포·효과"는 항목별 §2). 덱 KPI 배지도 "검증됨" 아닌 **"개발 이력 집계 · 논리 작업 단위"** 로 표기(정직성).
- **note**: baseline 2026-05-29(44d42997) 대비 전량 신규. **"16"의 도출(방어용)**: 채널1(git)이 1,084+ 커밋을 사용자 체감 **논리 작업 단위**로 그룹핑한 수(상위 테마 6의 하위). 9개 feature-id + **feature 미귀속 인프라 작업**(보안 기초·마이그레이션 프레임워크·RAG 활성화·비동기 오프로드·비용 회계 등)을 포함하므로 feature 수(핵심 9 / git ~14)·커밋 수·TASK 수와 **다른 축**이다(1 논리 단위 = 다수 커밋/TASK). 발표 시 "16이 뭘 센 값이냐" 질문에는 이 규칙으로 답한다. 정확한 16 단위 목록은 채널1 수집표.
...
- **status**: 확인 (머지 + 배포 2026-07-02) — 단 배포 근거는 PR#526 머지 + unit REPORT(회귀 스위트); **운영 배포 로그 원문은 원장 미인용**.
- **note**: 계측 범위 정직 표기 — chat 4경로 계측 가능, embedding/provider probe는 구조적 불가(read-time cost 계산 유지). 상부보고 시 "머지·배포 반영" 수준으로 표현하고, 6월 전반 기능 전반의 라이브 효과는 §2 U-005 로 분리.
...
- **note**: **그래프 화면 렌더 속도 수치는 U-003(확인필요).** 60x는 **depth1 이웃조회 서버측 실측**(9454→151ms; 원인=vertex properties 인덱스 부재 → GIN/btree 추가)이고, 화면 렌더 속도는 **브라우저 측**(headless 재현 불가)이라 별개 축이다. (렌더 엔진은 보고일 후속 교체 — U-003 참조.)
...
### E-012 · 웹 라우터 모듈화 (148 route → 21 도메인 APIRouter, app.py 약 35% 축소) — 배포됨
- **kind**: unit-doc / release-note / git
- **ref**: feature-0012 REPORT Phase 0~Final; PR#506/513/516(2026-07-01). app.py 29,111→18,917줄(-10,194 ≈ 35.0%; 보고 시점 스냅샷, 이후 후속 라우트로 소폭 변동), DI seam 선행, byte-동치 검증, route-parity 골든. 라이브 bare-name 500 3건 즉시 수정(ruff F821 전수).
...
### U-003 · 그래프 화면 렌더 속도 개선 수치 (렌더 엔진 개선 반영)
- **왜 확인필요**: 수치 미측정(실측) — 설계 근거이나 headless 재현 불가. (DB 이웃조회 60x[E-010]와 별개 — 이건 브라우저 화면 렌더.)
- **가진 것 / 없는 것**: 렌더 성능 개선 설계·전환 근거 있음 / 실 화면 렌더 속도 계측 결과 없음.
- **엔진 stale 주의**: 보고 작성일(2026-07-02) 후속 커밋(PR#528/ADR-004)으로 그래프 **렌더 엔진이 교체**됨. 발표자료·발표 시 특정 엔진명·FPS 수치를 확정 서술하지 말고 "그래프 렌더 성능 개선(설계 반영)"으로 표현, 엔진 전환·실측은 **다음 기간 성과로 이월**.
- **확인 방법**: 실 브라우저 성능 프로파일 계측(교체 후 엔진 기준).
...
- **성능 개선 수치**: 확인=이웃조회 60x(depth1 서버 실측)·무중단 부하 104건 중단 0. 확인필요=그래프 화면 렌더 속도(U-003, 엔진 교체 반영)·NL→SQL 정확도(U-006).
