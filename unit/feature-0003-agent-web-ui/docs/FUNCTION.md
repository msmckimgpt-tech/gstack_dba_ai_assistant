...
## REQ-20260910T120000-client-entry-gate — 일반 브라우저는 설치 안내로

**사용자 요청 (2026-09-10)**: "DQA클라이언트 외, 일반적인 웹브라우저로 접속할 경우 클라이언트
설치 안내페이지로 연결해주세요. 또한, 안내 페이지도 사람 입장에서 친숙하게, AI Slop 느낌이
나지 않도록 웹 리서치를 통해 모범적인 디자인으로 구성해주세요."

**사용자 결정 (2026-09-10, AskUserQuestion)**: 게이트 대상 = **앱 루트 + 관리 콘솔** ·
**탈출구 없음**(앱 전용 일관 — REQ-20260908-share-client-entry 의 「웹 폴백을 두지 않는다」와
같은 방향).

**위험도 Major (§12.3)** — 제품 전체의 **진입 경로**를 바꾼다. 인증·인가 코드·스키마·권한
데이터는 무변경이고 비파괴·가역이지만, 오판정 1건의 대가가 「사용자가 제품에 들어가지
못함」이라 등급을 낮추지 않는다.

- AC-20260910T120000-client-entry-gate-1 (막는 자리는 열거된 셋뿐): `static/client-gate.js` 의
  `GATED` = `{"/", "/index.html", "/admin"}` 이며 이는 정본 모듈 `static/app/client-bridge.js`
  의 `_PERSIST_SURFACES` **∪ `{"/admin"}`** 이다. 기본값이 「통과」라 새 페이지가 생겨도 모르는
  사이에 닫히지 않는다(공유 화면 allow-list 극성 사고의 반대 축).
- AC-20260910T120000-client-entry-gate-2 (통과 신호는 브리지 좌표 하나, 관측 자리가 둘):
  ① 주소의 `?client_port=&client_nonce=`(앱이 창을 연 첫 적재) ② `sessionStorage["dqa.bridge"]`
  (새로고침·앱 안 화면 이동). 둘 중 하나면 통과. **판정 불가는 통과시킨다** — 저장소가 막힌
  환경·`URLSearchParams` 부재. 잘못 막으면 제품 전체를 잃고 잘못 통과시키면 안내를 못 볼
  뿐이라, 두 오류의 크기가 다르다(`share.js appPlatformSupported` 와 같은 규칙).
- AC-20260910T120000-client-entry-gate-3 (안 막는 자리): `/share/{token}`(익명 열람은
  2026-09-08 결정으로 **브라우저가 정상 경로**) · `/ai/connect` · `/ai/oauth/callback` ·
  `/install` · `/healthz` · `/client/{filename}` · `/api/**` · `/static/**`.
- AC-20260910T120000-client-entry-gate-4 (판정 자리는 브라우저다): 게이트는 서버가 아니라
  `<head>` 의 **동기 classic 스크립트**다. 좌표는 첫 적재 뒤 주소에서 지워지므로 서버는 두
  번째 요청부터 그 사실을 볼 수 없고, 서버에서 막으면 앱 창이 새로고침 한 번에 자기 자신에게
  차단당한다. 스타일시트보다 앞서고 `type="module"`·`defer`·`async` 를 쓰지 않는다(뒤에 두면
  화면이 반쯤 그려졌다가 튕긴다). ES module 로 두지 않는 이유는 `app.js` 와 적재 실패 축을
  공유하지 않기 위해서다 — 모듈 하나가 깨지는 날 앱 창이 자기를 브라우저로 오인한다.
- AC-20260910T120000-client-entry-gate-5 (안내 페이지는 있는 것만 광고한다): `/install` 은
  `GET /api/ai/client/entry` 가 준 값만 그린다. `release` 는 `current_release()` 가 실물·크기·
  sha256 대조를 통과시킨 것만 담고 없으면 `null` — 그때는 받기 버튼 자체를 그리지 않는다
  (feature-0046 §P0-I). 서버에 못 물어본 경우도 같은 자리로 떨어진다.
- AC-20260910T120000-client-entry-gate-6 (딥링크는 정본이 조립하고 자격증명을 싣지 않는다):
  「이미 설치했다면 열기」의 `dqa-connect://open?…` 은 `shared.dqa_identity.app_open_url` 이
  만든다. 프런트가 문자열을 조립하면 개명이 도달하지 않는 자리가 하나 더 생긴다. 토큰
  미포함 — 이 화면은 미로그인 방문자에게도 뜬다.
- AC-20260910T120000-client-entry-gate-7 (다른 OS 는 막다른 길로 두지 않는다): DQA 앱은
  Windows 전용이므로(feature-0046 §4) macOS·Linux·모바일에서는 받기 대신 **그 사실과 어디서
  열면 되는지**를 말한다. 사용자 결정에 따라 「브라우저로 계속」 탈출구는 두지 않되, 공유
  링크 열람이 그 기기에서도 그대로 된다는 사실은 적는다. 판정 불가(UA 를 못 읽음)는 받기를
  보여 준다 — 받기 블록이 지원 OS 를 문자로 함께 말하므로 오판의 대가가 작다.
- AC-20260910T120000-client-entry-gate-8 (표시이지 집행이 아니다): 자격은 서버의 로그인 세션·
  RBAC 이 그대로 집행한다. 이 cycle 은 인증·인가 코드를 건드리지 않으며, 우회해서 얻는 것은
  권한이 아니라 「어느 화면에서 봤는가」뿐이다(REQ-20260908-share-client-entry 와 동형의
  **알려진 성질**이며, 기록해 두지 않으면 다음 사람이 이 유도를 통제로 오인한다).
- AC-20260910T120000-client-entry-gate-10 (외부 AI 인가의 «로그인 순간» 은 열어 둔다):
  `GET /api/ai/oauth/authorize` 는 외부 AI 도구가 시스템 브라우저로 여는 사람-동의 흐름이고,
  미로그인이면 서버가 `/?next=<그 인가 URL>` 로 되돌린다(`routers/oauth_as.oauth_authorize` —
  로그인 UI 가 SPA 안에만 있다). 게이트는 **그 착지점 하나만** 통과시킨다(`next` 가
  `/api/ai/oauth/authorize` 로 **시작**할 때). 막으면 `next` 가 소실돼 외부 AI 연결을
  브라우저에서 끝낼 방법이 사라지고, 앱의 WebView2 는 로그인 저장소가 브라우저와 별개라
  앱에서 로그인해도 복구되지 않는다. 이것은 사용자 결정의 「탈출구 없음」(= 앱을 못 쓰는
  사람에게 주는 우회로)과 **다른 축**이다 — 사람이 고르는 우회로가 아니라 기계 흐름이
  요구하는 로그인 한 순간이다. 서버 라우트 경로와 게이트 접두의 일치는 테스트가 정본
  대조로 잠근다.
- AC-20260910T120000-client-entry-gate-11 (신호가 «살아남을 수 있는 창인가» 를 묻는다):
  저장소 읽기가 예외 없이 빈 값을 내는데 쓰기가 막힌 창(quota 초과 등)에서는 정본 모듈이
  좌표를 보관하지 못한 채 끝난다. 그 빈 값을 「앱이 아니다」로 읽으면 정상 앱 창을 쫓아내므로,
  신호가 없을 때 **쓰기 probe** 로 한 번 더 갈라 판정 불가면 통과시킨다.
- AC-20260910T120000-client-entry-gate-12 (판정 전에는 주소의 좌표를 지우지 않는다):
  보관 허용 표면 **밖**(`/admin` 등)의 보관은 로컬 브리지 ping 판정을 기다리는 비동기다.
  종전처럼 흡수 직후 주소를 비우면, 판정이 오기 전 새로고침한 앱 창에 신호가 **하나도**
  남지 않는다. `static/app/client-bridge.js` 는 주소 정리를 **판정 이후**(수용·거절·오류
  모두)로 미룬다 — 새로고침이 같은 주소를 다시 보내므로 그 사이에도 통과한다.
  ⚠ 보관 허용 표면에서 `setItem` 이 **실패한** 경우도 같다: 주소의 좌표가 그 창의 유일한
  신호이므로 지우지 않는다.
- AC-20260910T120000-client-entry-gate-13 (「못 물어봤다」와 「없다」를 가른다): `/install` 은
  통신 실패·비-200 을 «설치 파일 정보를 불러오지 못했습니다» + [다시 시도] 로, 게시된 배포본
  부재를 «지금은 내려받을 설치 파일이 없습니다» 로 **따로** 말한다. 사용자가 할 일이 다르다.
  그리고 「이미 설치했다면 열기」 딥링크는 **받기 블록 밖**에 둔다 — 설치기를 철회해도 이미
  설치한 사람은 앱을 열 수 있어야 한다(두 축을 결합하지 않는다).
- AC-20260910T120000-client-entry-gate-9 (안내 화면은 제품의 디자인 시스템을 상속한다):
  `install.html` 은 `css/base.css` 토큰(warm canvas `--bg` · `--font` Geist · 시맨틱 색 ·
  다단 radius)을 쓰고 자기 값을 정의하지 않는다. 리서치가 짚은 AI-slop 지문(Inter 기본값,
  보라-파랑 그라디언트, 공허한 헤드라인, 동일 가중치 카드 나열, 장식용 모션)을 도입하지 않고,
  화면을 **실제 값**(버전·용량·게시일·SHA-256·SmartScreen 경고)으로 채운다. 문안 예산은
  §16.8 을 따른다 — 화면이 이미 보여 주는 조작법·내부 구조는 적지 않는다.

## REQ-20260909-deploy-refresh — 서버 배포 완료 자동 반영

DQA 대화 화면은 `/api/ui-release`를 3초 주기로 확인한다. 두 web replica가 같은 revision·asset stamp로 준비되고 배포 검증까지 끝난 완료 신호만 적용한다. 서버 변경만 있는 동일 자산 배포도 감지한다. 통신 실패·혼합 배포·오래된 신호는 현재 화면을 유지한다.

안전한 상태에서는 현재 대화·모델·저장된 첨부 선택·메시지 위치·계보 비교 쌍과 줄 위치를 복원하며 화면을 자동 갱신한다. 초안, 새 첨부/업로드, 다른 대화를 포함한 진행 요청, 미저장 편집, IME 조합 및 드래그 중에는 안내 후 안전해질 때까지 기다린다. 본문·파일 내용·자격증명은 복원 저장소에 적재하지 않는다. 자동 재로드는 같은 대상에 5분 내 2회로 제한한다. 로그아웃/재초기화로 감지기를 교체하면 이전 감지기의 지연 응답은 폐기한다.

적용 대상은 로그인한 주 대화 화면이다. 별도 관리/공유/연결 페이지와 네이티브 실행 파일 업데이트는 이 감지기의 대상이 아니다. 최초 도입 시 감지 코드가 없는 기존 페이지는 한 번 새로 로드해야 하며, 그 다음 배포부터 자동 적용한다.
...
- REQ-20260909T163000-attach-csv-table (TASK-20260909T163000-attach-csv-table, **Minor §12.3** — `static/app/attach-diff.js` · `static/css/chat.css` + 신규 테스트 2종. 서버·스키마·RBAC·엔드포인트·저장형식 0, 비파괴 표시 계층): **첨부 `.csv`/`.tsv` 를 본문 보기에서 «표(격자)» 로 출력한다.** 사용자 원문(2026-09-09): "DQA클라이언트 첨부파일 중, csv 확장자가 표 형태로 출력될 수 있도록 구성해주세요." 종전에는 CSV 첨부를 열면 줄번호 + 평문 표가 나왔고 구문 색이 구분자만 강조했다 — 열이 눈으로 갈리지 않아 사용자가 표를 머릿속에서 재구성해야 했다. `.md` 첨부를 문서로 렌더한 선행 경로(REQ-20260812T2030-attach-md-render)와 같은 성격의 요청이며 같은 관용구(원문은 토글로 남기고 기본은 렌더)를 쓴다. AC-20260909T163000-attach-csv-table-1 ~ -4.
  - AC-20260909T163000-attach-csv-table-1 (값을 잃지 않는다): `parseDelimitedText` 는 RFC 4180 을 따르되 **깨진 입력에서도 값을 버리지 않는다** — 짝 없는 따옴표, 인용 필드 중간에서 끊긴 절단본(서버가 `_ASSISTANT_EDIT_SIZE_CAP_BYTES`·`_VERSION_DIFF_ROW_CAP` 으로 잘라 보낸 본문이 실제로 이 화면에 온다), 행마다 다른 열 수, BOM, CRLF 를 모두 값 보존으로 처리한다. 파싱이 값을 삼키면 사용자는 «잘렸다» 는 배너만 보고 **무엇이** 사라졌는지 모른다. 인용 시작은 필드 첫 글자에서만 인정한다(Excel·RFC 동일) — 중간 따옴표를 인용으로 읽으면 열이 통째로 밀린다.
  - AC-20260909T163000-attach-csv-table-2 (셀은 값이지 마크업이 아니다): 셀·머리글은 `textContent` 로만 채운다. 마크다운 경로가 `marked`+`DOMPurify` 를 거치는 이유는 그쪽 **입력이 마크업**이기 때문이고, 여기 입력은 **값**이다 — 값에 파서를 물리면 없는 공격면을 새로 만든다. 첨부 본문은 사용자가 올린 임의 바이트이고 그룹 대화에서는 멤버 전원이 연다.
  - AC-20260909T163000-attach-csv-table-3 (거짓 어포던스를 만들지 않는다): «표로 보기» 토글은 **그릴 본문이 실제로 온 화면**(`_bodyState(...).sourceView`)에서만 뜬다 — 바이너리·빈 문서·조회 실패·**변경이 있는 비교 화면**에서는 뜨지 않는다(줄 대조를 격자로 바꾸면 어느 줄이 바뀌었는지가 사라진다). 표를 보는 동안 구문 색 토글은 숨는다(칠할 원문 줄이 화면에 없다). 표로 만들 수 없는 본문은 조용히 빈 격자를 그리지 않고 원문 표 + 사유 배너로 강등하되, 강등은 사용자의 선택이 아니므로 저장값은 건드리지 않는다.
  - AC-20260909T163000-attach-csv-table-4 (상한을 조용히 넘기지 않는다): 서버는 줄 수만 자르고 **열 수·셀 총량은 모른다**. `TABLE_COL_CAP=200` · `TABLE_CELL_CAP=30000` 을 프론트에서 걸고, 잘린 사실은 배너로 알린다(§16.7 G9-b). 첫 레코드를 머리글로 쓰되 행 번호를 **파일 레코드 번호 그대로**(머리글=1) 매겨 그 추정이 화면에서 읽히게 한다 — 머리글이 아니었더라도 값은 사라지지 않는다.
...
- REQ-20260908T204500-step-narration-seal (TASK-20260908T204500-step-narration-seal, **Minor §12.3** — `routers/conversations.py` · `routers/_conv_store.py` · `static/app.js` · `static/css/profile.css` + 테스트. 스키마·RBAC·엔드포인트·데이터 0): **실행 단계 문구의 남은 노출 경로를 닫고, 앞 cycle 의 조치를 되돌릴 수 없게 잠근다.** 계기는 `verification_debt`(subagent 3차 교차검증)를 갚은 확인 검증이 낸 지적 7건이다. AC-20260908T204500-step-narration-seal-1 ~ -4.
  ...
  - AC-20260908T204500-step-narration-seal-3 (지어낸 근거는 그렇게 표시한다): `reason_source == "derived"` 인 근거는 「근거(추정)」 라벨 + muted 색 + 툴팁으로 AI 가 말한 근거와 구분한다. 「지어내지 않는다」가 이 기능의 계약이므로, 파생을 남기려면 구분이 함께 있어야 한다.
  - AC-20260908T204500-step-narration-seal-4 (census 의 부분 실패를 삼키지 않는다): 조회 출처 둘 중 **하나라도** 비면 WARN + 폴백 보충 + **비캐시**로 처리한다. 캐시하면 기동 순서 때문에 한 번 좁아진 모수가 프로세스 수명 동안 고정되고, 좁아진 census 는 판정기와 라벨 가드의 모수를 동시에 줄여 「가드가 결손을 못 보는」 상태를 만든다.
- REQ-20260908T125500-step-tool-syntax-leak (TASK-20260908T125500-step-tool-syntax-leak, **Major §12.3** — `routers/_conv_store.py` · `routers/ai_tools.py` · `app.py` · `static/app.js` · `static/app/progress.js` · `static/css/profile.css` + 신규 테스트 2종. 스키마·마이그레이션·RBAC·엔드포인트 0, 비파괴 표시 계층): **실행 단계 패널이 «도구 호출 표기» 를 사용자에게 그대로 보여주지 않는다.** 사용자 원문(2026-09-08): "실행 단계에서 나타나는 각 도구의 구문이 그대로 클라이언트에 노출되는 이슈가 확인되어 수정이 필요합니다." 라이브 원장 실측으로 출처가 확정됐다 — `agent_runtime.steps` 의 `work_source='external-ai'` **30행 중 19행(63%)** 이 `describe_table {'schema_name': 'coupon', ...}` 형태였다. 이 `work` 는 연결된 개인 AI 가 `POST /api/ai/tools/<name>` 본문에 실어 보내는 값인데, 브리지 프롬프트는 `reason` 만 규정하고 `work` 는 **규정조차 하지 않는다** — 계약 없는 비신뢰 입력을 서버가 그대로 저장하고 화면이 그대로 그렸다. 프롬프트로 닫지 않는 이유: 프롬프트 계약은 지시이지 집행이 아니고, 러너는 사용자 PC 에 있어 낡은 빌드가 남는다. REV-20260908T131500-step-tool-syntax-{backend,security,qa,ux-design,codex}. AC-20260908T125500-step-tool-syntax-leak-1 ~ -5.
  - AC-20260908T125500-step-tool-syntax-leak-1 (판정 축은 «호출 표기» 이지 «식별자처럼 보임» 이 아니다): `_conv_store._step_text_is_tool_syntax` 는 둘만 서명으로 본다 — (a) 인자 매핑 리터럴(`{'k': …}`·`{"k": …}`) (b) **서버 도구 census 의 이름**이 독립 식별자로 등장. 판정 전에 NFKC 정규화 + 제로폭·bidi 제거를 거쳐 전각 밑줄(`describe＿table`)·전각 라틴 우회를 닫는다. **초판의 「인용 없는 snake_case 로 시작」 규칙은 폐기됐다** — 라이브 원장 9,759행 전건 재생에서 그 규칙이 걸러낸 412행 중 실제 도구 구문은 18행뿐이고 394행(96%)이 정상 제목이었다(이 도메인은 테이블 이름이 곧 사용자의 어휘다). census 는 `modules.tools` 정의 객체 조회 + 코드가 직접 단계로 남기는 이름(`materialize_attachment`·`query_sql`)으로 구성한다 (§16.7 G12-b).
  - AC-20260908T125500-step-tool-syntax-leak-2 (표시 문구를 만드는 이음매는 하나다): `_conv_store._step_display_narration` 이 `(work, work_source, reason, reason_source)` 를 만들고, 완료 경로(`_resolve_step_display`)와 진행 중 경로(`ai_tools._bridge_live_steps`)가 **그것만** 부른다. 계약 셋 — ① 나가는 값은 저장 문구든 파생 문구든 **같은 판정을 통과한다**(파생값도 비신뢰 `args` 에서 만들어지므로 재정화가 없으면 그 자체가 우회로다) ② 길이 상한이 파생값에도 걸린다(255/500) ③ 출처를 정직하게 가른다(원문 부재 `legacy` / 원문을 버림 `derived` / 통과 `llm`). 두 경로가 각자 정화·파생하면 제출 순간 같은 단계의 제목이 바뀐다.
  - AC-20260908T125500-step-tool-syntax-leak-3 (사유 축은 (b) 를 쓰지 않는다): `reason` 에는 (a) 만 적용한다. 사유는 산문이라 `… 단일 execute_sql 쿼리로 UNION 집계 …` 처럼 도구 이름을 **설명 안에서 언급**하는 것이 자연스럽고, 그것을 지우면 「왜」 칸이 통째로 빈다. 표시 경로에도 `agent_core._derive_step_reason` 파생을 배선해, 같은 행이 적재 시점에 따라 근거 줄이 있고 없고가 갈리지 않게 한다.
  - AC-20260908T125500-step-tool-syntax-leak-4 (파생도 배지도 식별자를 내보내지 않는다): 종전 파생 폴백 ``f"`{tool}` 도구를 실행한다"`` 와 배지의 `TOOL_LABEL_MAP[tool] || tool` 이 같은 결함의 다른 갈래였다. `_derive_step_work_tail` 은 표시층 고유 도구의 문구를 **인자를 받는 콜러블**로 갖고(대상 파일명·키워드가 문구에 남는다 — 상수면 형제 행 사이 정보 밀도가 비대칭이 된다), 그 밖은 내부 경로에 위임하며, 어느 쪽도 식별자를 담으면 `요청한 작업을 수행한다` 로 강등한다. 배지 표는 census 전체를 덮고 미지 도구는 `"도구"` 로 떨어진다. 제목 폴백의 두 단(`intent` = `<도구명>: …` · `tool` = 식별자)은 제거하고 `stepTitleText` 하나로 모은다 — 렌더러 **전수**(상세·활동·진행 카드·대기 말풍선)가 그것을 쓴다.
  - AC-20260908T125500-step-tool-syntax-leak-5 (기계 표기도 사용자 표기가 아니다): 서버 파생 문구는 식별자를 마크다운 백틱으로 감싸는데 제목은 `textContent` 라 백틱이 **글자 그대로** 찍힌다. 표시층 `stripDisplayTicks` 가 벗긴다(서버 원문의 백틱은 감사 가치가 있어 그대로 둔다). `intent` 는 익명 공유 화이트리스트와 진행 payload에서 제거한다(라이브 실측 21행이 `<도구명>: <유출문구>` 였다). 프런트에는 인자 리터럴을 일반 문구로 강등하는 **3줄 심층 방어**를 둔다 — 서버가 정본이되 단일 실패점이 되지 않게 한다.
...
- REQ-20260908-share-client-entry (20260908T1255-share-client-entry, **Major §12.3** — feature-0003 프론트 `static/share.html`·`static/share.js`·`static/share.css`·`static/share-client-context.js`(신규) + 라우터 `routers/share.py`(응답 1블록 추가) + 정본 `shared/dqa_identity.py` + feature-0046 클라이언트 `client/{core,gui,window}.py`; 인증·인가 코드 무변경·스키마 0·비파괴·가역): **공유 링크의 열람과 참가를 서로 다른 자리에 둔다.** 대화를 링크로 공유했을 때 받는 사람은 **평범한 웹브라우저로 내용을 그대로 볼 수 있고**(익명 포함, 종전과 동일), 그 대화에 **참여(join)하거나 자기 계정으로 fork 하려면 DQA 클라이언트를 거친다**. 사용자 원문: "DQA클라이언트가 구성됨에 따라, 대화를 링크로 공유할 때 일반적인 웹브라우저로 해당 링크를 통해 내용을 확인할 수 있지만 대화에 참여하거나 fork 하려면 클라이언트를 통해 진행되도록 정합하게 구성해주세요." 사용자 결정 2026-09-08: **앱 전용 + 받기 안내**(웹 폴백을 두지 않는다 — `connect-modal` 이 2026-09-07 에 터미널 경로를 접은 것과 같은 판단) · **집행은 프런트 유도까지**(서버 join/fork 게이트 무변경). `/_template:entry` arg-given dispatch. REV-20260908T125500-share-client-entry. AC-20260908T125500-share-client-entry-1 ~ -6.
  - AC-20260908T125500-share-client-entry-1 (열람은 그대로 브라우저에서 끝난다): `/share/{token}` 은 익명 접근 가능한 정적 페이지이고 `/api/public/share/{token}` 의 메시지·SQL·결과셋 렌더 경로는 **바뀌지 않는다**. 이 cycle 은 하단 액션 바만 건드리며, 공유 window·redaction·버전 페이징 계약은 무변경이다.
  - AC-20260908T125500-share-client-entry-2 (참가 진입은 앱 하나): 평범한 브라우저에서 하단 바에 보이는 것은 `[링크 복사]` 와 **`[DQA 앱에서 참여 · fork]`** 뿐이다. `[대화에 참여]`·`[내 계정에서 fork]` 직접 실행 버튼은 **앱 창 안에서만** 나타나고, 「로그인하고 참여 · fork」 웹 링크는 **제거**됐다(웹에서 로그인해도 누를 버튼이 없어 그 링크가 곧 막다른 길이 된다).
  - AC-20260908T125500-share-client-entry-3 (판정 신호는 브리지 좌표 하나): 「지금 앱 창 안인가」는 `window.__dqaClientBridge` 로만 판정한다. 그 값은 `static/share-client-context.js` 가 **정본 모듈 `static/app/client-bridge.js` 를 import 해** 얹는다 — 공유 화면이 `?client_port=&client_nonce=`·`sessionStorage["dqa.bridge"]` 규약을 다시 구현하지 않는다(규약 사본이 세 곳이 되면 「앱 안인데 앱 밖으로 보이는」 화면이 생기고, 그 화면은 사용자를 앱으로 되보내는 무한 왕복을 만든다).
  - AC-20260908T125500-share-client-entry-4 (딥링크는 서버가 정본으로 조립하고, 토큰을 싣지 않는다): `/api/public/share/{token}` 응답에 `client: {app_link, download_url}` 이 실린다. `app_link` 는 `shared/dqa_identity.app_open_url(origin, "/share/<token>")` = `dqa-connect://open?path=…&base=…` 이며 **베어러 토큰이 없다** — 이 링크는 *공유 링크를 받은 사람*의 화면에서 만들어지고, 앱 창은 브라우저 세션(또는 내장 창 자신의 세션)으로 인증되므로 토큰을 한 번 더 실으면 노출 지점만 는다. 프런트는 스킴 문자열을 조립하지 않는다(개명이 도달하지 않는 자리를 만들지 않는다 — 이 이름은 한때 리터럴로 21곳에 흩어져 있었다).
  - AC-20260908T125500-share-client-entry-5 (앱은 **그 대화를** 연다): 클라이언트가 `path` 를 받아 앱 창의 첫 자리로 쓴다(`ConnectPlan.path` → `appwindow.panel_url`). 이미 떠 있는 인스턴스로 온 경우에도 목적지가 함께 전달되어(`core.request_show(home, path)` + `show.path`) 내장 창은 `Shell.navigate` 로 옮긴 뒤 보이고, 브라우저 셸은 그 경로로 새 창을 연다. 경로 검증(`safe_app_path`)은 **조립·수용 양쪽**에 같은 표로 있으며 `//evil` protocol-relative·역슬래시·`..`·제어문자·길이 초과를 거부한다. 부적격 값은 **키만 버려 루트로 열화**한다 — 나쁜 링크 하나가 앱 실행 자체를 막으면 「눌렀는데 아무 일도 없다」가 된다. `path` 를 모르는 구버전 클라이언트도 `base` 를 읽어 루트를 연다(의도된 degrade).
  - AC-20260908T125500-share-client-entry-6 (막다른 길을 만들지 않는다): 브라우저는 **스킴 핸들러 부재를 감지하지 못하므로**, 진입 버튼을 누른 직후 안내가 뜨고 `[DQA 앱 받기]` 는 **서버가 실물을 확인했을 때만**(`oauth_as._client_download_url`) 함께 보인다 — 실물이 없으면 안내 문구도 그 버튼을 가리키지 않는다. 참여도 fork 도 불가능한 링크에서는 진입 버튼 자체를 보이지 않는다(할 수 없는 일을 권하면 사용자는 앱을 연 뒤에야 그것을 안다). 서버가 `app_link` 를 내지 못한 회차에는 **종전 웹 버튼으로 열화**한다 — 「앱 전용」의 예외가 아니라 우리 쪽 장애일 때의 열화다.
...
- REQ-20260901T031500-interrupt-preserve-bridge (20260901T0315-interrupt-preserve-bridge, **Minor §12.3** — `feature-0003/src/routers/conversations.py` + `feature-0002/src/agent_core.py`(`header` 파라미터) + 신규 테스트 12건. 스키마·RBAC·엔드포인트 0, 비파괴 텍스트 추가): **브리지(개인 AI) 경로의 중단도 진행 단계를 다음 요청의 맥락으로 넘긴다.** 앞 REQ 는 서버 run 경로를 고쳤는데, 라이브는 `feature-0043` 전환 모드(서버 계정 LLM 차단 — 배포 스모크가 매번 확인)라 **사용자가 실제로 누르는 중단은 브리지 경로**를 탄다. 그 경로를 축별로 추적하면 화면·이력(취소 안내 말풍선)과 단계 접이식(말풍선이 `run_id = task_id` 각인을 유지 + `_record_bridge_step` 이 개인 AI 의 도구 호출을 steps 에 적재)은 **이미 성립**했고, **다음 요청의 맥락만** 비어 있었다 — 브리지가 개인 AI 에게 넘기는 대화 맥락(`ai_tools._recent_conversation_context`)은 표시 store 의 **content 텍스트만** 읽고 steps·meta 를 보지 않기 때문이다. 그래서 중단 뒤 "아까 그거 이어서" 라고 물어도 개인 AI 는 자기가 직전에 무엇을 조사했는지 몰랐다. REV-20260901T031500-interrupt-preserve-bridge. AC-20260901T031500-interrupt-preserve-bridge-1 ~ -3.
  ...
  - AC-20260901T031500-interrupt-preserve-bridge-3 (실패·부재는 종전 동작으로 degrade): 단계 조회 실패(PG 다운)와 단계 부재(도구 호출 전 중단)는 모두 빈 문자열을 반환해 **종전 취소 안내 그대로**가 된다 — 단계 기록은 취소의 조건이 아니며, 여기서 예외를 올리면 이미 확정된 취소의 화면 표시조차 사라진다. 머리말도 꼬리와 **함께만** 붙어, 단계가 없을 때 "진행된 내용" 을 단정하지 않는다. 개인 AI 의 부분 추론 자체는 러너 쪽에 있어 미커버이며, 우리가 남기는 것은 우리 도구로 관측한 사실뿐이다.
- REQ-20260901T020746-interrupt-context-preserve (20260901T0207-interrupt-context-preserve, **Minor §12.3** — `feature-0003/src/routers/{conversations,ai_discovery}.py` + `static/app.js` + `feature-0002/src/agent_core.py` + 신규 테스트 2종 28건. 스키마·마이그레이션·RBAC·신규 엔드포인트 0, 비파괴 메시지 1건 추가·가역): **대화 중 요청을 중단해도 그때까지의 추론·맥락·단계가 남는다.** 종전에는 보존 축(`preserve_reasoning`)이 이미 있었는데도 **명시 '중단' 버튼만 거기서 빠져** 있었다 — `/api/cancel` 의 `bool(data.get("preserve_reasoning"))` 가 미지정을 폐기로 접어, 그 경로에서는 ① 화면(진행 카드가 지워지고 앵커 메시지 없음) ② 이력 ③ 다음 요청의 LLM 맥락(`core_messages` 미기록) **세 축이 동시에** 비었다. 진행 중 새 발화로 끼어드는 경로(`_interruptCurrentRunForResend`)만 `preserve_reasoning: true` 를 보내 보존됐다. 부수적으로 보존 본문의 근거인 `steps` 는 **도구 step 만** 담아(`steps.append` 는 tool 분기 1곳), 도구 호출 전("맥락 불러오는 중"·"추론 중")에 중단하면 보존 경로에 태워도 남길 문장이 0 이었다 — 사용자가 중단을 누르는 전형적 시점이 정확히 그 국면이다. 사용자 원문: "프로젝트 내 서비스에서, 대화 중 assistant에게 요청했던 작업을 중단(interrupt) 하더라도 추론했던 내용, 맥락, 단계가 손실되지는 않도록 구성해주세요." 사용자 결정(AskUserQuestion, 2026-09-01): 화면+다음 맥락 모두 보존하되 "방향이 틀려서 재요청한 경우도 LLM이 충분히 판단할 수 있는 구조로" · 보존 범위 = 단계 목록 + 근거 요약 · 재개(이어서 진행) 기능은 범위 밖. `/_template:entry` arg-given dispatch. REV-20260901T020746-interrupt-context-preserve. AC-20260901T020746-interrupt-context-preserve-1 ~ -4.
  - AC-20260901T020746-interrupt-context-preserve-1 (기본값 = 보존, 두 표면 공통): `/api/cancel` 은 `preserve_reasoning` 을 **미지정 시 `True`** 로 읽고(`data.get("preserve_reasoning", True)`), 폐기는 호출자가 **명시적으로 falsy** 를 보낸 경우뿐이다(향후 '버리고 중단' UI 의 자리). 기본값을 프런트가 아니라 **서버**에 둔 이유는 `/api/cancel` 이 외부 AI 도구 표면(`ai_discovery.py` 의 discovery 목록 + OpenAPI)에도 공개돼 있어, 프런트만 고치면 그 경로는 계속 폐기하기 때문이다. 외부 스펙에도 파라미터와 기본값(`default: true`)을 노출한다.
  - AC-20260901T020746-interrupt-context-preserve-2 (보존 본문 = 미완 라벨 + 진행 단계 + 근거): 보존 메시지 본문은 `_build_interrupted_note` 가 세 구획으로 만든다 — ① **미완 라벨**("완료된 답변이 아니라 중단 시점까지 진행한 중간 기록 … 이어지는 지시가 이와 다른 방향이면 그 지시를 따르세요") ② `진행 단계:` 도구 step 목록(`[도구] 무엇을 했는지`, 상한 20줄 + "외 N단계", 줄당 160자) ③ 기존 `단계별 근거` 요약, 그리고 있으면 `진행 중이던 답변`. 라벨이 **본문 안**에 있어야 하는 이유: 이 텍스트는 `core_messages` 를 타고 다음 run 의 recall 에 그대로 실리는데, mirror meta 의 `interrupted: true` 는 화면 렌더 전용이라 모델이 읽지 못한다 — 라벨이 없으면 모델이 중간 기록을 확정 결론으로 읽어, 방향을 바꾸려 중단한 사용자에게 폐기된 가설 위에서 답을 잇는다. 도구 결과 원문은 싣지 않는다(접이식 "쿼리 결과" 로 화면에서 이미 볼 수 있고, recall 에 넣으면 중단 1회가 다음 run 입력을 크게 부풀린다).
  - AC-20260901T020746-interrupt-context-preserve-3 (도구 호출 전 중단도 남는다 / 빈 손이면 안 남긴다): 도구 step 이 0 이면 **활동 라벨 trail**(`_emit_activity` 가 `_append_activity_trail` 로 in-process 적재, 상한 40·꼬리 우선)로 `진행 상황:` 을 대신 싣는다(중복 제거, 최근 8줄). activity step 은 진행 단계 목록에서 제외해 화면 접이식(`bubbleVisibleSteps`)과 같은 기준을 쓴다. 반대로 단계·활동·근거·부분답변이 **모두 비면 빈 문자열을 반환해 메시지를 저장하지 않는다** — 헤더만 남은 말풍선은 정보가 0 인데 다음 run 의 맥락만 오염시킨다.
  - AC-20260901T020746-interrupt-context-preserve-4 (단계 UI 복구는 앵커 1건으로 / 화면 반영은 유한 재확인): 취소해도 `agent_runtime.steps` 행은 지워지지 않고(`_purge_run_steps` 는 호출부 0), history 조립부가 assistant 메시지마다 `_load_steps_for_message` 로 그 run 의 steps 를 붙여 내려보내므로, **보존 메시지 1건이 남는 것만으로** 접이식 "실행 단계"·"쿼리 결과" 가 복구된다(별도 저장·별도 렌더 경로 없음). 프런트는 중단 시 진행 폴링을 멈추므로 뒤늦게 쓰이는 보존분을 볼 트리거가 없어, `_reloadForPreservedInterrupt` 가 **1.5s·4s·10s 3회만** 다시 읽는다 — `preserveScroll: true` 로 읽던 위치를 지키고, 보존분이 붙었거나 사용자가 대화를 옮겼거나 새 요청을 시작하면 즉시 물러난다(무한 재로드 순환 금지). 중단 토스트는 "요청을 중단했습니다. 진행된 내용은 대화에 남습니다." 로, 중단이 버리는 동작이 아님을 알린다.
...
- REQ-20260908-attach-folder-tree (TASK-20260908T124500-attach-folder-tree, **Minor~Major §12.3** — feature-0003 업로드·목록·프론트 + cross-cut feature-0002 LLM 컨텍스트; 비파괴 컬럼 추가(nullable, backfill 없음)·RBAC/엔드포인트 shape 무변경): 첨부 전달의 단위를 **파일에서 폴더(디렉토리 트리)** 로 넓힌다. 사용자가 폴더를 통째로 첨부할 수 있고, 각 파일이 폴더 안 어디에 있었는지(**상대 경로**)가 보존되며, **assistant 도 그 구조를 인지**한다. `/_template:entry` arg-given dispatch(사용자 요청 원문: "DQA에 첨부파일을 전달할 때, 폴더 또한 전달할 수 있도록 개선해주세요. 디렉토리 트리도 가능하다면, 보존해주세요. assistant 또한 이러한 구조를 인지해야 합니다."). REV-20260908T124500-attach-folder-tree. AC-20260908T124500-attach-folder-tree-1 ~ -6.
  - AC-…-1 (폴더 첨부 두 경로): 컴포저 `+` 메뉴의 "폴더 첨부"(`#attachDirInput`, `webkitdirectory` — 각 File 의 `webkitRelativePath` 사용)와 **폴더 드래그&드롭**(`_collectDroppedUploadItems` 가 `webkitGetAsEntry()` 로 디렉토리를 재귀 순회 — `dataTransfer.files` 는 폴더를 버리므로 순회 없이는 드롭이 무반응). `readEntries` 는 빈 배열이 올 때까지 반복해 큰 폴더의 뒷부분 유실을 막는다. 한 번에 300개 상한(`ATTACH_FOLDER_MAX_FILES`) — 초과 시 **자르지 않고 멈춘 뒤 알린다**(일부만 조용히 올리면 사용자는 전부 올라간 줄 안다).
  - AC-…-2 (경로 저장·승계): 업로드가 `relative_path` 폼 필드를 받아 `shared.attachment_path.normalize_relative_path` 로 정규화한 뒤 `WebConversationAttachments.RelativePath`(+ PG `core_attachments.relative_path`, dual-write 미러 3면)에 저장한다. NULL = 단일 파일. `OriginalFilename` 은 계속 basename — 경로는 **추가 축이지 대체가 아니다**(첨부를 파일명으로 지칭하는 기존 계약 전부 유지). assistant 편집본(`attachment-edit` 새 버전)과 fork 복사가 원본 경로를 승계한다.
  - AC-…-3 (assistant 구조 인지): `_build_attachment_context_section`(feature-0002 agent_core)의 SELECT 에 경로를 **끝에 append**(row[14] — 기존 positional index 보존), 파일 목록 라인에 `path="..."` 를 싣고, 순회 후 `## DIRECTORY STRUCTURE OF ATTACHED FOLDERS` 블록에 실제 트리를 **1회** 렌더한다(`shared.attachment_path.render_directory_tree` — 사람 표시와 같은 함수). **폴더 첨부가 없으면 블록을 렌더하지 않는다**(단일 파일 대화의 프롬프트는 종전과 동치 — 토큰 회귀 0). `read_attachment` 가 파일명뿐 아니라 **경로로도** 파일을 지칭한다(경로 정확 → 이름 정확 → 경로 접미 → 이름 부분), 실패·다중후보 안내도 경로로 한다.
  - AC-…-4 (동명 파일 비충돌): 버전 체인 스코프가 `(ConversationId, AccountId, RelativePath 또는 OriginalFilename)` 이다. 파일명만으로 스코프하면 `src/config.json` 과 `test/config.json` 이 한 체인으로 합쳐져 **서로를 supersede** 해 사용자가 올린 파일이 목록에서 사라진다. 경로 있는 행과 없는 행은 서로 배제한다(폴더 안 `a.txt` ≠ 따로 올린 `a.txt`). 프론트 dedup 축에도 경로가 들어간다.
  - AC-…-5 (경로는 신뢰하지 않는다): `webkitRelativePath` 는 브라우저가 보내는 사용자 입력이므로 traversal(`..`)·절대경로·드라이브 접두(`C:`)·제어문자를 제거하고 깊이 32·길이 1024·세그먼트 255 상한을 건다. 위험하면 **거절이 아니라 폴더 정보 폐기(None)** — 파일 자체는 올라간다. 경로의 마지막 세그먼트는 업로드된 파일명으로 고정(둘이 어긋나면 목록·트리가 실제 파일과 다른 것을 가리킨다).
  - AC-…-6 (스키마 배선·무회귀): MySQL 컬럼은 `_ensure_attachment_relative_path_schema` 가 **fast path(`_ensure_seed_catchup`)·slow path(`_ensure_web_tables`) 양쪽**에서 보장한다(운영 재기동은 slow 를 안 타므로 fast 누락 시 컬럼이 영영 안 생긴다 — 테스트가 배선을 소스로 고정). ALTER 는 CONVENTIONS §13.1 online DDL(`ALGORITHM=INPLACE, LOCK=NONE`) + 컬럼 존재 선확인(bare `try/except: pass` 금지). PG 는 alembic `0059_attachment_relative_path`(expand-only) + 부트스트랩 DDL. 신규 테스트 32건, main 대비 실패 차집합 0. 마이그레이션 전 배포(row 가 짧은 경우)는 폴더 없는 종전 동작으로 자연 폴백.
...
- REQ-20260618-0321 (TASK-20260618T044611-ai-claude-release-notes, **Major §12.3** — 릴리즈 노트(작업 화면 + 관리 콘솔)): 각 작업의 내역·개선 사항을 일반 사용자가 이해할 수 있도록 정리한 릴리즈 노트를 두 진입점으로 제공한다 — `작업 화면 > 사용자 프로필 > 릴리즈 노트(탭)` (모든 로그인 사용자), `관리 콘솔 > 릴리즈 노트(카테고리)` (콘솔 진입자). 두 화면은 **동일한 정적 큐레이션 콘텐츠**(`src/static/release-notes-data.js` 의 `window.RELEASE_NOTES`)를 **동일한 공유 렌더러**(`src/static/release-notes.js` 의 `window.ReleaseNotes.render`)로 표시한다. 업데이트는 **일자별 그룹**으로 정리하고 각 그룹은 **접기/펼치기**가 가능하며(기본: 최신 1개만 펼침), 상단 **영역 필터 칩**(전체/작업 화면/관리 콘솔/공통) + **모두 펼치기/접기**로 **탐색**할 수 있다. 콘텐츠는 일반 사용자가 바로 이해할 단순·명시적 문장으로 작성하고, **내부 동작(로직·네트워크·보안 처리 방법 등)은 노출하지 않고 "안정성/보안 개선" 수준으로만 간략화**한다. frontend-only(`index.html`/`admin.html` 스크립트+컨테이너+탭/카테고리, `app.js`/`admin.js` lazy 렌더 디스패치, `styles.css` 릴리즈 노트 스타일, 신규 정적 파일 2종) — 백엔드·RBAC·스키마·엔드포인트·DB·신규 권한 **0**. 콘텐츠가 정적이지만 모든 텍스트는 `textContent` 로 주입해 XSS-safe. 관리 콘솔 카테고리는 비민감 정보라 `ADMIN_TAB_PERMISSIONS` 에 매핑하지 않아 콘솔 진입자 모두에게 노출(데이터 자체가 공개 가능한 사용자-대상 릴리즈 노트). 새 업데이트 추가는 `release-notes-data.js` 의 `releases[]` 맨 앞에 일자 블록을 추가하면 되며 렌더 로직 변경 불필요. REV-20260618T044611-ai-claude-release-notes. AC-0578 ~ AC-0579.
  ...
  - **운영 이력(기능 계약 무변경)**: `releases[]` 블록 추가는 REQ-20260618-0321 의 운영 규약대로 데이터 큐레이션이며 렌더 계약을 바꾸지 않는다. 최신 적재 = **2026-09-09 블록**(doc_sync 09-10, `generated: 2026-09-09`, releases 64 — 착륙일 블록이 owning feature 커밋으로 self-add 돼 있어 신규 블록 prepend 가 아니라 그 블록 `items` 에 append(9→12)했다) — CHG-20260910T010301-ai-claude-doc-sync-rn-0910.
...
- 동작(`static/app.js`, 2지점 additive):
  ...
  - **위임 핸들러**: `document` 단일 `contextmenu` 리스너(`_onUniversalContextMenu`) + 설정표 `_CTX_MENU_TARGETS`(`{host,trigger}` 2행: `.conv-item`→`.conv-item-menu-trigger` / `.message`→`.message-menu-trigger`). **폴더 헤더는 이 표에서 빠져 있다**(folder-newconv 2026-09-10) — 폴더 행의 가시 버튼이 '📝'(새 대화)로 바뀌어, 재발화 방식으로 두면 우클릭이 메뉴 대신 대화를 만든다. 폴더 메뉴는 `renderFolderNode` 가 헤더에 직접 단 `contextmenu` 가 `openFolderMenu(folder, header, 커서좌표)` 를 부르며, 좌표는 `openFloatingMenu` 의 `anchorPoint` 옵션으로 전달하고 헤더의 `aria-expanded`(=폴더 접힘 상태)를 덮어쓰지 않도록 `ownsAriaExpanded:false` 로 연다. `ev.target.closest(host)` 매칭 + 호스트 내 trigger 존재 시 `preventDefault` + 커서 좌표 세팅 후 **기존 트리거 synthetic click 재발화**(`dispatchEvent(new MouseEvent("click"))`) → 각 트리거의 기존 open 함수(openConversationItemMenu/openFolderMenu/openMessageBubbleMenu)·권한 게이트·항목 구성·토글 로직 100% 재사용(중복 0).
...
- 수정(SSOT·drift-proof): `openFloatingMenu` 가 만든 모든 메뉴에 `data-floating-menu` 마커 부여 → `closeFloatingMenus` 가 `[data-floating-menu]` id 무관 일괄 제거(향후 신규 메뉴 자동 포함) + 트리거 리셋 selector. 정합: `_attachShareRangeEsc`·`_maybeSyncConversationListUnread` 의 열린-메뉴 가드에 folderMenu 포함. **현행(folder-newconv 2026-09-10)**: 폴더의 '···' 트리거는 사라졌고, 리셋 selector 는 `.conv-item-menu-trigger`·`.message-menu-trigger`(aria-expanded 복원) + `.is-menu-open`(클래스만 제거 — 그 요소의 aria-expanded 는 메뉴가 아니라 다른 의미를 갖는다) 두 갈래다. 시각 상태는 `shell.css` `.conv-folder-header.is-menu-open`(열린 메뉴의 대상 폴더 강조).
...
- AC-20260831T1827-connect-modal-autoclose-2: 이미 연결된 사용자가 모달을 열면 **닫히지 않는다**
  — 자동 관측 경로(폴링·탭 복귀)의 판정 대상은 여는 순간의 상태가 아니라 **열려 있는 동안의
  전이**다(기준선 = 창을 열 때 이미 알고 있는 값).
- AC-20260901T0330-connect-modal-launch-close-1: 그러나 사용자가 **`[내 AI 실행]` 을 직접 눌러**
  «대기 중» 이 확인되면, 기준선과 무관하게 토스트로 알리고 창을 닫는다 — 버튼을 누른 것은
  명시적 의도이고, 그 결과를 확인하고도 창을 남기면 성공을 말하면서 아무것도 하지 않는 것이 된다
  (사용자 제보 2026-09-01).
- AC-20260901T0330-connect-modal-launch-close-2: 단 그 성공이 **다른 창**을 닫지는 않는다 —
  실행 대기 중 창을 닫거나 새로 열었으면 그 대기는 물러난다(창 세대 검사).
...
### 릴리즈노트 콘텐츠 갱신 이력 (doc-sync-rn-0901, 2026-08-31 블록 신설)
- 사용자향 릴리즈노트 데이터(`static/release-notes-data.js`)에 **신규 `2026-08-31` 블록 prepend** — items 14 + `summary`. `releases` 56→**57** · `generated` "2026-08-31"(== `releases[0].date`) · 08-28 이하 블록 바이트 불변. 기능 계약·렌더 로직 변경 없음(콘텐츠 데이터 전용).

### 내 AI 연결 상태 칩의 자리 (20260901T1200-ai-conn-chip-to-profile-row)
- AC-20260901T1200-chip-to-profile-row-1: 연결 상태 칩(`#aiConnState`)은 **사이드바 프로필 행**에
  산다 — 프로필 버튼이 쓰고 남은 오른쪽 여백. 컴포저 하단이 아니다. 마크업상
  `.profile-trigger` 의 형제이며(버튼 중첩 금지) CSS flex 흐름으로 그 여백을 채운다. 화살표는
  프로필 버튼 안(흐름)에 남는다 — 칩은 버튼 밖의 별개 클릭 대상(연결 모달)이고, 그 경계가
  화살표로 드러난다.
- AC-20260901T1200-chip-to-profile-row-2: 컴포저 하단(`.composer-footer`)은 상태·힌트가 비고
  provider 상태점이 숨겨지면 **접힌다**. 칩은 더 이상 그 접힘의 예외가 아니다 — 예외 조건이
  남아 있으면 규칙이 영영 매칭되지 않아 빈 한 줄이 입력창 아래에 상시 남는다.
- AC-20260901T1200-chip-to-profile-row-3: 자리가 모자라면 **이름을 뭉개지 않고 칩이 다음 줄로**
  내려간다. 판단은 폭 임계값이 아니라 실제 계정 이름 길이가 한다(`flex: 1 0 auto` — shrink 0 +
  `flex-wrap`). 사이드바는 180~640px 사이를 오간다.
- AC-20260901T1200-chip-to-profile-row-4: 상태 라벨은 `연결 안 됨` / `대기 안 함` /
...
### 릴리즈노트 콘텐츠 갱신 이력 (doc-sync-rn-0902, 2026-09-01 블록 신설)
- 사용자향 릴리즈노트 데이터(`static/release-notes-data.js`)에 **신규 `2026-09-01` 블록 prepend** — items 17 + `summary`. `releases` 57→**58** · `generated` "2026-09-01"(== `releases[0].date`) · 총 항목 424→**441** · 08-31 이하 블록 바이트 불변. 기능 계약·렌더 로직 변경 없음(콘텐츠 데이터 전용).

## 점유 응답이 KB 근거를 함께 보낸다 (`claim_request`, 2026-09-02)

설계·AC 정본은 `unit/feature-0002-agent-core/docs/FUNCTION.md`
`## KB 근거는 «자동 주입»이다 — 도구 호출에 기대지 않는다`. web 계약만 여기 요약한다.

`POST /api/ai/tools/claim_request` 응답에 두 필드가 **가산**된다(기존 필드·상태코드 불변):

| 필드 | 뜻 |
|---|---|
| `kb_context` | 질문에 매칭된 큐레이션 근거(용어사전·ENUM·테이블/컬럼 설명·샘플쿼리·관계). 없으면 `""` |
| `kb_notes` | 층별 로드 실패·제품 미바인딩 등 사유. 조용한 누락 금지(§16.7 G9-b) |

- 매칭은 **가드 래퍼가 붙지 않은 원문**으로 한다(`marked` 는 canary·각인이 섞여 있다).
- 상한 `_CTX_BUNDLE_MAX_CHARS`, 초과 시 **절단 고지**를 본문에 남긴다.
- 러너는 `kb_context` 가 비어 있으면 블록 자체를 생략한다 — 그 생략이 **서버·러너 버전 조합이
  어긋나도 깨지지 않게** 하는 계약이다(러너가 낡으면 근거가 안 실릴 뿐, 오류가 나지 않는다).

### 릴리즈노트 콘텐츠 갱신 이력 (doc-sync-rn-0903, 2026-09-02 블록 신설)
- 사용자향 릴리즈노트 데이터(`static/release-notes-data.js`)에 **신규 `2026-09-02` 블록 prepend** — items 15 + `summary`. `releases` 58→**59** · `generated` "2026-09-02"(== `releases[0].date`) · 총 항목 441→**456**. 스키마·렌더 계약 무변경(콘텐츠 데이터만).

---

## P0-CHIP-CSS — 연결 칩 상태는 **무조건 규칙**을 갖는다 (TASK-20260903T210000)

화면이 `data-state` 로 칩 상태를 갈라도, 그 상태의 CSS 규칙이 `@media` 같은 **조건부
블록 안에만** 있으면 그 조건이 아닌 환경에서 칩은 **무스타일**로 그려진다 — 색은 상속된
검정, 배경 투명, `::before` 표지 없음. 코드는 갈라도 화면은 갈라지지 않는다.

라이브에서 이것이 실제로 있었다(2026-09-03 실 Windows 브라우저 실측). `@media
(prefers-color-scheme: dark)` 하나가 닫히지 않아 그 뒤 칩 규칙 전부가 dark 전용이 됐고,
`idle`(대기 안 함)·`stale`(업데이트 필요) 칩이 **라이트 모드에서 무스타일**이었다.
두 상태의 주석에는 「'연결됨' 과 뭉치면 헛되이 기다리게 된다」·「초록(정상)과는 확실히
갈라야 한다」고 적혀 있었고, 그 구분이 주간 화면에서는 성립하지 않았다.

| AC | 내용 |
|---|---|
| `AC-20260903T210000-chip-css-1` | CSS 파일의 중괄호가 균형을 이룬다. 블록 하나가 안 닫히면 그 뒤 규칙 전부가 조용히 조건부가 되고, 그 사실은 **화면에서만** 드러난다 |
| `AC-20260903T210000-chip-css-2` | 각 칩 상태(`on`·`off`·`idle`·`stale`·`checking`)가 **중괄호 깊이 0** 에 규칙을 갖는다. dark-mode 덮어쓰기 자체는 정당하다 — 잠그는 것은 「조건부만 있고 기본이 없는」 상태다 |
| `AC-20260903T210000-chip-css-3` | CSS 검사 모수와 `_paintConn` 이 실제로 대입하는 `dataset.state` 값 집합이 **1:1** 이다. 한쪽만 늘면 「분기는 타는데 스타일이 없다」(또는 죽은 CSS)가 된다 |
| `AC-20260903T210000-chip-css-4` | 「확인 중」의 색이 정상(`on`)과 다르다 — 갈라지는 것이 그 규칙의 존재 이유다 |

⚠ **문자열 검사로는 원리적으로 불가능하다.** 직전 cycle 의 검사는 「규칙이 파일에 있는가」만
보았고, 규칙은 있었다 — 그래서 초록이었고 화면은 무스타일이었다. 정적 축은 「블록 미닫힘」
형태를 커밋 시점에 잡지만, **캐스케이드 덮어쓰기**는 잡지 못한다. 그 축은 PB-0008 실 렌더
+ `getComputedStyle` 이 담당하며, 이 결함도 그것이 적발했다.

## 연결값은 **이 창의 로그인 세션**이 발급한다 (TASK-20260904T200000)

앱 창의 연결 패널이 [이 서비스에 연결] 직전에 `/api/ai/connect/token` 을 부르고, 서버가 딥링크용
으로 이미 만드는 `launch.protocol` **문자열 그대로** 연결 프로그램에 넘긴다.

**왜 화면이 주는가.** 종전에는 연결 프로그램이 딥링크로 받아 온 토큰만 썼다. 그러면 시작
메뉴에서 그냥 켠 앱 창은 토큰이 없어 연결을 걸지 못한다 — 프로그램이 웹의 부속물로 남던
지점이다(사용자 제보 2026-09-04). 토큰을 발급하는 주체는 원래부터 로그인 세션이고 이 창은 그
세션을 갖고 있으므로, 값을 주는 쪽은 화면이 자연스럽다.

⚠ **봉투를 여기서 조립하지 않는다.** 필드를 새로 만들면 같은 뜻의 봉투가 둘이 되고 한쪽만
고쳐지는 드리프트가 난다. 서버가 만든 문자열을 그대로 옮기고, 뜯는 것은 클라이언트의 파서
하나다.

⚠ **실패를 삼킨다.** 토큰을 못 받아도 `connect` 는 나간다 — 딥링크로 켠 창은 이미 값을 갖고
있어 이 호출 없이도 성립한다. 여기서 예외를 던지면 **되던 경로까지 막힌다.**

⚠ nonce 는 **이 컴퓨터로만** 나간다. 서비스 호출에는 싣지 않는다(`test_the_service_call_carries_no_nonce`).

## 앱 창 안의 문구는 «별도 프로그램»을 말하지 않는다 (TASK-20260904T220000)

연결 프로그램이 앱 창을 **직접 그리게** 되면서, 그 창 안에서 「연결 프로그램」이라고 부르는 것은
사실과 어긋난다 — 사용자에게 그것은 DQA 자체다.

⚠ **어디서 보느냐로 갈린다.** 브라우저로 방문한 사람에게는 「받아서 설치할 것이 있다」가
여전히 사실이므로 그 안내는 남는다. 바꾸는 것은 **앱 창 안에서 보이는 문구**뿐이다:
상주 안내 · 확인창 안내 · 브리지 도달 실패 문구.

⚠ 테스트는 **주석을 뺀 뒤** 검사한다. 설계를 설명하는 주석에 그 낱말이 남는 것은 옳고,
그것까지 걸면 「우리는 X 를 하지 않는다」는 문장에 걸려 헛도는 단정이 된다.

## 앱 창의 두 결함 — 상태 칩 자리 · 자기 자신 실행 (사용자 제보 2026-09-07)

### 1. 상태 칩은 **계정 이름 옆**을 떠나지 않는다

사용자가 두 번 말했다: *"계정 옆에 칩 자체가 확인되지 않았습니다."* 칩은 있었다 — **사이드바
맨 아래 왼쪽 구석, 계정 행 아래 줄**에.

종전 규칙은 자리가 모자라면 칩을 다음 줄로 흘려보냈다(`flex-wrap: wrap` + 트리거
`flex-shrink: 0`). 「이름을 뭉개는 대신 칩을 내린다」는 2026-09-01 의 교환이었는데, 기본
사이드바 폭(252px)에서는 계정 행이 그 폭을 다 써서 **거의 항상** 내려갔다.

**그 교환을 뒤집는다.** 이름은 잘려도 앞부분으로 알아보지만, 상태 칩은 자리를 옮기면
**찾지 못한다** — 그리고 이 칩의 목적은 «질문 전에 상태를 알린다» 이다.

⚠ 칩 자신은 **줄지 않는다.** 잠깐 `flex-shrink: 1` 로 두었다가 실측에서 되돌렸다 — 이름이
길면 칩이 함께 줄어 «업데이트 필…» 처럼 **상태 낱말이 잘렸다**(기본 폭에서도). 잘린 상태
칩은 상태를 알리는 일을 못 한다.

실측(라이브 브라우저): 폭 180·252·320px × 이름 3종 × 상태 문구 2종 = **전 조합에서 같은 줄 ·
잘림 없음 · 클리핑 없음**.

### 2. 앱 창은 **자기 자신을 실행하지 않는다**
...
방식이었으므로 새 입구 하나가 그대로 빠져나갔다.

⚠ 그래서 가드를 **입구가 아니라 출구**에 둔다. 스킴을 쏘는 자리를 `_fireScheme()` 하나로
모으고 거기서 막는다 — 앞으로 어떤 경로가 생겨도 이 함수를 거치는 한 앱 창에서는 스킴이
나가지 않는다(`test_the_scheme_is_fired_from_exactly_one_place`).

⚠ 앱 창에서는 자동 진입을 **아예 하지 않는다**(토큰 발급도). 쏘지도 않을 실행을 위해 매
로그인마다 토큰을 받을 이유가 없고, 이 창에서 연결은 패널이 한다 — 사용자가 요청하지 않은
확인창을 대신 띄우면 같은 불안을 다른 모양으로 되돌려 줄 뿐이다.

## 로그인은 앱을 껐다 켜도 유지된다 (TASK-20260907T030000, 사용자 제보)

세션 쿠키에 `max_age`/`expires` 가 **없었다**. 즉 브라우저가 「이 브라우징 세션이 끝나면
버림」으로 다루는 값이었다.

**웹에서는 이 결함이 보이지 않았다** — Chrome·Edge 가 재시작 시 세션 쿠키를 복원해 준다.
데스크톱 앱 창(WebView2)에는 그 복원이 없다. 프로세스가 끝나면 그 브라우징 세션도 끝난다.

실측(2026-09-07, 사용자 프로필): 영구 쿠키 저장소 **0건** · 새 창에서
`/api/ai/connect/status` → `{"logged_in": false}` · 상태 칩 `hidden`.

즉 **서버가 정한 14일(무활동 슬라이딩, 상한 90일)이 사용자 디스크에 도달한 적이 없다.**

⚠ 수명은 슬라이딩 창이 아니라 **절대 상한**을 준다. 슬라이딩 창(14일)을 쿠키에 주면 쿠키가
**제한 요인**이 된다 — 서버는 활동마다 만료를 미는데 쿠키는 로그인 시점에 한 번만 정해지므로,
매일 쓰는 사용자도 14일째에 로그아웃된다. feature-0043 TASK-20260828T150000 이 서버 쪽에서
고친 바로 그 증상을 쿠키 쪽에서 되살리는 꼴이다.

⚠ **권한은 여전히 서버가 정한다.** 14일 무활동이면 서버가 그 세션을 거절하고, 남아 있는
쿠키는 죽은 값일 뿐이다. 쿠키는 운반체이지 권한이 아니다.

⚠ 이 변경으로 «브라우저를 닫으면 로그아웃» 이 사라진다(사용자 결정 2026-09-07). 다른 방어
(`HttpOnly`·`SameSite=lax`·https 에서 `Secure`)는 **그대로**이며 테스트가 잠근다.

### 이 결함이 조용했던 이유

미로그인이 되면 `refreshConnState` 가 칩을 감추고 잠금 안내를 지운다(「로그인 전에는 말할
것이 없다」). 그런데 **이미 그려진 대화 목록·계정 이름은 그대로 남는다.** 그래서 화면은
로그인된 것처럼 보이는데 상태 표시만 사라진 모양이 되고, 사용자는 그것을 «칩이 안 보인다»
로 겪는다. 실제로 그렇게 제보됐다.


## 컴포저 위에는 안내 문단을 두지 않는다 (TASK-20260907T060000-chip-copy-and-note)

모델·추론 항목이 왜 없는지를 입력창 위 문단(`#composerActionsSelectorNote`)으로 말하던 UI 를
**제거**했다 (사용자 결정 2026-09-07: *"의도하지 않은 UI"*).

그 문단은 **두 번** 레이아웃을 깼다. 2026-09-02 에는 `.composer-box` 안의 플렉스 항목이라
입력창을 옆으로 밀었고(「대화창 UI가 무너진다」), 자리를 입력창 위로 옮긴 뒤에는 자기 줄을
차지해 **컴포저 전체 높이**를 바꿨다 — 사이드바 프로필 행과의 하단 정합이 안내가 켜질 때마다
어긋났다. 자리를 옮기는 수정이 두 번 다 증상만 옮겼으므로, 계약을 «어디에 두는가» 가 아니라
**«두지 않는다»** 로 바꾼다: 조건부로 나타나 컴포저 높이를 바꾸는 요소는 어디에 두든 정합을
흔든다.

- 서버 필드 `model_selector_reason` · `runner_download_url` 은 **그대로 둔다** — 판정 근거이자
  진단 필드이고, 사라진 것은 그 값을 화면 문단으로 그리던 소비처 하나뿐이다.
- 상태를 말하는 자리는 **프로필 행의 연결 칩**(`#aiConnState`)과 연결 모달로 모은다. 같은
  사실을 두 자리에서 말하면 한쪽이 낡는다(§16.8 A-1). 제거된 문단의 `aria-live` 는 칩이
  **승계**한다(`role="status" aria-live="polite"`) — 승계하지 않으면 상태 전이가
  스크린리더에 한 마디도 가지 않는다.
- ⚠ **칩이 전부를 덮지는 못한다.** 잠금 안내(`#composerGate`)는 `compose_blocked =
  bridge_mode and not (connected and listening)` 일 때만 뜨므로 「연결·대기는 성립했는데
  목록만 비었다」를 못 덮고, 칩 문안에는 「모델」이라는 말이 없다. 그래서 갈라 둔다:
  - **카탈로그 미수신**(`/api/api-vault/options` 실패) → 전송 시 **토스트**로 고지한다.
    고른 등급이 실리지 않은 채 나가는 조용한 강등이므로 말하지 않으면 안 된다.
  - **능력 협상 실패**(연결·대기는 성립) → 화면은 조용히 선택기만 없다. 이것이 문단 제거가
    받아들인 대가이며, 같은 주기의 러너 수정(카탈로그 경로·재시도·원장 폴백)이 그 상태
    자체를 드물게 만든다.

## 연결 칩 툴팁은 **최대 3문단** (TASK-20260907T060000-chip-copy-and-note)

사용자 지시(2026-09-07): *"툴팁이 사용자 입장에서 너무 장황합니다. 최대 3문단으로 단순화."*

- 6상태 모두 **2문단**(「지금 무엇인가」 + 「무엇을 하면 되는가」)으로 쓴다. 러너가 실어 보낸
  사유(`ai_unready_reason`)가 있으면 그것이 첫 문단이 된다.
- 내부 어휘를 대상 사용자 어휘로 낮춘다 (§16.8 B-2) — 「머신을 재시작했다면」→「컴퓨터를 다시
  켰다면」 · 「러너가 서버 배포본과 다릅니다」→「연결 프로그램이 최신본이 아닙니다」.
- 칩 **라벨**(연결 안 됨 / 대기 안 함 / 답할 수 없음 / 업데이트 필요 / 확인 중 / 대기 중)은
  불변이다 — 상태 구분이 그 라벨에 걸려 있고, 툴팁은 그 라벨의 부연이다.
- 예산은 테스트가 **센다**(`test_chip_tooltips_stay_within_the_copy_budget`). 분량 축은
  정확성 게이트가 원리적으로 잡지 못한다 — 틀린 말이 아니고 diff 도 작기 때문이다.

## 패널이 고를 것이 없으면 묻지 않는다 (TASK-20260907T060000)

같은 플랫폼이 두 자리에 있을 때만 사람에게 고르게 한다. 그 외에는 패널이 뜨자마자 연결한다.
자동·수동이 **같은 경로**를 쓰므로 봉투 전달·오류 표시가 갈리지 않는다.

⚠ 위험 동작의 확인 창은 2026-09-07 에 사라졌다(사용자 결정). 브리지가 끝난 뒤 알림 영역으로
알린다 — 그래서 `declined` 응답을 다루던 분기도 함께 지웠다. 도달하지 않는 분기를 남기면
다음 사람은 그것을 보고 확인 창이 아직 있다고 읽는다.

## 정적 모듈 참조는 **모두** 스탬프를 단다 (TASK-20260907T160000)

`inject_asset_stamp.py` 는 `?v=` 토큰을 **content-hash 로 치환**한다 — 삽입하지 않는다.
그러므로 토큰 없는 참조는 치환 대상 밖이고, 캐시 정책상 「`?v=` 없음 = Cache-Control 미설정」
이라 브라우저 휴리스틱 캐시가 걸린다(Last-Modified 기준 수 시간).

    ⇒ 서버는 새 파일을 서빙하는데 페이지는 구 모듈을 쓴다. 「배포됨」과 「도달함」이 갈린다.

라이브 실측 2026-09-07: 정적 트리의 상대 참조 103건 중 `./client-bridge.js` **한 건**만
토큰이 없었다. 그 결과 자동 연결(TASK-20260907T060000)은 배포·재적재 후에도 앱 창에 도달하지
못했다 — 서빙되는 파일만 확인하는 방법으로는 보이지 않는다. 결정적 관측은 페이지가 실제로
적재한 모듈 인스턴스를 꺼내 본 것이다:

    await import('/static/app/client-bridge.js')   // → 신규 export 부재

**계약**: `src/static/**/*.js`(vendor 제외)의 모든 상대 import/export-from/dynamic import
specifier 는 `?v=` 토큰을 갖는다. `tests/test_static_module_stamp_census.py` 가 전수 고정한다.
그 테스트는 **추출이 깨졌는지 먼저 가른다**(양성 대조군 + 표본 하한) — 참조를 못 찾는
스캐너는 언제나 초록이기 때문이다.

## 연결하는 길은 **DQA 앱 하나**다 (TASK-20260907T190000)

사용자 결정 2026-09-07: 「터미널 및 AI에게 연결을 요청하는 부분은 제거해주세요」 (범위 선택
「그냥 전면 제거」). 그래서 두 화면(대화 모달 · `/ai/connect`)에서 사라진 것:

- 「연결 준비」 버튼과 그것이 그리던 토큰·명령·지시문
- 터미널 1단계 (OS 탭 · 명령 블록 · 명령 복사)
- AI 지시문 두 갈래 (「조사만 맡기기」 · 「전부 맡기기」)
- 「이 창을 닫으면 다시 볼 수 없습니다」 토큰 경고 — **그릴 토큰이 없다**

남은 것은 둘이고, 그 둘은 **한 화면에 동시에** 있다:

| 상황 | 보이는 것 |
|---|---|
| 앱 창(연결 프로그램이 연 창) | 「이 컴퓨터의 AI」 패널 — 고유 플랫폼이면 자동 연결 |
| 브라우저 · 이미 연결해 본 적 있음 | **[내 AI 실행]** (스킴으로 앱 기동) |
| 브라우저 · 앱을 받을 수 있음 | **[DQA 앱 받기]** (서버가 실물을 확인했을 때만) |

### 토큰은 여전히 발급된다 — **그리지 않을 뿐**

스킴 URL 안에 실려 프로그램으로 곧장 건네지므로 사용자가 그 값을 보거나 복사할 일이 없다.
그래서 「닫으면 지운다」 계약이 「애초에 넣지 않는다」로 **강해졌다**.

### 실행 URL 은 **창을 열 때** 받는다

클릭 시점에 발급을 시작하면 그 앞에 `await` 가 생기고, 사용자 활성화가 끊겨 크롬이 스킴
이동을 조용히 거른다. 그래서 모달은 열릴 때 미리 받아 둔다(`_offerLaunch`). 자격은 **서버가
아는 연결 이력**(`last_os`)이다 — 없으면 받지 않는다(쓰이지 않을 토큰이 쌓인다).

⚠ 자격을 **나중에 알게 되는** 경우가 있다. 창을 여는 순간 상태 조회가 한 번도 끝나지 않았으면
「이력 없음」으로 읽고 물러나므로, 알게 된 자리(`refreshConnState`)에서 한 번 더 준다.

단독 페이지 `/ai/connect` 에는 「창을 여는 계기」가 없다. 그래서 그쪽 버튼은 **누를 때** 받고,
받은 URL 을 들고 있다가 다음 클릭은 동기적으로 쏜다 — 첫 클릭이 걸러져도 두 번째가 통과한다.

### 대가를 적어 둔다

앱을 아직 받지 못했고 서버가 배포본을 내놓지 못하는 상태의 사용자에게는 **연결할 길이 없다.**
사용자가 그 대가를 알고 고른 결정이고, 완화는 받기 안내 하나다. 이것이 다시 문제가 되면
되돌릴 자리는 이 문단이 가리키는 곳들이다.
...
가리키며, 제거된 터미널 1단계 명령을 안내하지 않는다. 갱신 실패 안내 MSG_RELAUNCH_NO_UPDATE를 실제 트레이 [업데이트 확인] 경로로 수정.
