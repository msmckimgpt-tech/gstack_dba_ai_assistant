...
## 1. Current Status
- State: **TASK-0226 (MSSQL per-DB 스캔 커버리지 + 권한 실패 가시화) 완료** — insight-worker 가 datasource(제품 접근가능 DB) 마다 재연결해 스캔하던 중 RO 로그인 GRANT 누락으로 일부 DB 가 조용히 누락되던 것을 telemetry(db_targets/db_failed)+status=degraded 로 가시화 + 멀티 DB RO 부트스트랩 템플릿 제공 (cycle: TASK-0226)
- Owner: AI (본 cycle: modules/insight.py per-DB telemetry + bin/datasource-mssql-ro-bootstrap-multidb.sql + 테스트 8)
- Priority: high
