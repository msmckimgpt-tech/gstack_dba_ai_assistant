...
### LRN-20260805-0002 — 부재 단정을 막는 봉인이 **자기도 부재를 단정하면** 정확히 그 마찰을 생산한다 — 권위는 사실이 완전할 때만 주장할 수 있다
- Source: conversation_audit FR-attachment-change-false-absence / CHG-20260805T160000 + §18.8 backend·qa 패널 BLOCK 흡수(CHG-20260805T173000)
- Category: mistake
- Frame: "첨부가 갱신됐는데 모델이 '변경 없음' 이라 단정한다" 는 마찰을 고치려고, 서버가 계산한 건수를 **코드-권위 사실**로 프롬프트 말미에 주입했다. 그런데 같은 블록에 **부정 분기**를 대칭으로 넣었다 — 신규 0건이면 "이번 턴에 첨부된 파일 없음" 을 역시 권위로 선언하고, red-team 리뷰어에게 그 반대 주장을 BLOCK 하라고 시켰다. 문제는 그 사실의 원천(`new_attachment_ids`)이 **클라이언트가 보내는 신호**라는 것이다. 비어 있다는 것은 "첨부 없음" 이 아니라 **"신호 없음"** 일 수 있다(대화 전환 후 복귀로 프론트 pill 이 session 으로 재수화 · 그룹 발신자 스코프 제외 · 비-브라우저 호출). 즉 봉인이 **자기가 막으려던 부재 단정을 스스로, 그것도 리뷰어의 강제력까지 얹어서** 생산할 수 있었다. 수정 전 초판은 사용자에게 도달하기 직전이었고, 적대 패널이 프로브로 재현해 잡았다. 같은 축의 두 번째 실수: 단정의 근거로 가리킨 섹션(`FILE UPDATES`)이 **일부 파일 종류에서는 렌더되지 않는데도** 무조건 "위에 있다" 고 말하면서 동시에 "볼 수 없다고 하지 말라" 고 막았다 — 없는 증거를 찾게 만든 뒤 못 찾았다는 말을 금지하는 fabrication forcing 이다.
- How to apply: (1) **긍정 사실과 부정 사실의 증명 부담은 대칭이 아니다.** "X 가 왔다" 는 X 하나로 증명되지만 "아무것도 안 왔다" 는 **관측 경로의 완전성**을 증명해야 한다. 신호가 클라이언트·베스트에포트·스코프 필터를 거쳤다면 부정은 주장하지 말고 **침묵**하라(침묵 = 변경 전 동작 = 안전). (2) 사실 목록은 **floor(하한)로 선언**하고 "목록에 없으니 없다" 는 추론을 명시적으로 금지하라 — ceiling 으로 읽히는 순간 스코프 제외분이 "존재하지 않는 것" 이 된다. (3) 프롬프트에서 **다른 섹션을 가리키는 문장은 그 섹션이 실제로 렌더된 경우에만** 내보내라. 렌더 여부를 아는 것은 그 섹션을 만든 코드뿐이니, 가리키는 쪽이 아니라 만드는 쪽에서 사실을 적재한다. (4) 금지 규칙은 **가장 좁은 명제 하나**로 — "제공 사실의 부정" 만 막고 내용·품질 판단은 전부 모델에게 남겨라. 넓게 금지하면 참인 답변까지 막고, 리뷰어가 그 참인 답변을 BLOCK 해 모델을 허구로 민다.

### LRN-20260805-0003 — 프롬프트 봉인의 **배선**은 단위 테스트가 구조적으로 못 잡는다 — 뮤테이션으로 역검증하지 않으면 "고쳤다" 는 주장 자체가 미검증이다
- Source: conversation_audit FR-attachment-change-false-absence §18.8 qa 패널 지적(뮤테이션 10종 중 6종 생존) + 자체 뮤테이션 8/8 KILLED
- Category: mistake
- Frame: 프롬프트 봉인의 핵심 주장은 대개 "이 문구가 **어디에** 실리는가" 다(이번엔 "72k 프롬프트의 마지막 발화"). 그런데 자연스럽게 작성되는 테스트는 builder 함수를 직접 호출해 **문자열 내용**만 검사한다 — 그러면 조립부(`compose_system_prompt` 의 append 한 줄, 거대 함수 안의 대입 한 줄, 두 번째 패스로 넘기는 kwarg 하나)를 **지워도 전부 통과**한다. 실제로 적대 패널이 뮤테이션을 돌리니 블록 append 삭제 · 위치 이동 · user 턴 합치기 삭제 · verify 패스 인자 제거 · 보안 게이트 제거 · 조기-return 경로 오적재가 **초판 18건 + 관련 224건을 모두 통과**했다. 더 나쁜 것은 위치 계약 테스트가 **자기가 이어붙인 문자열을 검사하는 tautology** 였다는 점이다 — 통과했지만 아무것도 보장하지 않았고, 통과했기 때문에 보장한다고 착각했다.
- How to apply: (1) 봉인 테스트는 **조립 함수를 실제로 호출**해 최종 산출물에서 위치·포함을 검사하라(fake conn 하나면 충분하다). (2) 수천 줄짜리 단일 함수 안의 한 줄 배선은 단위 호출로 태울 수 없으니 **소스 검사**(호출 존재 + 순서)로라도 고정하라 — 이 저장소에 선례가 있다. (3) 무엇보다 **뮤테이션으로 역검증**하라: 지키려는 동작을 하나씩 되돌려 각 테스트가 실제로 FAIL 하는지 확인한다. KILLED 되지 않는 뮤턴트는 그 축이 무방비라는 뜻이고, 이건 몇 분이면 끝난다. (4) 컨테이너에서 뮤테이션을 돌릴 때 함정: `conftest.py` 가 실제 `src` 를 `sys.path[0]` 에 넣으면 `PYTHONPATH` override 가 무시돼 **원본을 테스트한다** — 파일을 실제로 치환하고 복원하는 방식이어야 한다.

### LRN-20260805-0001 — **대상 선정의 단위**와 **저장의 단위**가 어긋나면 그 작업은 무한 순환한다. 그리고 그 순환은 "일이 많은 것"과 구분되지 않아 오래 숨는다
- Source: 사용자 관측 "관리 콘솔 > AI 운영 현황 > 운영 현황에서 '분석문 사실성 검증'이 대부분의 분석 작업을 차지한다" → feature-0036 라이브 실측 (2026-08-05, ADR-0036-08)
- verified: false (수정·테스트·라이브 쿼리 실측까지. "배포 후 호출량이 실제로 떨어지는지" 는 배포 후 재실측분)
- Mistake: feature-0036 은 판정 결과를 **노드당 1행**으로 저장한다(ADR-0036-04 — 무한 증식 차단). 그런데 판정 **대상**은 `node_analysis_jobs` 를 **행 단위**로 골랐다. 같은 노드에 **여러 분석 run 의** done 행이 쌓이므로(라이브: done 2,682행 / 노드 2,052 — 다세대 노드 489개 전부 run_id 상이. 초판 진단은 이를 back-refine 탓으로 적었으나 오귀속이었다: `_backrefine_neighbors` 는 행을 만들지 않고 기존 행을 제자리 전이시킨다. **순환은 맞게 봤는데 귀속이 틀렸고, 그 틀린 전제가 '중복 정리는 이 층의 경계가 아니다'라는 대안 기각의 근거로 쓰였다** — 적대 패널이 라이브 데이터로 반증) A세대를 판정해 저장하면 B세대가 미판정으로 남고, B세대를 저장하면 A행이 지워져 다시 미판정이 된다 — **영원히 번갈아 판정**한다. 라이브에서 7일간 7,896콜(배경 LLM 호출의 **62.8%**, 토큰 38.3%)을 태우고 판정 행은 **91개로 고정**돼 있었다(노드당 평균 87회 재판정, 정보 증가 0). 두 정책을 각각 리뷰할 때는 둘 다 옳다 — 결함은 **정책 사이의 단위 불일치**에 있어서 어느 한쪽을 읽어도 보이지 않는다. codex 적대 리뷰 P1 4건도 이 축은 잡지 못했다(대상 조회의 `analysis_hash` 비교 누락은 잡았는데, 그 비교가 **어떤 행 집합 위에서** 도는지는 묻지 않았다).
- Correct approach: (1) **한 자원의 "선정 단위"와 "기록 단위"는 같아야 한다.** 저장이 X당 1행이면 대상도 X당 1건이어야 한다(`DISTINCT ON` 등). 다르면 그 차집합이 매 pass 되살아나는 영구 잔여물이 된다. (2) **"이미 처리했음"을 판정하는 키가 저장 키와 다른 축을 포함하면**(여기서는 해시 vs 노드) 그 조합이 순환을 만들 수 있는지 명시적으로 따진다. (3) 고정 정렬(`stage DESC` 등)만으로 후보를 뽑으면 처리된 항목이 완충 구간을 채워 **미처리 항목이 영영 밀린다** — "한 번도 처리 안 된 것 우선"을 정렬 첫 키로 둔다.
- 관측 설계(핵심): **재실행을 신규 실행과 분리해 세지 않으면 순환은 계기판에서 정상 부하와 똑같이 보인다.** 운영 현황 화면은 task 별 호출 수만 보여줘서 "검증이 62.8%"까지는 알려줬지만 "그중 정보 증가분이 0"은 말해주지 않았다. telemetry 에 `rejudged`(이전 결과가 있는데 다시 처리한 수)를 분리하면 pass 마다 상한을 채우는 것이 곧 알람이 된다. 백그라운드 반복 작업에는 **"처리량"과 "순증"을 나란히** 두는 지표가 필수다.
- 진단 레시피(재사용): ① task 별 호출 집계로 지배 소비자 식별 → ② **`target` distinct 대비 호출 수** 비교(여기서 91 vs 7,896 = 노드당 87회가 결정적 신호) → ③ 산출 테이블 행 수·`created_at` 분포와 대조(행이 안 늘고 같은 소수만 갱신되면 순환) → ④ 상위 소비 대상의 **원천 행 수**를 세어 대조군 확인(중복 행이 있는 대상만 반복, 1행짜리는 조용 → 원인 확정). LLM 응답 품질을 의심하기 전에 **대상 선정을 먼저 의심**한다 — 이번에도 응답은 전부 정상이었다(수동 1콜 재현으로 확인).
- How to apply: 백그라운드 배치를 만들 때 "대상 쿼리"와 "저장 쿼리"를 **같은 화면에 놓고** 단위가 일치하는지 확인한다. 그리고 배포 후 첫 주에 `distinct target` 대비 호출 수를 한 번 본다 — 이 결함은 기능이 "돌고 있다"는 신호를 정상적으로 내보내기 때문에, 보러 가지 않으면 드러나지 않는다.

### LRN-20260731-0001 — "거부하되 방향을 주라"는 코칭은, 그 방향이 **정적 문구**면 모델이 이미 한 일을 되풀이하라는 말이 된다
- Source: `/_dqa:conversation_audit "에러로그 분석 및 원인 대안 제시"` — 사용자 보고 "'무거운 쿼리' 블로킹이 너무 심하다" 근본 진단 (2026-07-31, `FR-loadgate-blind-coaching`)
- verified: false (코드/테스트·라이브 EXPLAIN 계획 dogfood 까지. "실제 대화에서 차단이 줄고 재작성이 성공하는지" 는 배포 후 실측분)
- Mistake: 선행 cycle(TASK-0304)이 부하 게이트를 "하드 차단" 에서 "더 가벼운 쿼리로 재작성하도록 코칭" 으로 바꿨다. 방향은 옳았는데 **코칭 내용이 상수 문자열**이었다 — "필요한 컬럼만 SELECT · WHERE 로 대상 한정 · 서버측 집계 · 표본은 LIMIT". 라이브에서 모델은 그 넷을 **이미 전부 적용한** 쿼리를 냈고, 차단당한 뒤 **똑같은 조언**을 다시 받았다. 정보량이 0인 피드백이라 모델은 형태만 바꿔 재제출했고 한 대화에서 **6연속 차단**된 뒤 목적을 포기했다. 더 나쁜 것은, 차단된 쿼리의 다수가 **전역 집계**(`COUNT(*)`)여서 "재작성하면 가벼워진다" 는 전제 자체가 그 쿼리들엔 **거짓**이었다는 점이다 — 원리적으로 불가능한 일을 반복해서 요구한 셈이다. 그런데 그 판단에 필요한 정보(`type=ALL`, `key=None`, 스캔 파티션 26/26)는 **게이트가 방금 실행한 EXPLAIN 안에 이미 있었고 그대로 버려지고 있었다**.
- Correct approach: (1) **거부 피드백은 거부 판정에 쓴 근거를 함께 돌려준다.** 판정에 쓴 데이터를 버리고 상수 문구를 주면, 수신자는 판정의 이유를 역추측해야 하고 대개 이미 시도한 것을 다시 시도한다. 게이트가 EXPLAIN·플랜·검증기 출력으로 판정했다면 그 출력의 **판정 근거 필드**가 피드백의 본문이어야 한다. (2) **"재작성하라" 는 재작성이 가능한 경우에만 말한다.** 원리적으로 불가능한 형태(전역 집계·전수 확인)에는 그 사실과 **대안 경로**(근사치 도구·범위 축소·명시적 승인)를 줘야 한다. 아니면 코칭이 곧 무한루프 지시문이 된다. (3) **탈출구가 있으면 그 존재를 조건부로 승격한다** — "최후수단" 이라고만 써 두면 모델은 반복 실패 상황에서도 그것을 쓰지 않는다. 반복 차단처럼 "재작성으로 못 푼다" 는 신호가 잡히면 탈출구를 명시 선택지로 올린다. 단 승격 카운터는 **대상 단위**로 세야 한다(run 단위로 세면 다른 테이블의 첫 쿼리가 남의 실패 횟수를 물려받아 좁혀볼 기회를 건너뛴다). (4) **정책이 막아 둔 탈출구는 안내하지 않는다** — 운영자가 모델의 `confirm` 을 무시하도록 설정했는데 "confirm 하면 실행됩니다" 라고 말하면, 통하지 않는 우회를 반복 시도하게 만든다. 이 cycle 이 없애려던 루프와 정확히 같은 형태다.
- 진단 레시피(재사용): **사용자가 지목한 표면 증상을 먼저 데이터로 기각해 본다.** 이번 보고는 "`LIMIT 1` 인데도 차단된다" 였는데, 그 쿼리는 `COUNT(*)` 집계라 LIMIT 과 무관하게 전체 스캔이 맞았다(차단 25건 중 **24건이 집계** — 게이트 판정은 정당). 표면 프레임을 그대로 수용해 임계를 낮추거나 집계를 통과시켰다면 **부하 회귀**를 출하했을 것이다. 대신 결함을 "거부 자체" → "거부 **피드백**" 으로 위치 재지정하고, 그와 **별개로** 실재하는 오판(순수 `LIMIT n` 은 실제 n행인데 `EXPLAIN.rows` 는 LIMIT 미반영 스캔 상한이라 테이블 전체로 추정)만 좁게 되돌렸다. **표면 증상과 진짜 근본이 인접하지만 다른 곳에 있을 때, 둘을 갈라 놓는 것이 진단의 핵심 산출물이다.** 갈라 놓는 도구는 라이브 실측이었다 — 같은 엔진에 쿼리 형태별로 EXPLAIN 을 돌려 "오판" 과 "정당" 을 표로 만들면 어디까지 되돌릴지가 자동으로 정해진다. 그 표에서 **통과하는 재작성 경로가 실재한다**(파티션 키로 좁히면 est 13.9M → 1)는 것이 드러났고, 그것이 곧 코칭이 알려줬어야 할 내용이었다.
- 완화 방향 수정의 자기 함정(§18.8 실측): 이 cycle 의 적대 패널이 [P1] 2건을 냈는데 둘 다 **내 보정이 실제 전체 스캔을 통과시키는** 결함이었다 — 주석 속 가짜 LIMIT(`-- LIMIT 5`)과 `SQL_CALC_FOUND_ROWS`(LIMIT 뒤에도 전체 행수 계산). **막으려던 부하 회귀를 봉인 자신이 만들고 있었다.** 교훈: *가드를 완화하는 수정은 "완화 조건을 우회하는 입력" 을 적대적으로 열거하는 것이 검증의 본체*다. 판정 실패가 항상 "완화 미적용 = 차단 유지" 쪽으로 떨어지도록 설계하고(원본·정규화본 양쪽 검사), 우회 후보를 테스트로 고정한다. 정규식으로 SQL 형태를 판정할 때 `_` 가 word char 라 `\bdistinct\b` 가 `DISTINCTROW` 를, `\bjoin\b` 가 `STRAIGHT_JOIN` 을 **놓친다**는 것도 같은 계열의 함정이다.
- Applies to: 도구 거부·검증 실패·정책 차단을 LLM 에게 되돌리는 모든 경로(sql_guard·부하게이트·스키마 검증·첨부 파서). 거부 문구를 쓸 때마다 "수신자가 이 문구만 보고 **다음에 무엇을 바꿔야 하는지** 특정할 수 있는가, 그리고 그 변경이 **가능하기는 한가**" 를 묻는다. 같은 축의 선례: `FR-show-create-routine-blocked`(거부에 대체 도구 안내 부재) · `FR-dataplane-conn-stale-no-reconnect`("쿼리를 좁혀라" 가 연결 장애에 오도적이었던 사례 — 원인이 다른데 같은 문구를 주면 모델이 엉뚱한 방향으로 축소한다).
### LRN-20260731-0002 — 운영자가 편집 가능한 설정이 코드 상수를 **대체**하는 구조에서는, 코드에 규칙을 추가하는 것이 라이브에 반영된다는 보장이 전혀 없다
- Source: `/_dqa:conversation_audit` — `FR-review-frames-live-db-as-spec` 배포 검증 중 부수 발견 → `FR-operator-global-prompt-shadows-code-seals` (2026-07-31)
- verified: true (배포본에서 직접 측정 — composed prompt marker 부재 + 운영자 row 길이·UpdatedAt + 코드-append 상수 전수 대조)
- Mistake: 6주에 걸쳐 시스템 프롬프트 **본문**에 grounding 규칙을 다섯 번 추가했다(절단·완전성 / 0행≠부재 / 첨부↔실DB 양측 조회 / 식별자 대소문자 / 커버리지 도구 유도). 매번 테스트가 "상수에 문구가 있다" 를 확인했고, 매번 배포했고, 원장은 두 건을 `fixed:deployed:verified` 로 닫았다. 그런데 `compose_system_prompt` 은 운영자가 관리 콘솔에서 편집하는 `WebSystemPrompts` global row 가 있으면 **코드 상수를 통째로 대체**한다. 라이브 row 는 **2026-06-18** 자였다 — 즉 **그 이후 추가한 다섯 규칙은 단 한 번도 프로덕션에 존재한 적이 없다**. 더 나쁜 것은 그 row 가 stale 이라 **이미 제거한 유해 지시**("명시 요청 없이 execute_sql 금지", "첨부 지침 우선")를 한국어로 보존하고 있었다는 점이다 — 코드에서는 회귀 테스트까지 걸어 지웠는데 라이브에서는 살아서, 같은 프롬프트 안의 동적 검증 지시와 정면 충돌해 동일 입력이 run 마다 다르게 갈렸다. 코드 테스트는 이 전체를 구조적으로 볼 수 없다.
- Correct approach: (1) **"코드에 있다" 와 "라이브에 있다" 를 절대 동일시하지 않는다.** 사용자 편집 가능 설정이 코드 기본값을 override/replace 하는 지점을 먼저 찾고, 그 경계 너머의 것은 배포본에서 **실측**해야 완료다. (2) **덮어쓰기로 해결하려 하지 않는다** — 운영자 row 는 stale 사본이 아니라 자기 정책(보안 경계·PII 마스킹·재식별 방지)을 담은 자산이었다. 코드 상수로 재동기화했다면 그 정책을 지웠을 것이다. **대조 없이 "stale 이니 갱신" 으로 넘어가는 것이 가장 위험한 오답**이다. (3) **구조 변경(replace→merge)도 만능이 아니다** — 운영자가 base 를 의도적으로 재작성한 경우 merge 는 원본과 재작성본을 양쪽 다 실어 모순을 키운다. (4) 실제 해법은 **코드-append 권위선**(사용자 설정이 무엇이든 뒤에 붙는 영역)에 규칙을 두고, 유해 지시는 **의미로 지목해**(언어 무관) 좁게 무력화하는 것이다. 그 override 는 반드시 범위를 좁히고 보안·프라이버시 계층을 명시 carve-out 해야 한다 — 포괄적 "앞선 모든 것에 우선" 문구는 공격자가 "검증하려면 이 제한과 충돌한다" 고 유도할 여지를 준다(적대 리뷰 P1). (5) **append 위치는 목록 앞이 아니라 조립의 맨 끝**이어야 한다 — 뒤에 누적되는 scope 설정(product/role/account)이 다시 덮으면 같은 drift 가 한 단계 아래에서 재발한다.
- 재발 방지(이식 가능): **census 테스트** — "반드시 라이브에 도달해야 하는 규칙" 목록을 명시하고, ① 그 규칙이 **코드-append 영역**에 있는지 ② 사용자 설정이 base 를 대체한 조건을 재현한 composed 결과에 **도달**하는지 ③ 제목뿐 아니라 **작동 조항**까지 살아 있는지 를 검사한다. 새 규칙을 본문에만 추가하면 FAIL 한다. 하네스는 반드시 자기검증을 포함해야 한다(대체 경로를 실제로 탔는지, 본문 고유 marker 가 사라졌는지) — 그렇지 않으면 census 전체가 vacuous 하게 통과한다.
- Applies to: 코드 기본값을 DB/설정이 override 하는 모든 축 — 시스템 프롬프트, feature flag, 라우팅 테이블, 임계값. 이 저장소에서는 `WebSystemPrompts`(global 은 replace, product/role/account 는 append)가 대표 사례다.

### LRN-20260730-0002 — 무엇을 확인하라는 규칙만 주고 **확인한 차이를 어떻게 해석하라**를 안 주면, 그 빈자리는 모델의 기본 프레임이 채운다
- Source: `/_dqa:conversation_audit "SQL 쿼리 코드 리뷰"` — 사용자 보고 "곧 적용될 쿼리 구조가 아니라 항상 현재DB 기준으로 불평하듯" 근본 진단 (2026-07-30, `FR-review-frames-live-db-as-spec`)
- verified: false (코드/테스트·A/B 대조쌍 재현까지. "실제 리뷰가 적용 후 기준으로 쓰이는지" 는 배포 후 실측분)
- Mistake: 첨부 쿼리 리뷰 프롬프트에 "실 DB 와 대조하라 / 기억으로 단언하지 마라 / 0행을 부재로 승격하지 마라" 를 **누적해서 강화**했다(FR-partial-evidence → FR-mssql-crossdb → FR-false-absence → FR-false-truncation). 규칙은 전부 옳았고 각각 실제 마찰을 없앴다. 그런데 어느 규칙도 **"확인한 차이가 무엇을 의미하는가"** 를 정하지 않았다. 그 결과 존재/부재 판정이 답변의 중심으로 올라온 상태에서 시간 방향이 비어 있었고, 모델의 기본 프레임(**라이브 DB=정본 스펙 / 첨부=그에 미달하는 후보**)이 그 자리를 채웠다. 그래서 변경이 *스스로 만들어내는* 차이 — 아직 없는 테이블, 스크립트가 추가할 PK, 바뀐 시그니처 — 가 전부 결함·경고로 보고됐고, 한 대화에서는 미적용 마이그레이션을 두고 "동적 ALTER 로직이 실제로 실행되지 않았거나 실패한 상태" 라는 **허위 결함 단정**까지 나왔다. 사용자가 원한 것은 적용 후 구조의 리뷰였는데, 받은 것은 현재 DB 와의 불일치 목록이었다.
- Correct approach: (1) **검증 규칙에는 항상 해석 규칙을 붙인다.** "X 를 확인하라" 는 "확인 결과가 A 면 …, B 면 …" 까지 가야 계약이다. 확인만 강제하면 분류는 모델 재량이 되고, 재량은 기본 프레임으로 수렴한다. (2) **시간 방향을 명시한다** — 리뷰 대상이 *적용될 변경*이면 라이브는 BEFORE, 첨부는 AFTER 다. 그러면 "차이" 는 결함이 아니라 변경의 정의 그 자체다. (3) **교정은 선행 봉인을 되돌리지 않는 축으로 넣는다.** 여기서 "대조를 줄이자" 는 정확히 이전 마찰(대조 없이 리뷰)의 재발이다. 대조는 유지하고 **분류만** 바꾼다 — 대조의 목적을 (a)적용 가능성 (b)미접촉 의존성 영향 (c)진짜 선행 누락 으로 재정의하면 라이브 조회는 그대로 살아남으면서 "지금 없다" 지적만 사라진다. (4) **면책 힌트는 반드시 양방향 분기로 쓴다** — "이건 결함이 아닐 수 있다" 만 주면 진짜 선행 누락과 권한·스코프·오타까지 면책된다. 분기 셋(적용 전제 / 진짜 누락 / 확인 부족)과 "교차확인 전에는 어느 쪽으로도 단정 금지" 를 함께 준다.
- 진단 레시피(재사용): **같은 입력의 자연 발생 A/B 대조쌍을 먼저 찾는다.** 이번 건은 사용자가 동일 5파일(sha256 일치)·동일 요청문을 3분 간격으로 두 번 보냈고, 한쪽은 코드 리뷰·다른 쪽은 현재-DB 부재 지적으로 갈렸다. 입력이 같고 출력 프레임만 갈렸다는 것은 **결함이 모델 능력이 아니라 계약 부재**라는 직접 증거이며, 프롬프트 수정이 옳은 레버임을 코드 독해 전에 확정해 준다. 첨부 해시(`core_attachments.sha256`)·요청 텍스트·`created_at` 근접으로 이런 쌍을 기계적으로 찾을 수 있다.
- Applies to: 검증·grounding 규칙을 누적해 온 모든 LLM 프롬프트. 규칙을 하나 추가할 때마다 "이 규칙이 만든 새 판정이 어떤 프레임에서 해석되는가" 를 같이 정한다. 같은 축의 선례: `FR-false-truncation-belief`(절단 epistemic 계약의 역방향 과발동) — **grounding 강화는 항상 2차효과 표면을 만든다**.
...
### LRN-20260730-0001 — 자동 폴백 안전망은 "도달성 장애"를 조용한 품질 저하로 번역한다 — 같은 장애에서 폴백 없는 형제 경로가 대조군이 된다
- Source: 사용자 보고 "18시 기준 모든 LLM 요청이 edge 로 호출" / CHG-20260730T191535-llm-edge-free-routing · feature-0007 ADR-003
- Category: pattern
- Frame: 게이트웨이 컨테이너가 재생성된 직후 **외부 DNS 해석이 34분간 실패**(계정·토큰은 모두 정상 — quota 가 아니라 **도달성** 장애)했는데, 사용자에게 보인 증상은 "장애" 가 아니라 **"모든 LLM 요청이 로컬 2B 모델로 서빙됨"** 이었다. 원인은 라우팅 체인 끝에 남아 있던 로컬 폴백(`edge-fallback`)이 도달불가를 흡수해 **정상 응답처럼 보이는 저품질 산출물**로 바꿔치기한 것. 주목할 점: 같은 장애 창에서 **폴백을 이미 걷어낸 형제 alias**(`*-chat`·`*-meta`)는 500 으로 즉시 실패해 **한 사건 안에 대조군이 존재**했다 — 두 규약의 거동 차이가 로그에서 나란히 관측됐다. 일반화: 안전망은 가용성을 사는 대신 **관측 가능성을 판다**. 폴백 대상이 원본보다 현저히 저품질(2B ctx 4096 vs frontier)이면 그 거래는 대개 손해다 — 사용자는 "느리다/안 된다" 는 알아채지만 "조용히 나빠졌다" 는 못 알아챈다.
- How to apply: (1) 폴백을 설계할 때 **폴백 대상의 품질이 원본과 동급인지** 먼저 묻는다. 동급이 아니면 폴백이 아니라 **명시적 실패**가 기본값이어야 한다(이 저장소의 `*-chat` 2026-07-07 결정이 그 선례이고, 2026-07-30 에 전 경로로 확대됐다). (2) 폴백을 유지하기로 했다면 **강등 발생 자체를 1급 신호로 관측**하라(폴백 서빙 시 경보·대시보드 카운터) — 로그를 뒤져야 알 수 있는 강등은 사실상 은폐다. (3) 장애 진단 시 "계정/quota" 를 먼저 의심하기 전에 **도달성(DNS·네트워크)** 을 배제하라: 자격증명 `expiresAt` 이 유효하고 429 가 0건인데 실패하면 그건 인증 문제가 아니다. (4) 안전망을 걷어내는 변경은 **그 안전망이 덮고 있던 촉발 조건(여기선 게이트웨이 DNS 단절)을 후속 과제로 명시**하라 — 제거 후에는 같은 창이 곧 전면 중단이다.

### LRN-20260730-0002 — 환경변수 override 존재 시 skip 하는 테스트는 정작 그 override 가 걸린 운영 환경에서 vacuous pass 한다
- Source: codex 적대검증 P1 (REV-20260730T191535-ai-claude-feature-0007-llm-edge-free-routing)
- Category: mistake
- Frame: "기본값이 X 여야 한다" 는 계약을 테스트하면서 `if os.getenv(KEY): pytest.skip("환경변수 override 존재 — 기본값 검증 대상 아님")` 패턴을 썼다. CI 는 env 가 비어 있어 통과하고, **정작 위반이 실재하는 운영 환경(`.env` 에 옛 값이 남은 상태)에서는 skip 되어 조용히 통과**한다. 잠그려는 계약이 *바로 그 환경의 거동*일 때, skip 은 게이트를 정확히 필요한 곳에서만 무력화한다. 이번 사례에서 실제로 운영 `.env` 에 `AGENT_INSIGHT_OFFHOURS_MODEL=edge` 가 남아 코드 변경만으로는 강등이 꺼지지 않는 상태였고, 그 테스트는 그것을 잡지 못하도록 설계돼 있었다. (§16.7 G4 boundary-analysis 와 같은 계열 — 경계를 건드리지 않은 케이스의 PASS 는 false-PASS.)
- How to apply: 기본값이 아니라 **실효 설정(effective config)** 을 검사하도록 반전한다 — "미설정이면 기본값 확인, 설정돼 있으면 그 값이 계약을 위반하지 않는지 확인". 그리고 판정은 **blocklist 가 아니라 allowlist** 로 한다(`edge|local|gemma` 문자열 검사는 `ollama/mistral` 을 놓쳤다 → `startswith("claude")` 로 반전). 마지막으로 **역검증을 반드시 수행**하라: 위반 값을 주입했을 때 실제로 FAIL 하는지 확인하지 않으면 "잠갔다" 는 주장 자체가 미검증이다(이번엔 `edge`·`ollama/mistral` 두 값으로 확인).
