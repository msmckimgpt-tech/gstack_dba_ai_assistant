...
class Api:
    ...
    def heartbeat(self, runtimes: list | None = None,
        ...
        **같은 사실의 두 면**이다. 따로 보내면 "살아 있다고 하는데 능력은 모르는" 또는 그
        ...
        # ⚠ **`bool()` 로 누르지 않는다** — 3상태다(`None` = 아직 확인되지 않음).
        #   누르면 「모른다」가 `false` 가 되어 정상인 러너가 「답할 수 없음」으로
        #   보이고, 반대로 초기값을 `True` 로 두던 종전 코드에서는 「모른다」가
        #   「정상」으로 보였다 — 사용자가 지적한 그 거짓이 바로 이 한 줄이었다.
        #   JSON `null` 로 그대로 실어 보내고, 3상태 해석은 서버·화면이 한다.
        body["ai_ready"] = None if _ai_ok is None else bool(_ai_ok)
