...
> **2026-07-07**: ㊵ **런타임 설정 콘솔** (feature-0018, 신규 슬라이스 — 코드 거주 feature-0003/0002) — 관리 콘솔 시스템>설정에서 실행 타임아웃·모델별 추론 예산 조정·저장·live/restart 하이브리드 반영(`WebRuntimeSettings`·`admin_settings` 라우터·`system.runtime.read/write` RBAC+audit·`shared/runtime_settings.py` resolver, env-fallback). runtime-settings Major·PB-0008 PASS. ㊶ **제품 카테고리 밴드 + 크로스-DB 관계 일반화** (feature-0016 §55) — 관계도 제품 카테고리 밴드(CAT/CATH/CATX)·크로스-DB 관계 일반화(xschema)·관계 수동 큐레이션(trust/break)·DB 단위 분석 재귀 전개·refine-not-override·back-refine(ADR-021, alembic 0038, PB-0008 PASS). ㊷ **대화 답변 안정성** (feature-0002) — 대화 답변 경로 edge(gemma) 폴백 완전 차단(맥락 손실 해소, conv-audit FR-edge-fallback). [[Features/feature-0016-metadata-graph]] · [[Features/feature-0003-agent-web-ui]].
