...
## 2. Task Queue
- [x] TASK-0124 (REQ-20260528-0001, Minor §12.3) MySQL 서버 설정에 `log_bin_trust_function_creators=1` + `local_infile=1` 영구 추가. 개발 편의 목적 — binlog 환경에서 SUPER 없이 stored function 생성 허용, 클라이언트 측 LOAD DATA LOCAL INFILE 허용. runtime `SET GLOBAL` 은 이미 적용됨; 본 변경은 다음 컨테이너 재시작 시 자동 적용.
