...
Review base: `76a76ddd517aad6001ddb62c53e2e360c0ebdfda`; current uncommitted worktree diff.
...
- **WF-QA-3 / P1 — newly enabled native-client suite is red (FIXED locally; independent full-suite confirmation pending).**
...
- **WF-QA-4 / P2 — compact status notes silently change identifiers and conditions (RESOLVED after independent re-review).**
  ...
  - Location: `bin/gen-status.sh:64`, `compact_note`; `bin/tests/gen_status_test.py` currently exercises long-note shape/idempotence but no semantic-preservation negative control.
...
- Collection is not execution. The targeted mixed run in WF-QA-3 exercised native-client tests and failed; therefore neither collection success nor entrypoint parity currently justifies a full-suite PASS. Root is running the complete configured suite; cross-suite `sys.modules` contamination must be resolved if exposed there.
...
- Restoring a previously shipped workflow block does not establish correctness by provenance. The restored freshness logic contains a reproducible false-success path and needs the same behavioral review as new implementation.

### 4. Verdict

**Workflow, suite-membership, installer, board fixture and generator changes: PASS after independent review. Overall WF-QA-3: pending independent verification.**
...
- The new runtime harness requires Node.js and fails explicitly when unavailable. The complete 521-test native suite ran with Node locally; CI/make dependency provisioning and the complete configured project suite remain root's responsibility; tests must not be silently skipped for a green result.
- Latest user steering: the **DQA native client is the primary service entry**, while browser handoff is compatibility behavior. This Node fake-DOM harness validates shared JS behavior only and does not substitute for actual DQA client/WebView/OS validation. Root must independently review the authored test changes and complete the applicable client verification before clearing WF-QA-3.
