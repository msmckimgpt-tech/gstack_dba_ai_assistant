...
- [~] TASK-0012-8 helper→web_context 이동 + APIRouter 추출(도메인 1개/커밋) + route-parity 골든 갱신
  ...
  - [ ] NS-BOUND=0 도메인 순차 추출(잔여): admin-conversations(1)→admin-usage(5 MIXED)→conversations(MIXED). **추출 전 grep `app.<handler>` 직접참조 + `app.<global>` monkeypatch 선행 점검(0012 학습).** 그 후 web_context 추출(NS-BOUND admin/metadata 등)→이연 핸들러 router 화.
