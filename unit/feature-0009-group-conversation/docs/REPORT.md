...
- In Progress: composer-clear-input-on-send — docs 완료 → verify-completion → PR → 머지 → 라이브 배포(web) → PB-0008. (composer-nonblock-interrupt·gc-unread-read-fix 등 직전 머지분은 라이브/배포대기.)
