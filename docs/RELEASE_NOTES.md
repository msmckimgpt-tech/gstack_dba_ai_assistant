...
서버 반영 및 설치기 공개 완료. 앱의 [업데이트 확인]에서 적용할 수 있습니다.
...
## 2026-09-08 — DQA 클라이언트 텍스트 선택·검색 설정 복구

- 답변과 표시되는 텍스트 첨부를 드래그해 선택하고 Ctrl+C/V로 활용할 수 있도록 창의 선택 차단을 제거했다.
- Ctrl+F 기본 페이지 검색 단축키를 활성화했다. PDF/스프레드시트 뷰어·OCR 추가는 포함하지 않는다.
- 실제 WebView2 텍스트 복사·붙여넣기 검증과 검색 UI 미검증 범위는 feature-0046 test-runs.d/20260908-text-interaction.md에 기록한다.

## 2026-09-08 — DQA 1.2.4: Codex 연결 완료·사용 불가 위치 안내

Codex 모델 조회 후 연결 완료가 실패하던 오류를 수정했다. 실제 응답 확인과 서버 수락 후 연결 완료를 표시하며, 위치 변경 도중 옛 결과가 섞이지 않는다. 실행 권한이 없는 AI는 목록에 사유를 표시하고 자동 연결에서 제외한다. CI용 gh-runner 계정은 AI 탐색 대상에서 제외했다. 다른 AI를 추가할 때 진행 중인 조회를 유지하고, 일시 실패 후 재시도를 복구했다.1.2.3의 텍스트 선택·검색 개선을 포함한다.

검증/출하 상태: feature-0046-native-client/docs/REPORT.md 및 test-runs.d/20260908-codex-connect-fix.md.
## 2026-09-09 — DQA 1.2.5 / 1.3.0: 작업을 끊지 않는 설치 · 서버 배포 자동 반영

### 변경 (운영자/사용자 영향)
- **DQA 1.3.0 — 앱을 쓰는 중에 새 버전을 설치한다.** 설치는 `versions/<버전>-<설치번호>` 새 슬롯에 실행 파일·내장 Python 을 배치하고 실제 실행 검사를 통과한 뒤 `active-slot.txt` 를 원자 교체한다. 현재 앱과 자식 러너를 종료하지 않으며 적용은 다음 정상 실행부터다. 고정 `DQALauncher.exe` 를 시작 메뉴·바탕화면·자동시작·스킴이 공유한다.
  - ⚠ 1.2.x 의 기존 업데이트 메뉴를 통한 **최초 전환은 한 번 재시작**된다. 중단 없이 넘어가려면 새 설치기를 직접 실행한다.
  - ⚠ 실행 중일 수 있는 옛 슬롯을 지우지 않으므로 **디스크 사용량이 설치 횟수에 따라 증가**한다(제거 프로그램이 정리).
  - ⚠ 미서명 자동 설치는 종전과 같이 기본 꺼짐이다.
- **DQA 1.2.5** — 공유 링크의 목적지(`dqa-connect://open?…&path=`) 수용을 설치본까지 반영했다.
- **서버 배포 완료가 열려 있는 화면에 자동 반영된다.** 배포 스파인이 두 replica 의 revision·asset stamp 일치와 검증 완료를 확인한 뒤 완료를 원자 게시하고(`bin/lib/ui-release.sh` → `/srv/ui-release:ro`), 대화 화면이 `/api/ui-release` 를 3초 주기로 확인해 안전한 상태에서만 갱신한다. 초안·새 첨부·진행 중 요청·미저장 편집·IME 조합 중에는 안내 후 기다린다.
  - ⚠ 이 기능 도입 **전부터 열려 있던 페이지는 새로 한 번 로드**해야 이후 배포부터 자동 적용된다.
- **배포 완료 검증 복구** — 엣지 복귀 점검이 실행 중 Caddy 안에 HTTPS 자식 프로세스를 만들어 프로세스 한도를 소진하던 것을 격리 프로세스(같은 network namespace · 호스트 CA 검증)로 분리했다. 상태 조회 실패와 실제 미기동을 구분한다.
- **첨부** — `.csv`/`.tsv` 를 「문서 원문」 모달에서 표(격자)로 보여 주고, 첨부 버전 비교를 SQL 유사 구문 기준으로 정렬한다.
- **대화** — 같은 대화에 답한 AI 세션을 이어 쓰고(미지원 런타임은 최근 대화 기록 전달), 그룹에서는 발언자를 구분한 최근 질문·AI 답변을 함께 전달한다.
- **운영: portproxy 동기화 예약작업의 실행 계정 교정(SYSTEM → 사용자 계정 + S4U).** WSL 은 LOCAL SYSTEM 을 지원하지 않아(`WSL_E_LOCAL_SYSTEM_NOT_SUPPORTED` · `EXIT=-1`) SYSTEM 등록은 5분마다 rc=1 로 죽으면서도 스케줄러 상태는 「사용」으로 보였다. `-RunLevel Highest` 는 유지하고 `RepetitionDuration` 을 1일 → 3650일로 늘렸다.
  - ⚠ 옛 이름(`mysql_ai_web_portproxy_sync`)의 유령 작업은 **재부팅 전까지** 스케줄러 메모리에 남는다.

### 미검증으로 남긴 것
- 공유 딥링크의 **설치본 종단**(브라우저 공유 화면 → 스킴 핸들러 → 설치된 앱이 그 대화를 여는 경로)은 `NOT-RUN` 이다.
- 실서비스 로그인·AI 위치 선택 복귀, 실제 작업표시줄 핀 클릭, 물리적으로 분리된 Windows 세션 2개 동시 설치, 사용자의 설치 취소 클릭은 이번 fixture 시험에 포함되지 않았다.

검증/출하 상태: feature-0046-native-client/docs/REPORT.md 및 test-runs.d/20260909-nondisruptive-update.md · test-runs.d/TASK-20260909T120000-client-125-share-entry.md, feature-0003-agent-web-ui/docs/FUNCTION.md REQ-20260909-deploy-refresh, feature-0014-zero-downtime-deploy/docs/FUNCTION.md REQ-20260909-caddy-probe, feature-0006-lan-proxy-access/docs/FUNCTION.md 실행 계정 절.
