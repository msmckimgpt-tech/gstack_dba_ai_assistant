...
## 4. Test Run History
- 2026-07-13 (TASK-20260713T053423-attach-user-version — 사용자 재업로드 첨부 버전 관리(해시 대조 → 버전 체인), **Major §12.3 + cross-cut feature-0002** — **PB-0008 Windows-browser DEFERRED(배포 후)**):
  ...
  - **Environment: Windows-browser — 배포 후 잔여(사유 명시)**: 본 cycle 은 미배포 worktree(`ai/claude/feature-0003-attach-user-version`)라 라이브 web 서버가 이 코드로 미기동 → 실 Windows 브라우저 시각검증은 배포 게이트(deploy_scope: included) 후로 연기. CHECK#13 미수행 사유 명시. **배포 후 PB-0008 확인 항목**: ① 대화에 `report.sql` 첨부 후 내용 바꿔 재첨부 → "새 버전(v2)으로 첨부" toast + 첨부 목록 `v2` 배지·"버전 N개" 펼침에 v1/v2 병존 ② 동일 파일 재첨부 → "동일 파일 — 기존 버전 사용" toast(새 버전 미생성) ③ 재업로드본으로 "무엇이 바뀌었어?" 질문 → assistant 가 변경점(diff) 인지·답변 ④ pageerror 0. Evidence(예정): `artifacts/shared/win-browser-shots-attach-user-version/`.
  - **Pass/Fail: PARTIAL(단위·정적 PASS, 라이브 PB-0008 배포 후 잔여)**.
