...
## 8. Completion Checklist (DI seam Phase 2 batch 4 = TASK-0012-7 history_anchor + cat-A 마무리)
- [x] history_anchor(P2_MULTI_CLOSE) byte-동치 전환 — 누적 cat-A 35
- [x] behavior-neutral — make test progress-chars 1238(baseline 동일)·0 fail, route-parity 179 불변, ruff clean, py_compile OK, `_require_account` exact 54→53
- [x] §18.8 적대 패널 — REV-20260630-0002: batch3 HIGH refute 의 근본원인(T2 전환 누락) 해소. 마이그 자체는 byte-동치 확인됨
- [x] 동반 테스트 전환 — T2(test_history_anchor_returns_pg_id) 직접호출→TestClient(as_account). 잔류 직접호출 0
- [x] cat-A 잔여 11(pre-auth gate 10 + ask_result) 아키텍처적 이연 판정·문서화(router-extraction 단계)
- [x] TASK/REPORT/MODIFY/REVIEW 반영, BLOCKED 없음
- [ ] verify-completion PASS(9 checks) → Git 커밋 (Phase 2 batch 4)
