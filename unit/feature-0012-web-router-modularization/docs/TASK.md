...
- [~] TASK-0012-7 DI seam Phase 2..7: 클러스터별 핸들러+테스트 동반 마이그(A 단순→B 인라인perm→C/E1 require_permission→F optional→D 동적→트랜잭션4+E2)
  ...
  - [ ] Phase 3 cat-B batch 3+: 확정 잔여 4(admin_llm_usage·admin_get_dashboard_prefs: 본문 `try: conn.close() except: pass` → try/except 제거; admin_list_datasources·admin_datasource_databases: AO 본문 finally:conn.close → try/finally 제거+dedent) + workflow 보류 8(동반 테스트 전환 누락 5·action분기 admin_get_system_prompt·fail-soft suggestions·admin_products_insight_coverage) + AND pre-auth gate/multi + OR·BRANCH 23 → cat C/D/E/F/트랜잭션.
...
## 8. Completion Checklist (DI seam Phase 3 cat-B batch 2 = TASK-0012-7 WRAP RP 5)
- [x] WRAP-style RP 5 핸들러 전환(외곽 try/finally:conn.close 제거+dedent+require_permission hoist) — 누적 cat-B 11
- [x] behavior-neutral — make test progress-chars 1238(baseline 동일)·0 fail, route-parity 179 불변, ruff clean, py_compile OK
- [x] §18.8 적대 패널 — REV-20260630-0004(REV-0003 확정분 재사용). 403 메시지 verbatim 보존 확인(actor 변수명 포함)
- [x] 동반 테스트 — delete_product_icon 직접호출→TestClient(as_account); 나머지 4 직접호출 0. 잔류 직접호출 0
- [x] TASK/REPORT/MODIFY/REVIEW 반영, BLOCKED 없음
- [ ] verify-completion PASS(9 checks) → Git 커밋 (Phase 3 cat-B batch 2)
