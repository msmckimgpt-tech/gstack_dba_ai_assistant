...
| `shared/` | feature-0002·0003·격리 컨테이너가 공유하는 Python 패키지 (feature-0011 P5a 추출 Step 1~5 완료 · 현재 20 모듈(`ls shared/*.py` 실측 2026-09-09, `__init__` 포함 — 09-08 창에서 `shared/attachment_path.py`(폴더 첨부 상대경로 정규화·디렉토리 트리 렌더 정본) 1종 신설 · 09-03 창에서 `shared/dqa_identity.py`(개인 머신 클라이언트의 제품명·URL 스킴·역-DNS **명칭 정본**) 1종 신설 · 09-01~09-02 창에서 `shared/self_review.py`(외부 AI 자가 검증 5축의 **계약 정본**을 서버에 두어 러너 재설치 없이 축을 고칠 수 있게 한다) · `shared/bridge_consent.py`(배경 배치 동의) · `shared/bridge_caps.py`(능력 신고 지문·계정별 baseline 원장) 3종 신설) · `shared/model_catalog` 는 LLM 카탈로그와 provider identity 주입 규칙의 단일 정본) | `docs/CODEBASE_MAP.md` §3 Shared Module Index (모듈 색인 정본) + `shared/README.md` + `shared/docs/MODIFY.md` |
...
- [[../Features/_Index|Features MOC]] — feature-0001 ~ feature-0046
