...
### 15.4-2 P1 — 자유 SELECT (`execute_sql`, 2026-08-14 개방)

구조 정보만으로는 **관계 주장을 데이터로 검증할 수 없다**는 실사용 제보로 열었다(FK 0건
스키마에서 실제 행수·고아행·뷰 정의 확인이 전부 막혀 있었다).

방어는 **내부 경로 것을 그대로 통과**한다 — sqlglot AST 가드(단일 SELECT/CTE · write verb ·
다중문 · lock · INTO · 금지 스키마) · 제품 스키마 allowlist · 무거운 쿼리 사전 게이트 ·
per-query 시간 cap. 외부 표면이 **추가로** 지는 것은 셋이다.

| 축 | 왜 |
|---|---|
| CSV **미생성**(`_suppress_csv`) | 응답에서 경로만 지우면 파일은 쌓이고, 소유 대화와 무관한 전체 결과가 서버에 남으며, 저장 실패 시 절대경로가 예외 문구로 샌다 |
| **실제 행수** 원장 기록(`_stats_out`) | 렌더 줄 수로 세면 미리보기 50행만 잡혀 시간당 행 상한이 장식이 된다 |
| **건당 행 상한** | 시간당 상한은 실행 전 누적 확인이라 원자적이지 않다 — 상한 직전 대형 쿼리 1회, 동시 요청이 같은 잔여를 본다. 이 값이 **초과분을 유계로** 만든다(hard cap 이라 부르지 않는다) |

운영자는 `AGENT_EXT_TOOL_SQL_ENABLED` 로 이 축만 끌 수 있다(구조 조회는 유지).

### 15.4-2-1 부하 게이트가 외부 AI 에게 하는 말 (2026-08-14 정정)

게이트의 차단 기준·임계·denylist 는 **내부 대화 경로와 공유**하며 이 feature 가 바꾸지 않는다.
바꾼 것은 **차단할 때 하는 말**이다.

- **집계 전용 쿼리**에는 집계 조언을 준다(엔진 무관). 예전엔 이 조언이 실행계획 사실이 있을
  때만 나가서, 계획을 주지 않는 MSSQL 에서는 `COUNT(*)` 쿼리가 "서버측 집계(COUNT/SUM)를
  쓰세요" 라는 **이미 한 일을 시키는** 안내를 받았다. 외부 AI 는 구간 2분할로 우회했고
  **총 스캔량은 동일**했다 — 게이트가 부하를 못 줄이고 마찰만 만들었다.
  이제 명시한다: 근사치는 `approx_rows`(구조 도구가 이미 준다), **분할은 총 부하를 줄이지
  않는다**, 정확한 값이 꼭 필요하면 `confirm_heavy`.
- **카탈로그 이름 함수**(`OBJECT_NAME`·`COL_NAME` 등) 차단 시 `INFORMATION_SCHEMA` 경로를
  안내한다. 차단 자체는 유지 — 권한 탐침 함수와 한 목록이라 선별 완화는 경계를 흐린다.
- **추정 실패** 문구에서 "쿼리가 무거워서가 아니다" 를 앞세우고 원인 3종(권한·연결·**SQL 형태**)을
  구분한다. "범위를 좁혀라" 만 주면 좁혀도 안 풀리는 부류에서 무한 재작성에 빠진다.

⚠ 계획 사실이 없을 때 **계획에서 유도한 문구(접근형태·인덱스·파티션)는 여전히 지어내지 않는다.**
집계 판정은 SQL 형태에서 나오므로 그 계약과 출처가 다르다 — 테스트가 둘을 함께 고정한다.

### 15.4-3 실행 스코프 — 세우는 곳이 모든 축을 진다

`scoped_execution` 이 **연결 · 스키마 allowlist · 방언/기본DB** 를 함께 세우고 finally 에서
전부 되돌린다. 이전에는 다중 바인딩일 때만 라우터를 세우고 단일 바인딩은 `None` 을 돌려주며
docstring 으로 호출측에 미뤘는데, 호출측이 memory DB 연결을 넘겨 **구조 조회가 내부 서버를
향했고**(다른 대화의 첨부 샌드박스까지 노출) allowlist 도 걸리지 않았다. 대부분의 제품이 단일
바인딩이라 예외가 아니라 기본 경로였다. datasource 미바인딩 제품은 **403**(전엔 내부 DB 노출).

⚠ `set_active_datasource` 는 **토큰을 반환하지 않는다**(setter). 반환값을 token 으로 받아
복원하면 절대 복원되지 않아 ContextVar 가 스레드에 남는다.
