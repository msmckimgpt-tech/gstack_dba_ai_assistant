...
- Deploy: web 재빌드(정적+엔드포인트). **선행조건**: PG(agent_kb)에 sample_feedback/sample_queries 스키마 + titan-embed 임베딩(1024-dim)이 가용해야 적재·승급 동작. 신규 RBAC 는 admin 자동 보유, 비-admin 검수자는 kb.sample.curate 명시 부여 필요(least-privilege).
