...
def _postprocess_attachment_blocks(cid: str, account_id: Any,
    ...
    **terminal 전이(finish_ask_job) 전에 호출**해야 한다. web long-poll 은 terminal 을 보고
    ...
    try:
        ...
        #
        # 저장·strip·미전달 고지는 **경로 공용 정본**(`_apply_assistant_attachment_blocks`)이
        # 한다(feature-0043). 종전에는 이 블록이 그 시퀀스의 유일한 구현이었고, 브리지 경로에는
        # 아예 없어 개인 AI 가 만든 `attachment-edit` 블록이 파일로 저장되지 않은 채 답변만
        # "수정했습니다" 라고 말했다(거짓 성공, 라이브 실측 2026-08-28). 구현이 갈리는 순간
        # 느슨한 쪽이 사용자가 보는 진실이 되므로 한 벌로 합쳤다.
        _res = _web._apply_assistant_attachment_blocks(
            conn, account=account, conversation_id=cid, message_id=message_id,
            answer=content, failed=_failed, tool_attachment_ids=_tool_ids,
        )
        edited = list(_res.get("edited") or [])
        created = list(_res.get("created") or [])
