...
- AC-20260729T140200-attach-full-scope-2 (보안 경계 불변): ① 그룹 대화는 발신자 본인 첨부만(`sender_scope` → `AccountId` 필터, feature-0009 CSO F1 권한상승 가드 유지 — 2026-07-29 사용자 결정). ② `ConversationId` 스코프로 타 대화 첨부 유입 차단(TASK-0284 IDOR 안전망). ③ 공유창 bounded 발신자에게는 리뷰어 첨부 근거를 넘기지 않는다(누출 게이트 불변).
