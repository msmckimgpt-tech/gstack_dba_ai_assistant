...
- [ ] verify-completion(pre-commit) → commit → PR → 머지 → web 재배포(deploy_scope: included) → **POST-DEPLOY PB-0008 라이브 재검증**: 재배포된 graph.css 에서 도움말 설명 어절 줄바꿈·카드 폭 브라우저 반응(다중 크기)·중앙정렬·pageerror 0. worktree `ai/claude/feature-0003-graph-help-wordbreak`(base main 14f54e64).
