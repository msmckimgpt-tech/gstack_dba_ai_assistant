...
## REV-20260729T142000-ai-root-feature-0021-rounds-fixes [CODEX:review]
- Related TASK: feature-0021-redteam-review (TASK-20260729T142000-rounds-visual-fixes)
- Related Change: CHG-20260729-0006
- Trigger: 코드 변경(agent 쓰기 경로 + 콘솔 렌더). 이전 cycle 의 사용자 결정(codex 리뷰)을 승계.
- Reviewer: OpenAI Codex CLI (`codex exec -s read-only`, `model_reasoning_effort=high`)
- Timestamp: 2026-07-29T14:20:00+0900
- Verdict: PASS (**P1 0건** · P2 2건 → 1건 반영, 1건 근거 기록 후 수용)
- Human Approval Needed: no

### 왜 이 cycle 이 생겼나 — 검증이 결함을 찾은 사례
회차 원장은 배포 전 단위·시각 검증을 통과했지만, 그때 라이브 표본은 **2단계**뿐이었다.
사용자가 다회차 표본(10단계)을 확보해 육안 검증을 요청하자 **분량이 늘어야만 드러나는**
결함 2건이 나왔다. 둘 다 "데이터는 맞는데 읽는 사람이 오해한다" 는 계열이다:
- 10개 목록에서 종료 원인이 **펼쳐야만** 보임 → 사유를 요약으로 승격.
- 시각 10개가 전부 같음 → 값이 있으니 "회차별 시각" 으로 읽히지만 실제로는 **배치 시각**.

### [P2] 동일 timestamp 숨김이 정당한 데이터도 숨긴다 — **반영**
codex: "legacy 여부를 판별할 표식 없이 감사 시각을 조용히 제거한다."
숨김 자체는 유지하되(값이 같은 10개를 회차별 시각으로 보여주는 것이 더 큰 오도) **숨겼다는
사실을 1줄 안내**로 표면화했다. 이 프로젝트의 `capped`/`rounds_truncated` 와 같은 계열
규약(무언의 절단 금지)이다. 실제 동일 시각이 정당한 경우에도 안내가 붙지만, 그 안내가
"시각이 원래 없음" 과 "일괄 저장됨" 을 구분해 주므로 정보 손실은 없다.

### [P2] 표시 시간대가 불명확 — **수용(근거 기록)**
codex: "`slice(11,19)` 로 offset 을 버려 DB 세션이 UTC 면 KST 사용자에게 9시간 이르게 보인다."
지적은 원리적으로 맞다. 다만 **이 콘솔 전체가 같은 규약**을 쓴다(리뷰 헤더 시각·통계·감사
로그 모두 `isoformat()` + slice). 라이브 실측에서 회차 시각과 리뷰 헤더 시각이 `14:08:57` 로
일치했고 이는 PG 세션 timezone 이 KST 로 반환한다는 뜻이다 — 즉 현재 환경에서 어긋나지 않는다.
회차만 `toLocaleTimeString` 으로 바꾸면 오히려 같은 화면 안에서 규약이 갈린다. 시간대 표기는
콘솔 전역 과제로 남긴다(별도 cycle).

### codex 가 확인한 것 (인용)
> "SQL 컬럼/바인딩은 행당 15개로 일치하며, 두 번째 행 `at` 위치 29도 정확합니다.
> `COALESCE(%s, now())` 의 타입 추론과 offset 은 문제 없습니다.
> 신규 summary 의 `note` 는 최종 `esc(...)` 를 거쳐 XSS 문제 없습니다."

## REV-20260729T173000-ai-root-feature-0021-reasoning-hint [SKIPPED:UI 카피 1줄 축약 — 데이터·로직 무변경]
- Related TASK: feature-0021-redteam-review (TASK-20260729T173000-reasoning-hint-trim)
- Related Change: CHG-20260729-0007
- Trigger: changeset 이 안내 문구 1줄 + 문서. §18.8 키워드(인증·스키마·API 계약·성능) 비매칭이고
  직전 두 cycle 의 codex 리뷰가 같은 코드 경로를 이미 훑었다 — 카피 변경에 패널을 재소집할
  근거가 없다.
- Timestamp: 2026-07-29T17:30:00+0900
- Verdict: PASS
- Human Approval Needed: no

### 판단 — 안내는 "무엇인가" 까지, 조작법은 화면이 말하게
회차 구조를 안내로 설명하려 한 것이 과했다. 접기/펼치기·정렬은 캐럿과 배치가 이미 보여주고,
설정 경로는 좌측 메뉴가 담당한다. 안내가 그것을 복제하면 **매 방문마다 읽히는 비용**만 늘고
정보는 늘지 않는다. 상세 계약(정렬·상한·폴백)은 FUNCTION.md §7.5 가 정본이며, 운영자가
그 수준을 원할 때 찾아갈 곳이 이미 있다.

### 함께 확정된 것 (이전 cycle 미커버 해소)
CHG-20260729-0006 이 "회차별 실제 시각이 라이브 원장에 남는지는 배포 후 다회차 표본 필요"로
