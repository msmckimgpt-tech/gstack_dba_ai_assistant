...
- **POST-DEPLOY PB-0008 (미수행·사유): 제 변경(admin.js/html/css 신규)은 배포 후에만 라이브에 존재** — deploy_scope: included 로 병합 직후 web 재배포(정적 자산 baked, 마이그 없음) 후 실 Windows Chrome 으로 육안 확인 예정(본 섹션에 POST-DEPLOY Run append): ① 상세 패널 ←뒤로/앞으로→ nav 바(이력≤1 숨김·끝단 disabled) ② 관계 행 단일=카메라만·더블=상세 전환 ③ 상세 패널 하단 3탭 범례 렌더·탭 전환·pg_trgm 문구 부재 ④ AI 능동 분석 재클릭 시 "이미 진행 중" 메시지·중복 run 미생성 ⑤ 샘플검수·용어 태그 데이터소스 라벨 표시 ⑦ 스키마 펼침 시 상세 패널 "▦ 접기" 버튼 축소 · pageerror 0.
