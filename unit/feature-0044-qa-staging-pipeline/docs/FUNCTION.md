...
| 11 | SVN 용량 정책 | **안 A** — 매니페스트·설정만 SVN, tar 는 사내 파일서버 | SVN 경량 유지. 매니페스트가 두 저장소를 `url`+`sha256` 으로 연결 |
| 12 | 로컬 LLM | **전면 미사용** — 임베딩까지 사내 MCP 이관 예정 | **GPU 불요** → 스펙 하향. 이관 완료가 QA 세팅의 선행 조건 |
| 13 | 임베딩 MCP 위치 | **사내 망 내부** | 아웃바운드 예외 불요. 대신 KB 기능의 새 SPOF |
...
- **로컬 임베딩(`embed-ollama` · bge-m3)의 사내 MCP 이관** — 본 feature 의 범위가 **아니다**
  (별도 작업). 다만 QA 스펙(GPU 유무)에 직접 영향을 주므로 현황·제약·순서를
  CICD_DESIGN §9 에 기록하고, 이관 완료를 **QA 세팅의 선행 조건**으로 명시한다.
