...
- **[POST-DEPLOY 갱신 2026-07-03] PB-0008 Windows-browser 라이브 실측 PASS**: PR #555 병합(7f2b9aa7) → deploy-web.sh 롤링(alembic 0031 적용·soak 통과) + insight-worker/ask-worker 재빌드 → role 백필 509/509 → 실 Windows Chrome 149(relay) 라이브: 역할 범례 행·roles 384 적재·dk_data_release 역할 칩 123개(색+아이콘+dark 라벨)·selected 어두운 테두리·상세 패널 역할 칩(📦 기타) 실측. 스크린샷 `artifacts/feature-0016-metadata-graph/20260703-node-role-viz-pb0008/`. → CHECK#13 실충족. 상세: feature-0016 TEST.md node-role-viz 절.
