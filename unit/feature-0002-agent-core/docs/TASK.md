...
## 1. Current Status
- State: DB 조회 사용자 경험(UX) 개선 — 스키마 grounding PG 복구 + system prompt 개편 + 멀티턴 맥락 보존 + 첨부 리뷰 정합 완료 (cycle: TASK-0151)
- Owner: AI (본 cycle: agent_core grounding/prompt/window + domain origin 가드 + app.py 첨부 cap + 단위테스트 12)
- Priority: high
