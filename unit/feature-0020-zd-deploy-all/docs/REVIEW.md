...
## REV-20260814T133000-ask-surge-selfcheck [SKIPPED:session-policy-no-subagent] — PASS
- **Trigger**: 사용자 요청 "자가 검증도 진행". 선행 `REV-20260814T120000` 이 subagent 패널을
  열 수 없어 남긴 공백을 **직접 적대 검증 + 라이브 실측 + 뮤테이션**으로 메운 cycle.
- **검증 설계(가설 → 실측)**: 코드를 다시 읽으며 "이 판정이 거짓으로 통과할 수 있는가" 만 물었다.
  가설 H1(`compose ps -q` 가 stopped 를 포함하지 않으면 drain 관측이 상수가 된다)을 **라이브에서
  직접 실측**해 확정 — surge 를 띄우고 stop 한 뒤 `ps -q`=빈 값 / `ps -aq`=cid.
  가설이 맞으면 즉시 결함이므로, 추측이 아니라 실행으로 갈랐다.
- **적발 2건(둘 다 이번 cycle 코드의 결함)**: D1 drain 보고가 항상 `after=0`(vacuous pass) ·
  D2 stopped leaked surge 미감지. 둘 다 **정상 경로에서는 아무 증상이 없고, 정확히 문제 상황에서만
  거짓말하는** 부류다 — 유휴 배포 로그가 초록이었던 이유가 그것이다.
- **동시 확인(설계 전제 실증)**: 본체/surge 공존에서 liveness 파일이 인스턴스별로 분리되고
  healthcheck 가 각각 독립 판정 · role reclaim 상호 탈취 0건. 즉 "다중 인스턴스 안전" 이라는
  surge 교대의 근거가 라이브에서 성립함을 확인했다(종전엔 SQL 계약 논증뿐이었다).
- **뮤테이션 11/11 KILLED** — 신규 3축(hostname 사후 캡처 · `ps -q` 복귀 · 잔존 run 은폐)이
  이번 적발분의 재도입을 잡는다. 판별력 있는 테스트임을 확인.
- **정직 — 여전히 남는 것**: ① "바쁠 때 기다리지 않고 완결" 궤적은 아직 미관측(유휴 배포만
  실측). 다만 이번 수정으로 **그때 나올 보고가 신뢰할 수 있게** 됐다 — 종전 로직이면 바쁜
  배포에서도 `0` 을 찍어 검증 자체가 불가능했다. ② subagent 패널은 여전히 미실행.
