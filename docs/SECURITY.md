...
| `/install` | GET | anonymous | DQA 클라이언트 **설치 안내 페이지** (`install.html` FileResponse). `static/client-gate.js` 가 앱 루트(`/`)·관리 콘솔(`/admin`)에서 브리지 좌표 신호를 못 찾으면 여기로 `location.replace` 한다 (사용자 결정 2026-09-10: 「DQA 클라이언트 외 일반 웹브라우저는 설치 안내로」 · **탈출구 없음**). 익명인 이유: 이 화면에 도착한 사람에게는 아직 로그인할 창(DQA 앱)이 없다 — 인증을 걸면 아무도 못 본다. 인스턴스 데이터 0(정적 HTML + 아래 `entry` 응답만). ⚠ 이 화면은 **미서명 설치기 실행**을 안내한다(SmartScreen 1회) — share 행의 같은 주의를 그대로 적용한다: 안내 문구·다운로드 출처를 바꿀 때 이 항을 함께 검토. ⚠ 게이트는 **표시이지 집행이 아니다** — 자격은 로그인 세션·RBAC 이 그대로 집행하고, 우회로 얻는 것은 「어느 화면에서 봤는가」뿐이다(share 행과 동형) | feature-0003 client-entry-gate (2026-09-10) |
| `/api/ai/client/entry` | GET | anonymous | 위 안내 화면이 쓰는 **화면 한 벌** — `{app_link, release}`. `app_link` 는 `shared.dqa_identity.app_open_url` 이 조립한 `dqa-connect://open?…` 이며 **자격증명 미포함**(토큰 없음, `/api/public/share` 의 `client.app_link` 와 같은 규약). `release` 는 `current_release()` 가 실물·크기·sha256 대조를 통과시킨 값만 담고, 없으면 `null`(없는 것을 광고하지 않는다). 업데이터 계약(`/api/ai/client/latest`)과 표면을 가른 이유는 `routers/client_release.client_entry` docstring 참조 | feature-0003 client-entry-gate (2026-09-10) |
