...
- 완료 증분: inc1(session id/token leaf) · inc2(proxy/client-IP) · **inc3(SESSION_COOKIE+RBAC 권한 카탈로그)** · **inc4(권한 오버라이드·계정 행 빌더 폐포)** · **b2(검증 정규식·인증 파라미터·seed 롤)** · **b3(세션쿠키·패스워드·TOTP 인증 클러스터)** · **b4(세션 경로·출력 정규화·미디어 URL)** · **b5(계정 로더·id 맵·RBAC seed 부트스트랩)** · **b6(A+B+C: 권한 read-model·제품 접근·인증 read/actor — 적대 판정표 기반, KEEP 3종·금지 목록 9종 확정)** — app.py 19,650→17,724줄(2026-07-11).
