...
### 2.4 연결 — 기본은 연결 프로그램, 터미널은 접힌 보조 경로

사용자에게 "당신의 AI 가 MCP 를 지원하는가" 를 묻지 않는다. 그건 만든 사람이나 답할 수 있는 질문이다.

> **2026-09-03 주 경로 교체.** 기본 진입점이 「명령 복사 → 터미널 붙여넣기」에서 **연결 프로그램**([[../Features/feature-0046-native-client|feature-0046-native-client]] · `DQA Connect`)으로 옮겼다. 터미널 1·2·3단계는 **접는다 — 없애지 않는다**: 연결 프로그램을 설치하지 않은 사용자에게는 그것이 유일한 길이고, 실행 실패 안내가 「1단계 명령」으로 되돌려 보내므로 접힌 블록은 **자동으로 펼쳐진다**(되돌아갈 길을 가리키면서 그 길을 닫아 두면 막다른 길이다).
...
flowchart TD
    ...
    C -.->|"연결 프로그램이 없다면<br/>(접힌 details)"| D{"OS 탭<br/>기본값 = 마지막으로 연결됐던 OS"}
