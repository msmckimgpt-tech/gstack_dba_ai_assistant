...
def _ensure_seed_roles(conn) -> None:
    ...
    if admin_row:
        ...
        for code in (
            ...
            "console.aiops.read",
            # feature-0018: admin 의 런타임 설정(실행 타임아웃·모델 추론 예산) read/write 2건 catchup.
            # **필수** — 신규 권한은 role 생성 시 seed=set(PERMISSION_CODES)로만 부여되어 기존 배포
            # admin row 에는 retroactive 미적용. 미보정 시 기존 admin 이 `시스템 > 설정` 의 신규 항목을
            # 못 본다(lockout). operator/sales/dba 미부여(least-privilege).
            "system.runtime.read",
