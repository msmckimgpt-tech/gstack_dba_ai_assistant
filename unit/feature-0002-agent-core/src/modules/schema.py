...
def _maybe_record_schema_usage(
    conn,
    conversation_id: str,
    sql_text: str,
    ...
    if not conn or _is_zero_summary(summary_data):
        return
    kv = kv or {}
    ...
    if _is_system_schema(schema):
        return
    allow_schema_pref = True
    allow_table_pref = bool(table)
    guard_reason = "strict_disabled"
    if AGENT_SCHEMA_USAGE_RECORD_STRICT:
        req = str(request_text or "").strip()
        req_schema = _sanitize_ident_part(_detect_requested_schema(req, KNOWN_SCHEMAS) or "")
        sql_ref = _normalize_object_ref(schema, table or "")
        known_tables = [table] if table else []
        explicit_refs, explicit_tables = _extract_object_hints_from_request(req, known_tables=known_tables)
        explicit_match = bool(
            sql_ref
            and (
                sql_ref in explicit_refs
                or (table and table.lower() in explicit_tables)
            )
        )
        anchor_ref, anchor_full = _extract_anchor_table_ref(kv)
        anchor_match = bool(
            sql_ref
            and (
                (anchor_ref and anchor_ref.lower() == sql_ref.lower())
                or (anchor_full and anchor_full.lower() == sql_ref.lower())
            )
        )
        if explicit_match:
            allow_schema_pref = True
            allow_table_pref = bool(table)
            guard_reason = "explicit_object"
        elif req_schema:
            if req_schema != schema:
                allow_schema_pref = False
                allow_table_pref = False
                guard_reason = f"schema_mismatch:{req_schema}!={schema}"
            else:
                allow_schema_pref = True
                allow_table_pref = bool(table and anchor_match)
                guard_reason = "schema_match"
        elif anchor_match:
            allow_schema_pref = True
            allow_table_pref = bool(table)
            guard_reason = "anchor_match"
        elif looks_like_sql(req):
            allow_schema_pref = True
            allow_table_pref = bool(table)
            guard_reason = "direct_sql"
        else:
            allow_schema_pref = False
            allow_table_pref = False
            guard_reason = "ambiguous_request"
    if not allow_schema_pref and not allow_table_pref:
        try:
            log_fact_quality(
                "schema_usage_skipped",
                {
                    "conversation_id": conversation_id or "",
                    "reason": guard_reason,
                    "request": " ".join(str(request_text or "").split())[:140],
                    "sql": _compact_sql_for_memory(sql_text, max_len=240),
                },
            )
        except Exception:
            pass
        return
    try:
        if allow_schema_pref:
            save_memory_kv(conn, conversation_id, "preferred_schema", schema)
    except Exception:
        pass
    try:
        if allow_schema_pref and GLOBAL_SESSION_CONVERSATION_ID:
            save_memory_kv(
                conn,
                GLOBAL_SESSION_CONVERSATION_ID,
                "last_global_domain_anchor_schema",
                schema,
            )
        if allow_schema_pref and AGENT_GLOBAL_KB_SHARE_ACROSS_SESSIONS:
            save_memory_kv(
                conn,
    ...
    if allow_schema_pref:
        _publish_fact(
            conn,
            conversation_id,
            f"schema_pref:{schema}",
            f"분석/집계에 사용된 주요 스키마: {schema}",
            5,
            source_type="schema_usage",
            source_run_id=source_run_id,
            source_sql=sql_text,
        )
    if table and allow_table_pref:
        try:
            ...
            if GLOBAL_SESSION_CONVERSATION_ID:
                save_memory_kv(
                    conn,
                    GLOBAL_SESSION_CONVERSATION_ID,
                    "last_global_domain_anchor_table",
                    table_ref,
                )
            if AGENT_GLOBAL_KB_SHARE_ACROSS_SESSIONS:
                save_memory_kv(
                    conn,
                    GLOBAL_CONVERSATION_ID,
                    "last_global_domain_anchor_table",
        ...
        _publish_fact(
            conn,
            conversation_id,
            f"table_pref:{schema}.{table}",
            f"주요 테이블: {schema}.{table}",
            4,
                source_type="schema_usage",
                source_run_id=source_run_id,
                source_sql=sql_text,
