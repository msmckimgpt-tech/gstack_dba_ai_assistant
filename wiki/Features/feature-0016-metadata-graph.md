...
- 2026-07-02 (doc_sync): 07-02 그래프 뷰 후속 진화 반영 — 렌더링 엔진 Cytoscape(WebGL)→AntV G6 v5(Canvas) 전면 교체(ADR-004, 07-01 WebGL 대체)·클러스터 다열 masonry/shelf-packing·노드/스키마카드 우클릭 상세 상호작용(컨텍스트 메뉴·관계 상세 패널·중심 보기 칩)·초기 진입 가시성(스키마-우선+줌클램프+미니맵)·검색 스키마 카드 badge 매칭·노드 펼침 논블로킹(ADR-005)·프리즈 잔존 해소(setData rebuild, ADR-006)·추정/신뢰 관계 자기교정 파이프라인 미가동 근본수정(config `__all__` 누락 NameError 3일 정지 결함 포함 4중 원인, ADR-007)·더블클릭 카메라 앵커-중심 애니 팬(ADR-008)·그래프 분석 전용 모델 claude-haiku 분리. 정본 REPORT 2026-07-02 항목 / DECISIONS ADR-004~008(feature-local).
