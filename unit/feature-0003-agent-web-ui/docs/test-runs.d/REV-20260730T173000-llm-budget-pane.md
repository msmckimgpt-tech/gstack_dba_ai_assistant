...
`bin/win-browser.py` relay 로 실제 Windows Chrome(150.0.7871.115)에서 관리 콘솔 →
**AI 운영 현황 → 운영 현황** 진입 후 확인:

- ✅ **'백그라운드 LLM 토큰 예산 (최근 24시간)' 섹션 노출**
- ✅ 실측 수치 표시: `6,719,639 / 20,000,000 토큰 (34% · 남은 여유 13,280,361)`
  — 같은 시각 API `snapshot()` 값(`spent=6,751,132`)과 정합(폴링 간격 차이 범위)
- ✅ 막대 녹색(34% < 80% 임계)
- ✅ 면제 안내 문구 노출: "대화 답변·자가검증·제목 생성처럼 사용자가 기다리는 호출은 이
  예산에서 제외되며 상한과 무관하게 항상 나갑니다."
- ✅ 하단 '워커 공유 자원' 표(T0b)도 정상 렌더 — 워커 6개 × `llm`/`ds`/`task` 자원별
  최대 점유/상한/거절/커넥션 누적

증거: `evidence/20260730-llm-budget-pane.png`

## 게이트 실동작 (콘솔 외 라이브 검증)

운영 설정을 건드리지 않고 **별도 프로세스 안에서만** 상한을 1000 으로 낮춰 실제 코드 경로를
라이브 데이터로 구동한 결과(`repo-insight-worker-1`):

```
노드 분석 보류 — 백그라운드 LLM 토큰 예산 소진(6739092/1000, 24h)
semantic_cluster pass 보류 — …
product_classify pass 보류 — …
node_analysis   -> llm_token_budget | claimed = 0
semantic_cluster-> llm_token_budget
product_classify-> llm_token_budget
```

`claimed=0` — 잡을 집어오지도 않고 반환한다(claim 전 게이트가 실제로 앞선다).

## "사용자를 막지 않는다"의 라이브 증거

| 최근 24시간 | 콜 | 토큰 |
|---|---|---|
| 전체 | 1,955 | 10,379,848 |
| 예산 대상(백그라운드) | 1,839 | 6,731,217 |
| **제외(사용자 경로)** | 116 | **3,648,631** |

합이 정확히 일치한다(6,731,217 + 3,648,631 = 10,379,848). 사용자 답변 경로의 365만 토큰
(전체의 35%)이 예산에서 완전히 제외된다 — 이것을 예산에 넣었다면 상한 도달이 훨씬 빨랐고
그 순간 사용자 답변이 막혔을 것이다.
