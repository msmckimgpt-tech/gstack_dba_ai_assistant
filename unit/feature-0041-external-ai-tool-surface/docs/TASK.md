...
- (미결 결정 1) **AC-7 대화 적재를 구현할 것인가** — §5.1 참조. 사용자 요구의 답변 축이
  미충족이고, 소급 복구는 불가(이미 제출된 2건의 답변 본문은 우리 쪽에 없다)
- (미결 결정 2) 08-14 에 제출된 `dbauth` 분석 task(`t_tLqaVEPlZ-DIxie6`)의 **정정 메모** 부착
  여부 — 08-14 스코프 결함 수정으로 같은 제품의 datasource 가 MSSQL 로 교체되어 그 답변의 대상
  DB 가 현재 스코프에서 도달 불가. 단 **답변 본문 자체가 우리 쪽에 저장돼 있지 않으므로**
  (미결 1) 메모를 붙일 대상은 task 행의 질문·상태뿐이다 — 결정 1 과 묶어 판단해야 한다
...
### P1 개방 + 스코프 결함 (2026-08-14)

- [x] `execute_sql` 노출(P1_TOOLS) — 방어는 내부 경로 재사용, 외부는 CSV 미생성·실제 행수 기록
...
- [x] 실사용 2차 제보 — 게이트 코칭·카탈로그 함수 안내·추정 실패 문구 (차단 기준 무변경)
