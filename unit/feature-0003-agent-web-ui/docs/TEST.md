...
- **Environment: Windows-browser (PB-0008) — 배포 후 라이브 재검증 예정**: cycle-finalize→재배포(cache-buster `?v=20260704-group-drag-hotfix`) 후 **패치 없이** 그룹 헤더 드래그 → groupOffset 설정·그룹 통째 이동·타 그룹 불변·rebuild 유지 실측하고 본 Run POST-DEPLOY 갱신.
