...
- [~] TASK-0012-7 DI seam Phase 2..7: 클러스터별 핸들러+테스트 동반 마이그(A 단순→B 인라인perm→C/E1 require_permission→F optional→D 동적→트랜잭션4+E2)
  ...
  - [ ] Phase 2 batch 2: list_conversation_shares(SQL heredoc dedent 수작업) + profile_usage_conversations(동반 테스트 TestClient 전환) + 적대검증 보류 3건(mark_conversation_read/list_conversation_attachments/profile_llm_usage — orphan-try edit 수정) + cat-A 잔여(~75)
...
## 8. Completion Checklist (DI seam Phase 2 batch 1 = TASK-0012-7 cat-A 19)
- [x] cat-A 19 핸들러를 `account=Depends(get_current_account), conn=Depends(get_conn)` 로 byte-동치 전환(부록 원자단위 ①②③④)
- [x] behavior-neutral 검증 — make test progress-chars 1238(baseline 동일)·0 fail, route-parity 179 불변, ruff clean, py_compile OK, `_require_account` 118→99
- [x] §18.8 적대 패널 실행(REV-20260629-0006 AGENT-TEAM, ultracode workflow 72 agents) — 24 분석/21 확정(NONE)/3 보류(orphan-try edit, batch 2 이월)
- [x] 동반 테스트 영향 분석 — getsource/AST 도메인-문자열 단언이라 전환 불요(직접호출 1건 profile_usage_conversations 는 batch 2 이월)
- [x] TASK/REPORT/MODIFY/REVIEW 에 상태가 반영되었다
- [x] BLOCKED 없음
- [ ] verify-completion PASS(9 checks) → Git 커밋 (Phase 2 batch 1)
- [ ] Git 원격 동기화: ai/* push (§16.3). PR/머지는 milestone 까지 보류(Phase 2 잔여 + 3..7 + Final 브라우저 QA 게이트 전).
