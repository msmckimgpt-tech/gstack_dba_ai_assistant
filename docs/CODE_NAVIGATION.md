...
> **대상 코드**: `unit/feature-0003-agent-web-ui/src/` — app.py(19,650→**3,722줄**, -81%)의 핸들러 전량이 `routers/` 29개 파일로 추출됨(24 route-module `@router` + 4 언더스코어 공유모듈 + `__init__` registrar — feature-0026 이 `admin_perf.py`(HTTP 성능 스냅샷, INCLUDE_ORDER=250)·leaf 계측 `src/perf_metrics.py` 추가; feature-0014 가 leaf `src/static_cache.py` — `/static` mount 를 감싸 "요청 `?v=` == 빌드 스탬프" 일 때만 immutable 을 부여하는 ASGI 래퍼 — 추가). leaf helper 는 `src/web_context.py`.
