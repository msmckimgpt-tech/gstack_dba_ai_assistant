...
같은 파일·모듈을 편집하는 작업은 착수 전에 TASK/REPORT에 `hot_paths`와 작업 범위를 적고
다른 활성 작업의 선언·실제 diff와 대조한다. 같은 핫스팟의 편집 동시진행 기본값은 2개다.
세 번째 작업자는 충돌 구간 편집을 대기하며 조사·검증 또는 독립 경로 작업을 자율 수행한다.
경로를 나눠 충돌을 제거할 수 있으면 범위·소유권을 기록하고 진행한다. 상한 변경도 근거와
프로젝트 합의 범위 내에서 판단하며 매번 사용자에게 재확인하지 않는다.
머지는 같은 핫스팟에서 한 번에 한 브랜치씩 한다. 첫 브랜치 랜딩 후 나머지는 최신 정본에
rebase/merge하고 양쪽 변경을 검증한다. cycle은 24시간 내 랜딩을 목표로 나누고 초과가
예상되면 더 작은 단위로 분할한다. 24시간을 넘는 대기는 원인과 다음 실행 조건을 남긴다.
편집 예정 주요 경로 1~5개와 순서는 자기 TASK 블록에 적고 다른 작업자의 블록은 변경하지 않는다.
존재하지 않는 `--hot-paths` 옵션이나 필수 REGISTRY 파일을 가정하지 않는다. §13.2.2 F2의
공유 경로 금지는 그대로 적용하며, 이 절을 그 경로의 편집 허가로 쓰지 않는다.
<!-- /inbox-autonomy:hotpaths:v1 -->
...
<!-- inbox-autonomy:scratch-cwd:v1 -->
**임시 사본에서 복귀한 뒤 첫 편집 전 확인 (v3.54.0)**: scratch·sandbox 실험의 `cd`는 해당
셀 안에서 끝낸다. 이후 첫 mutation 전에 편집 대상 realpath, `git rev-parse --show-toplevel`,
현재 branch를 TASK의 실제 worktree와 대조한다. 사본 테스트는 사본의 증거이며, 원본 worktree에
수정이 반영됐는지와 그 상태에서의 필요한 검증을 별도로 확인한다.
<!-- /inbox-autonomy:scratch-cwd:v1 -->
...
<!-- inbox-autonomy:resume-arrival:v1 -->
**명시 활성 세션의 재개 도달 확인 (v3.54.0)**: §22.12의 활성 등록·재개 권한·정본 상태 검사를
통과한 재개만 대상으로 한다. 재개 입력을 보낸 사실과 assistant 작업 이벤트가 실제 발생한
사실을 구분해 TASK/REPORT에 기록한다. “No response requested.” 같은 무작업 응답만이면
도달 실패로 기록한다. 프로젝트의 응답 관측 임계(미정이면 60초) 안에 작업 이벤트가 없으면
해당 경로의 실패로 남기고 같은 입력을 반복 주입하지 않는다. 독립적으로 진행 가능한 작업은
계속한다. 재시도는 실패 원인 수정과 현재 등록·정본의 재검증이 있는 경우에만 한다.
이 조항은 관측자에게 다른 세션의 재개·종료 권한을 새로 주지 않는다.
<!-- /inbox-autonomy:resume-arrival:v1 -->
...
<!-- inbox-autonomy:policy-refresh:v1 -->
**Cycle 경계 정책 재독 (v3.54.0)**: 진입과 각 cycle 시작·재개·완료 검증 전에 적용되는 정책
문서의 실제 경로와 내용 SHA-256을 TASK/REPORT에 기록·대조한다. `sha256sum AGENTS.md`
또는 동등한 내용 해시를 쓰며 `git log -1`만으로 대체하지 않는다(미커밋 정책을 놓친다).
해시가 바뀌면 해당 문서의 변경 구간과 참조 규정을 읽고 계획·검증에 반영한다. linked worktree는
현재 작업에 적용되는 정책과 공유 main의 새 정책을 함께 확인하되, 다른 경로의 정책을 조용히
정본으로 바꾸지 않는다. 권한·방향 충돌이 없다면 이미 승인된 작업을 계속한다.
<!-- /inbox-autonomy:policy-refresh:v1 -->
...
**사용자향 화면 사전 설계 (v3.54.0)**: 신규 화면이나 주요 사용자 흐름을 바꿀 때 구현 전에
핵심 안과 판단 근거를 짧게 제시한다. 사용자가 설계 선택까지 위임했거나 기존 승인 디자인이
있으면 그 범위에서 작업자가 선택해 구현한다. 목표·주요 동작에 실질적인 선택이 남을 때만
사용자에게 묻는다. 단순 문구·간격·버그 수정에 새 승인 단계를 추가하지 않는다.
Claude Code `/design`(v2.1.234부터 research preview)은 선택 수단이다. 가용하지 않으면
로컬 mockup/HTML/기존 디자인 도구를 사용한다. 외부 canvas 공개는 기존 외부 공개 권한을
...
- (d) **검증 입력의 자기-출처 한정** — 검증에 투입하는 자원(파일·레코드·첨부)은 **이 세션이 직접
  ...
  투입하지 않는다.
<!-- inbox-autonomy:upstream-stimulus:v1 -->
**상류 계약을 검증하는 입력의 출처 (v3.54.0)**: URL·payload·파일·이벤트의 전달 계약은
상류 시스템이 실제 생성한 결과를 캡처해 하류에 넣고 검증한다. AI가 직접 조립한 정상 입력은
보조 대조군으로 쓸 수 있지만 실제 연동 성공의 유일한 증거가 될 수 없다. 앞의 자기-출처 규칙은
타 계정의 자산을 임의로 시험하지 않는 권한·자산 소유 규칙이고, 이 규칙은 상류 생성 경로의
정확성을 검증하는 규칙이다. 허가된 테스트 자산으로 실제 상류 출력을 확보한다.
<!-- /inbox-autonomy:upstream-stimulus:v1 -->
...
<!-- inbox-autonomy:negative-control:v1 -->
**G7 보강 — 부재·0건의 측정 도달 확인 (v3.54.0)**: “없음/0건/전부 정상” 결론 전 대상 접근과
도구 실행 성공을 확인하고, 같은 경로로 알려진 양성 표본 1건을 검출하는 대조를 수행한다.
실제 표본이 없으면 통제된 fixture를 사용하고 적용 범위를 명시한다. 검색·테스트 수집 실패,
권한 거부, 필터 오적용을 결과 0으로 읽지 않는다. 양성 대조가 실패하면 결론은 미확인이다.
<!-- /inbox-autonomy:negative-control:v1 -->
...
<!-- inbox-autonomy:diagnosis-common:v1 -->
**진단의 공통 원인과 사용자 실패 처리 (v3.54.0)**: 여러 증상을 독립적인 권고로 나누기 전에
공통 원인 가설을 세우고 이를 반증할 실제 probe를 수행한다. 공통 원인이 아니라고 판단했으면
그 근거를 기록한다. 증상·권고 숫자만 맞춰 완료하지 않는다.
상류 API·네트워크·하네스 장애는 원인 귀속이고, 우리 서비스가 실패를 알릴 책임과 별개다.
허가된 환경에서 상류 실패를 재현하고, 제품의 명시된 timeout/SLO 안에 우리 화면/API가 원인과
가능한 다음 행동을 전달하는지 검증한다. 기준이 없으면 작업 범위에 맞는 제한시간을 정해
기록한다. 상류 원인이라는 설명만으로 사용자에게 남는 무응답을 완료 처리하지 않는다.
<!-- /inbox-autonomy:diagnosis-common:v1 -->
...
<!-- inbox-autonomy:verification-debt:v1 -->
**일시 불가로 남은 검증의 추적 (v3.54.0)**: 위 기준으로 채널 불가를 선언했더라도 원인이
quota·rate limit·일시 네트워크 장애면 REVIEW/TASK에 `verification_debt`를 남긴다:
채널, 미검증 범위, 원인, `retry_after`(알면 시각, 모르면 unknown), 검증 대상 commit/content hash,
대체 검증 결과, 후속 담당 작업을 기록한다. `[SKIPPED:*]`는 통과한 검증이라는 뜻이 아니다.
다음 명시적 재개 또는 cycle 경계에서 복구 여부를 확인해 미검증 범위를 자율 재검증한다.
해소 시 실제 검증한 revision과 결과를 연결하고 닫는다. 현재 정책이 허용하는 대체 검증으로
작업을 계속할 수 있지만 미검증 사실을 숨기거나 검증부채만을 위한 자기 재호출 예약은 하지 않는다.
<!-- /inbox-autonomy:verification-debt:v1 -->
...
<!-- inbox-autonomy:budget-bounds:v1 -->
**무인 위임 비용·동시성 경계 (v3.54.0)**: Claude Code v2.1.217의
`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` 기본 동시성은 20이다. `--max-budget-usd`는
지원되는 비대화 실행에서 예산에 도달하면 새 spawn뿐 아니라 실행 중 background agent도
멈출 수 있으므로 checkpoint·부분 결과 회수를 함께 설계한다. 실제 CLI help·버전에서 지원을
확인하고 사용자가 정한 비용·동시성 범위 내에서 값을 선택한다. 예시 기본값을 프로젝트 예산으로
강제하지 않는다. §22.12 무인 recipe에서도 같은 경계를 적용한다.
공식 근거: https://raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md
<!-- /inbox-autonomy:budget-bounds:v1 -->
...
<!-- inbox-autonomy:deny-surfaces:v1 -->
**deny의 집행 표면 확인 (v3.54.0)**: 결정적이라는 설명은 해당 도구·해당 버전에서 실제 집행되는
표면에 한정한다. 공식 v2.1.260은 v2.1.259의 Bash 인자 대상 Read deny 확대를 되돌렸으므로
한 가지 최소 버전으로 모든 경유 경로의 차단을 보장하지 않는다. 허가된 격리 환경에 무해한
표식 파일을 두고 Read 도구, Bash 리다이렉트, Bash 인자 경로를 각각 대조한다. 실제 비밀을
시험 입력으로 쓰지 않는다. 이미 금지된 경로를 이 검사 명목으로 우회하지 않고, 독립 테스트
설정에서 허용된 canary로 집행 여부만 확인한다. 미검증 표면은 보장하지 않는다.
공식 근거: https://raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md
<!-- /inbox-autonomy:deny-surfaces:v1 -->
...
<!-- inbox-autonomy:native-resume:v1 -->
**하네스 네이티브 자동 재개 예외 — 명시 활성 세션만 (v3.54.0)**: 위 stale 종료 규칙의
예외는 사용자가 재개를 허용한 특정 세션·작업이 현재도 활성인 경우다. `Continue automatically
at usage limit`는 Claude Code v2.1.234부터의 기능이며, 켜진 설정만으로 재개 권한을 추론하지 않는다.
TASK/REPORT에 `auto_resume` 등록(세션 ID, 작업 ID, worktree/branch, 허용한 사용자 지시,
active 상태, 마지막 checkpoint)을 남기고 재개 시 현재 정본과 대조한다. 기존 사용자 지시가
이 범위를 충족하면 작업자가 기록하며 동일한 승인을 다시 묻지 않는다.
완료·취소·inactive·승인 대기·정본 불일치이면 작업을 재실행하지 않고 현 상태만 보고한다.
명시 등록이 없거나 네이티브 이벤트임을 확인할 수 없으면 기존 stale 규칙을 따른다. 오래 지연된
self-scheduled wakeup·자기 재호출 예약은 이 예외에 포함되지 않는다. 다른 세션을 임의 활성화하거나
재개하지 않는다. 적격 재개 후에는 §13.2.11의 도달 확인과 checkpoint 기반 중복 방지를 적용한다.
이 문서 변경은 기존 계정의 auto-continue 설정을 일괄 켜는 작업이 아니다.
<!-- /inbox-autonomy:native-resume:v1 -->
