...
- CHG-20260701-0009: **전략 전환 + batch1 6 도메인 35 핸들러 전체추출**(profile·integrations·attachments·admin_audits·admin_accounts·admin_roles). 핵심발견: 핸들러 추출은 DI 전환 불요(monkeypatch=`app.X` 동적참조 보존) → DEFER 도 인라인 유지 byte-identical 추출. 범용 추출기(완전 mod_syms·동적 import·self-healing)+reref(커플링 6유형: 직접호출·monkeypatch·getsource·소스텍스트·hasattr문자열·app.routes재귀). **app.py 26,734→24,648(~2,086↓)**, 12→18 router, make test 1313·0 fail, route-parity 192.
- CHG-20260701-0010: **batch2 datasources 6 + auth 18 전체추출** → routers/{admin_datasources,auth}.py. datasources CLEAN-DI 3·auth PUBLIC 7·DEFER 인라인 유지. 커플링 7유형(신규: source-text 호출식 count — auth_me 필터 이동 적발). **app.py 24,648→23,536(~1,112↓)**, 20 router, make test 1313·0 fail.
...
- CHG-20260702-0013: **batch4 bare-name 회귀 수정(라이브 500 3종)** — 추출된 핸들러가 app-모듈 전역/import 심볼을 `app.` 로 한정 안 해 `NameError`→500. 6심볼/10개소 `app.` 한정: admin_console(`MEMORY_DB`)·admin_quotas(`LLM_QUOTA_ENFORCE`)·conversations(`load_memory_kv`×5·`mark_cancel_requested`·`set_run_status`·`mark_finalize_requested`). graph-panel-perms 배포 후 라이브 검증 중 발견 → `ruff --select F821` 전수 감사로 6심볼 확정. ruff F821 clean·py_compile·45 테스트 PASS·§18.8 SUBAGENT 패널 PASS(4축). 배포 후 500→200. **"byte-neutral" 게이트가 미커버 핸들러 NameError 를 놓침 — 추출 게이트에 F821 추가 권장.**
- CHG-20260702-0014: **route-parity 골든 stale 해소(CI red 192→193)** — feature-0016 #514(`24445e94`)가 `GET /api/admin/metadata/graph/analyze/status` route 를 추가하며 골든 미갱신 → 실 193 vs 골든 192 상시 CI red. `_build_table()` 재생성으로 골든 갱신(정밀 diff 로 +1 legit route 확정, 제거·중복 0). `test_route_parity_p5b` PASS. **test fixture 전용 — runtime 무변경(web 이미지 tests/ 미포함).** route 추가 feature 의 골든 동반갱신 책임 학습.
- CHG-20260710-0015: **세션 재개 — origin/main 359-커밋 동기화** + 추출 건전성 F821 감사(전 23 routers `All checks passed` — 타 워커 bare-name 회귀 수정 + 신규 admin_settings/ai_ops 포함 clean) + 잔여 workstream 준비. 코드 clean 병합(app.py @app 0 유지), docs 3 union 해소. 프론트 분할 blocked(admin.js 30일 237 커밋). 잔여 3 test 실패=feature-0018/dbanalysis main-상속(범위 밖).
- 총 변경 횟수: 39
