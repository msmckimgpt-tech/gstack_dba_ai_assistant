...
- In Progress: gc-unread-read-500-fix — 서버 import 수정+회귀 테스트+docs 완료 → verify-completion → PR → 라이브 배포(web 재빌드, deploy_scope included) → web 로그 read 500→200 + 고착 커서 전진 실증. (composer-clear-input-on-send·composer-nonblock-interrupt·gc-unread-read-fix 등 직전 머지분은 라이브/배포대기.)
...
## 3. Recent Changes
- (최신) gc-unread-read-500-fix: 읽음 API `POST /read` 가 web 에 없는 `modules.db` 를 import → 매 호출 500(best-effort 삼킴) → 서버 읽음 커서 미전진 → 새로고침 시 unread 배지 복원. `modules.db`→`shared.db`(다른 9곳과 정합) 1줄 + 핸들러 회귀 테스트 신설. 이 silent 500 이 unread 수정 3건(badge/baseline/read-fix)을 무력화하던 진짜 근본 원인. (CHG-20260625T202817)
