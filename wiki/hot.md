...
## Recent Changes
- 덱 상호작용 발견성: **Ctrl 키 누르면 클릭 가능 항목 주위 glow 가 천천히 fade-in/out**(outline 제거, `@keyframes findglow` `filter:drop-shadow` alpha 0↔.55, prefers-reduced-motion 정적 폴백, keyup/blur 해제) + **팝업 열린 채 다른 항목 클릭 시 교체**(`.pop{pointer-events:none}`/`.pop-card{auto}` 오버레이 통과, 빈 곳 클릭=팝업만 닫힘·페이지 유지).
