...
새 대화 시작 시 같은 계정의 과거 대화 맥락이 통째로 사라지는 문제를 완화한다 — 특정 명칭이 아니라 문맥/인사이트를 어렴풋이 이월. 설계 정본 `docs/DESIGN-account-insight-recall.md`. outside-voice 2명 검증(REV-20260617T082131).
- **회상 모집단 = 계정 소유 대화**: `modules/account_recall._load_account_scoped_conv_ids(account_id, exclude_conversation_id, limit)` 가 `agent_runtime.core_conversations.owner_account_id`(idx `ix_core_conv_owner`)로 계정 소유 과거 대화 conv_id 를 최근순 도출. 가드 G2: `owner_account_id` 일치 + NOT NULL + `archived_at IS NULL` + 현재 대화 제외 + `__global__`/세션샤드 sentinel 제외(SQL+Python 이중). `_pg_connect_ro`(agent_kb_ro least-priv), schema-qualified(ADR-0027), 예외/PG미가용 fail-soft `[]`.
- **의미 회상 재사용**: `recall_account_conv_facts(account_id, query_text, exclude_conversation_id)` 가 도출 conv_ids 를 기존 `kb_retrieval._load_rag_documents_for_request_pg`(pgvector `<=>` cosine, 1536/text-embedding-3-small) 에 주입 + `MIN_SIM` 임계·`TOP_K` 로 노이즈 억제. `RECALL` flag OFF=no-op.
- **주입 지점**: `_build_knowledge_context(..., account_id, conversation_id)` 가 회상 결과를 "CONTEXT FROM YOUR PAST CONVERSATIONS (reference only)" 블록으로 주입 — **`INJECT` flag ON 일 때만**. OFF=shadow(회상만 로깅).
- **flag(전부 default OFF)**: `AGENT_ACCOUNT_INSIGHT_RECALL`/`INJECT`/`MAX_CONVS`(20)/`TOP_K`(3)/`MIN_SIM`(0.75). 두 flag OFF → 기존 동작 0 변경.
- **본 Phase 범위 밖(INJECT 활성 전 선행 필수)**: G1(fork 대화 회상 제외 — fork 는 타 계정 메시지를 복사+owner 재귀속하므로 owner 격리만으론 누출), G3(값-패턴 PII 마스커 또는 PII-free 구조화 인사이트만 — `_mask_rows` 가 prose 미적용). Phase 2 = insight worker 의 PII-free 구조화 인사이트 추출 pass.
