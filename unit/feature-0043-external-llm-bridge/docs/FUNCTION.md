...
- 결과 CSV 다운로드. 외부 표면은 서버 CSV 경로를 응답에서 제거한다(경로 유출 + 외부 호출자에겐
  없는 다운로드 약속).
- ~~답변의 실행 단계(`AI 추론` 탭)~~ → **복원됨**(P0-N · P0-T). 「서버 run 이 없으니 단계도
...
> **후속 정정 (P0-T)**: 이 절의 "사유는 비워 둔다" 는 그대로 두면 화면에 `SQL을 실행한다` 만
...
### P0-T. 대화 제목과 단계 사유 — **빠진 두 축** (사용자 제보 2026-08-27)
...
- AC-20260828T040000-external-llm-bridge-53: 브리지로 질문을 보내면 **그 즉시** 대화 제목이
  질문 앞머리로 바뀐다(사이드바가 "새 대화" 로 남지 않는다).
- AC-20260828T040000-external-llm-bridge-54: 개인 AI 가 `submit_answer` 에 `title` 을 보내면
  제목이 맥락 요약으로 갱신되고, **사용자가 손으로 바꾼 제목은 갱신되지 않는다**.
- AC-20260828T040000-external-llm-bridge-55: 실행 단계에 「사유(왜) + 작업(무엇)」이 함께
  표시되며, 사유의 출처(`external-ai` / `derived`)가 구분되어 기록된다.
- AC-20260828T040000-external-llm-bridge-56: MCP 어댑터 2종의 조사 도구 9종이 `reason` 인자를
  받아 서버로 전달하고, 도구 설명이 사유 제공을 요구한다.
- AC-20260828T040000-external-llm-bridge-57: 호출 시점 단계가 기록된 task 는 제출 시 원장
