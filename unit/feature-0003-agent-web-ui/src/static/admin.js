...
function switchTab(tabName) {
  ...
  // TASK-0108 Sprint 3: KB 등록 tab 첫 진입 시 list fetch + handler bind.
  if (tabName === "kb-ingest" && !adminState.kbIngest.initialized) {
    adminState.kbIngest.initialized = true;
    mountKbIngestPane();
  }
}
 ...
 * BRIEFING-attachment-multi-cycle.md §6.3 정합. text/markdown/.sql 첨부 본문
 * 을 AgentMemoryFactEntries 의 manual fact 로 등록한다. RBAC: backend gate
 * 가 `attachment.kb.write.any` (admin only). frontend 는 표시 자체는 허용
 * (TASK-0098 패턴 — 표시 + backend 403 fallback). */
...
function mountKbIngestPane() {
  const refreshBtn = $("kbIngestRefreshBtn");
  if (refreshBtn && refreshBtn.dataset.bound !== "1") {
    refreshBtn.dataset.bound = "1";
    refreshBtn.addEventListener("click", () => loadKbIngestList());
  }
  const submitBtn = $("kbIngestSubmitBtn");
  if (submitBtn && submitBtn.dataset.bound !== "1") {
    submitBtn.dataset.bound = "1";
    submitBtn.addEventListener("click", () => submitKbIngest());
  }
  const cancelBtn = $("kbIngestCancelBtn");
  if (cancelBtn && cancelBtn.dataset.bound !== "1") {
    cancelBtn.dataset.bound = "1";
    cancelBtn.addEventListener("click", () => {
      adminState.kbIngest.selectedId = null;
      renderKbIngestDetail();
    });
  }
  const list = $("kbIngestList");
  if (list && list.dataset.bound !== "1") {
    list.dataset.bound = "1";
    list.addEventListener("click", (ev) => {
      const btn = ev.target.closest("[data-kb-ingest-attachment-id]");
      if (!btn) return;
      const id = Number(btn.getAttribute("data-kb-ingest-attachment-id") || 0);
      if (id > 0) {
        adminState.kbIngest.selectedId = id;
        renderKbIngestList();
        renderKbIngestDetail();
      }
    });
...
async function loadKbIngestList() {
  const list = $("kbIngestList");
  const empty = $("kbIngestListEmpty");
  const count = $("kbIngestListCount");
  if (empty) empty.textContent = "불러오는 중…";
  try {
    const res = await fetch("/api/admin/attachments?kind=text,markdown&limit=200", {
      credentials: "include",
    });
    if (!res.ok) {
      const msg = res.status === 403
        ? "권한이 없습니다 (attachment.kb.write.any 필요)."
        : `목록 조회 실패 (HTTP ${res.status}).`;
      if (empty) empty.textContent = msg;
      if (count) count.textContent = "—";
      adminState.kbIngest.items = [];
      return;
    }
    const body = await res.json();
    const items = Array.isArray(body && body.items) ? body.items : [];
    // .sql ext 도 포함 — backend 가 kind=text,markdown 필터만 적용했으므로 client 에서
    // 추가로 ext=='sql' 인 항목 합쳐 보여줘야 한다. 본 cycle 은 admin 의 typical workflow
    // (kind 기반 list) 만 충당 — .sql 첨부는 같은 endpoint 호출에 kind 인자 무시 시 결합
    // 또는 별 호출. 단순화: kind 필터 + client 에서 ext='sql' 첨부도 별 호출.
    let extra = [];
    try {
      const res2 = await fetch("/api/admin/attachments?limit=200", { credentials: "include" });
      if (res2.ok) {
        const body2 = await res2.json();
        extra = (Array.isArray(body2 && body2.items) ? body2.items : []).filter(
          (it) => it && (it.ext === "sql"),
        );
      }
    } catch (_) { /* fail-soft — 1차 list 만으로도 사용 가능 */ }
    ...
    adminState.kbIngest.items = merged;
    renderKbIngestList();
  } catch (err) {
    if (empty) empty.textContent = `오류: ${err && err.message ? err.message : err}`;
    if (count) count.textContent = "—";
    adminState.kbIngest.items = [];
  }
}

function renderKbIngestList() {
  const list = $("kbIngestList");
  const empty = $("kbIngestListEmpty");
  const count = $("kbIngestListCount");
  if (!list) return;
  ...
  items.forEach((it) => {
    ...
    btn.setAttribute("data-kb-ingest-attachment-id", String(it.id));
    if (Number(adminState.kbIngest.selectedId) === Number(it.id)) {
      btn.classList.add("is-active");
    ...
    const name = document.createElement("span");
    name.className = "admin-list-row-name";
    name.textContent = it.original_filename || `attachment-${it.id}`;
    title.appendChild(name);
    main.appendChild(title);
    const meta = document.createElement("span");
    meta.className = "admin-list-row-meta";
    const sizeKb = ((Number(it.size_bytes) || 0) / 1024).toFixed(1);
    meta.textContent = `kind=${it.kind || "—"} · ${sizeKb} KB · ${(it.created_at || "").slice(0, 19).replace("T", " ")}`;
...
function renderKbIngestDetail() {
  const detailEmpty = $("kbIngestDetailEmpty");
  const panel = document.querySelector('[data-kb-ingest-panel="form"]');
  const selectedId = Number(adminState.kbIngest.selectedId || 0);
  if (!panel) return;
  if (!selectedId) {
    panel.hidden = true;
  ...
  const it = (adminState.kbIngest.items || []).find((x) => Number(x.id) === selectedId);
  if (!it) {
    panel.hidden = true;
    if (detailEmpty) detailEmpty.style.display = "";
    return;
  ...
  panel.hidden = false;
  const fname = $("kbIngestSelectedFilename");
  const meta = $("kbIngestSelectedMeta");
  const submit = $("kbIngestSubmitBtn");
  const result = $("kbIngestResult");
  if (fname) fname.textContent = it.original_filename || `attachment-${it.id}`;
  if (meta) {
    const sizeKb = ((Number(it.size_bytes) || 0) / 1024).toFixed(1);
    meta.textContent = `attachment_id=${it.id} · kind=${it.kind || "—"} · size=${sizeKb} KB · sha256=${(it.sha256_prefix || "").slice(0, 8)}… · conv=${(it.conversation_id || "").slice(0, 24)}`;
  }
...
// server 의 `^[A-Za-z0-9_.-]{1,96}$` 와 동일 — UX 즉시 피드백.
const KB_INGEST_SCOPE_KEY_RE = /^[A-Za-z0-9_.\-]{1,96}$/;

async function submitKbIngest() {
  if (adminState.kbIngest.submitting) return;
  const selectedId = Number(adminState.kbIngest.selectedId || 0);
  if (!selectedId) return;
  const scopeKeyEl = $("kbIngestScopeKey");
  const result = $("kbIngestResult");
  const submit = $("kbIngestSubmitBtn");
  const scopeKey = (scopeKeyEl && scopeKeyEl.value || "").trim();
  if (!scopeKey) {
    if (result) result.textContent = "ScopeKey 를 입력하세요.";
    return;
  }
  if (!KB_INGEST_SCOPE_KEY_RE.test(scopeKey)) {
    if (result) result.textContent = "ScopeKey 형식 오류 — 영숫자 / 점(.) / 언더스코어(_) / 하이픈(-) 만, 길이 1~96자.";
    return;
  }
  adminState.kbIngest.submitting = true;
  if (submit) submit.disabled = true;
  if (result) result.textContent = "ingest 진행 중…";
  try {
    // REV-20260526-0002 B-2 흡수: source_type / weight 는 서버 상수 — body 에 안 보냄.
    const res = await fetch("/api/admin/attachments/kb-ingest", {
      method: "POST",
    ...
    if (!res.ok) {
      ...
      if (result) result.textContent = `등록 실패: ${msg}`;
      showToast(`KB 등록 실패: ${msg}`, true);
      return;
    }
    const supersededHint = body.reactivated
      ? "(동일 본문 reactivate)"
      : (Number(body.superseded_count || 0) > 0
        ? `(기존 ${body.superseded_count} 건 supersede)`
        : "(신규 등록)");
    if (result) {
      result.textContent = `등록 완료 · fact_entry_id=${body.fact_entry_id} · text_hash=${String(body.text_hash || "").slice(0, 12)}… ${supersededHint}`;
    }
    showToast(`KB 등록 완료: ${scopeKey} ${supersededHint}`, false);
  } catch (err) {
    if (result) result.textContent = `오류: ${err && err.message ? err.message : err}`;
    showToast(`KB 등록 오류: ${err && err.message ? err.message : err}`, true);
  } finally {
    adminState.kbIngest.submitting = false;
    if (submit) submit.disabled = false;
  }
}
