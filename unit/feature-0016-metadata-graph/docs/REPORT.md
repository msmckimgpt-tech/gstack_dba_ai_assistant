...
## 2026-07-28 · graph-hdr-label-fit — 위계 헤더 라벨을 클러스터 범위만큼 확장 (사용자 요청 · 웹 리서치 동반)
label-lod 배포 확인 직후의 후속 요청: "컨텐츠 카테고리 클러스터 텍스트는 줌 아웃 시에도 상대적으로 명확하게 보이게 **클러스터 범위만큼 텍스트 크기가 확장되는 방안**을 검토" + "**웹 리서치**로 줌아웃 시 상위 노드를 더 잘 표현할 수단 검토".
- **진단**: `group-hd`(컨텐츠 카테고리 헤더)는 폰트 **10.5 고정**이라 칩 폭만 박스에 맞춰 늘고 폰트는 그대로 → 박스가 248px 든 920px 든 동일하게 zoom 0.305 에서 억제됐다. **박스 크기라는 정보가 이미 있는데 폰트가 쓰지 않았다.** 위계 한 단계 위 `cat-hd`(12 고정)도 동형(0.2667).
- **해법**: 폰트 = `clamp(base, min(base/zoom, 박스fit), MAX)`. `base/zoom` 이 화면 크기를 유지하고(지도 라벨 관례·§85 선례), `박스fit` 이 라벨을 자기 범위 안에 묶어 헤더 충돌 폭발을 구조적으로 막는다(지도가 라벨 충돌 컬링으로 푸는 문제를 박스 상한이 그 복잡도 없이 해결). `MAX` 는 **줌 하한에서 파생**(3.2/0.05 = 64) — 리터럴 26 은 실측 개요 줌 0.2524 에서 이미 감쇠해 요구 미달.
- **칩 기하(사용자 결정)**: 폰트에 비례해 커지되 **하단 앵커**로 박스 위로만 자란다 → 예약 헤더 행 아래 멤버 영역 무침범 = **reflow 0**(좌표가 바뀌는 것은 헤더 칩 자신뿐).
- **stale 방지**: 폰트가 `base/z` 로 연속 변하는데 rebuild 는 밴드 전이에서만 걸리므로, 역보정 배율을 25% 승법 스텝으로 양자화해 밴드에 합류시켰다(§85 와 동일 허용 오차). `base/z > MAX` 부터는 폰트가 z-무관이라 **스텝 상한 클램프** — 없이 넣었을 때 기존 F3·F7 '무의미 rebuild 차단'이 실제로 깨졌고, 그 실측을 근거로 도입했다.
- **개선 실측**: `group-hd` 억제 시작 **0.3048 → 1열 0.1416 / 2열 0.0642 / 3열 이상 0.0500(줌 하한 = 사실상 안 사라짐)**. 실측 "전체 조망" 줌 0.2524 화면 크기 **2.65px → 2열 이상 10.5px(base 그대로)**. `cat-hd` 는 0.2667 → 0.1405~0.0500, 3.03px → 12.0px.
- **리서치 결론(비채택 2건 근거 보존)**: ① **SDF/MSDF 폰트** — "any size 에서 crisp" 로 근본 해법처럼 보이지만 PixiJS v8 공식 문서가 **CJK 대형 문자셋은 텍스처 메모리 제약으로 비현실적**이라 명시(§79 T79.5 와 같은 벽) → 비채택 확정. ② **극단 줌아웃 metanode 강등** — 리서치가 지지하는 정석이나 **§67 에서 사용자 피드백("집계=규모 파악 어려움")으로 이미 폐기**된 방향이라 재논의 대상으로 분리.
- **§18.8 적대 검증(codex-review) — P1 1건 · P2 1건 in-cycle 흡수**: ① **P1(GATE)** — 칩을 폰트에 무제한 비례시키면 폰트 64 에서 높이 ~110 world 가 되고 하단 고정이라 **위 그룹 블록 안으로 72 world 침범**한다(행 간격 GGY=16). GH/CATH 는 테이블 칩보다 z 가 높은 드래그 핸들이고 hit-test 가 `style.size` bbox 로 판정하므로 **위 그룹 테이블의 클릭을 가로챈다** — memory 에 2회 재발로 기록된 z-order↔hit-test 결함 계열의 재발이다. **"reflow 0 이니 팔출은 안전" 이라는 내 판단이 불완전했다**(자기 멤버 영역만 보고 이웃 블록·hit 영역을 안 봤다). 수정 = **hit 영역(칩)만 묶고 텍스트는 자유** — 라벨이 hit 대상이 아니라는 사실이 이 분리를 가능하게 한다. ② **P2** — `/h` 밴드가 헤더 미방출 경로(products 조기 return)에서도 헛 rebuild 를 유발 → `fit > base` 헤더 계수 게이트(z-독립 — self-lock 회피). 상한 제거판으로 신규 K2·K3·K7 이 **실제로 FAIL** 함을 실측했다.
- 검증: `test_g6build_labellod.js` **36 → 71 PASS**(신규 35 — 유닛 12·결합 11·리뷰흡수 10·B5 재진술 2) + 헤드리스 **전 스위트 21개 910 PASS / 0 FAIL** · `node --check` PASS · REV-20260728T181000. PB-0008 라이브는 HF.6(배포 후).
...
## 2026-07-29 · detail-panel-typo — 상세 패널이 "시각적으로 불편"했던 원인과 교정 (사용자 리포트)

**요청**: "디자인적으로 모범적이진 않은것처럼, 시각적으로 불편하게 느껴지는데 원인을 파악하고 개선."

**원인 (한 줄)**: 목록 행의 **주 라벨에 부-액션 버튼 스타일(`.amgr-link`)을 재사용**했다 — `모두 펼치기`·`관계 상세`
같은 보조 버튼과 같은 테두리 10.5px 칩을 목록의 주인공에 입힌 것. 여기서 네 증상이 파생됐다: ① 부가정보(참조 컬럼)에
크기 규칙이 없어 base 13px 를 상속 → **주 라벨보다 24% 큰 위계 역전** ② 칩 폭이 내용대로 벌어져 **우측 경계 18종
톱니** ③ 컬럼 목록 줄바꿈 시 **행 높이 19px↔38px** ④ `h4` 가 본문 항목과 같은 색이고 `h4` 내 muted 가 bold 를
상속해 제목/항목/부가정보 구분이 사실상 없었다.

**처리**: 같은 패널의 컬럼 섹션이 이미 쓰던 검증된 관용구(전폭 무테두리 행 + hover 채움 + 12px)로 시각 언어를 통일.
행을 **하나의 전폭 flex 버튼**으로 만들어 부가정보 영역까지 클릭·hover 타깃이 되게 하고, 주 라벨 12px/본문 ink ·
부가정보 10.5px/우측 정렬/폭 상한 38%/말줄임으로 위계와 정렬을 고정했다.

**효과(실렌더 정량)**: 행 높이 2종→**1종(25px)** · 행 폭 가변→**295px 균일** · 우측 경계 18종→**1종** ·
부가정보/주라벨 크기비 1.24배→**0.88배**. 말줄임 전문은 이름·컬럼 양쪽 `title` 에 보존(11/11).

**검증**: 신설 `test_detail_panel_typo.js` **24 PASS**(수정 전 CSS 로 **17 FAIL** = 반증 확인) · 헤드리스 그래프 전
스위트 **996 PASS/0 FAIL** · pytest **2835 passed/0 failed** · ruff clean. PB-0008 은 배포 후 POST-DEPLOY.

### §8 개선 제안 (기록만)
- **툴바 위계**: `보기 옵션`/`초기화`/`상세 ⇆`/`❓ 도움말` 4개가 동일 outline 버튼이라 주·부 구분이 없고 `❓` 이모지만
  튄다. 우하단 **미니맵이 거의 빈 큰 사각형**으로 시각적 무게만 차지한다. 이번 범위(상세 패널) 밖이라 기록만 한다.
- **행 밀도**: 57행 1열 유지(이름 최대 236px 로 2열 불성립). 밀도 개선은 그룹 접기 기본값 또는 가상 스크롤 검토 필요.

### POST-DEPLOY 확인 (2026-07-29, detail-panel-typo)
PR #1033 → main `36618965` → 롤링 배포(soak PASS) 후 실 Windows Chrome 150 실측: 주 라벨/부가정보 크기비 **0.88배**
(종전 1.24배 역전) · 행 높이 **25px 단일**(종전 19~38px) · 우측 경계 **1종**(종전 18종 톱니) · `h4` 본문 ink ·
`h4` 내 muted 500. 신 코드 실행은 DOM 증거로 선확정(`li.amgr-rtli` 57 · 구 칩 0). 적대검증이 잡은 부수 피해도
관계 상세 뷰 카드 57개 간격 3px 균일로 **회귀 0**. 상세: feature-0003 `docs/test-runs.d/20260729T1113-detail-panel-typo-postdeploy.md`.

## 2026-07-29 · graph-hdr-label-typo — 위계 헤더 라벨 재설계 (사용자 시각 피드백)
hdr-label-fit 배포 후 사용자: "디자인적으로 모범적이진 않은 것처럼 **시각적으로 불편하게 느껴지는데** 원인을 파악하고 개선해줄 수 있을까요?"
- **진단(라이브 확대 3× + 방출 기하 덤프)** — 6개 결함: ① 형제 GH 폰트가 **24.6/27.1/30.1** 제각각(박스별 `fit`) → 크기 차이가 정보가 아니라 **노이즈** ② 칩(38)이 텍스트(24~30)보다 큰데도 알약이 `fillOpacity 0.45` 로 옅어지고 좁은 박스는 알약 유지 → **유무 불일치** ③ 두 레벨이 같은 상한 64 로 수렴 → **위계 역전** ④ 폰트 확대로 **ellipsis 증가**(`캐릭터 프로필 · …`) ⑤ 칩 38 > 예약 행 26 **팔출**로 텍스트가 박스 테두리를 물음 ⑥ 라벨이 박스 밖으로 나가 **박스보다 라벨이 주인공**.
- **근본 원인**: 폰트를 **박스마다 연속적으로** 파생한 것. 지도학·디자인 시스템은 **레벨별 discrete type scale** 을 쓰고 크기 차이는 *위계* 에만 쓴다. 거기에 상방 팔출이 겹쳐 ②⑤⑥ 을 만들었다.
- **재설계**: (A) 폰트를 **줌만의 함수**로(박스 무관, arity 3) → 형제 전원 동일 크기 (B) 상한을 **예약 헤더 행 기하 파생**(GH 20 / CATH 24) → **팔출 0**, 알약이 항상 텍스트를 감싼다 (C) 부모 상한 > 자식 상한 → **위계 역전 구조적 차단**. 알약 소프트닝 분기 폐기, 반동 스텝 상한 9→4.
- **halo 불가(기록)**: 지도 area-label 표준인 텍스트 halo 는 **어댑터에 label stroke seam 이 없어** 막혔다(`_makeText` = size/fill/weight, Pixi BitmapText stroke 부재) — SDF·mipmap 과 같은 계열의 벤더 한계.
- **대가 명시**: 억제 시작 줌이 1차 구현(0.05)보다 후퇴 — `group-hd` 0.3048 → **0.1600(1.91배)** · `cat-hd` 0.2667 → **0.1333(2.00배)**. 이득과 불편이 같은 뿌리라 분리 불가여서 **시각 정합성을 택했다**.
- **배포 전 기하 시각 대조**: 구/신 방출 기하를 렌더해 대조 — 구는 알약이 박스 밖으로 나가 테두리를 물고 옅으며 크기가 제각각, 신은 라벨이 박스 안에 단정히 들어가고 전원 동일 크기·밴드 헤더가 더 크다.
- **§18.8 codex-review — P1/GATE 0 · P2 1건 흡수**: 밴드 스텝 상한을 비율에서 뽑아 4 였는데 실효 상한은 3 이라(전 레벨 폰트가 z≤0.5053 에서 굳는데 밴드는 z≈0.4579 에서 전이) 렌더 동일 구간에 헛 rebuild → 상한을 **레벨 스펙에서 파생**. ⚠️ 첫 초안 테스트는 검증점을 하드코딩해 이 결함을 **놓쳤고**, 클램프 지점을 스펙에서 파생한 뒤에야 재현판 FAIL 이 잡혔다.
- 검증: `test_g6build_labellod.js` **72 PASS**(Section I·J·K 신 계약 35건 — 폐기 API 잔재 0·형제 동일·arity 3·위계 스윕·팔출 0·알약 수용·reflow 0·zoom 1 회귀 0·products 게이트) + 헤드리스 **전 스위트 21개 936 PASS / 0 FAIL** · `node --check` PASS · §18.8(REV-20260729T093000). PB-0008 라이브는 HT.9.
- **남은 수단(별도 항목)**: 더 큰 개요 판독이 필요하면 리서치가 제시한 '범위를 캔버스로 쓰는 배치'(줌아웃 시 박스 중앙 **워터마크 area-label**). 어디도 침범하지 않고 폰트를 크게 쓸 수 있으나 semantic zoom 전환 + 워터마크 노드 hit 제외 규약이 필요해 범위가 크다.
...
## 2026-07-29 · graph-cap-audit — 상한이 데이터를 잘라내던 지점 전수 감사 (사용자 리포트 2차 + 방향 지시)
사용자: "여전히 컬럼이 누락된 항목이 있으며, **노드 또한 누락된 대상**이 확인되었습니다." → 이어서
**"다른 모든 상한값이 설정된 항목들을 검토해주세요. 이와 같은 현상은 오류이며, 개수를 줄여 출력하는
최적화는 다른 방향으로 이루어져야 합니다."**
- **왜 직전 cycle 로 안 끝났나**: 직전 보강 게이트는 "컬럼 0 **또는** `truncated`" 였다. 그런데 그래프
  `Column` 정점은 `column_descriptions`(설명이 달린 컬럼만) 원천이라 **부분 투영**이 흔하고, 그 경우
  백엔드는 있는 걸 다 준 것이므로 `truncated` 도 false 다 — **"부분"이라는 사실이 응답 어디에도 없다.**
  게이트가 그 2개 때문에 보강을 skip 했다(실측 `cc_pyron.DT_Character_New` = 그래프 **2** vs 실 **55**).
- **노드 누락의 정체**: `_ROUTINE_CAP_DEFAULT = 300` 은 조회가 아니라 **sync 투영 단계**의 상한이라,
  잘린 루틴은 그래프에 **정점 자체가 없었다**(어떤 조회로도 복구 불가). 실측 `cc_pyron` 실제 **597** 중
  **300**만 투영 · 루틴 보유 스키마 **232개 중 47개(20%)가 정확히 cap 에 포화** — 전부 조용히.
- **전수 감사 결과**: 데이터 누락을 유발하던 상한 **19건**을 비현실 극단 전용 **안전 가드**(실사용
  최대치의 수십 배)로 전환하고, 걸리면 **WARN 으로 표면화**한다(조용한 절단 제거). `IN` 절 크기처럼
  **배치 성격**인 것(`_COLUMNS_TABLE_CAP` 400)은 상한이 아니라 배치 크기이므로 **반복 조회로 전량**
  처리하도록 바꿨다(배치 단위 실패 격리 — 한 배치 실패가 전량을 버리지 않는다). 표시 절단
  (`_META_RTCOL_SHOW` 8 + "외 N")은 **표시와 툴팁 양쪽**이 잘려 나머지 컬럼명을 화면 어디서도 되찾을 수
  없었으므로 전량을 싣고 축약은 CSS ellipsis 에 위임했다.
- **유지한 것(= 사용자가 말한 "다른 방향")**: 뷰포트 컬링(§65)·노드 LOD(§67)·컬럼 LOD(§61)·레이아웃
  위상서명 메모이즈(§73)·라벨 BitmapText(§80)·scene diff 풀(§79) — **데이터는 전량 보유하고 표시만
  조절**하는 최적화. `_META_DBGRP_ROW_CAP`(4000, 이미 "실사용 무제한" 가드)·`_META_DBGRP_BIG`(초기 접힘 —
  펼치면 전량)·연출 상수도 그대로.
- **§18.8 codex 적대 검증 — P1(GATE) 1건 흡수**: 컬럼 펼침 함수 **초입의 조기 return**(`_metaTableHasCols`
  단독)이 내가 추가한 보강 로직보다 앞이라, 부분 펼침 테이블은 함수에 들어오자마자 반환돼 나머지 컬럼이
  영영 오지 않았다 — **사용자 스크린샷이 정확히 그 상태**였으므로, 이 지적이 없었다면 "고쳤다" 고 보고하고도
  증상이 그대로 남았을 것이다. "펼쳐졌다" 를 *완전히* 펼쳐졌다(컬럼 있음 AND introspect 완료)로 좁혀 해소.
- 검증: pytest **전량 통과**(FAILED 0) · 헤드리스 **585 PASS** · ruff PASS · 상한 상수 런타임 실측 확인.
- **비용 정직 표기**: 상한 상향은 **LLM 호출을 늘리지 않는다**(루틴 introspect·정의 파싱·AGE MERGE 는
  LLM-free). 늘어나는 것은 sync 1회 처리량과 조회 응답 크기다 — 그래서 상한을 *제거* 하지 않고 안전
  가드로 남겼고, 실제 부하는 배포 후 관측 대상이다(feature-0026 성능 관측 인프라).

### §8 개선 제안 (기록만)
- **AI 분석 진행 패널의 완료 12 / 실패 4 표시 상한**: 그래프 데이터가 아니라 실시간 진행 로그 요약이고,
  전량 표시하려면 패널에 스크롤 컨테이너를 추가하는 레이아웃 변경이 동반된다 — 본 cycle 의 "데이터
  누락" 축과 성격이 달라 별도 항목으로 남긴다.
- **근본 해소는 투영 커버리지**: 프론트 introspect 보강은 그래프 공백을 화면에서 메우는 것이지 공백
...
## 20260730T2055-embed-congestion-fix — 시그니처가 길어지자 타임아웃이 cliff 가 됐다

### 사실관계

임베딩 드레인이 19:39 부터 처리량 0 으로 고착했다(큐 8,371 고정, 25분+). 워커 로그는
`embedding API attempt 1/3..3/3 실패: Request timed out.` 만 반복했고, 백엔드는 그동안
CPU 99%·GPU 99% 로 **계속 일하고 있었다**.

**무경합 실측**(insight-worker 정지 + 게이트웨이 우회, ollama 직접 호출):

| 입력 | n=100 | 건당 |
|---|---|---|
| 짧은 텍스트(16자) | 22.30s | 0.22s |
| 실포맷 근사(1022자) | **119.83s** | 1.20s |
| 실포맷 근사, n=25 | 30.02s | 1.20s |

**bge-m3 비용은 텍스트 길이에 비례하고, 총 처리량은 건수에 선형이다**(100건 119.83s ≈ 25건 30.02s × 4).

그리고 대기 중인 텍스트의 길이 분포:

```
미임베딩 8,371건  평균=210  중앙=154  p95=464  최대=12,374 자
```

직전 cycle 에서 시그니처를 content-forward 로 보강했다(설명 + 컬럼 + `used by:` + `related:` +
`analysis:`). 그 결과 텍스트가 16자급에서 **평균 210자·p95 464자**로 커졌고, 건당 비용 모델
(≈ 0.22 + 길이×0.00096 초)로 환산하면 **100건 배치가 42~67s** 가 된다. 클라이언트 타임아웃은
**60s** 였다. 즉 길이 분포의 위쪽을 포함한 배치는 **완료 직전에 매번 버려졌다.** 버려진 요청도
백엔드에서는 계속 연산 중이므로 재시도는 큐를 줄이는 게 아니라 늘렸다.

처리량이 21,600 → 18,000 → 12,000 → 9,600 → 2,400 → **0** 으로 *점진 감소*한 것도 이 설명과 맞는다 —
큐 앞쪽의 짧은 구포맷 텍스트가 소진되고 긴 신포맷 텍스트 비중이 올라가면서 배치 지연이 60s 선을
넘어간 것이다. 특정 시점의 사건이 아니라 **분포가 임계를 넘은 것**이다.

### 원인 귀속을 두 번 틀렸다 (기록)

1. **1차 오귀속 — "내가 버스트를 1000 으로 올려서"**: 눈앞의 내 변경이라 먼저 의심했고 그렇게 쓰기
   시작했다. 타임라인을 맞춰보니 그 변경은 붕괴 **90분 전**부터 12,000~21,600/h 로 정상 동작했다.
   인과 순서가 맞지 않았다.
2. **2차 오귀속 — "외부 GPU 경합"**: `nvidia-smi` 가 GPU utilization 96~100% 를 보이고 컨테이너
   재기동으로도 회복되지 않아 WSL2 가 공유하는 Windows 호스트 측 경합으로 판단했다. **embed-ollama 를
   정지시켜 보니 GPU 가 99%→6%·300MHz·16W 로 떨어졌다** — GPU 를 쓰는 건 embed-ollama 하나뿐이었고,
   48°C·45W(제한 125W)라 스로틀링도 아니었다. "GPU 가 바쁘다" 를 "남이 쓰고 있다" 로 읽은 오독이다.

두 번 모두 **상관을 인과로 승격**한 것이고, 결정적 실험(부하 주체 분리·무경합 재측정)을 뒤늦게 했다.
그 실험을 먼저 했다면 한 번에 끝났다.

### 진짜 불변식

혼잡의 원인은 pass 당 **버스트 횟수**가 아니라 **요청 1건의 작업량**(= 배치 건수 × 텍스트 길이)이다.
총 처리량이 건수에 선형이므로 배치를 잘게 쪼개도 손해가 없고, 요청당 지연만 줄어 여유가 생긴다.

| knob | before | after | 근거 |
|---|---|---|---|
| `AGENT_KB_EMBEDDING_BATCH_SIZE` | 100 | **25** | 요청당 최악 작업량 120s→30s. 총 처리량 불변(선형) |
| `AGENT_KB_EMBEDDING_TIMEOUT_SEC` | 60 | **300** | "영영 안 온다" 판정값으로 복원. 요청당 작업량의 10배 여유 |
| `AGENT_KB_EMBEDDING_BATCH_MAX_ROWS` | 1000 | **600** | 서브배치가 작아졌으므로 pass 를 잘게 끊으면 60s 유휴만 늘어난다 |

회귀 잠금은 계약 자체를 바꿔 걸었다 — `요청당 작업량 × 4 ≤ 타임아웃`. "서브배치 ≤ N회" 는 틀린
불변식이라 폐기했다(원인이 아니라 노출량을 재는 지표였다).

### 교훈

- **타임아웃은 "영영 안 온다" 를 판정하는 값이지 "느리다" 를 판정하는 값이 아니다.** 정상 지연 근처로
  잡으면 입력이 무거워졌을 때 시스템은 *느려지는* 대신 *완전히 멈춘다*. graceful degradation 이 아니라
  cliff 가 된다. 이번엔 그 cliff 를 **내가 만든 입력 변경**이 밟았다.
- **임베딩 입력의 "품질" 을 올리는 변경은 곧 "비용" 변경이다.** 시그니처에 설명·컬럼·관계·분석문을
  넣어 클러스터 품질을 얻었고(지표 역전 해소), 그 대가로 건당 비용이 5배가 됐다. 품질 변경을 할 때
  파이프라인의 시간 예산(타임아웃·배치·주기)을 함께 재검토했어야 했다.
- **부하 주체는 추측하지 말고 분리해서 확인한다.** `docker stop` 한 번이 GPU 경합 가설을 끝냈다.

### 남은 항목 (이번 범위 밖, 기록)

시그니처 최대 길이가 **12,374자**로 꼬리가 길다. bge-m3 컨텍스트(4096 토큰) 상한에 근접해 실효가
없으면서 비용만 큰 구간이다. 다만 시그니처는 뒤쪽에 `related:`/`analysis:`(지표 역전을 고친 핵심 신호)가
오므로 **단순 말단 절단은 유해**하다 — 길이 상한을 두려면 구성요소별 예산 배분이 필요하고, 별도 cycle 로 다룬다.


## 20260730T2200-cluster-outcome-baseline — 측정 기준선과 문제의 소재

재클러스터 결과를 판정하려면 먼저 **무엇을 세는지**가 정확해야 한다. 두 번 틀렸다.

### 밴드를 세는 키

`semantic_cluster_id` 는 **전역 고유가 아니다**(scope·datasource·schema 안에서만 유효). id 단독으로
`GROUP BY` 하면 서로 다른 스키마의 클러스터가 한 덩어리로 합쳐진다 — 그렇게 세면 밴드 157개·평균
96.7멤버라는 실물과 동떨어진 수가 나온다. 캔버스가 그리는 밴드 1개 = `(datasource, schema,
semantic_cluster_id)` 이고, 이 키로 세야 **781개·평균 19.44** 가 된다.

교차검증: 이 키로 잰 중복 라벨 **174쌍**은 이전 cycle 이 별도로 기록해 둔 기준선 177쌍과 사실상
일치한다. 잘못된 키로 잰 1,168쌍은 그 어느 쪽과도 맞지 않았다 — **기존 수치와 자릿수가 다르면
계산부터 의심한다**는 신호였는데 한 번 놓쳤다.

### 기준선 (2026-07-30 22:00, 전환 전 ≈99%)

| 지표 | 테이블(`rag_objects`) | 루틴(`routine_objects`) |
|---|---|---|
| 밴드 수 | 781 | **4,572** |
| 평균 멤버 | 19.44 | **6.56** |
| 1~2멤버 밴드 | 90 (12%) | **966 (21%)** |
| ≥100멤버 밴드 | 16 | 0 |
| 중복 라벨 | 65종 / 174쌍 | **584종 / 7,600쌍** |
| 미클러스터 객체 | 2,351 | — |

### 사용자가 지적한 문제는 **루틴 축**에 있다

과세분화도 유의어도 루틴(함수·프로시저) 쪽이 압도적이다. 사용자가 든 두 예가 그대로 실측된다:

- `메일 시스템`(5밴드·59) ↔ `우편 시스템`(5밴드·52) ↔ `우편 메일 시스템`(1밴드·15) — **한 의미, 세 어휘**
- `경매 시스템`(7밴드·114) ↔ `경매`(3) ↔ `경매 거래`(2) ↔ `경매 거래 시스템`(1) ↔ `경매장`(1)
- `메일 삭제`(8밴드) ↔ `우편 삭제`(5밴드) — 같은 연산이 두 어휘로 갈림

그리고 남아 있던 임베딩 드레인도 **전부 루틴 시그니처**였다 — 즉 개선 효과가 가장 크게 나타날 축과
가장 늦게 도착하는 축이 같다.

### 전환 진행 (부분 관측, 5/27 scope)

| 지표 | 기준선 | 5/27 전환 시점 |
|---|---|---|
| 테이블 밴드 | 781 | 679 (−13%) |
| 테이블 1~2멤버 | 90 | 58 (−36%) |
| 테이블 중복 라벨 | 65종/174쌍 | 54종/167쌍 |
| 루틴 밴드 | 4,572 | 4,519 (정체) |

테이블 축은 의도한 방향으로 움직인다. 루틴 축이 정체인 것은 루틴 클러스터 멤버의 80%(23,735건)를
가진 `mssql-06656002eda6` 가 아직 전환 전이기 때문이다 — **최종 판정은 이 scope 전환 이후에만 성립한다.**


## 20260731T0000-cluster-outcome-measured — 재클러스터 결과 (E.6)

시그니처 드레인 완료(0건) 후 scope 별 재클러스터가 순차 실행됐고, 루틴 클러스터 멤버의 80%를 가진
`mssql-06656002eda6` 의 결과가 반영된 시점에서 측정했다. 기준선은 같은 쿼리로 2026-07-30 22:00 측정분.
...
|---|---|---|---|
| 테이블 밴드 | 781 | **598** | −23% |
| 테이블 1~2멤버 밴드 | 90 | **48** | −47% |
| 테이블 중복 라벨 | 65종 / 174쌍 | **53종 / 138쌍** | −21% |
| **루틴 밴드** | 4,572 | **2,669** | **−42%** |
| **루틴 평균 멤버** | 6.56 | **10.36** | **+58%** |
| **루틴 1~2멤버 밴드** | 966 (21%) | **41 (1.5%)** | **−96%** |
| **루틴 중복 라벨** | 584종 / 7,600쌍 | **349종 / 4,067쌍** | **−46%** |

과세분화(요청 a)는 루틴 축에서 1~2멤버 밴드가 **966 → 41** 로 사라졌고, 평균 멤버가 6.56 → 10.36 으로
올라 밴드 하나가 실제로 "묶음" 역할을 하게 됐다.

### 배치가 이름이 아니라 관계를 따르는가 (요청 b·c)

`cluster_id` 가 곧 배치 순서이고(centroid MDS 2-D serpentine), **인접한 밴드끼리의 실제 centroid
유사도**를 세 가지 순서로 비교했다 — 값이 높을수록 "가까이 놓인 것이 실제로 가깝다".

| 스키마 | 밴드 | **MDS 배치(현행)** | 라벨 이름순 | 루틴명순(옛 방식) | 전체 평균 |
|---|---|---|---|---|---|
| atum2_db_arena | 64 | **0.8115** | 0.8013 | 0.7869 | 0.7782 |
| cc_data_main | 40 | **0.7604** | 0.7095 | 0.7468 | 0.6776 |
| cc_pyron | 52 | **0.7937** | 0.7657 | 0.7729 | 0.7445 |
| cc_obt | 49 | **0.7950** | 0.7778 | 0.7552 | 0.7404 |

**네 스키마 전부에서 현행 배치가 두 대조군보다 높다.** 옛 배치(멤버 min(key) = 루틴명 순) 대비
+0.014~+0.040, 무작위 기준선인 전체 평균 대비 +0.033~+0.083. 사용자가 지적한 "라벨링된 이름 순서대로
나열되고 있는 것 뿐"은 해소됐다.

### 사용자가 든 두 사례

**메일 ↔ 우편 — 해소.** `우편` 계열 밴드가 **12밴드/92멤버 → 1밴드/12멤버**(남은 것은
`우편·아이템 관리` 라는 복합 개념)로 줄고 `메일 시스템` 이 17밴드/168멤버로 수렴했다.
`우편 시스템`·`우편 삭제`·`우편 메일 시스템` 은 전부 소멸.

**경매 기록 ↔ 경매 시스템 — 어휘는 통합, 병합은 의도적 거부.** `경매장`·`경매 거래 시스템`·
`분해 경매`·`경매캐시`·`캐시경매` 가 `경매 시스템`(12밴드/190멤버)으로 흡수됐다. 다만
`atum2_db_arena` 스키마 안에서 `경매 기록`(3멤버)은 `경매 시스템`(7멤버)과 **여전히 별도 밴드**다.

실측한 이유:

```
centroid 유사도      = 0.8599   (MERGE_SIM 0.90 미달)
완전연결 최소 쌍     = 0.6950   (← 이 값이 병합을 막는다)
최대 쌍 0.8473 / 평균 0.7867
```

**완전연결(complete-linkage) 이 설계대로 작동한 결과다.** 두 밴드의 *최악* 쌍이 0.695 로, 평균만 보고
합치면 관계가 약한 항목까지 한 밴드에 끌려들어온다(체이닝). 그래서 병합하지 않는 편이 옳다.

그리고 사용자의 원 지적은 "**관계에 비해 거리가 먼**" 이었지 "같은 것이니 합쳐라" 가 아니다. 이 축의
정답은 병합이 아니라 **인접 배치**이고, 실제로 64개 밴드 중 `경매 기록`(id=57)과 `경매 시스템`(id=59)이
**두 칸 차이**로 놓였다 — MDS 배치가 의도대로 두 밴드를 붙여 놓았다.

### 남은 것 (정직 표기)

- 측정 시점에 `mssql-06656002eda6` 의 pass 가 **아직 마크를 갱신하지 않았다**(데이터는 기록됨, 마크는
  pass 반환 후). 잔여 스키마가 더 처리되면 수치가 추가로 개선될 수 있고 악화되지는 않는다.
- 27 scope 중 21개가 최신, 6개가 순차 대기 중이다. 대기분은 루틴 보유량이 작아 전역 지표에 미치는
  영향이 제한적이다.
- 라벨 어휘는 스키마 경계를 넘어 완전히 통일되지는 않았다(`경매`·`경매 처리`·`파괴 경매` 등 잔존).
  같은 스키마 안의 유의어는 `_merge_clusters_by_label` 이 처리하지만, **스키마 간 어휘 통일**은
  LLM 프롬프트의 `existing_labels` 힌트에만 의존한다 — 강제 규칙이 아니다.


## 20260731T0100-label-canon — 스키마 간 어휘 통일: 임계를 감으로 잡지 않았다

### 남은 문제

직전 측정에서 `메일 ↔ 우편` 은 해소됐지만 스키마를 넘는 어휘 통일은 남았다고 기록했다
(`경매`·`경매 처리`·`파괴 경매` 잔존). 원인은 구조적이다 — 클러스터링이 **스키마 단위**로 돌고
라벨도 스키마별로 붙는데, 스키마 간 일관성은 LLM 프롬프트의 `existing_labels` **힌트**에만
의존했다. 힌트는 강제가 아니다.

### 임계는 측정해서 정했다

`mssql-06656002eda6`(밴드 2,083 · 스키마 115 · 교차-스키마 쌍 213만)의 실측 분포:

| 쌍 종류 | 중앙 | p95 | p99 | 최대 |
|---|---|---|---|---|
| 교차-스키마 **같은 라벨** | 0.949 | — | — | — |
| 교차-스키마 **다른 라벨** | 0.672 | 0.788 | **0.848** | 1.000 |

다른-라벨 쌍의 99%가 0.848 이하다. 기본 임계 **0.97** 은 그 꼬리보다 한참 위라, 잡히는 것은
"같은 클러스터인데 라벨만 다른" 경우뿐이다. 실제로 0.9998 로 붙어 있으면서 라벨만 갈린 쌍들이 있다:

```
0.9998  거래 시스템  <->  거래 처리
0.9998  아이템 관리  <->  아이템 획득
0.9998  성 관리     <->  성 구축
0.9997  길드 신청    <->  길드 가입
```

같은 게임 DB 의 **사본 스키마**들이 각자 라벨링된 결과다.

### 설계 판단 셋

**① 병합하지 않는다.** 스키마가 다르면 두 밴드는 각자 자기 DB 의 테이블·루틴을 가리키므로 별개가
맞다. 바꾸는 것은 **라벨 텍스트뿐**이다.

**② union-find 를 쓰지 않는다.** A~B, B~C 가 임계를 아슬아슬하게 넘고 A~C 는 먼 경우, 전이 병합은
무관한 어휘까지 한 이름으로 뭉갠다. centroid 병합에서 완전연결을 택한 것과 같은 이유다. 각 밴드는
**직접 이웃(1-hop)** 만 보고 그중 가장 큰 밴드의 라벨을 따른다 — 결정론이라 pass 마다 흔들리지
않는다(흔들리면 라벨 캐시가 전패해 LLM 재호출이 폭주한다).

**③ 스키마-내 충돌은 되돌린다.** 통일 결과 한 스키마 안에 같은 라벨이 둘 생기면 작은 쪽을 원래
라벨로 복원한다. 스키마-내 동일 라벨은 `_merge_clusters_by_label`/`_disambiguate_labels` 의
관할이고, 여기서 새로 만들면 "왜 같은 이름이 둘인가" 가 다시 생긴다.

### 부수 결함을 하나 만들 뻔했다

합성 요약(`cluster_summary`) 프롬프트는 **`label` 을 입력으로 받는다.** 그리고 요약 캐시 키는
`멤버셋 + L1 지문 + L0 지문` 이라 **라벨이 들어 있지 않다.** 그래서 통일 전에 요약을 만들면
밴드는 `거래 시스템` 인데 요약문은 `거래 처리` 를 말하고, 라벨이 바뀌어도 캐시가 유효해
**그 불일치가 영구히 고착**된다. 어휘를 맞추려다 다른 층에 부정합을 새로 심는 셈이다.

해소는 두 겹이다. ① 요약 생성을 스키마 루프 안에서 **보류**했다가 어휘 통일 직후 최종 라벨로
만든다(pass 당 상한 계산은 그대로). 런타임에서만 드러나는 순서 결함이라 일반 단위 테스트로는 안
잡히므로 **AST 로 호출 순서를 잠갔다**. ② 그런데 이것만으로는 **신규 요약만** 고쳐진다 — 적대
리뷰(codex P1)가 짚었듯 **이미 저장된 요약**은 캐시 키에 라벨이 없어 재생성되지 않는다. 그래서
`label` 을 요약의 **신선도 축**에 추가했다(조회·miss 판정·UPSERT `WHERE` 절 3곳). 라벨이 바뀌면
요약이 낡은 것이라는 판단이 이제 캐시 계약에 들어 있다.

### 하지 않기로 한 것 (측정 결과)

- **시그니처 길이 상한** — 현행 참조 시그니처 46,122건 중 >1000자는 **237건**뿐이고, 1000자 상한을
  걸어도 총 문자수의 **1.8%** 만 준다. 직전 REPORT 에서 최대값(12,374자)만 보고 "비용만 큰 구간"
  이라 적었던 것은 분포를 보지 않은 과장이었다 — 정정한다.
- **미참조 `texts` GC** — 149,558건 중 **49,335건(33.0%, 959만 자)** 이 구포맷 시그니처 잔존이다.
  회수 이득은 스토리지(~200MB)뿐인데, 잘못 지우면 재임베딩에 수 시간이 든다(현행 처리량 기준).
  되돌리기 비용이 이득보다 크므로 수치만 남기고 실행하지 않는다.


## 20260731T0230-label-canon-verified — 라이브 검증 (E.11)

배포본 `4d5e6678` 에서 재클러스터 pass 를 직접 1회 유발해 측정했다(자동 cadence 는 6시간이라
그때까지 기다릴 이유가 없다). 두 데이터소스 모두 **밴드는 그대로 두고 어휘만 줄었다** — 설계 의도가
실제로 그렇게 동작하는지가 이 검증의 요점이다.

| 데이터소스 | 스키마 | 라벨 종수 전→후 | 밴드 수 전→후 | 통일 밴드 | 재생성 요약 | 스키마-내 중복 |
|---|---|---|---|---|---|---|
| `mysql-42371f8d92bc` | 13 | **62 → 56** (−9.7%) | 64 → **64** | 8 | 8 | — |
| `mssql-ba175631e9fc` | 9 | **150 → 130** (−13.3%) | 162 → **162** | 23 | 40 | **0 → 0** |

읽는 법:

- **밴드 수 불변**이 핵심이다. 어휘를 맞추려다 클러스터를 합쳐 버리면 각 DB 의 객체가 뒤섞인다.
  두 경우 모두 밴드가 1개도 변하지 않았다.
- **스키마-내 중복 0 유지** — 충돌 가드가 라이브에서도 성립한다(통일 결과 한 스키마에 같은 라벨이
  둘 생기지 않았다).
- **요약 재생성이 따라왔다**(8건·40건). 라벨을 요약 신선도 축에 넣은 §18.8 codex P1 수정이 실제로
  발동한 것 — 이게 없었으면 밴드 이름만 바뀌고 요약문은 옛 어휘로 남았다.
  `mssql-ba175631e9fc` 의 40건은 pass 당 상한(`_SUMMARY_MAX_PER_PASS`)에 걸린 값이라 잔여는 다음
  pass 들에서 이어서 채워진다.

### 아직 남은 관측

가장 큰 `mssql-06656002eda6`(루틴 23,859 · 스키마 115 · 밴드 2,083)은 **이번에 유발하지 못했다.**
insight-worker 컨테이너 메모리 한도가 1GiB 인데, 상시 워커와 별도로 pass 프로세스를 하나 더 띄우면
임베딩 적재만으로 한도를 넘어 OOM(exit 137)이 난다. 워커 자신의 정기 pass 는 같은 프로세스 안에서
돌기 때문에 문제없이 끝난다 — 이 scope 는 cadence(마크 23:55 + 6h ≈ **05:55**)에 자연 전환된다.
그때 `거래 처리`(17밴드) ↔ `거래 시스템`(2밴드), `아이템 거래`(3밴드) ↔ `아이템거래`(2밴드, 공백
변형) 가 통일되는지가 남은 확인점이다.

> 운영 메모: 외부에서 pass 를 유발할 때는 **데이터소스 크기를 먼저 본다.** 대형 scope 는
> `docker exec` 로 별도 프로세스를 띄우지 말고 정기 cadence 에 맡긴다.


## 20260731T0300-band-visual-fit — 육안검증: 수치가 맞아도 화면이 안 맞을 수 있다

앞선 cycle 들은 밴드 수·라벨 종수·유사도 같은 **수치**로 성과를 확인했다. 이번엔 라이브 화면을
확대해 보면서 그 결과가 **디자인적으로도 정합한지** 봤다. 두 개가 걸렸다 — 둘 다 수치로는 절대
드러나지 않는 종류다.

### F1 — 멤버 1개짜리 "밴드"

`dk_game_integrate` 한 스키마에서 이런 상자 3개가 나란히 있었다:

```
…integrateunion · 1      spget… · 1      spupdate… · 1        기타 · 7
   (1개 노드)             (1개 노드)        (1개 노드)          (7개 노드)
```

바로 위에는 `경매 거래 · 46` 이 있다. 문제는 세 가지가 겹친다.

1. **멤버 1개는 묶음이 아니다.** 헤더·테두리·접기 토글을 다 갖춘 상자가 노드 하나를 감싼다.
2. **라벨 어휘가 깨진다.** `spget…` 은 기계 어간이다. 한국어 의미 라벨(`경매 거래`·`기타`) 옆에
   서면 같은 계층의 이름처럼 보이는데 성격이 전혀 다르다 — 사용자가 지적한 "동일한 의미가 다른
   이름" 문제의 다른 얼굴이다.
3. **시각 무게가 같다.** 46-멤버 밴드와 1-멤버 밴드가 같은 크기의 헤더를 갖는다.

원인은 `graph-simgroups.js` 의 싱글턴 흡수 한 줄이었다:

```js
// 역할/기타 family 도 싱글턴이면 기타로 흡수 (nm: 은 2차 attach 로 커질 수 있어 유지)
if (f !== "misc" && !f.startsWith("nm:") && !f.startsWith("be:") && cnt.get(f) < 2) → misc
```

주석의 근거가 **그 시점에 이미 성립하지 않는다.** 2차 attach 는 바로 위에서 끝났고, 여기 남은
`nm:` 싱글턴은 더 자랄 수 없다. `nm:` 예외를 걷어내면 세 상자가 `기타` 로 흡수된다.
(`be:` 예외는 유지 — 배정 시점에 `beCnt >= 2` 를 요구하므로 싱글턴이 될 수 없다.)

싱글턴이 사라지면 라벨의 방향 말줄임(`tok…` = 이 스템으로 시작 / `…tok` = 끝남)이 **범위 신호**로
제 뜻을 되찾는다. 멤버가 하나면 "범위" 가 없어 그 신호가 공회전한다. — 처음엔 이 방향 불일치를
별개 결함으로 적으려다 코드를 보고 **의도된 인코딩**임을 확인해 철회했다.

### F2 — 긴 이름이 알약 밖으로 나간다

같은 배율로 두 칩을 나란히 놓으면 명확하다.

| 이름 길이 | 관측 |
|---|---|
| 짧음(`spGetCastle`) | 아이콘·글자가 알약 **안**, 좌우 여백 정상 |
| 긺(`spDeleteIntegrateUnionCAG…`) | 아이콘이 알약 **왼쪽 밖**, 글자가 여백 0 으로 꽉 참, 말줄임표가 **오른쪽 밖** |

원인은 `_metaRoutineStyle` 한 줄이다:

```js
const w = Math.min(190, _METLAY.TW + (rel ? Math.round(rel*40) : 0));   // TW = 150
... labelMaxWidth: 176 ...                                              // ← 상수
```

폭은 rel 에 따라 150~190 인데 라벨 예산만 **176 고정**이다. rel 부스트가 없는 칩(대부분)은 폭 150
안에 176 짜리 라벨을 그리라는 뜻이라 26px 넘친다. 라벨 문자열이 `아이콘 + " " + 이름`
(graph-core `rLabel`)이라 이 한 값이 아이콘 위치까지 함께 관장한다 — 아이콘 밀림과 말줄임표 넘침이
**한 원인**이다.

결정적인 건 바로 위 `_metaTableStyle` 의 주석이다:

> feature-0016 §45: 폭이 rel 무관 고정(TW=150)이 되며 라벨이 박스를 넘치지 않도록 labelMaxWidth 를
> 박스 안으로 클램프(**예전 176 은 rel 부스트로 최대 190 폭일 때 기준**).

§45 가 같은 결함을 테이블에서 이미 고쳤고 **루틴 칩을 빠뜨렸다.** 같은 처방(`w - 10`)을 적용한다.

### 정합했던 것

- **패널 ↔ 캔버스**(사용자 요청 d): 컨텍스트 메뉴 `컨텐츠 카테고리 / 경매 거래 · 테이블 46` 과
  캔버스 밴드 `경매 거래 · 46`, 패널 그룹 목록이 **완전히 일치**. SSOT 가 라이브에서 성립한다.
  (F1 의 싱글턴도 양쪽에 똑같이 노출됐다 — 부정합이 아니라 **정합하게 잘못된** 상태였다.)
- **어휘 통일**: 검색 패널에서 서로 다른 스키마의 두 밴드가 모두 `🏷 인벤토리 관리` 로 나온다 —
  병합되지 않고 이름만 같아진, 의도한 그 모습이다.

### 회귀 잠금의 제약 (정직 표기)

이 두 결함을 덮는 기존 헤드리스 테스트(`test_g6build_simgroups_p2.js` 등)는 **번들 조립이 필요해
CI 에서 돌지 않는다**(CI 는 pytest 전용). 그래서 CI 가 실제로 실행하는 pytest 로 **소스 계약**을
걸었다(`test_static_cache_integrity.py` 의 선례와 같은 방식). 렌더 결과가 아니라 소스 형태를 재는
간접 계약이라는 한계는 그대로 남는다 — 실제 픽셀 확인은 배포 후 PB-0008 로 한다.

### 측정 과정에서 버린 시도 (기록)

픽셀로 "넘침 N px" 를 재려다 세 번 실패했다. ① 측정 창 경계를 텍스트 끝으로 오독 ② `±60px` 창이
옆 노드 글자를 주워 짧은 이름까지 "넘침" 판정 ③ 배경색(248,244,239)이 흰색 임계를 통과.
관계선을 끄고(객체 168→106) 배경을 정리한 뒤에야 pill 폭이 일정한 값(~109px)으로 잡혔다.
**밀집한 화면에서 픽셀 측정은 노이즈를 먼저 제거해야 성립한다** — 그전 수치는 전부 버렸다.
최종 판정은 같은 배율 확대 비교(육안)로 했다.

## 20260813T1121-expand-camera-anim — 펼침 시 선택 노드 추종 + 재배치 애니메이션

### 요청과 진단

사용자 요청 2건: ① "노드를 클릭하여 확장할 때 펼쳐지는 크기가 커 전체적인 노드들의 재배치가 일어나면
기존에 선택한 노드의 위치를 카메라에서 잃어버려 다시 찾아야 한다 — 벗어나면 탄력적으로 추적"
② "다른 노드의 재배치가 깜빡이는 순식간이라 각 이동을 인지하기 어렵다 — 애니메이션".

코드 실측으로 확인한 기전:

- **카메라**: 더블클릭 이웃확장(`_metaGraphExpand`)만 적응형 follow tween 을 갖고 있었다. 사용자가 말한
  "노드를 클릭하여 확장" = 단일클릭 컬럼 펼침(`_metaGraphToggleColumns`)에는 **카메라 보정이 전혀
  없었다** — `_metaG6Apply(false)` 만 부르고 끝난다. 접기(`_metaGraphCollapse`)도 동일. 스키마
  펼침은 반대로 **항상 중앙 `focusElement`** 라, 이미 잘 보이던 카드까지 매번 화면을 튀게 했다.
- **애니메이션**: 재배치는 `setData()`+`draw()` 1프레임 점프다. `draw()` 의 오브젝트 풀 diff 서명
  (`nodeSig`)에 x/y 가 들어 있어 **이동 노드는 파기 → 최종 위치 재생성** — 중간 프레임이 존재하지 않는다.

### 무엇을 했나

**① keep-in-view (벗어날 때만, 최소 이동)** — `_metaKeepInViewDelta`(순수) + `_metaGraphKeepInView`
(rAF 적응형 follow). 안전영역(짧은 변의 12%, 24~120px) 안이면 **카메라를 1프레임도 움직이지 않는다**.
벗어나면 잔여 delta 의 26%씩 ease-out 으로 되돌린다. 안전영역보다 큰 요소(펼쳐진 스키마 combo)는
전체를 넣는 것이 불가능하므로 **중심 기준**으로 판정한다 — 아니면 큰 요소마다 카메라가 끝없이 밀린다.
컬럼 펼침/접기·스키마 펼침/접기 **4경로**에 배선했고, 스키마 펼침의 무조건 중앙 이동은 제거했다
(제자리 원칙 ADR-004 ② 회복. 카메라 API 부재 폴백 번들은 구 동작 보존).

**② 이동 트윈 (360ms ease-out)** — 어댑터가 `setData` 직전에 **직전 씬 좌표를 스냅샷**하고, `draw()` 가
오브젝트를 최종 위치로 만든 뒤 **출발 위치로 되감아** `from→to` 로 옮긴다. 모델(`n.style`)은 끝까지
건드리지 않는다(배치 메모이즈·미니맵 서명·fit 이 전부 모델을 읽는다). 트윈 중에만 유효한 `_tweenPos`
오버레이를 **관계선 끝점·hit-test·요소 bounds** 가 공유해, 선이 노드를 따라가고 클릭이 보이는 대로
떨어지며 카메라가 **보이는 노드**를 추종한다.

비용 상한(전부 관측에 사유가 남는다 — `window.__META_GRAPH_PERF.move`):
`prefers-reduced-motion` · rAF 부재 · 씬 교체(직전 씬과 겹침 < 30%, 분모는 양쪽의 큰 쪽) · 화면 밖 이동 ·
노드 600 · **관계선 1500**(프레임 비용의 지배항은 노드가 아니라 따라 그릴 선의 수다) 초과 시 트윈을
열지 않고 즉시 최종 위치 = 기존 동작.

### 적대 검증이 바꾼 것 (codex review 4라운드)

초판은 "동작은 하지만 상호작용이 어긋나는" 결함을 여럿 갖고 있었고, 그중 어느 것도 자동 테스트가
스스로 드러내지 못했다 — 전부 리뷰가 지적한 뒤 회귀 가드를 신설했다.

| 라운드 | 지적 | 실제 결함 | 대응 |
|---|---|---|---|
| 1차 P1 | 상태 변경이 트윈 오브젝트 참조를 무효화 | `setElementState`(busy 해제·선택 점등)가 노드를 파기·재생성 → 캡처한 참조가 죽어 **노드는 멈추고 관계선만 보간**돼 선이 떨어짐. 펼침 경로에서 항상 발생 | 프레임마다 `_objs` 재조회(자가 치유) + 교체 시 새 오브젝트를 현재 좌표에 배치(1프레임 깜빡임 제거) |
| 1차 P2 | hit-test 가 최종 좌표 기준 | 360ms 동안 **보이는 자리는 안 눌리고 빈 목적지가 눌림** | 이동 노드를 grid 에서 제외 + 화면 좌표 선형 스캔(`hitTestMoving`) |
| 2차 P2 | override 가 `_pick` 에만 | `_probeHover`·`_pickEdge` 누락 → "클릭은 되는데 hover 는 안 되는" 반쪽 | 단일 판정기 `_hitNodes` 로 통일 + `_pickEdge` 는 `_resolvePos` |
| 2차 P2 | 씬 교체 판정 분모 | 새 씬이 이전의 **부분집합**(검색 prune 500→1)이면 겹침 100% 로 계산돼 가드 통과 | 분모를 양쪽의 큰 쪽으로 |
| 2차 P2 | focus tween 양보가 영구 포기 | fire-and-forget 이라 재시도 주체가 없음 → focus 중 다른 노드를 펼치면 **영영 보정 안 됨** | 양보 후 재개(양보 구간은 예산 제외, 총 수명은 HARDMS) |
| 3차 P1 | 드래그와 트윈 충돌 | 트윈 프레임이 손끝 위치를 덮어씀 | `_moveElement`·`translateElementTo` 가 트윈 선점(**조작이 애니메이션을 이긴다**) |
| 4차 P2 | 카메라가 목적지를 추종 | 노드가 반대편에 그려진 동안 카메라가 목적지로 가 **애니메이션 내내 화면 밖** — 요구의 정반대 | `_boundsOf` 가 트윈 좌표 반환 |
| 4차 P2 | 프레임당 O(전체 노드) grid 재구성 | 22프레임 × 전 노드 재할당 | grid 재구성 제거 — 이동 노드는 상한이 걸린 **선형 스캔**(포인터 이벤트당 1회, 프레임당 0) |

### 검증

- 헤드리스 그래프 스위트 **1155 → 1264 PASS / 0 FAIL**(신규 109: `test_pixi_adapter` T28·T29·T30 68건,
  신규 `test_graph_expand_camera.js` 41건). 기준선은 착수 전 실측값이다.
- **뮤테이션으로 포착력 실증**(각 가드가 그 결함을 실제로 잡는지): 항상-중앙정렬 복원 10 FAIL · 큰 요소
  중심판정 제거 3 · 씬교체 가드 제거 1 · cap 무음절단 1 · 가시영역 필터 제거 2 · 되감기 제거 1 ·
  오버레이 미해제 2 · 엣지예산 제거 1 · 오브젝트 재조회 제거 2 · grid 이동노드 미제외 3 · `_pick`
  override 미전달 1 · 드래그 선점 제거 2 · `_boundsOf` 트윈 미반영 2 · 양보→즉시포기 2.
- 관련 pytest(`test_static_cache_integrity`·`test_metadata_perm_split`) 46 PASS.
- `node --check` 3파일 PASS.

### 남은 것 / 정직 표기

- **렌더 성능은 아직 미검증이다.** 헤드리스에는 GPU 합성이 없어 프레임레이트 주장의 근거가 되지
  못한다(AGENTS.md §16.6 — 헤드리스 rAF-FPS 는 ground-truth 불인정). 부드러움·jank 는 배포 후 PB-0008
  실 Windows 브라우저 + CDP 실-paint 로 확인하고 그 결과를 Run 기록에 남긴다.
- combo(스키마 배경 상자)는 트윈하지 않는다 — 자식 bbox 파생이라 매 프레임 전체 재계산이 필요하다.
  상자는 최종 크기로 먼저 서고 내용물이 안착하는 그림이 되며, 이는 의도된 절충이다.
- 미니맵은 최종 기하로 그린다(트윈 중 재복제 없음 — §74/§77 재사용 게이트 유지).

### POST-DEPLOY 라이브 검증 결과 (2026-08-13, 배포 3d3adb4b)

배포본을 실 Windows Chrome 150 으로 검증했다. 요지:

- **애니메이션은 라이브에서 동작한다** — 신규 계측 `__META_GRAPH_PERF.move` 실측: 컬럼 펼침에서
  `items 12 / moved 12 / skipped 0 / incident 5`(12노드 트윈 + 관계선 5개 추종), 스키마 펼침 `items 2`.
  카테고리·데이터소스 드릴은 `reason:"scene-switch"` — 비용 가드가 의도대로 발동해 새 화면을
  애니메이션하지 않는다.
- **카메라 "보이면 움직이지 않는다" 를 픽셀로 확인** — 컬럼 펼침으로 형제 테이블이 약 225px 이동하고
  그룹 박스가 넓어지는 실제 재배치 중에도, 선택한 테이블은 화면 좌표 (426,592) 그대로였다.
- **1차 구현의 회귀 1건을 라이브가 잡았다** — 스키마 펼침에서 기존 중앙 focus 를 keep-in-view 로 바꾼
  것이 문제였다. 펼친 combo 가 뷰포트보다 커지면 큰 요소 규칙(중심 기준)이 "이동 0" 을 내고, 그 결과
  status 는 "테이블·함수 9개 펼침" 인데 **화면엔 카드만 남았다**. 사용자 요청은 "선택한 노드를 잃지
  않게" 였고 이 경로의 중앙 이동은 그 요청과 충돌하지 않았다 — **요청 범위를 넘어 바꾼 것이 회귀의
  원인**이다. 해당 1줄을 되돌리고, 회귀 재발을 배선 계약 테스트로 잠갔다(EA.8).
- **측정하지 못한 것(정직 표기)** — ① 트윈 중간 프레임의 시각 캡처: CDP relay 가 호출 사이 렌더러를
  재워 360ms 창을 샘플링할 수 없었다. 프레임 간격은 실측 **16.7ms(60fps vsync)** 이나 **연속 5프레임
  단일 표본**이므로 전체 구간의 부드러움으로 일반화하지 않는다. ② "재배치로 화면을 벗어날 때의 추종"
  분기는 라이브에서 조건이 발생하지 않아 헤드리스 계약(C2·C9)으로만 검증됐다.
  즉 **체감 부드러움과 이탈-추종은 사용자 육안 확인이 남은 항목**이다.

**POST-FIX 재검증(배포 f9cfd2a0)**: 같은 조작(`gunzlogin` 스키마 카드 클릭)에서 펼쳐진 9개가 전부
화면에 보이는 것을 확인했다 — 회귀 재현본과 동일 조건에서 결과가 갈리므로 원인·수정이 확정됐다.
그 조작에서 재배치 트윈도 함께 동작(`move items 6`)해 중앙 이동과 애니메이션이 상충하지 않음을 확인.
잔여 미측정 2건(트윈 중간 프레임 시각 캡처 · 이탈-추종 라이브 재현)은 변동 없다.
