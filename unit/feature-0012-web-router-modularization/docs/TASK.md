...
- [~] TASK-0012-7 DI seam Phase 2..7: 클러스터별 핸들러+테스트 동반 마이그(A 단순→B 인라인perm→C/E1 require_permission→F optional→D 동적→트랜잭션4+E2)
  ...
  - [ ] Phase 3 cat-B batch 4+: workflow 보류 8(동반 테스트 전환 누락 5: admin_test_datasource·admin_product_db_insights·admin_usage_conversations·admin_archived_conversations·admin_overview; action분기 admin_get_system_prompt; fail-soft suggestions; admin_products_insight_coverage) + AND pre-auth gate/multi(~18) + OR·BRANCH 23 → cat C/D/E/F/트랜잭션.
...
## 8. Completion Checklist (DI seam Phase 3 cat-B batch 3 = TASK-0012-7 sole-in-block 4)
- [x] sole-in-block conn.close 4 핸들러 전환(WRAP/FB-finally RP 2 + AO 본문 try/finally 2) — cat-B 확정 15 완결(누적 cat-B 15)
- [x] behavior-neutral — make test progress-chars 1238(baseline 동일)·0 fail, route-parity 179 불변, ruff clean, py_compile OK
- [x] §18.8 적대 패널 — REV-20260630-0005(REV-0003 확정분 재사용). strip_conn_finally pass(본문 try/finally·defensive try/except 제거+dedent) 정확성 + RP 메시지 verbatim
- [x] 동반 테스트 — 4개 직접호출 0(grep). 잔류 직접호출 0
- [x] TASK/REPORT/MODIFY/REVIEW 반영, BLOCKED 없음
- [ ] verify-completion PASS(9 checks) → Git 커밋 (Phase 3 cat-B batch 3)
