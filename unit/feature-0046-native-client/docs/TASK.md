...
트레이 상주 + 자식 창 숨김까지 완료 — 테스트 171건, **실 Windows 4종 실측 전부 PASS**.
...
## TASK-20260904T100000 — 트레이 상주 + AI 호출 백그라운드화
...
- [x] 창 숨김 인자 오용 방지 표지 — `appwindow` 의 `CREATE_NO_WINDOW`-만 은 **의도된 중복**임을
      명시(통합하면 앱 창이 안 뜬다)
- [ ] **웹 셸 주 경로의 트레이 — 별도 cycle** (사용자 결정 2026-09-04). main 의 `run_client` 가
