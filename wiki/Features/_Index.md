...
| feature-0002-agent-core | active | agent core · 멀티 데이터소스(MySQL·MSSQL) · KB Postgres · insight worker — (08-14) 대화 LLM **스트리밍 전환**(per-attempt 상한이 성공 지연 꼬리 안쪽이라 정상 진행 중인 무거운 추론이 전량 폐기되던 것 해소·반환 계약 유지) · 조기 종료 run 의 KV terminal **3중 봉인**(`_ensure_kv_terminal`·`ask_jobs` backstop·`_persist_early_exit`) · 타 멤버 첨부 본문 턴의 작업공간 쓰기 도구 provenance 게이트(이미지·csv/xlsx 축 포함) · 데이터소스 연결 제한 안내 정본(`shared/db.py`) · (08-16) 첨부 변경-인지 신호를 **클라 신호 ∪ 서버 파생**으로 봉인 | [[feature-0002-agent-core]] | `unit/feature-0002-agent-core/docs/FUNCTION.md` |
| feature-0003-agent-web-ui | active | FastAPI Web UI · 관리콘솔(데이터소스·대시보드·LLM 사용량 차트 지표 선택기) · audit · 첨부(버전 diff·원문 보기·이름순 정렬·공유 대화 스코프) · 그룹 대화 멤버 권한 게이트 · ask-worker — (08-14) 사용량 지표 정의를 `static/usage-metrics.js` 단일 정본으로 추출해 관리·프로필 두 번들이 공유(지표 8종 선택기·전환 시 재조회 0) · 사용 기록 표 열 정렬 + 페이지네이션(관리·프로필 양 판, 규칙을 테스트가 두 소스 대조로 잠금) · 첨부 버전 계보를 **작성 주체별로 분기**(assistant 수정본이 사용자 최신본을 supersede 하지 않음) + 계보 간 비교 축 토글 · 첨부 `CreatedAt` 시간축 로컬→UTC 정정 · 실행 단계 패널 단계별 시각·간격·누적 경과 | [[feature-0003-agent-web-ui]] | `unit/feature-0003-agent-web-ui/docs/FUNCTION.md` |
