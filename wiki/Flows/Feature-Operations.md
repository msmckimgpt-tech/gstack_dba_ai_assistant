...
[[User-Journeys|사용자 화면 흐름]] 이 "무엇이 보이는가" 라면, 여기는 그 아래에서 **각 기능이 실제로 무엇을 하는가** 다. 46 개 feature 전부가 아니라, 사용자가 자주 만나거나 흐름이 비자명한 것들을 골랐다. 각 절의 정본은 해당 feature 의 `FUNCTION.md` 이며 본 문서는 그 흐름 mirror 다.
