...
- 2026-05-27 Phase 2 insight-worker PG primary write 전환 + MySQL agent* 10 테이블 실 DROP — commit 0745862 (TASK-0119). 배경: AR-M5 DROP 직후 insight-worker의 ensure_memory_schema() + _ensure_memory_tables()가 PG guard 없이 MySQL 테이블을 재생성 (CREATE TABLE IF NOT EXISTS), _save_message() / save_memory_kv() 등이 DUAL_WRITE=0에도 MySQL primary write 경로 유지 → 실 DROP 불가 상태. 수정: memory.py 5함수 + agent_core.py 4함수에 AGENT_RUNTIME_READ_BACKEND=postgres 시 PG direct write + MySQL bypass guard 추가. orphaned 데이터 사전 PG 마이그레이션 (14:57 run: core_messages 25행+steps 15행+kv 9행 upsert → PG core_messages=55, steps=33, kv=13). insight-worker 재빌드 + 재기동 후 MySQL 테이블 재생성 없음 확인. 최종 백업: artifacts/shared/agent-memory-final-backup-2026-05-27T061637Z/ (sha256: 104eea69). DROP 완료: agentcoreconversations/agentcoremessages/agentmemoryfactentries/agentmemorykv/agentmemorymessages/agentmemoryragdocuments/agentmemoryragobjects/agentmemorysteps/agentmemorysummary/agentmemorytexts (10 테이블). MySQL ABSENT: agent_memory DB 내 agent* 테이블 COUNT=0 확인. **Phase 2 agent* MySQL 완전 제거 완료. agent_memory DB는 web* 18 테이블만 잔존 (Phase 3 대상).**
```
