...
~~**부수 확인 필요**: `claude -p` 가 2026-06-15부터 Agent SDK 크레딧 풀에서 차감된다는 보고~~ →
**SPIKE-02 항목 4 에서 정정됨 (2026-09-03)**: 그 변경은 2026-05-14 발표됐다가 **2026-06-15 취소**됐다.
현재 `claude -p` 는 여전히 **구독 한도**에서 차감되며 별도 계량기는 없다 — 안내에 반영할 것이 없다.
근거·출처는 `./SPIKE-02-native-client.md` §4.
