...
function _paintConn(connected, listening, epoch, runnerStale, runnerBuild,
  ...
  if (!connected) {
    ...
    //
    // 종전에는 러너 기동 직후(능력 협상 중)의 **미확인** 구간이 「대기 중」(정상)으로
    // 보였다. 러너 원장의 초기값이 `true`(fail-open)였고 그 값이 그대로 하트비트에 실려
    // 나갔기 때문이다. 그 구간에 AI 가 실은 응답 불가여도 사용자는 협상이 끝날 때까지
    // (실측 ~200초) 그 사실을 듣지 못했고, 그 사이 보낸 질문은 침묵으로 남았다.
    //
    // 이제 러너는 3상태를 신고한다 — `true`(답을 받아냈다) · `false`(못 쓴다는 관측) ·
    // `null`(아직 확인되지 않음). **`null` 은 정상이 아니다.**
    //
    // ⚠ 「정상은 `=== true` 일 때만」으로 판정한다. 종전 형태(`aiReady === false` 만
    //   걸러내고 나머지를 정상으로)는 「모른다」를 정상으로 흘려보내는 그 구조 그대로다 —
    //   여기서 고치는 결함이 바로 그 형태였다. 구 러너(키 미신고)도 이 갈래로 오는데,
    //   그쪽은 실제로도 확인된 바 없으므로 「확인 중」이 참이다. 그리고 구 러너는 자기갱신
    //   으로 곧 신 러너가 되어 이 갈래를 스스로 벗어난다.
    //
    // `runnerStale` **뒤**에 두는 이유: 낡음은 확정된 사실이고 이것은 미확정이다. 확정된
  ...
  try {
    ...
    if (atStart === _launchAttempt) {
      _lastObserved = { attempt: atStart,
                        // ⚠ `ai_ready` 는 **`true` 여야** 성공이다 (TASK-20260903T200000).
                        //   종전에는 `!== false` 였고, 그래서 「아직 확인되지 않았다」(`null`)
                        //   가 곧 「연결 완료!」로 읽혔다 — 사용자는 그 확인을 받고 질문을
                        //   보냈고, 그 AI 가 실은 응답 불가여서 침묵을 겪었다.
                        //   지금은 모달이 확인이 끝날 때까지 폴링을 계속하고, 확인이
                        //   실패로 떨어지면 그 사유를 그대로 말한다.
                        ok: !!(b && b.logged_in !== false && b.ai_ready === true && b.listening
