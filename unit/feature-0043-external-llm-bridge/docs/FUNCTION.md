...
- AC-20260828T060000-external-llm-bridge-53: 브리지 대기 중 컴포저에 **중단 버튼이 뜬다**(입력이 비어
  있을 때). 누르면 대기 말풍선이 취소 안내로 바뀌고 **새로고침해도 유지**된다.
- AC-20260828T060000-external-llm-bridge-54: 취소는 `_cancel_bridge_tasks` **한 함수**가 판정한다 —
  `/api/cancel` · 재전송 supersede · 중단 버튼이 모두 그것을 부른다(술어 중복 0).
- AC-20260828T060000-external-llm-bridge-55: 미점유 task 는 삭제되고, 점유된 task 는
  `Status='canceled'` 가 된다. 취소된 task 는 `list_open_requests`·`wait_for_request` 에 나타나지 않고
  `claim_request` 로 집히지 않는다.
- AC-20260828T060000-external-llm-bridge-56: 취소된 task 에 대한 `submit_answer` 는 **409** 이고,
  그 답변은 대화에 전달되지 않는다.
- AC-20260828T060000-external-llm-bridge-57: 대기 중 새 질문을 보내면 같은 대화의 이전 대기 task 가
  supersede 된다 — 답변이 **질문 순서대로** 오고 옛 질문이 최신 문맥으로 답해지지 않는다.
- AC-20260828T060000-external-llm-bridge-58: `wait_for_request` 가 **취소에도 즉시 반환**하며
  응답에 `canceled_task_ids` 가 실린다. 도구 **개수는 변하지 않는다**(가이드 수 대조 통과).
- AC-20260828T060000-external-llm-bridge-59: `GET /api/ai/bridge_stream` 이 phase 전환과 도구 호출
  단계를 **답변 전에** 흘리고, **55초 상한**으로 정상 종료해 `active_streams` 가 0 으로 복귀한다.
  SSE 가 실패하면 폴링으로 폴백해 답변 도달성이 유지된다.
- AC-20260828T060000-external-llm-bridge-60: 러너가 `--workers N` 으로 동시 처리하고, 취소된 task 는
