...
## CHG-20260730T160000-ai-claude-feature-0016-meta-llm-edge-free — 개발용 메타데이터 LLM 의 로컬 gemma 강등 경로 봉인 (2026-07-30)
- 대상: `unit/feature-0007-bedrock-llm-provider/src/config/litellm_config.yaml`(edge-free `claude-haiku-4-meta`/`-meta-root` 별칭 신설 + fallback 체인)·`shared/config.py`(`AGENT_NODE_ANALYSIS_MODEL` 기본값 → `claude-haiku-4-meta`)·라이브 `.env` override 정합화(gitignored). 테스트 신규 `test_meta_llm_edge_free.py`(5) + 기존 `test_llm_env_naming.py` 계약 2건 갱신.
- 변경(사용자 결정 재확인 "로컬LLM은 특수목적으로만 사용하고(야간, 업무 외 탐색) 실제 개발용 작업은 계정으로 연결된 claude-code를 사용하도록 구성했습니다. 해당 구성과 정합하게 동작하도록 수정"): `llm_cluster_label`·`node_analysis` 는 애플리케이션 레벨 off-hours 강등은 타지 않지만(설계대로), **litellm fallback 체인이 `edge-fallback`(로컬 gemma)로 끝나** 두 claude 계정이 401/429 면 조용히 강등됐다. 이 산출물은 일회성이 아니라 **영구히 남는다** — 라벨은 멤버셋-해시 kv 캐시로 재사용되고, 능동 분석문은 시그니처에 섞여(cluster-signal-repair) 클러스터 구조 자체를 오염시킨다. 대화 답변의 `*-chat` 규약(2026-07-07 FR-edge-fallback: "명백한 실패처리로")을 그대로 확장해 **edge 를 배제한 `-meta` 별칭**(claude-corp → root 2계정, root 이후 폴백 없음)으로 이관했다. **오염 실측: 없음** — `cluster_label` 1,810회 전부 claude, `node_analysis` 의 `edge` 519회는 라우팅 분리 결정(2026-07-04) **이전**인 07-02 가 최근값. 즉 구조적 경로만 닫았다.
- 근거: 등급 **Minor**(라우팅 별칭 추가·기본값 변경. 스키마·API·인가 불변, 마이그레이션 0). **과잉 차단을 피했다** — 배경 insight 배치(`claude-haiku-4` → root → edge-fallback)의 야간·주말 gemma 강등은 사용자가 말한 "특수목적" 이므로 **그대로 유지**하고, 대화 `*-chat` 체인도 무변경이다(테스트가 양쪽을 잠금). edge 제거가 기능 정지를 만들지 않음도 확인: 두 계정 실패 시 `llm_cluster_label` 은 affix 라벨 폴백, `node_analysis` 는 미분석 유지 후 재시도(fail-soft 기존 경로). 검증: 신규 5 + 기존 갱신 2 · `make test` **3191 passed / 3 skipped / 0 failed** · ruff clean. 상세는 TASK.md `## 20260730T1600-meta-llm-edge-free`.
## CHG-20260730T175000-ai-claude-feature-0016-embed-throughput — 임베딩 워커 스로틀 해소(병목 이동분) (2026-07-30)
...
## CHG-20260731T030000-ai-claude-feature-0016-band-visual-fit — 육안검증에서 찾은 시각 부정합 2건 (2026-07-31)

| 파일 | 변경 |
|---|---|
| `unit/feature-0003-agent-web-ui/src/static/graph/graph-simgroups.js` | 싱글턴 흡수에서 `nm:` 예외 제거 — 멤버 1개짜리 밴드 소멸 (F1) |
| `unit/feature-0003-agent-web-ui/src/static/graph/graph-roleviz.js` | `_metaRoutineStyle.labelMaxWidth` 176 → `w - 10` (F2) |
