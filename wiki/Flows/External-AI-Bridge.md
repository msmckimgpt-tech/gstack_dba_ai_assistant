...
2. [상세](#2-상세)
     ...
     - [2.4 연결 — 길은 DQA 앱 하나다](#24-연결--길은-dqa-앱-하나다)
...
flowchart TD
    ...
    E["KB 검색 (pg_trgm 문자 후보)"] --> OK["게이트 비대상 — LLM 호출 없음<br/>(2026-09-07: 로컬 임베딩 bge-m3/ollama 폐기)"]
...
- 집행 지점은 chat 클라이언트 생성 chokepoint 2곳(`modules/llm._get_llm_client`, `agent_core` 의 직접 `OpenAI(...)` 생성)이며, `litellm_config.yaml` 의 계정 alias 14종은 주석 처리됐고 **2026-09-07 로컬 LLM 전면 폐기**로 마지막 활성 alias `titan-embed` 까지 제거돼 활성 `model_list` 는 **0건**이다(`model_list: []`).
- 따라서 [[../Architecture/Data-Flow|Data-Flow]] 의 `agent → bedrock-gateway → Bedrock` 구간은 **활성 모델 0개로 소진**됐다 — 게이트웨이 컨테이너 자체 철거는 결정 축이 다르다는 이유로 이월됐다.
...
> **2026-09-03 주 경로 교체 → 2026-09-07 단일 경로 확정.** 기본 진입점이 「명령 복사 → 터미널 붙여넣기」에서 **연결 프로그램**([[../Features/feature-0046-native-client|feature-0046-native-client]] · `DQA Connect`)으로 옮겼고, 2026-09-07 에 **「연결 준비」 버튼·터미널 1단계(OS 탭·명령·복사)·AI 지시문 두 갈래·토큰 경고가 두 화면(대화 모달 · `/ai/connect`)에서 전면 제거**됐다(사용자 결정 「그냥 전면 제거」). **접은 것이 아니라 지웠다** — 한쪽만 지우면 링크를 새 탭으로 연 사용자가 옛 화면을 보므로 함께 지웠고, 그 길이 막다른 길이 되지 않도록 **받기 안내**를 같은 자리에 둔다(서버가 실물을 확인해 `client_download` 를 낼 때만). ⚠ 대가: 앱을 아직 받지 못했고 서버가 배포본도 내놓지 못하는 사용자에게는 **연결할 길이 없다** — 사용자가 그 대가를 알고 고른 결정이다. ⚠ 서버는 여전히 `handoff`·`launch` 를 만들어 보낸다(화면이 그리지 않을 뿐 — 되돌릴 자리다). 정본: `unit/feature-0003-agent-web-ui/docs/FUNCTION.md` §「연결하는 길은 **DQA 앱 하나**다」.

```mermaid
flowchart TD
    A["사이드바 프로필 행 연결 칩<br/>또는 /ai/connect 페이지"] --> B["연결 모달 / 연결 페이지"]
    B --> C["창을 열 때 실행 URL 을 미리 받는다<br/>(_offerLaunch · 자격 = 서버가 아는 연결 이력 last_os)"]
    C --> K["[내 AI 실행]<br/>dqa-connect 스킴 딥링크<br/>(토큰 · 서버 주소 · CA 지문 · 러너 체크섬)"]
    K --> L["연결 프로그램이 열리고<br/>나머지는 그 창에서 끝난다"]
    B -.->|"앱이 없고<br/>서버가 실물을 확인했을 때만"| G["[DQA 앱 받기] — 받기 안내"]
    G --> K
    L --> H{"연결 성립?"}
    H -->|예| I["칩이 '연결됨' 으로 전환<br/>모델·추론강도 목록이 새로고침 없이 나타남"]
    H -->|아니오| J["연결 단계 체크리스트가 '어디서 막혔는지' 를 말한다<br/>(판정은 서버 한 곳 · 연결 축과 AI 축을 갈라 보고 ·<br/>서버 연결은 멀쩡한데 '연결 확인 실패' 로 보이지 않게)"]
```

체크리스트는 `연결 정보 발급` → `내 컴퓨터가 듣는 중` → `답할 AI 있음` → `첫 답변 받음` 각 행을 **다음 행동과 함께** 낸다(`action` 이 없는 행은 할 일이 없는 행이다). 판정을 서버 한 곳에 두는 이유는 프런트가 `connected && listening` 같은 곱을 스스로 조립하면 **판정이 두 벌**이 되고, 축이 하나 늘 때 화면과 서버가 갈리기 때문이다. ⚠ 연결 프로그램 다운로드가 없으면 클라이언트를 **권하지 않는다** — 없는 다운로드를 안내하면 사용자는 안내받은 대로 갔다가 막힌다.

아래 방법 순서는 **2026-09-07 에 화면에서 제거된 AI 지시문 표면**의 이력이다(위 §2.4 — 두 화면 모두 지시문을 더 이상 발급·표시하지 않는다). 그 순서는 "간단해 보이는 순" 이 아니라 **사람을 다시 부르지 않는 순**이었다.
...
flowchart TD
    ...
    C -->|양쪽 다| D["토큰 발급 순서(Id) 비교<br/>= 연결 토큰이 발급된 순서<br/>(2026-09-07: 「연결 준비」 버튼은 사라졌고 토큰은 창을 열 때 발급된다)"]
