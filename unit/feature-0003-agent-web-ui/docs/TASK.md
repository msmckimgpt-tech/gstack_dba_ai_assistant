...
- 접근: 캔버스 그룹 키의 **fam(구분자 뒤 토큰)** 을 선택 신호로 상세 렌더까지 실어 보내고, 렌더 직후 같은 fam 의 패널 헤딩으로 국소 스크롤 + 1.8s 도착 강조. 캔버스 키(`<comboId>|fam`)와 패널 키(`panel:<name>|fam`)는 `_metaSimGroups(schemaId, …)` 의 네임스페이스가 달라 **fam 만 대조**한다.
- 완료 판정(acceptance):
  - [x] [AC-CPS-1] 캔버스 컨텐츠 카테고리 좌클릭 시 패널이 같은 카테고리 헤딩으로 스크롤되고, 헤딩이 sticky 이력 바 아래(+6px)에 놓인다.
  - [x] [AC-CPS-2] 우클릭 메뉴 '소속 스키마 상세' 도 동일하게 그 카테고리 위치로 이동한다.
  - [x] [AC-CPS-3] fam 미전달 경로(스키마 카드 클릭·뒤로/앞으로 이력 복원)는 기존 동작(스크롤 미개입·이력 scrollTop 복원)을 유지한다.
...
- 검토 결론: **AA 는 이미 활성**이라 해법이 아니다. 원인은 서브픽셀 폭이며, MSAA 는 커버리지를 양자화할 뿐이다. → hairline 처리 채택(사용자 승인: "hairline + base 재조정").
- 완료 판정(acceptance):
  - [x] [AC-GEH-1] 1물리픽셀 이상 무보정(A13).
  - [x] [AC-GEH-2] 서브픽셀 폭 승격 + alpha 보상·단조(A13).
  - [x] [AC-GEH-3] 임계 = 1/dpr, 고DPI 검증(A13).
  - [x] [AC-GEH-4] 동일 줌 4× 확대 대조에서 점선 파편 → 연속 실선(라이브 육안) · 1px 구멍 740→635.
- 상태: **완결** — 라이브 검증·원복 완료.
...
- 적발·수정: F1 전체보기 관계선 비가시(서브픽셀) → 화면 기준 최소 굵기 + alpha 바닥 / F2 곡률 상한이 카드 높이와 동일 → 26 결속 / F3 분석완료 halo 대비 위계 역전 → 관계선 강화로 균형.
- 완료 판정(acceptance):
  - [x] [AC-GEL-1] 전체보기 관계선 대비 개선 — 라이브 실측 27→51 · 37→55, 밴드 간 흐름 육안 판독 가능.
  - [x] [AC-GEL-2] 줌아웃 보정이 굵기 서열 보존 + zoom 2 무보정. (A11 4건)
  - [x] [AC-GEL-3] 곡률 상한 26 < 카드 높이 44 · 행 간격 52 — 확대 시 카드 여백 안에 머묾(육안).
  - [x] [AC-GEL-4] alpha 누적 단조 증가·미포화(1겹 0.58 → 4겹 0.97).
- 상태: **완결** — 라이브 검증·개선·재검증 완료(주입 QA 후 이미지 원본 원복).
...
## TASK-20260714T015432-step-scroll-preserve — 실행 단계 폴링 갱신 시 펼친 "결과 보기" 스크롤 보존 (Minor §12.3 — feature-0003 프론트 단독. /_template:entry arg-given dispatch)
- 트리거(사용자): "서비스 내 assistant에게 요청함으로서 내부적으로 단계 를 진행할 때, 각 단계가 갱신될때마다 펼쳐둔 '결과 보기' 의 스크롤이 초기값으로 되돌아가는 이슈가 확인되었습니다. 단계 진행에 따른 기존 항목들에 대한 스크롤이 보존될 수 있도록 구성해주세요." REQ-20260714T015432-step-scroll-preserve.
- [x] 근본원인 확인: 라이브 폴링(`refreshStepSidePanel`@5770 / `pollProgress`→`applyProgressPayload`→`renderProgress`)이 컨테이너를 `innerHTML=""` 로 통째 재렌더 → 펼침 상태(`state.stepResultExpanded`)는 `_stepResultKey` 로 복원되나 **스크롤 위치는 미복원**(결과 표 `.result-table-wrap` `max-height:min(60vh,460px)`·미리보기 `.step-result-preview` `max-height:120px`·외부 목록 `.step-side-panel-body`/`.progress-steps` `overflow-y:auto` 모두 0 리셋).
- [x] 위험도 Minor(§12.3): 비파괴 프론트 UX, 인증/인가/데이터/스키마/마이그 무관, 단일 파일(app.js)+테스트. 단일 파일이라 §7.1 plan 생략 가능.
- [x] 수정 `static/app.js`: ① `buildStepDetailEl` 결과 wrap 에 `data-step-result-key=stepKey` 부여(AC-SSP-1). ② 제네릭 헬퍼 `_snapshotStepResultScroll`/`_restoreStepResultScroll`(컨테이너 무관, `[data-step-result-key]` 비-hidden 매칭, top/left 캡처·복원, 접힘 skip). ③ `_renderStepSidePanelBody`: 재렌더 전 스냅샷(외부 `prevScrollTop`+내부 결과) → 재렌더 후 복원(내부 + 외부: 하단추종=최하단 / 미추종=이전 위치 유지). ④ `renderProgress`: 동일 규약을 `progressStepsEl` 에 적용(AC-SSP-2).
...
- [x] verify-completion(pre-commit) → commit → PR #798 → 머지(main 8d1285d0) → web 재배포(deploy_scope: included, deploy-web --web-only soak PASS) → **POST-DEPLOY PB-0008 라이브 재검증**: 재배포된 graph.css(스탬프 d5f26a416089)에서 `.amg-help-overlay` 파싱 복구·`position:absolute`·팝업 카드 canvas-wrap 중앙정렬(dx:0/dy:6)·줌 컨트롤 미겹침(card_overlaps_zoom:false)·스크린샷 육안·pageerror 0 ✓. worktree `ai/claude/feature-0003-graph-entry-help-postdeploy`(base main 1f705a9e). 재배포 자산 최종 확인 = postverify(ai/claude/feature-0003-graph-help-overlay-postverify).

## TASK-20260715T025509-doc-sync-rn-0715 — 07-14 머지분 릴리즈노트 정합(메시지 편집 Phase 1+2 완성·작업화면 애니메이션/계정탭·그래프 첫입장 도움말/카테고리밴드 우클릭/AI분석 권한분리·결과보기 스크롤·답변 정확도) (doc_sync, 비-정책 콘텐츠 doc, 2026-07-15)
- 트리거: `/_dqa:doc_sync ultracode`(스케줄·무인, cron wrapper v3 — landing/배포 위임). 직전 릴리즈노트 블록(797a83df @ 07-14 03:09 — generated 07-13, 07-13 오후 델타) 이후 07-14 주간 main 병합 user-facing 델타 정합. releases 배열 head 에 **date "2026-07-14" 새 블록 prepend**(10항목) + generated 07-13→07-14. 기존 블록(07-13 이하 28개) 무삭제·무변형(총 29 블록).
- [x] 신규 07-14 블록 10항목: [work] ① new 메시지 편집(feature-0019 Major — 1:1 단순수정('편집됨')/요청사항 수정=분기 재답변 `< n/m >` 페이징, 그룹·공유 단순수정만·@assistant 잠금) ② new 작업 화면 애니메이션 효과 설정(복원+UI 탭 3옵션) ③ improved 프로필 '계정' 탭 세분화(계정/알림/UI/사용 내역) ④ improved 첨부 수정요청 시 새 버전 파일 전달 ⑤ improved SQL Server 여러 DB 검색(cross-DB) ⑥ fixed '결과 보기' 스크롤 보존(세로·가로) / [admin] ⑦ new 그래프 첫 입장 도움말 팝업(중앙 배치·중간버튼 커서) ⑧ new 카테고리 밴드 우클릭 전용 메뉴 ⑨ improved 'AI 능동 분석 실행' 권한 분리(기존 관리자 무영향) / [common] ⑩ improved 답변 정확도·안정성 개선(첨부↔실데이터 대조·부분 미리보기 단정 방지).
- [x] **사용자향 평이화**: feature-id·§번호·PR#·테이블/함수명·마이그·권한키(metadata.graph.analyze 등)·라이브러리명 등 내부표현 누출 0(node vm 구조검증 leak 스캔 0). 화면 체감 변화만 사용자 언어로.
- [x] **제외(비-노출)**: feature-0020 무중단 배포 커버리지(내부 운영)·feature-0016 §82 그래프-sync flock 가드·cluster_label target(내부 운영/관제)·@@ 시스템변수 과차단(a206ea36 — 07-13 블록 기존 항목 '정상 조회 과차단 수정' detail 이 '설정 확인 조회'로 이미 포괄 → 익일 중복 회피)·ROUTEMAP/ANCHOR/POST-DEPLOY 기록(내부).
- [x] **cache-buster**: 소스 `?v=dev` 고정 placeholder 유지 — index/admin.html 수기 bump **안 함**(§13.1 ITEM-09 what#3 — Dockerfile `inject_asset_stamp.py` 배포 시 static content-hash 주입, deploy-web `asset_stamp_verify` 가 baked `?v=dev` 잔존 하드 차단). release-notes-data.js 내용 변경만으로 전역 content-hash 변화 → wrapper 재빌드 시 서빙 토큰 자동 갱신. **system-prompt 의 '수동 bump' 지시는 07-12 ITEM-09 이전 모델 기준이라 부적용**(수동 변경 시 게이트 무력화·content-hash 불변). 직전 doc-sync-rn-0713/0714 동일 판단.
...
- [x] verify-completion → commit → PR #811 머지(f6cb0b14) → web 재배포(soak PASS) → **POST-DEPLOY PB-0008 PASS**: 묶음 8개(12후보) 그룹핑·전체 승인 마스터·일부 해제 indeterminate/힌트·등록 count/disabled 라이브 실증(pageError 0). 이 검증에서 카드 flex 붕괴 적발 → 20260715T1132-flex-fix 로 해소. worktree `ai/claude/feature-0003-enum-review-bundle`(base main 7f169007). REV/CHG-20260715T105337-enum-review-bundle.
...
- 검증: [x] `node --check`(module) PASS · [x] §18.8 적대 리뷰([SUBAGENT] 6축 diff 도입 결함 0; #1 GB-키 fam-divergence caveat → 타깃을 첫 멤버 노드로 전환해 근본 제거, REV-20260716T012805) · [x] verify-completion PASS → PR #838 머지(main d2c72fdc) → web 재배포(deploy_scope: included, soak PASS) → [x] **POST-DEPLOY PB-0008 라이브 PASS**(DK dk_data_release_main: 66그룹 전부 data-pan-key=첫 멤버 노드; 헤딩 hover→카메라 팬 — "NPC 콘텐츠"↔"게임 콘텐츠 조회" 서로 다른 영역으로 이동·미니맵 뷰포트 이동·행 hover/클릭 불변·pageerror 0). POST-DEPLOY CHG-20260716T015146-graph-cluster-detail-group-hoverpan-postverify.
...
## TASK-20260716T010501-doc-sync-rn-0716 — 07-15~16 머지분 릴리즈노트 정합(관계도 우클릭/상세목록/드래그·ENUM 묶음 큐·권한 원자화/카테고리 계층·'AI 추론' 콘솔·답변 정확도) (doc_sync, 비-정책 콘텐츠 doc, 2026-07-16)
- 트리거: `/_dqa:doc_sync ultracode`(스케줄·무인, cron wrapper v3 — landing/배포 위임). 직전 릴리즈노트 블록(0958451b @ 07-15 03:04 — generated 07-14, 07-14 델타) 이후 07-15~16(01:00) main 병합 user-facing 델타 정합. releases 배열 head 에 **date "2026-07-15" 새 블록 prepend**(7항목) + generated 07-14→07-15. 기존 블록(07-14 이하 29개) 무삭제·무변형(총 30 블록).
- [x] 신규 07-15 블록 7항목(admin 6·common 1): ① fixed/admin 관계도 우클릭 메뉴 대상 정합(ctxmenu hittest/band-priority/content-category) ② improved/admin 관계도 상세 목록 함수·프로시저 포함+카테고리별 접기(cluster-detail routines/cap/fulllist/collapse) ③ fixed/admin 관계도 드래그 시 연결선 즉시 추종+부드러움+도움말 줄바꿈(edge-follow-drag/drag-perf/help-wordbreak) ④ new/admin 코드값(ENUM) 검토 큐 묶음 승인·일괄 등록(enum-review-bundle) ⑤ improved/admin 계정·역할 권한 화면 묶음별 '접근'+추가/수정/삭제 원자화(perm-category-hier/atomic-split) ⑥ improved/common AI 답변 정확도·안정성(첨부↔실데이터·이름 대소문자/cross-DB·자체 점검·응답 안정성) ⑦ new/admin 관리 콘솔 'AI 추론' 화면(feature-0021).
- [x] **사용자향 평이화**: feature-id·§번호·PR#·테이블/함수명·마이그·권한키(console.reasoning.read 등)·라이브러리명(PixiJS 등)·내부표현(red-team/choke-point/Bedrock 등) 누출 0(node vm 구조검증 leak 스캔 0). 화면 체감 변화만 사용자 언어로.
- [x] **제외(비-노출)**: 각 기능 *-postverify 머지(배포 후 검증 기록·문서)·#805 friction-ledger(내부 원장)·#833 probe-throttle/#826 llm-probe 내부 안정성(⑥ 안정성 포괄)·feature-0021 red-team 백엔드 정확도 메커니즘(사용자 비가시 — 콘솔 'AI 추론' 표면만 ⑦ 노출).
- [x] **cache-buster**: 소스 `?v=dev` 고정 placeholder 유지 — index/admin.html 수기 bump **안 함**(§13.1 ITEM-09 what#3 — Dockerfile `inject_asset_stamp.py` 배포 시 static content-hash 주입, deploy-web `asset_stamp_verify` 가 baked `?v=dev` 잔존 하드 차단). release-notes-data.js 내용 변경만으로 전역 content-hash 변화 → wrapper 재빌드 시 서빙 토큰 자동 갱신. **system-prompt 의 '수동 bump' 지시는 07-12 ITEM-09 이전 모델 기준이라 부적용**(수동 변경 시 injector placeholder 매칭 무력화·해당 2파일 캐시무효화 상실). 직전 doc-sync-rn-0713/0714/0715 동일 판단.
- [x] 검증: `node --check release-notes-data.js` PASS · vm 파서 구조검증(30 releases·releases[0].date=2026-07-15/items=7[admin 6·common 1]·releases[1].date=2026-07-14 보존[10항목]·스키마 type/area/title·누출 스캔 0). item #6 area='common' 은 직전 07-14 블록 ⑩ 답변정확도 area='common' 선례와 정합(직접 재확인).
...
- [x] PB-0008 POST-DEPLOY 라이브 실측(2026-07-22, 배포본 22c3b9cb·PR #873): **A(막대 범위화)·B(클릭 위치 비례) 라이브 PASS** — rail 14 뱃지 height% 비례·상단클릭 scrollTop 107/하단클릭 734. C 자동 페이징은 환경 대화 모두 <20 메시지(hasMoreHistory=false)라 미트리거(코드/리뷰 검증+회귀없음). **gap**: 20개 미만 대화 4배 상한 미적용(서버 페이징 재사용 트레이드오프 — 후속 검토 여지). evidence/point-rail-range-window-live.png.
- [x] C 강화(DOM 윈도잉 — 사용자 후속 요청 "긴 대화 일부만 로딩 검증"): 서버 페이징만으론 20개 미만 긴 대화(conv 14개·높이 뷰포트 20배)가 전부 렌더되던 gap 해소. `state.renderCount`(최근 N개)로 `renderMessages`·`renderMessagePointRail` 이 최근 창만 DOM 렌더(뱃지도). 대화 로드 8개→높이 4배까지 확장(상한)·부족+hasMoreHistory 면 서버 페이징(하한)·최상단 스크롤 시 DOM 창 확장→서버 로드·검색/캘린더 창밖 점프 `_ensureMessageRendered` 확장. 적대리뷰 발견1(절대 인덱스 복원 `_windowBase` — 피드백 nl_question/투표/공유 idx)·2(floor 칩)·5(검색 optimistic)·6(주석) 반영, 3(live-poll 슬라이딩)·4(연쇄 확장 total 상한) known-limitation.
...
- [x] 배포(무중단 web-only, 2026-07-22): PR #887 → main **622b7434** → `deploy-web.sh --web-only`(web-a/b soak PASS). 서빙 자산 반영(share.js `buildShareBranchPager`·app.js `branchView`). 워커/마이그 무관.
- [x] POST-DEPLOY 라이브 검증(양 surface, 배포본 622b7434, 대화 20260722015229-79da15cb=is_group+활성 share): **공유-링크(익명 curl)** — 편집 메시지에 pager 메타(1/2, sibs[1269,1313])·`branch_view=1313` 읽기전용 전환·`branch_view=999999999` **fail-closed**(활성 1269 폴백). **인앱 그룹(인증 curl)** — pager 메타(window-scoped)·`branch_view` 전환·**active_leaf 불변**(branch_view 후 기본 히스토리 원래 활성 1269 유지 = 읽기전용·INV-4). **Windows-browser 시각 스크린샷 미수행** — 브리지 다운(Windows Chrome 종료·재기동 실패), §15.4.1 escape(브리지 불가 시 문서화)로 API end-to-end 실측 + 단위 12 PASS 로 보완. 시각 확인 잔여(사용자 브라우저 또는 브리지 복구 후).

## 20260723T010501-doc-sync-rn-0723 — 릴리즈노트 07-22 블록: 대화 UI 안정화·탐색 (doc_sync maintenance, 비-정책 doc-only, 무인 cron)
- [x] `static/release-notes-data.js` releases head 에 "2026-07-22" 블록 prepend(6항목: fixed/work 3 [재답변 후 내 메시지 소실 복구·'수정' 창 글자 비가시·말풍선 '여기부터/여기까지 공유' 회귀] + improved/work 3 [상단 '위에 더 있음' 흐림 신호·오른쪽 위치 막대 클릭 이동+로그 윈도잉·공유/그룹 편집 버전 읽기전용 페이징]) + generated 2026-07-21→2026-07-22. 렌더 로직·cache-buster(`?v=dev` 빌드 자동주입) 무변경.
- [x] 검증: `node --check` PASS · vm 구조검증(33 releases·07-22 head 6항목[fixed 3·improved 3, area=work]·07-21 보존·상위 8블록 내림차순[말단 '이전' sentinel 정상·pre-existing]·스키마 type/area/title/detail·누출0). 근거 정본 = 07-22 머지 owning POST-DEPLOY 커밋(4edb7701/5dbe7992·0ebbac8e/a8fb33d1·7a5b092a/94e2cadd·0db6fc36/109befe1·a92492e6/a70f8382·f0a32980/20562ff9) + git log cfa647df..HEAD.
- [x] operational gate(feature-0003): TASK#2·MODIFY#3·FUNCTION#4·REVIEW#9([SKIPPED:non-policy-doc])·TEST#13(Windows-browser 미수행 사유). 무인 cron — 로컬 commit 까지만, push/merge/deploy=wrapper(v3). 적대검증: ULTRACODE wf_4ab4d814 — RN 6 INCLUDE holds·feature-0023 등 8 EXCLUDE 사유 정확 confirmed.

## 20260723T024724-paging-scroll-preserve — 브랜치 버전 페이징 시 스크롤 위치 보존 (Minor §12.3 — feature-0003 web/UI 프론트 단독, cross-cut 정본 feature-0019. /_template:entry arg-given dispatch)
- [x] 진단(사용자 신고): 편집 버전 `< n/m >` 페이징 시 스크롤이 맨 아래로 튀어 연속 페이징 UX 번거로움. RC = `renderMessages()`(app.js:4576)가 매 재렌더마다 `scrollTop=scrollHeight`(맨 아래) + 비-append loadHistory 는 `_applyRenderWindowSoon()`(rAF 재렌더)로 다시 맨 아래. 공유 뷰(share.js `render`)도 `#shareMessages` 통째 교체로 문서 스크롤 튐.
- [x] 수정(app.js): `loadHistory({preserveScroll})` 신설 — 재렌더 전 `messageLogEl.scrollTop` 저장, 재렌더 후 `_applyRenderWindowSoon` 생략 + 저장 위치 rAF 복원(clamp). `_pageBranch` 그룹=`loadHistory({branchView,preserveScroll})`, 1:1=`refreshWorkspace(cid,{preserveScroll})`(→ loadHistory 로 전달). append/일반 로드 경로는 무변경(기존 맨-아래·prepend 보존).
...
- [x] PB-0008(POST-DEPLOY, 2026-07-23 라이브 PASS): PR #891 → main 2cdb7907 → deploy-web --web-only(soak PASS). Windows Chrome '간단한 덧셈 계산' 3→4: rendered 3→**6**(전체 렌더), pager **'4/4' 유지**(소실 안 됨), 브랜치 메시지 뷰포트 위치 **298px→298px 동일**(scrollTop 4985 유지, maxTop 11286 scrollable, atBottom=false)·스크린샷 육안(pager 화면 중앙). verdict PRESERVED.

## 20260724T010501-doc-sync-rn-0724 — 릴리즈노트 07-23 블록: 대화 폴더·탐색·관리 콘솔 (doc_sync maintenance, 비-정책 doc-only, 무인 cron)
- [x] `static/release-notes-data.js` releases head 에 "2026-07-23" 블록 prepend(7항목: new/work 2 [대화 폴더·폴더별 AI 지침] + improved/work 2 [대화목록 월·연 날짜 묶음·우클릭 메뉴] + fixed/work 1 [메시지 버전/긴 이전 대화 페이징 스크롤 튐] + improved/admin 2 [AI 답변 다듬기 전·후 보기·DB 전체 AI 자동 분석 완결성/재분석]) + generated 2026-07-22→2026-07-23. 렌더 로직·cache-buster(`?v=dev` 빌드 자동주입) 무변경.
...
## 20260727T010501-doc-sync-rn-0727 — 릴리즈노트 2026-07-24 블록: 대화·공유·관리 UX (doc_sync maintenance, 비-정책 doc-only, 무인 cron)
- [x] `static/release-notes-data.js` releases head 에 "2026-07-24" 블록 prepend(8항목: improved/work 4 [SQL 코드블록 구문 강조·'새 폴더' 폴더 아이콘 버튼·공유 링크 진입 시 최신 메시지 스크롤·공유 링크 위치 막대] + fixed/work 2 [답변 CSV 실제 내려받기·'요청사항 수정' 재답변 모델/추론강도 승계] + improved/admin 1 [메타데이터 검토 목록 datasource 스코프+등록시각] + fixed/admin 1 [관계도 이모지 색 렌더]) + generated 2026-07-23→2026-07-24. 렌더 로직·cache-buster(`?v=dev` 빌드 자동주입) 무변경.
