...
## REV-20260728T170500-graph-label-lod [CODEX:frontend-render+lod] — PASS-WITH-FIXES (P1/GATE 0 · P2 2건 전량 in-cycle 흡수)
- 대상: CHG-20260728T1604-graph-label-lod (`graph-state.js` 라벨 하한·밴드 양자화·`_metaPerf` / `graph-core.js` `_metaApplyLabelLod`·밴드 훅·상태줄 마커 / `graph-renderer-pixi.js` 라벨·draw 계측 / 신규 `test_g6build_labellod.js`).
- 방법: **codex-review(`codex review --uncommitted`, codex-cli 0.145.0)** — §18.8 적대 검증. `[CODEX:*]` 는 §18.4/§18.9 의 check #9 accepted verdict. 본 세션은 `Agent` 툴 사용이 제한된 환경이라 subagent panel 대신 codex 를 택했고, 이 변경은 **repo 전체 문맥**(라벨 폰트 상수의 실제 사용처·기존 LOD 계약과의 겹침)을 봐야 판정되므로 bundle-only reviewer 보다 repo 접근 리뷰어가 적합하다. 반박 대상으로 band-invariant·정보 손실 0·밴드 양자화·헤더 우대·계측 무분기 5개 계약과 6개 의심 축(누락 경로·클램프 경계·`_makeText` 래핑·`_labelStat` stale·col-lod 겹침·테스트 기준 약화)을 명시 제시.
- **P1(GATE) 0건** — band-invariant·`_makeText` 래핑(반환값·BitmapText/Text 판별·예외 경로 불변)·`_labelStat` 초기화 시점·col-lod 겹침 처리·`collod` T3 줌 이동의 정당성에는 반박이 성립하지 않았다.
- **CONFIRMED(P2 #1, 수정)** — *상태줄 오독-가드가 배지-단독 억제 구간에서 뚫린다.* 스키마 카드는 제목(`_cardLF`≈13)보다 배지(`_cardBF`≈10) 폰트가 작아 **제목은 살아 있고 배지만 억제되는 줌 구간이 실재**한다(zoom 0.45 부근: 12×0.45=5.4 유지 / 10×0.45=4.5 억제). 그런데 마커 게이트가 `dropped` 만 보므로 그 구간에서 카드 개수 정보가 **마커 없이 사라진다** — §57 오독-가드의 재발이며, 이 cycle 이 '이름표' 항을 추가한 목적 자체가 뚫리는 결함. **수정**: 게이트를 `dropped > 0 || badgesDropped > 0` 로 확장(마커 항은 '이름표' 하나로 유지 — 항을 늘리면 상태줄이 두 줄로 길어진다는 기존 결정 보존). 신규 테스트 Section H(H1~H5)가 게이트 진리표 + '배지 폰트 < 제목 폰트' 불변식을 고정.
- **CONFIRMED(P2 #2, 수정)** — *정수 `ceil` 밴드 양자화가 소수 폰트의 임계 교차를 놓친다.* 실사용 라벨 폰트에는 정수 외에 `10.5`(컨텐츠 그룹 헤더, graph-core.js:626)·`11.5`(제품 개요, graph-core.js:1033)가 있다. `ceil(MIN/z)` 는 정수 경계에서만 바뀌므로 헤더 하한 3.2 와 10.5px 의 교차점 z*≈0.30476 을 지나도 밴드가 동일(0.3048→`17/11` = 0.3040→`17/11`)해 **rebuild 가 걸리지 않고, 그 10.5px 헤더가 판독 하한 밑에서 계속 렌더된다** — 다른 밴드가 우연히 바뀔 때까지 이 cycle 이 없애려던 붕괴 라벨이 남는다. **수정**: 밴드를 **반포인트(0.5) 격자**로 양자화(`ceil((MIN/z)/0.5)*0.5`) — 0.5 배수인 모든 실사용 폰트에 대해 억제 집합 변화와 1:1 대응하고, 런타임 계산 폰트(`Math.round` 정수)도 정수 격자를 포함하므로 함께 커버된다. 수정 후 같은 두 지점이 `16.5/10.5` vs `16.5/11` 로 갈라진다. 신규 테스트 F5·F6 이 두 소수 폰트의 교차를, F7 이 '격자 세분화가 통상/극단 구간 안정성(F1/F3 무의미 rebuild 차단)을 깨지 않음' 을 고정.
  - **수정 전 재현 확인**: 두 지점의 밴드를 구/신 양자화로 직접 대조해 구 구현이 **MISS**(동일 밴드)·신 구현이 **detect**(상이 밴드) 임을 실측했다 — 테스트가 결함 자체를 잡는지 검증한 것이며, 통과만 보고 넘기지 않았다.
  - PB-0008 이 기록한 실측 band `9/9`(zoom 0.7713)는 격자 변경 후에도 **불변**(재계산 확인)이라 TL.8 증적의 유효성은 유지된다.
- **회귀 확인**: 수정 후 `test_g6build_labellod.js` **36 PASS**(28 → +F5·F6·F7·H1~H5) + 회귀 16 스위트 **502 PASS** + PixiJS 어댑터 **190 PASS** = **728 PASS / 0 FAIL** · `node --check` PASS(3 파일).
- **정직 표기 — 수정이 도입한 새 표면은 PRE-LANDING PB-0008 범위 밖이다**: TL.8 실촬·계측은 수정 *이전* 코드로 수행됐고, (a) 배지-단독 억제 구간의 마커 노출 (b) 반포인트 밴드 경계의 rebuild 는 그 probe 가 통과한 줌(0.126/0.325/0.771)에서 발화하지 않는다. 두 항목을 **POST-DEPLOY 1-probe(TL.9)에 명시 추가**했다. 수정은 억제 판정(`fs * z < minPx`)을 건드리지 않고 밴드(=rebuild 시점)와 마커 게이트(=추가 발화만, 제거 없음)만 바꾸므로 TL.8 의 ②③⑤⑥ 결과는 계약상 그대로 성립한다 — 다만 그 성립을 '실촬로 재확인' 한 것은 아니므로 POST-DEPLOY 로 넘긴다.
## REV-20260728T161940-ai-claude-feature-0016-routine-column-edges [SKIPPED:session-policy-no-subagent] — 사용 관계선 컬럼 단위 연결 자체 적대 검토
- **§18.8 subagent 패널 미수행 사유**: 본 세션 사용자 환경 정책이 Agent tool 호출을 명시적으로 금지한다("Do not call the AgentTool unless the user requested it"). 같은 사유의 선례가 feature-0003 REPORT 의 §83~§87 cycle 에 연속 기록되어 있다. 대신 자체 적대 검토 H1~H6 을 수행하고 그 중 **2건은 테스트가 실제 결함으로 확증**했다.
- **H1 (확증·수정) — "접힌 상태에서는 기존대로" 계약 위반**: 초판 구현은 `ref_columns` 의 read/write 별로 테이블 승격선을 만들어, 컬럼이 하나도 렌더되지 않은 접힘 상태에서 선이 1→2개로 갈라졌다. 사용자 요구의 전제(접힘 무변경)를 정면으로 깨는 회귀. 헤드리스 ①-1/①-4 가 적발 → **렌더된 컬럼이 0이면 분해 자체를 포기**하고 기존 단일선 경로로 흐르도록 수정.
- **H2 (확증·수정) — 부가 기능이 본 경로를 죽일 수 있음**: `_fetch_columns` 의 `cursor()` 획득이 try 밖이라, 컬럼 조회 실패(권한·뷰 부재)가 상위로 전파돼 **그 스키마의 routine upsert 전체가 무산**될 수 있었다. pytest `test_fetch_columns_failure_is_nonblocking` 이 적발 → cursor 획득을 try 내부로 이동(컬럼 승격은 부가라 절대 본 경로를 막지 않는다는 원칙 명문화).
- **H3 (반증) — 컬럼 오귀속 환각**: alias 재사용(`FROM T_User a … FROM T_Order a`)은 흔하고, 그대로 두면 컬럼이 엉뚱한 테이블에 붙어 "없는 관계"를 사실처럼 보여준다. → `_alias_map` 이 **같은 alias 의 다른 테이블 바인딩을 감지해 그 alias 를 통째로 폐기**한다. `dbo.T_User` 의 `dbo` 처럼 스키마 qualifier 가 alias 로 오인되는 경로도 alias map 부재로 자동 폐기. 테스트 `test_ambiguous_alias_dropped_entirely`·`test_schema_qualifier_is_not_an_alias`.
- **H4 (반증) — 성능 회귀**: 컬럼 인벤토리 조회가 대형 스키마(8K 테이블)에서 비용이 될 수 있다. → introspect 를 2-pass 로 나눠 **실제 참조로 채택된 테이블에만** `INFORMATION_SCHEMA.COLUMNS` 를 1회 질의(cap 400)하고, routine 이 없거나 참조 테이블이 없으면 **질의 자체를 하지 않는다**. 프론트 측 케이스 보정 인덱스도 `ref_columns` 를 가진 사용선이 있을 때만 lazy 구축.
- **H5 (반증) — 데이터 이행 위험**: `cols` 는 jsonb 안 additive 키라 마이그레이션이 없고, `referenced_tables` 가 바뀌면 기존 `IS DISTINCT FROM` 가드가 다음 sync 에서 자동 갱신한다. AGE 쪽은 `sync_routine` 이 매번 `ROUTINE_USES` 를 전량 회수 후 재-MERGE 하므로 stale `ref_columns` 잔존 경로가 없다. 값이 없으면 속성을 아예 SET 하지 않아 기존 엣지와 동치(무회귀).
...
## REV-20260729T172000-graph-detail-columns-postdeploy [SKIPPED:non-policy-doc]
- Related TASK: feature-0016-metadata-graph
- Reason: changed paths are docs/evidence only outside policy-doc list — POST-DEPLOY 라이브 검증 기록(실행 코드 0줄). 검증 대상 코드의 적대검증은 같은 cycle 의 REV-20260729T065900-graph-detail-columns [CODEX:frontend-async+empty-state] 가 담당했고(P2 2건 in-cycle 흡수), 그 코드는 이미 main 머지·배포됐다.
- Deploy approval: `deploy_scope: included`(FIRST_REQUEST.md 전역, 2026-06-11 사용자 결정) — PR #1056 머지 후 자동 배포. 첫 배포 직전 1줄 표면화 이행.
...
## REV-20260731T011000-label-canon [CODEX:cross-schema-label-canon] — PASS-WITH-FIXES

도구: `codex exec --sandbox read-only` (§18.8.2 tool-restricted carve-out). 대상:
`git diff --cached -- shared/ unit/feature-0002-agent-core/`.

**P1 2건 · P2 1건 — 전부 in-cycle 수정.**

| # | 등급 | 지적 | 처리 |
|---|---|---|---|
| 1 | **P1** | 요약 캐시 미스 판정이 `l1_version`+`evidence_version` 만 봐서, **이미 존재하는 요약**은 라벨이 바뀌어도 재생성되지 않는다. 생성 시점을 통일 이후로 옮긴 것은 신규분만 고친다 → `cluster_summaries.label` 이 옛 어휘로 영구 잔류 | **수정.** `_summary_cache` 가 `label` 을 함께 읽고, miss 판정과 UPSERT `WHERE` 절에 라벨을 신선도 축으로 추가. 회귀 테스트 `test_cache_miss_when_label_changed` |
| 2 | **P1** | 라벨 kv 캐시에 통일 결과를 저장하지 않아, 통일이 실패·비활성이면 pass 간 라벨이 되돌아갈 수 있다 | **설계 의도 유지 + 관측성 보강.** kv 는 **LLM 원본**만 담는다(통일 결과를 심으면 기능을 꺼도 되돌아가지 않고, kv 가 "LLM 이 뭐라 했나" 를 더 이상 답하지 못한다). 통일은 매 pass 재계산되는 **결정적 파생 뷰**라 정상 경로에서는 흔들리지 않는다. 실패 시 1-pass 원본 라벨 기록 후 자연 복구되며, 그 창을 놓치지 않도록 로그를 `debug`→`warning` 으로 올리고 동작을 주석에 명시 |
| 3 | P2 | docstring 은 "유사도 임계 이상" 이라 했는데 구현은 공백 정규화 라벨도 허용 — 계약 불일치 | **문서 수정.** 예외를 명시하고, 그 경로의 hub 는 정의상 **철자 변형**이라 의미가 다른 라벨이 끼어들 수 없음을 근거로 기록 |

**리뷰어가 확인한 무결점 항목**: `(eff, cid)` ↔ `_canon_members` 배선이 스키마-로컬 cluster id 와
일치 · 보류 요약이 pass 전체 잔여 cap 을 쓰고 열린 커서를 그대로 사용 · 스키마-내 충돌 가드 동작.

**리뷰 전 자체 점검에서 잡은 것(리뷰어 미지적)**: 충돌 가드가 크기만으로 정렬해, **그 라벨을 이미
갖고 있던 밴드가 더 작을 때** 되돌림 대상이 뒤바뀌어 no-op 이 되고 충돌이 남았다. 정렬 키를
`(통일로 바뀐 쪽인가, -크기, 라벨)` 로 바꿔 원래 그 라벨이던 밴드가 항상 우선하게 했다
(`test_collision_guard_holds_when_incumbent_band_is_smaller`).
...
## REV-20260803T173000-visual-postdeploy [SKIPPED:docs-measurement-only] — PASS

- Reason: 문서·증거 전용(실행 코드 0줄). 대상 코드의 적대 검증은
  `REV-20260731T030000-band-visual-fit`·`REV-20260731T011000-label-canon [CODEX:...]` 가 담당했고
  모두 머지·배포됐다.
...
## REV-20260901T203000-f3-graph-retract [SKIPPED:non-policy-doc] — 그래프 유령 정점 회수 기록

- Cross-ref: CHG-20260901T203000-f3-graph-retract. 코드 변경 0 — 리뷰 패널 대상 아님.
- 파괴적 작업이라 **dry-run 리포트 → 사용자 범위 선택(A+B+D) → 매니페스트 → 적용** 순서를 지켰다.
- 근거: 정점 수 78,362→71,120 · 축별 재집계(product 축 물리 스키마 0) ·
  GlossaryTerm 634=634 · 삭제 후 물리 스키마 표본 3종 datasource 축 생존.
- 남긴 위험: C·D' 미처리 · MERGE-only 로 인한 재발(원인 미제거). 둘 다 MODIFY 에 기록.

## REV-20260902T140000-graph-sync-retract [CODEX:graph-sync-retract] — sync 용어 회수 단계

### 0. codex 채널 복구 + 지적 3건 판정

이 세션에서 codex 는 **4회 전건 무산출**이었다(한도 → 무출력 → 파일 덤프 → 프롬프트 에코,
모두 exit 0). 호출 형태를 **① 명령 실행 금지 ② 파일 열지 말고 코드를 프롬프트에 인라인
③ 출력은 3줄 고정** 으로 바꾸자 **19k 토큰에 결론 3건**을 냈다. 무산출의 원인은 모델이 아니라
**호출 형태**였다(코드베이스 탐색을 시키면 예산을 거기 다 쓰고 결론을 못 낸다).

| # | 지적 | 판정 | 조치 |
|---|---|---|---|
| **P3** | `str(r[0]).strip('"')` 가 AGE 문자열을 역직렬화하지 않아 `"`·`\` 포함 key 를 오판 | **실증 확인 — 진짜 결함** | `json.loads` 로 교체 + 파싱 실패는 **건너뛴다**(삭제 쪽으로 접지 않는다) |
| P1 | 소유권 표식 없이 `GlossaryTerm` 전체를 삭제 후보로 삼는다 | **현재 위험 없음, 지적은 타당** | 정점 생성처는 `sync_glossary_term` 뿐임을 확인. 미래에 다른 출처가 생기면 문제가 되므로 FUNCTION 에 명시 |
| P2 | 조회~삭제가 원자적이지 않아 동시 등록분을 stale 로 오판 가능 | **유효하나 창이 좁다** | 배치 트랜잭션 + `_sync_row_guard` SAVEPOINT 안이고, 오판돼도 다음 full sync 가 다시 투영한다. 잠금 도입은 과함 — 아래 §3 에 기록 |

**P3 실증**(라이브 `_vkey` 로 왕복):

    'He said "hi"'  → strip 불일치 · json.loads 일치
    'back\slash'   → strip 불일치 · json.loads 일치

현재 라이브 용어의 특수문자는 **작은따옴표 3건**뿐이라 아직 안 터졌을 뿐이다
(`TABLE_TYPE='BASE TABLE'` 등). 자율수집이 임의 용어를 쓰므로 언제든 들어올 수 있었다.
⚠ **내 자체 리뷰는 이걸 못 찾았다** — 독립 채널의 값이 이것이다.

- Verdict: PASS. 뮤테이션 7/7 KILL(baseline green 확인 후) · `make test` rc=0.
- **[가장 위험한 지점] 증분 실행의 회수는 파괴적이다.** `since` 가 있으면 조회 결과가 부분집합
  이므로 「없다 = 삭제됐다」가 성립하지 않는다. 가드가 빠지면 **전 용어가 지워진다**.
  뮤턴트 R1 이 이를 잠근다.
- **[의도] 물리 스키마 축은 회수하지 않는다.** 원본 판정이 다르다 — 운영 DB 를 실제 조회해야
  하고, 접속 실패를 「없다」로 오판하면 멀쩡한 정점을 지운다(라이브 datasource 다수가 현재
  접속 실패 상태다). 이번 수동 회수분은 **중복이거나 원본 자체가 없는** 경우라 안전했다.
### 3. 남긴 위험 (P2)

조회(`RETURN g.key`)와 삭제 사이에 다른 트랜잭션이 용어를 등록하면 그 정점이 stale 로
오판될 수 있다. 창은 배치 트랜잭션 안이라 좁고, 오판돼도 **다음 full sync 가 다시 투영**한다
(용어 자체는 `kb_glossary` 가 정본이라 유실이 아니다). 테이블 잠금은 sync 전체를 직렬화해
비용이 크므로 택하지 않았다.
