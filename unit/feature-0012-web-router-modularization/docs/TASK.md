...
- [~] TASK-0012-7 DI seam Phase 2..7: 클러스터별 핸들러+테스트 동반 마이그(A 단순→B 인라인perm→C/E1 require_permission→F optional→D 동적→트랜잭션4+E2)
  ...
  - [ ] Phase 2 batch 4+: 보류 12 — pre-auth gate 10(P1: 인증 전 early-return 404/400 → Depends hoisting 시 401 우선순위 역전, account-only/inline variant 또는 게이트-우선 재배치 필요), ask_result(P3: 복수 _connect_memory), history_anchor(적대 HIGH refute — 재조사). + 전체 cat-A 재survey.
...
## 8. Completion Checklist (DI seam Phase 2 batch 3 = TASK-0012-7 P2_MULTI_CLOSE 10)
- [x] P2_MULTI_CLOSE 10 핸들러 byte-동치 전환(산재 conn.close 전부 제거 + auth 블록 + 시그니처 Depends, dedent 불요) — 누적 cat-A 34
- [x] behavior-neutral — make test progress-chars 1238(baseline 동일)·0 fail, route-parity 179 불변, ruff clean, py_compile OK, `_require_account` exact 64→54
- [x] §18.8 적대 패널 — REV-20260630-0001(batch3 workflow, 44 agents) 22 후보 3패턴 분류+2렌즈 → 10 확정(P2)/12 보류. LOW verdict 는 prose 오류로 실결함 아님
- [x] 동반 테스트 전환 — history_dates(test_history_calendar_pg_routing T1/T3) 직접호출→TestClient(as_account); 나머지 route-snapshot/getsource 무영향. 잔류 직접호출 0 확인
- [x] ask_status terminal-finally 수작업 / history·history_dates multi-line sig 변환기 보강 / py_compile OK
- [x] TASK/REPORT/MODIFY/REVIEW 반영, BLOCKED 없음
- [ ] verify-completion PASS(9 checks) → Git 커밋 (Phase 2 batch 3)
