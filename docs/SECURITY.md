...
만드는 것이 아니라 이미 있던 표면에 첨부를 정합시키는 것이다. 다만 완전한 통제는 아니므로,
provenance 기반 tool 게이트(타 멤버 파일이 실린 턴에서 쓰기·외부영향 도구를 제한하거나 사용자 확인을
요구)는 **후속 과제로 남긴다** — 채택하려면 별도 설계·승인이 필요하다(도구 사용성과의 교환).
