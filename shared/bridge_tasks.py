...
__all__ = [
    ...
    "RUNNER_FEATURE_CAPS_SELF_REPORT",
...
#: **모델 목록의 출처가 그 AI 자신의 응답뿐**임을 러너가 선언하는 자격 (사용자 제보 2026-09-01).
#:
#: ## 왜 자격이 필요한가 — 같은 결함의 4번째 재발
#:
#: 「화면 목록의 출처는 연결된 AI」 는 이 기능의 계약인데, 그 계약은 **러너 안에서만** 지켜진다.
#: 서버가 받는 것은 클라이언트가 준 JSON 뿐이고, 그것이 AI 의 응답인지 러너 소스에 적혀 있던
#: 내장 표인지 구별할 방법이 없다. 폴백을 제거한 빌드를 배포해도 **사용자 머신에서 도는 러너를
#: 우리가 갱신할 수는 없으므로**, 낡은 러너가 계속 내장 표를 신고하고 화면은 그것을 그대로
#: 그린다 — 라이브 실측(2026-09-01): 08-31 16:55 빌드가 `codex: 응답을 받지 못해 내장 기본값을
#: 씁니다` 로그를 남기며 `gpt-5.1-codex` 를 신고 중이었고, 그 계정이 실제로 쓸 수 있는 것은
#: `gpt-5.6-*` 였다.
#:
#: ## 왜 버전·지문이 아니라 기능 신고인가
#:
#: - **버전**(`RUNNER_MIN_AGENT_VERSION`)은 날짜 단위라 같은 날 세 번 바뀐 러너를 구분하지
#:   못한다 — 이 결함의 3차 재발이 정확히 그것이었다.
#: - **지문**(`agent_build`)은 동일성 축이라 «배포본과 같은 파일인가» 만 답한다. 그것을 게이트로
#:   쓰면 러너 파일을 고치는 **모든 배포**가 전 사용자의 선택기를 지운다 — 목록이 멀쩡한
#:   러너까지 함께.
#: - **기능 신고**는 «이 빌드가 그 계약을 지키는가» 라는 정확히 그 질문에 답하고, 한 번 참이면
#:   이후 배포에도 계속 참이다. 낡은 러너는 이 이름을 모르므로 신고할 수 없다 — 자격의 부재가
#:   구조적으로 보장된다.
RUNNER_FEATURE_CAPS_SELF_REPORT = "caps_self_report"
