...
def test_r8_frontend_subtab_perm_map_split():
    # feature-0038 Cycle 6: 메타데이터 콘솔은 admin/metadata.js 로 분리(byte-동치 이동) — 합본 검사.
    text = ADMIN_JS.read_text(encoding="utf-8") + (ADMIN_JS.parent / "admin" / "metadata.js").read_text(encoding="utf-8")
    m = re.search(r"const _METADATA_SUBTAB_PERM\s*=\s*\{(.*?)\};", text, re.DOTALL)
    assert m, "_METADATA_SUBTAB_PERM 블록을 admin.js+admin/metadata.js 합본에서 찾지 못함"
    body = m.group(1)
