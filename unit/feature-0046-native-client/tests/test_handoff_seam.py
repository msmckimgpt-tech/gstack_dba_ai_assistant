...
def test_launch_button_never_lies_about_what_it_can_do(page, script):
    ...
    if "hidden" in decl:
        ...
        registration = re.search(rf'[^\n]*\$\("{bid}"\)[^\n]*addEventListener[^\n]*', js)
        assert registration, f"{script.name}: {bid} 가 배선되지 않았다"
        handler = js[registration.start():].split("\n  });", 1)[0]
        assert "/api/ai/connect/token" in handler, \
...
def test_silent_scheme_failure_is_explained():
    ...
    js = (_STATIC / "ai-connect.js").read_text(encoding="utf-8")
    body = js[js.index("function _fire(url)"):].split("\n  }", 1)[0]
