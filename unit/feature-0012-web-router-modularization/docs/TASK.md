...
## 20260711T2140-item10-routers-p15 — 잔여 전수 census 기반 128종(2,188줄) 최대 sweep + standalone 검증기 2건 수리 (parallel-work-structure ITEM-10)
- [x] 전수 census(182종): MOVE 131 → 직수입/DI-seam 판정으로 128 확정. _conv_store 38·conversations 20·admin_products 13·admin_accounts 8·admin_roles 6·_bootstrap_schema 6·_prompt_context 5·admin_metadata 3·admin_console 5·admin_usage 4·admin_quotas 4·admin_datasources 3·system 3·share 3·attachments 2·media 2·기타 5. app.py 6,974→4,786(-2,188 / 누적 -76%).
...
## 20260712T0230-item11-census-정정 — DEFER 모집단 16→50 교정 + §18.8 3렌즈 패널 (parallel-work-structure ITEM-11)
- [x] **census 정정(CRITICAL)**: ITEM-11 DEFER 모집단 = 인라인 authn 라우트 핸들러 **50개**(16 아님). AST 독립 census(@router 데코 + body `_require_account`/`_get_authenticated_account`/`_optional_account` + 시그니처 `Depends(get_current_account/get_optional_account/require_permission)` 無)가 50 확정 — 코드베이스 자체 MODIFY.md:38 "DEFER 50"·ROADMAP ~46·DI-seam ~46 과 수렴. 종전 "16" 은 DI-seam cat-A 적대검토 subset(pre-auth gate 10+ask_result+suggestions+public_share_view+txn 3)일 뿐. ITEM-10 은 DEFER 를 인라인 유지로 추출만 함(REPORT.md:52) → 모집단 불변. fail-soft 도 3→**6**(progress/suggestions/public_share_view + auth_me/get_session/get_llm_health).
- [x] **§18.8 적대 패널 3렌즈(인증 인접 필수 게이트)**: (1)설계 판정=**NOT-SHIP**(옵션 B 전면 intentional-inline over-general — me_put_system_prompt 은 try/finally 부재 실제 conn leak 이라 DI 가 더 안전; "6 regressions" 는 strawman, blueprint §1.3 get_optional_account 이 risk 3/4/5/6 byte-동치 해결; TASK.md:83 이 pre-auth DI-rework 를 planned refinement 로 이미 기록) (2)계약 강건성=**WEAK**(AST/string 테스트 5종 bypass 허용·status 미검증·동적 완전성 無) (3)census 완전성=**CRITICAL**(16→50, 34 누락). → 약한 계약 테스트 폐기.
- [x] **방향 전환: 옵션 B(16 종결) 철회 → 옵션 C(split)**. ITEM-11 = 50 핸들러 개별 판정 다-batch:
  - **DI-rework 대상(안전+이득, runtime TestClient snapshot 필수)**: me_put_system_prompt(conn leak fix) · admin_create/update/delete_product(형제 upload_product_icon 이미 `Depends(require_permission("product.manage"))` — 단 403 메시지 verbatim 함정: 인라인="제품 관리 권한이 필요합니다." vs 형제="…(product.manage)." 상이) · suggestions(get_optional_account) · 34 미열거 중 평범한 auth-then-body 다수(admin_accounts 7·admin_roles 5·admin_console 2·admin_products datasource 6·conversations CRUD 6·auth_me_patch·purge_audit_events·get_profile_audit_event 등 개별 cat-B/C 판정).
...
## 20260712T0300-item11-batch2 — rename_conversation_title DI-rework + admin_products 이연 판정 (parallel-work-structure ITEM-11)
- [x] **admin_products CRUD 3 이연 판정**: admin_create/update/delete_product 정독 결과 txn(autocommit 토글)+audit 2차 commit+산재 close 로 복잡하나 **실제 버그(leak) 없음**(except/finally 로 close·autocommit 복원 완비). batch1(me_put leak fix)과 달리 DI-rework 이득=스타일뿐·리스크=데이터 쓰기 txn 파손 → **txn 전용 개별 마이그(우선순위 하향)**. census/blueprint "bulk 금지·verbatim 보존" 경고와 정합.
- [x] **leak 후보 재선정**: try/finally 부재·conn.close 산재·mutate·txn無 = batch1형 명확 leak fix 후보 12개(admin_update_account close×15·admin_update_role 14·admin_create_role 12·admin_put_system_prompt 10·update_conversation_product 10·admin_account_password_reset 9·admin_delete_account 8·admin_delete_role 8·admin_account_unlock 7·admin_update_product_databases 6·auth_me_patch 6·rename_conversation_title 5) 우선 처리.
- [x] **batch2=rename_conversation_title DI-rework**: 게이트 헬퍼(_account_can_access_conversation·_conversation_owner_account_id) raise 시 leak → account=Depends(get_current_account)+conn=Depends(get_conn). byte-동치: 401/500/400×3(invalid json·empty title·too long)/404/403/500(update)/200. runtime snapshot 10종+leak-fix 회귀(게이트 raise→conn.close 관측). 유일 차이=malformed/empty body+unauth 400→401(benign). 게이트: route snapshot 205 byte-동치·F821·py_compile·전 스위트 pytest rc=0. DEFER 49→48.

## 20260712T0320-item11-batch3 — auth_me_patch conn-only DI-rework (parallel-work-structure ITEM-11)
- [x] **패턴 발견**: auth_me_patch 는 `_get_authenticated_account`(not `_require_account`) + 미인증 401 body="**unauthorized**"(get_current_account "로그인이 필요합니다."와 상이). → `Depends(get_current_account)` 대체 시 401 byte-동치 위반. **conn-only DI(get_conn)** 로 leak fix 하고 auth 는 인라인 유지가 정답. conn None 체크를 json parse 뒤에 둬 ordering(invalid json 400→db fail 500)+500 body byte-동치 보존.
- [x] auth_me_patch conn.close×6 제거→get_conn finally. runtime snapshot 6종(500·**401 unauthorized**·400 invalid json·400 pw-no-current·200·leak-fix commit raise→conn.close). 게이트: route snapshot 205 byte-동치·F821·py_compile·전 스위트 pytest rc=0. DEFER 48→47.
- **후속 함의**: `_get_authenticated_account` 기반 핸들러(auth_me/get_session/get_llm_health 등 fail-soft 포함)는 conn-only DI 패턴 적용(401 메시지·fail-soft 계약 보존). `_require_account` 기반은 batch1/2 처럼 account+conn 완전 DI 가능.
