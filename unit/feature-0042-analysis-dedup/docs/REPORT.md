...
- **ITEM-15 착수 조건 미확보** — 프롬프트를 바꿀 때 품질 회귀를 관측할 수단이 없다.
  ...
  프롬프트 출력 계약 회귀 테스트 0건.
  **선행 확정(2026-08-14)**: (b) 출력 계약 회귀 테스트 신설 채택 — 지금 만들 수 있고 결정론적이다.
  (a) 증거 커버리지 확대는 워커가 시간을 두고 채우는 축이라 ITEM-15 를 무기한 대기시킨다(배타적이지
  않으므로 병행 시 품질 축 검정력이 함께 오른다). **구현은 별도 cycle** — 본 cycle 범위 밖.
