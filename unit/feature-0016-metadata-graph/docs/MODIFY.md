...
## CHG-20260707T031149-ai-claude-feature-0016-graph-dataflow-tooltip
- Date: 2026-07-07
- Related Requirement: 사용자 요청(관리 콘솔 > 지식베이스 > 메타데이터 > 그래프 뷰) — 노드 관계 화살표 데이터흐름 정합 + AI 능동 분석 지침 툴팁 (TASK §56).
...
- admin.js `_metaG6Build` 레이아웃(frontend-only): ① 적응형 shelf 폭 `MAXROWW=max(2400, maxClusterW,
  ...
  balance(ADR-004 ② 재선회). 캐시버스터 20260709-graph-vpack.
- 비겹침 불변식 보존(COLW 간격·realH push-down·(w,h)=실 bbox). §60 headless 16 PASS + 기존 54+26 회귀 0.
  §18.8 적대 리뷰(SUBAGENT, ux/layout): PASS-WITH-FIXES(BLOCKING 0). 상세 REVIEW.md.

## CHG-20260709T163000-ai-claude-feature-0016-graph-vpack2 — §60.2 simGroups 경로 세로폭주 해소 (PB-0008 회귀, 2026-07-09)
- PB-0008 1차 라이브 실측(배포 d5cf0fec)이 초기 수정의 gap 포착: simGroups 경로 스키마(cc_* 557T)가 여전히
  822×10272 aspect 0.08 세로폭주(그룹 블록이 고정 TRW 4열-상당 폭으로만 래핑 → 그룹 행 세로 스택).
- admin.js: ① 그룹 블록 행 목표 폭 `TRW` 를 총 블록 면적 기반 적응(`max(TRW, round(sqrt(ΣblockArea×2.0)))`)
...
- 대상: `admin.js`(_faKeep/_clusterHasFocus 헬퍼 + 3 컬 지점 예외 가드) + `admin.html` 버스터 + headless `test_g6build_cullrefkeep.js`(신규 9).
- 변경: 뷰포트 컬링(§65/§67)이 화면 밖 노드 미방출로 관계선 드롭 + 상세 네비 skip 시키던 회귀 수정. 선택 노드의 관계 상대(focusAdj)를 화면 밖이어도 방출 예외 → renderEndpoint 가 엣지 렌더, _metaRenderedIdFor 가 네비 팬. 무선택 시 예외 0(컬링 무손실).
- 근거: 사용자 피드백 "draw는 하지 않되 참조·상호작용은 가능하도록"(ADR-037). frontend-only.
- 검증: headless 10(cullrefkeep)+169(회귀) = **179 PASS**·node --check. §18.8 적대 리뷰. POST-DEPLOY win-browser(TEST §76). 버스터 `admin.js?v=20260710-cullrefkeep`.
