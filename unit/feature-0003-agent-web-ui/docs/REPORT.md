...
DQA 클라이언트가 아닌 평범한 웹브라우저로 **앱 루트(`/`)나 관리 콘솔(`/admin`)** 에 들어오면
설치 안내 페이지(`/install`)로 보낸다. 사용자 결정 2026-09-10: 대상은 그 둘, **탈출구 없음**.
2026-09-08 에 공유 화면의 참여·fork 를 앱으로 모은 결정의 확장이며, 이번에는 **제품 진입 자체**
가 대상이다. 공유 열람(`/share/{token}`)·외부 AI 연결(`/ai/connect`)·설치기 다운로드는 그대로
브라우저에서 열린다.

**판정은 브라우저에 있다.** 「앱 안인가」의 신호는 브리지 좌표 하나이고 그 좌표는 첫 적재 뒤
주소에서 지워져 `sessionStorage` 로 옮겨간다 — 서버는 두 번째 요청부터 그것을 볼 수 없으므로,
서버에서 막으면 앱 창이 새로고침 한 번에 자기 자신에게 차단당한다. 게이트는 `<head>` 의
**동기·무의존 classic 스크립트**이고, 판정할 수 없으면 **통과시킨다**(잘못 막으면 제품 전체를
잃고 잘못 통과시키면 안내를 못 볼 뿐이다).

안내 화면은 제품 디자인 시스템(`css/base.css`)을 상속하고 값을 하나도 정의하지 않는다. 리서치가
짚은 AI-slop 지문(Inter 기본값·보라 그라디언트·공허한 헤드라인·동일 가중치 카드·장식 모션)을
도입하지 않는 대신, 화면을 **실제 값**으로 채운다 — 1.4.0 · 24.7MB · 게시일 · SHA-256 ·
`Get-FileHash` 명령, 그리고 미서명 설치기라 반드시 뜨는 **SmartScreen 경고 예고**.

서버 표면 2개 추가(둘 다 익명, 인스턴스 데이터 0): `GET /install`(안내 문서) ·
`GET /api/ai/client/entry`(`{app_link, release}` — 딥링크는 `shared.dqa_identity` 정본이
조립하고 토큰 미포함, `release` 는 실물·크기·sha256 대조를 통과한 것만·없으면 `null`).
인증·인가 코드·스키마·권한 데이터 변경 0.

**codex 독립 리뷰가 P1 2건·P2 3건을 냈고 전건 재현 후 수정했다** — 그중 셋이 「게이트가 정당한
사용자를 막는」 축이었다: ① 외부 AI 인가의 미로그인 로그인 착지점(`/?next=…`)이 막혀 브라우저
연결이 불가 ② `/admin` 딥링크에서 판정 전 새로고침이 앱 창을 쫓아냄 ③ 저장소 **쓰기** 실패가
fail-open 에서 누락. 나머지 둘은 설치기 철회 시 「앱 열기」까지 사라지는 결합과, 통신 장애를
「파일 없음」으로 확정 표시하던 문제다.

검증: jsdom 행위 하네스 67 PASS(뮤턴트 4종 KILL — 그중 1건은 **등가 뮤턴트**로 식별해 되메우는
방어선까지 함께 걷어 낸 뒤 KILL) · 계약 테스트 28 PASS · **실 Windows Chrome 12 PASS**(역검증
포함, 400px 실측 치수 대조). 첫 캡처를 눈으로 읽어 한국어 음절 중간 줄바꿈 1건을 추가로 잡았다 —
그 시점에 자동 검증은 전부 초록이었다.

배포 완료 — release `4b2dd68c` · asset_stamp `485a9a2696e0`(두 replica 일치) · soak PASS.
라이브 실 브라우저에서 6건 PASS(무신호 `/` → `/install` · 좌표·새로고침·`/admin` 통과 ·
안내가 라이브 릴리스 값 렌더). 실행 중이던 사용자 DQA 앱(PID 12344)은 배포 후에도 **정상 대화
화면**을 유지했다(창 캡처 2회, 수동 관측 — 앱을 종료·조작하지 않았다). 다만 그 앱이 이미 새
빌드로 재적재됐는지는 접근 로그 부재로 확인하지 못했고, 재적재 경로 자체는 라이브 L5 가 별도로
실측했다 — 두 사실을 합산하지 않는다.

**실제 DQA 클라이언트(WebView2) 자동화 검증은 NOT-RUN** 이다 — 자동화 진입점이 없고 사용자의 앱이
실행 중이라 종료하지 않았다(§16.6). 같은 Chromium 엔진의 보조 검증으로 대체했고 그 한계를
Run 에 명시했다. 배포 후 실행 중인 앱이 자동 갱신 뒤에도 대화 화면에 머무는지 확인해 Run 에
append 한다.
