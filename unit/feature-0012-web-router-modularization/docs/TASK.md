...
## 1. Current Status
- State: in_progress (DI seam Phase 0 — 가산적 토대 구현·검증; 클러스터별 마이그 후속)
- Owner: AI / Human
...
- [ ] TASK-0012-6 DI seam Phase 1: `conftest.py`(TestClient+`as_account`) + 파일럿 핸들러 1개 전환 + 401/403 회귀 스위트
...
## 4. In Progress
- TASK-0012-6 DI seam Phase 1(테스트 인프라 + 파일럿) — 다음 단계.
...
- **DI seam Phase 0(TASK-0012-5)**: app.py L10269~ 에 가산적 토대(`_AuthError`+`@app.exception_handler` 로 `{"error":msg}` 셰이프 보존, `get_conn` yield 의존성+autocommit rollback 안전망, `get_current_account`/`get_optional_account`/`require_permission(*perms,message=)`). 미사용 → behavior-neutral. 검증: py_compile / route-parity 179 / `_AuthError` handler 등록 / **make test 1205 passed·2 skip·0 fail**.

## 7. Next Action
- TASK-0012-6 Phase 1: `conftest.py` 에 TestClient 픽스처 + `as_account(perms={...})` 헬퍼 + 401/403 회귀 스위트(§4.2) 신규, 파일럿 핸들러 1개(`get_llm_health@10760`) DI 전환 → make test + route-parity 게이트.
- **§18.8 패널 이월(REV-20260629-0003) — 첫 소비자 전 필수**: (HIGH) `get_conn` 의 `_connect_memory()` 실패 → curated 500(`{"error":"db connection failed"}`) 동치 보정(메시지 다양성 grep 선분석); (LOW) `require_permission()` 빈 perms 가드; deps wire 후 401/403 셰이프 동치 회귀 테스트.

## 8. Completion Checklist (DI seam Phase 0 = TASK-0012-5)
- [x] AC(가산적 DI seam 토대 — deps + `_AuthError` handler + get_conn 이 응답셰이프 보존하며 정의됨)가 구현되었다
- [x] behavior-neutral(미사용) — route-parity 179 불변, make test green(1205 passed/2 skip/0 fail)
- [x] DI_SEAM_BLUEPRINT.md(정본 설계)가 적대적 비평 보정을 반영한다
- [x] TASK.md/REPORT.md에 상태가 반영되었다
- [x] BLOCKED 없음(브라우저 QA env 가용 확인)
- [ ] Git 커밋 (Phase 0)
