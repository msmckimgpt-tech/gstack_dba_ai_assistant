...
def _run_cli_cancelable(cmd: list[str], cancel_check, cwd: str | None = None,
    ...
    if proc.returncode != 0:
        msg = describe_cli_failure(int(proc.returncode), box.get("out", ""),
                                   box.get("err", ""))
        # 러너 로그에도 남긴다. 종전에는 이 실패가 로그에 **한 줄도** 남지 않아, 운영자가
        # 사용자 화면의 빈 콜론 말고는 볼 것이 없었다(라이브 2026-09-01).
        _log(f"내 AI 호출 실패 — {msg.splitlines()[0]}")
        return False, msg
