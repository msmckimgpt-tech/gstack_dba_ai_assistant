...
- [~] TASK-0012-7 DI seam Phase 2..7: 클러스터별 핸들러+테스트 동반 마이그(A 단순→B 인라인perm→C/E1 require_permission→F optional→D 동적→트랜잭션4+E2)
  ...
  - [ ] Phase 3 cat-B batch 5+: 확정 4(testconv: admin_test_datasource·admin_product_db_insights·admin_usage_conversations·admin_archived_conversations — 동반 테스트 직접호출 전환 필요) + 보류 3(suggestions fail-soft·admin_products_insight_coverage·admin_overview) + preauth/longpoll ~32(아키텍처 이연) + OR·BRANCH 잔여 → cat C/D/E/F/트랜잭션.
...
## 8. Completion Checklist (DI seam Phase 3 cat-B batch 4 = TASK-0012-7 no-flag 10)
- [x] no-flag 10 핸들러 전환(RP 5 OR-of-negations require_permission 다중 hoist + AO 5) — 누적 cat-B 25
- [x] behavior-neutral — make test progress-chars 1238(baseline 동일)·0 fail, route-parity 179 불변, ruff clean, py_compile OK
- [x] §18.8 적대 패널 — REV-20260630-0006(cat-B 잔여 workflow 49 agents) no-flag 17→14 확정. RP 메시지 verbatim·OR-of-negations AND 의미·order 보존 적대검증
- [x] 변환기 보강 — or-chain 다중 perm 인식 + auth-perm adjacency 주석 허용. 동반 테스트 10개 직접호출 0(grep)
- [x] TASK/REPORT/MODIFY/REVIEW 반영, BLOCKED 없음
- [ ] verify-completion PASS(9 checks) → Git 커밋 (Phase 3 cat-B batch 4)
