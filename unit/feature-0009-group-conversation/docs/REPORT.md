...
- **읽음 커서 전진 누락 보정(gc-unread-read-fix, 본 worktree, Minor frontend)**: 사용자 보고 — 읽은 대화의 회색 unread 배지가 안 줄어듦. 원인=`refreshWorkspace`(페이지 복원/갱신)·이미-active 재선택 경로에서 읽음 처리(`_markActiveConversationRead`) 누락 → 서버 커서 미전진(DB: cursor 3466 backfill 값에 멈춤, conv_max 3590). 수정=refreshWorkspace `loadHistory` 후 + selectConversation 가드 시 읽음 처리. `node --check` PASS, 패널 SKIP(frontend). 캐시버스터 gc-unread-read-fix. (CHG-20260625T194159/REV-20260625T194159)
- In Progress: gc-unread-read-fix — node --check PASS → verify → commit → PR → 머지 → 배포(web) → PB-0008(새로고침→복원 대화→배지 0).
- In Progress: composer-nonblock-interrupt — docs 완료 → verify-completion → PR → 머지 → 라이브 배포(web+ask-worker) → PB-0008. (gc-unread-baseline/participant-product-select 등 main 머지분은 직전 배포로 라이브.)
