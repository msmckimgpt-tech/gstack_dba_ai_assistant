...
- **설치본(exe) 재빌드 후의 트레이** — 실측은 소스 상태의 Windows 파이썬으로 했다. 동결
  빌드에서 `ExtractIconW(sys.executable)` 가 **앱 아이콘**을 내는지는 다음 빌드에서 확인한다
  (실패해도 `IDI_APPLICATION` 으로 폴백하므로 아이콘이 사라지지는 않는다).
