...
## ADR-20260713T105932-content-cluster — 의미 클러스터를 DB 단위·루틴 합동·분석문 기반으로 재설계 (사용자 요청)
- 상태: 채택 (2026-07-13)
- 맥락: 사용자 리포트 — cc_data_main 'AI 능동 분석' 후에도 카테고리(sim-group) 밴드 대부분(특히 함수·프로시저)이 단순 명칭 구분. 진단(RC1~5): numpy 부재로 Phase C 클러스터가 전 시스템 0건(silent 무산), 클러스터 단위가 ds-전역이라 대형 ds 통째 skip(N 가드), 루틴은 대상 밖, 분석문이 그룹핑에 미연결, 라벨이 affix 스템.
- 결정: ① 클러스터 계산 단위를 **표시 단위(스키마=DB 클러스터)와 정합**하게 effective schema 별로 분할 — N 가드는 스키마 국소로만 발동, cluster id 는 pass-전역 순번(결정론). ② 루틴을 rag 테이블과 **하나의 임베딩·id 공간에서 합동 클러스터**(0040 additive 컬럼) — 루틴 시그니처는 참조 테이블(touches)·분석문을 포함해 자기가 만지는 테이블과 자연 응집. ③ 'AI 능동 분석' 산출(summary+usage)을 시그니처에 주입 — **분석 실행 → 재클러스터** 인과 성립(사용자 기대). analysis 줄은 비어있지 않을 때만 append — 미분석 객체 해시 불변(§12.3 2차-효과 비용 한정: 재임베딩은 분석 보유분만). ④ 라벨은 LLM 한국어 컨텐츠 명(멤버셋-해시 kv 캐시·fail-soft affix 폴백·게이트) — membership 은 임베딩이 결정, LLM 은 이름만(환각 영향면 최소).
- 대안 기각: (a) node_analysis 가 노드별 자유 category 문자열을 내게 해 그룹핑 — 노드 간 라벨 불일치("상점"vs"샵")로 그룹 파편화, 정규화 인프라 필요 → 임베딩 클러스터가 이미 그 정규화. (b) pgvector SQL kNN 으로 대형 ds 전역 클러스터 — DB 경계를 넘는 클러스터는 표시 단위(스키마 내부 그룹)에서 소비 불가(be: 싱글턴 강등)·복잡도만 증가. (c) 프론트에서 ROUTINE_USES 관계로 루틴을 테이블 그룹에 부착 — nm: affix family(sp_ 접두)가 관계 attach 보다 선행해 무효(§ _metaSimGroups 1차 배정), 백엔드 신호가 근본.
- 트레이드오프: 분석 갱신 시 해시 변경 → 재임베딩(로컬 embed-ollama, 배치 상한 500행/15분 스로틀 유지 — 외부 비용 0). LLM 라벨은 클러스터 변경분만(캐시)·DB당 1 call 유계, 실패해도 id 배정 무영향. 초대형 단일 DB(>2,000 객체)는 여전히 affix 폴백(가드 보존 — OOM 방지).
...
- 결정: ① **실패를 분류**한다 — `bad_model`/`context_length`(요청-레벨 확정)만 permanent, 나머지(네트워크·타임아웃·429·5xx·자격만료·빈 응답·JSON 파싱)는 transient. 신호 전달은 `llm_node_analysis(error_sink=dict)` — 반환 계약(None)을 바꾸지 않아 기존 호출자 무회귀. ② **일시 실패는 pending 되돌림** — `attempts+1` + `next_attempt_at = now() + LEAST(BASE×2^attempts, MAX)`, 단일 statement 로 상한 판정까지 원자 처리. **`runs.failed` 를 올리지 않는다**(run 이 `running` 을 유지 → finalize 되지 않고 복구 시 스스로 완주). `MAX_ATTEMPTS`(4) 도달 시 종전과 동일한 terminal. ③ **run liveness heartbeat** — 대기 잡을 가진 `running` run 의 `updated_at` 을 매 틱 갱신. ④ **회로차단** — 연속 일시 실패가 임계(3)면 claim 을 canary 1건으로 축소, 성공 1건으로 즉시 복귀. ⑤ **회수는 사람 명시 경로만** — `retry_failed_jobs` 가 `attempts=0` 리셋 + `runs.failed` 차감 + `status='running'` 복원을 단일 CTE 로 처리하고, 대상을 일시 실패·레거시 LLM 실패로 한정한다.
- 대안 기각: (a) **자동 reaper 로 모든 failed 를 주기 회수** — 영구 실패·인위적 실패('verification-cleanup(취소)' 580건)까지 되살려 LLM 예산이 무한히 샌다. 회수 판정을 error 문자열에 맡기는 것도 취약. (b) **backoff 없이 즉시 재시도** — 단절 창에서 큐를 그대로 태워 소실 규모만 키운다(현행 결함의 재생산). (c) **`llm_provider_health` 단독 게이트** — 그 모듈은 네트워크 타임아웃을 의도적으로 분류하지 않고(`classify_llm_provider_error` → None) PG 영속에 의존해, 정작 이 결함의 주 원인 경로에서 신호가 없다. 자체 연속-실패 카운터를 1차 신호로 두고 health 는 보조로 존중한다. (d) **프론트 폴 cap 상향(240→N)** — 값만 미루는 수단이라 더 긴 단절·더 큰 run 에서 같은 결함이 재발한다. cap 자체를 없애고 적응 백오프 + 총 지속 시간 상한으로 바꿨다.
