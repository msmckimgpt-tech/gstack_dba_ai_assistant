...
test:  ## ci: 단위 테스트(pytest) + 린트(ruff) — 전용 compose 프로젝트(라이브 네트워크 미참여) + 라이브 DB/스냅샷 차단 env
	...
	@$(DC_TEST) run --rm --no-deps $(TEST_ISOLATION_ENV) -v "$(CURDIR):/work" -w /work --entrypoint sh agent -lc '\
	  ...
	  echo "=== pytest ==="; \
	  python -m pytest -q unit/feature-0002-agent-core/tests unit/feature-0003-agent-web-ui/tests unit/feature-0023-conversation-api-access/tests unit/feature-0014-zero-downtime-deploy/tests unit/feature-0020-zd-deploy-all/tests; rc=$$?; \
	  echo "=== ruff (참고용, 비차단) ==="; \
