...
- **Environment: Windows-browser — pre-commit 라이브 미수행 사유**: 정적 자산 web 이미지 baked → merge + `deploy-web` 재배포 선행(§51·§57 동일 패턴). **POST-DEPLOY 2차 PB-0008 append 예정**(T60.8) — 검증 항목: ① 관계 있는 제품 스키마(cc_* 등 simGroups) 펼침 시 클러스터 landscape(세로폭주 해소 실증) ② 여러 스키마 펼침 전체 그래프 판독 가능 landscape ③ **성질 다른 노드·클러스터 비겹침 육안**(테이블·컬럼·routine·그룹박스·카테고리 밴드) ④ 펼침/접기 후 fitView·미니맵 무충돌 ⑤ pageerror 0.
