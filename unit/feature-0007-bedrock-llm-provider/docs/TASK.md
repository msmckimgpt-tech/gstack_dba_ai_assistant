...
- [x] **§18.8 적대 검증 (codex review, §18.8.1 accepted 채널) — R1 에서 P1 1건·P2 1건 검출 → 전건 수정**
  - **P1** `kb_embedding_worker.py` — `base_url` 미지정 시 OpenAI SDK 기본값 `https://api.openai.com/v1` 로 나간다. 즉 `BEDROCK_GATEWAY_API_KEY` 만 있고 URL 이 없는 구성에서 **게이트웨이 자격증명 + KB 텍스트가 OpenAI 로 전송**된다. `docs/SECURITY.md`(CHG-20260522-0006 — "LLM 호출 entry 는 게이트웨이만 허용", OpenAI direct 의도적 폐기) 위반. → URL·KEY **paired 요구**(fail-closed)로 수정. 내가 로컬 fallback 을 없애면서 만든 구멍이다
  - **P2** 백필 데몬 — `AGENT_KB_EMBEDDING_MODEL` 기본값을 빈 값으로 내렸으나 `AGENT_KB_EMBEDDING_AUTO` 는 기본 활성이라, insight-worker 가 `INTERVAL_SEC` 마다 **빈 모델명으로** 임베딩을 반복 시도한다. 쿼리 임베딩만 no-op 이 되고 **백필 경로는 그 방어를 공유하지 않았다**(§16.7 G8-a 재현). → `run_embedding_pass` 진입 가드(`skipped: embedding-model-unset`, `error` 는 비움 — 비활성은 실패가 아니므로 매 tick 경고를 쌓지 않는다)
  - 회귀 잠금 `test_kb_embedding_gateway_failclosed.py` 신규 **6건** + 결함 재주입 실증(수정 전 형태로 되돌리면 3건 FAIL → 원복 후 6건 통과). 정상 경로 실측 포함(§16.7 G9-c — 차단 로직이 정상 경로를 막지 않는지)
- [x] **확인 라운드 R2 (§18.8 (a) — 수정 라운드 자신은 종결 근거가 아니다)** — **P1 0건** 확인, **P2 1건 신규**: R1 의 P2 를 `run_embedding_pass` 진입 가드로만 고쳐 **형제 진입점 `main()`**(= `bin/kb-embedding-worker.sh`)이 우회해 `model=""` 를 게이트웨이로 보냈다. **내가 직전 REVIEW 에서 자기 진단한 것과 같은 클래스의 재발**이라 §16.7 G10 대로 점수정이 아니라 **구조로 승격**: 두 진입점이 반드시 지나는 `call_openai_embeddings` 를 chokepoint 로 삼아 빈-모델 가드를 두고(앞으로 추가되는 진입점도 자동 커버), `main()` 은 **exit 0 + `[DISABLED]` 안내**로 끝낸다(비활성 ≠ 실패 — API 실패로 끝내면 cron·래퍼가 장애로 보고 재시도·알림을 쌓는다). `--model` 명시 override 는 보존
- [x] **모수 검증 테스트 신설** (§16.7 G12-b) — `test_all_gateway_entrypoints_route_through_chokepoint`: AST 로 `OpenAI(` 직접 생성 지점이 `call_openai_embeddings` **하나뿐**임을 강제한다. 새 진입점이 자기 클라이언트를 만들면 가드를 우회하므로 그 형태를 CI 에서 잡는다. 테스트 6 → **10건**
- [x] **결함 주입 3형태 실증** — ⓐ `main()` 조기 종료만 제거(R2 P2 정확한 형태) → CLI 테스트 FAIL ⓑ chokepoint 가드 제거 → chokepoint 테스트 FAIL ⓒ chokepoint 밖 `OpenAI(` 생성 → AST 모수 테스트 FAIL. 원복 후 10건 통과 · `git diff --stat` 로 원복 확인
- [x] **전수 스위트 기준선 대조** — branch 86 FAILED / main 86 FAILED, **집합 차이 0건**(branch-only 0 · main-only 0). 86건은 전부 pre-existing(컨테이너 밖 로컬 실행: `pymysql`·`psycopg` 부재, bash fixture) — `test_oauth_exhaustion_gate`(68) · `test_share_redaction_invariant`(7) · `test_scratch`(6) · `test_mssql_auth_cooldown`(5). **내가 만든 회귀 0건**
- [x] **확인 라운드 R3 — PASS (P1 0 · P2 0)**. 수렴: R1(1/1) → R2(0/1) → R3(0/0), P1 단조 감소·진동 없음. ⚠ codex 가 자기 sandbox 에서 테스트를 못 돌렸다고 명시했으므로(소켓 생성 금지) R3 의 PASS 는 **정적 분석 기준**이다 — 런타임 축은 본 세션이 직접 측정한 것으로 덮었고 REVIEW REV-20260907T180000 에 수행자를 분리 기재했다
- [x] **의도적 미적용 3건을 ADR 로 기록** (§16.7 G8-c — 기록 없는 미적용은 누락과 구분되지 않는다) — `docs/DECISIONS.md` ADR-20260907T175000-local-llm-decommission-scope-boundary: ① `bedrock-gateway` 철거(결정 축이 다름 + feature-0020 배선 폭) ② `embed_ollama_models` 볼륨 보존(되돌리기 경로) ③ 선행 프로젝트 `mysql_ai/.env`(VCS 부재·휴면·별 프로젝트)
