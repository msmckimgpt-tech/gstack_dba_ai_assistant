...
- [x] 배포 후 라이브에서 `RunnerCapabilities` 가 7종으로 차고 화면에 선택기가 뜨는지 실측
      — **PASS** (2026-09-08, 사용자 계정 앱 창까지 확인. 상세: `docs/test-runs.d/20260908T010000-live-model-list-reached.md`)
...
## Cycle — TASK-20260908T010000-live-model-list-record — 배포 후 라이브 실측 기록 (문서 전용)
...
```
해당 러너 실행 및 실측도 직접 진행해주세요.
```

- [x] **① 사용자 계정 러너를 직접 기동** — 설치본과 같은 방식(DQA Connect 동봉 런타임 ·
      `--ai codex`). 토큰은 앱 창의 사용자 세션이 발급했고 값은 어디에도 노출하지 않았다.
- [x] **② 종단 실측** — caps `[]` → **591바이트(codex 7종)** 30초 · 서버 `visible` ·
      **사용자 앱 창 화면**에 CODEX 7종·기본값 `GPT-6-Astra` 렌더 · 안내 문단 없음 ·
      하단 정합 781↔781.
- [x] **③ 독립 확인** — 손대지 않은 다른 사용자(AccountId=27)도 새 빌드로 자기갱신해
      caps 465바이트를 신고 중이었다.
