...
## 1. Current Status
- State: in_progress (DI seam Phase 1 — 테스트 인프라 + 파일럿 전환·검증 완료; Phase 2 클러스터 마이그 후속)
- Owner: AI / Human
...
## 4. In Progress
- TASK-0012-7 DI seam Phase 2(cat A 단순 require, ~95 핸들러 PR당 15-25) — 다음 단계. 파일럿 패턴(부록 마이그 원자단위 ①~⑤) + conftest fixture(`as_account`/mini-app) + 401/403/500 회귀 가드를 클러스터에 확대 적용.
...
## 7. Next Action
- TASK-0012-7 Phase 2(cat A 단순 require, ~95 핸들러): 부록 마이그 원자단위(①인라인 conn 삭제 ②수동 close 삭제 ③`_require_account`+`if error` 삭제 ④`account=Depends(get_current_account),conn=Depends(get_conn)` ⑤동반 테스트 TestClient 전환) 를 PR당 15-25 핸들러로 적용. 파일럿/conftest 의 `as_account`·mini-app·401/403/500 회귀 패턴 재사용. 게이트: make test + route-parity + 401/403 회귀.
- 주의(파일럿 교훈): conn-acquire-fail 과 auth-query-raise 의 legacy 의미가 사이트별로 다를 수 있음(fail-soft vs fail-loud) — 전환 전 핸들러별 except 구조를 확인해 get_optional_account(삼킴) vs inline(전파) 선택.

## 8. Completion Checklist (DI seam Phase 1 = TASK-0012-6)
- [x] AC(테스트 인프라 conftest + 파일럿 DI 전환 + 401/403/500 회귀 스위트)가 구현되었다
- [x] behavior-neutral(파일럿 4경로 byte-동치 + deps 계약 적대 검증) — route-parity 179 불변, make test green(1236 passed/2 skip/0 fail), ruff clean
- [x] §18.8 적대 패널 실행(REV-20260629-0004 AGENT-TEAM) — HIGH 2 적발·보정, LOW 보정/수용
