...
- **클라이언트는 알림 영역(트레이)에 상주한다** — [X] 로 닫아도 연결 유지, 우클릭 메뉴로 열기·연결 토글·종료. ⚠ **트레이 생성 실패 시 그 동작을 켜지 않는다**(닫기=종료 폴백) — 「닫아도 유지」는 아이콘이 **지금 살아 있을 때만** 하는 말이고, 판정은 `tray is not None` 이 아니라 `Tray.alive`(backend 생존)다. `pystray` 불채택(Pillow 동반) — `ctypes`+`Shell_NotifyIconW` 직접.
...
- **트레이 미실측 2건** — 동결 exe 에서 `ExtractIconW(sys.executable)` 가 앱 아이콘을 내는지(실패해도 기본 아이콘 폴백) · 탐색기 재시작(`TaskbarCreated`) 후 재등록.
