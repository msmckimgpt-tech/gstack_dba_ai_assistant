...
- [ ] PB-0008 Windows-browser 시각검증: 정적 자산 baked — 라이브 반영은 merge+`make deploy-web` 재배포 선행 필요. **POST-DEPLOY 에서 PB-0008 수행 후 TEST.md §3 Run 에 라이브 append 예정** — 검증 항목: ① composer '+' 메뉴에 "추론 강도" 항목 노출·클릭 시 4단계 팝업 ② 레벨 선택 시 라벨 갱신·전송 후 유지 ③ 대화 전환/새로고침 후 저장값 복원 ④ 비-thinking 모델(로컬 LLM 노출 시) 선택 시 "미지원" 비활성 ⑤ pageerror 0. 사유는 TEST.md §3 Windows-browser Run 기록(CHECK#13).
- [ ] verify-completion --pre-commit PASS → commit → push → PR → main merge → cycle-finalize → web 재빌드·재배포(deploy_scope: included, frontend+backend → web 이미지) → /healthz + 라이브 thinking override smoke.
