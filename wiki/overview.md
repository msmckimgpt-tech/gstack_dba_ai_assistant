...
본 프로젝트는 **기존 `mysql_ai` 운영 자산을 AI 위임 개발 (`ai_delegated_dev`) 템플릿 구조로 이관한 실행형 사본** 이다. 자연어 입력 → LLM tool-call loop → **N 개 데이터소스 (MySQL·MSSQL)** 에 대한 read-only DBA 작업 자동화를 22 개 feature unit 디렉토리(feature 넘버 0001~0022 — feature-0016 은 metadata-graph + zd-pg-pause-caddy 2 슬라이스라 디렉토리 2개, feature-0018 은 디렉토리 없는 슬라이스로 feature-0003 에 코드 거주) + 단일 docker-compose 로 제공한다. 정책 정본 (`AGENTS.md`) + 도메인 정본 (`docs/`) + 기능 정본 (`unit/<id>/docs/`) 의 3-tier 문서 체계로 다중 AI 가 동시에 작업 가능.
...
22-feature 위에 다음 결정이 누적되어 현재 시스템을 형성한다:
