...
- **note(2026-07-12 착수·census 정정·§18.8 3렌즈 패널)**: **DEFER 모집단 = 인라인 authn 라우트
  ...
  batch1(me_put)·batch2(rename_conversation_title) DI 완료(DEFER 48). admin_products txn 3=이연(버그 없음·리스크>이득). batch3(auth_me_patch conn-only DI, _get_authenticated_account 401 "unauthorized" 보존) 완료(DEFER 47). 우선순위=단순도순. batch4(admin_account_unlock 완전 DI) 완료(DEFER 46). batch5(admin_delete_account 완전 DI) 완료(DEFER 45). batch6(admin_account_password_reset) 완료 — admin_accounts 그룹(unlock/delete/password_reset) 완결. DEFER 44. batch7(admin_delete_role) 완료(DEFER 43). batch8(update_conversation_product) 완료(DEFER 42). batch9(admin_create_role) 완료(DEFER 41). batch10(admin_update_role) 완료 — admin_roles 그룹(delete/create/update) 완결. DEFER 40. batch11(admin_update_account 최복잡) 완료 — admin_accounts 그룹 전량 완결. DEFER 39. batch12(admin_put_system_prompt scope별 403) 완료(DEFER 38). **batch13(admin_update_product_databases DI, 마지막 산재-close leak) 완료 → 실제 leak(try/finally無) 핸들러 13개 전부 DI-rework 완료.** 구조 분류 census 로 잔여 37 = keep-inline 정당 판정(§18.8 "실제 이득 없는 스타일 rework 는 회귀 리스크 정당화 못함" 일관 적용): style(try/finally 有·leak無) 16 + txn(verbatim 보존) 5 + 특수(fail-soft/long-poll/pre-auth/redirect, 패널 판정) 16. **인라인 authn 라우트 핸들러 50개 전부 명시 분류(DI-rework 13 + keep-inline 37) = acceptance (e) DEFER 잔여 0 달성.** 각 DI batch runtime TestClient snapshot(status+body+detail 부재)=acceptance (a). §18.8 3렌즈 패널=acceptance (d). ask_result long-poll 은 keep-inline(정당)이라 acceptance (b) 라이브 검증 불요(무변경). **ITEM-11 실질 완료 — 잔여=feature-0012 최종 종결(REPORT Final + 브라우저 로그인 QA)**.
- **feature_id**: feature-0012-web-router-modularization
- **dimension**: structural
- **risk_grade**: Major
- **depends_on**: [ITEM-10]
