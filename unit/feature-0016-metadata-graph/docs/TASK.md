...
- [x] TCR.11 POST-DEPLOY 라이브 확인 (2026-08-03, 배포 D+7) — **오탐 0**(`candidates` 전 구간 0) · 정탐 정상(`SchemaAuto` 4 run / 67 노드 전부 done·failed 0·analysis 67/67) · 요청 3축(테이블·컬럼·프로시저) 실동작 · MSSQL 발동 · 쿨다운 준수(7,624s·4,217s) · 예산 회계 정확(`enqueued==done==node_budget`) · 수동 run 과 이중 지출 0 · 자격 게이트 정상(116 중 112 blocked) · **라이브 3-state 정지 스위치 실증**(50→0 `disabled`→-1 `shadow`→원복, 스냅샷 바이트 일치) — **범위: 스냅샷 override → 새 프로세스 읽기 → 게이트 전환까지. 상시 워커의 TTL 반영은 미관측이며, 라이브 전환은 cap 정수만(env 문자열은 재배포 필요)**. 상세는 TEST.md `### Run — POST-DEPLOY 라이브 … TCR.11`.
- [x] TCR.11a **계측 도달 규약 역전** (2026-08-03) — POST-DEPLOY 관측이 드러낸 결함: `auto_reanalysis_absorbed`·`auto_reanalysis_axis_dropped` 가 `report` 에 기록되면서 payload allow-list 에 미등재라 **7일간 무음**이었다(이 cycle 이 1라운드에서 경계했던 "allow-list 미등재 = 관측 소실" 을 계측을 늘리며 그대로 반복). §18.8 적대 리뷰가 이 보정의 1차 시도를 **BLOCK** — 2키만 등재하고 정규식 감시자를 얹은 것은 (a) 같은 클래스의 고아 카운터 **6건**(`coverage_seeded`·`insight_llm_calls`·`tables_fanout`·`relationships_*` — 그 중 넷은 FUNCTION.md 가 "관측된다"고 선언)을 남기고 (b) 감시자 자체가 유실 3방식 중 2개(주석 처리·지역변수 이동)를 통과시켰다(mutation 실측). → **규약을 뒤집었다**: `_telemetry_sweep` 이 `scan_report` 의 미등재 스칼라를 payload 로 흘리고, 빼야 할 것만 `_TELEMETRY_SWEEP_DENY` 에 사유와 함께 명시한다. "기록했다 ⇒ 도달한다" 가 사람의 기억이 아니라 **구조**가 되고, 고아 6건도 함께 해소된다. 0 이어도 보여야 하는 핵심 지표는 명시 등재를 유지(sweep 은 truthy-only). 테스트도 소스 텍스트 검사 → **동작 검사**로 교체.
      추가 흡수: `shadow`/`unknown` status 무집계(C1 — 안전 모드가 걸렸는지 텔레메트리로 확인 불가했다) · 예외 경로 카운터 부재(C2 — 매 사이클 예외가 "건강한 무변경"과 구별되지 않아 "오탐 0"이 관측이 아니라 추론이었다) · `baseline` 이 unknown 으로 오집계되던 회귀 방지(`_AUTO_INFO_STATUSES`) · per-schema 로그에 `absorbed=` 필드(없으면 TCR.11b 하루 경계 관측이 총합 역산이라 판정 불가) · 문서 정정(예산 불변식 2개 분리 — `3/4` 는 미소진 정상 · blocked/자격 합이 스키마 수를 넘는 이유 · **env 문자열은 재배포 필요, cap 정수만 라이브**인 비대칭) · 범위 밖 파일 모드 변경 복원.
- [ ] TCR.11b POST-DEPLOY 추가 관측 — `absorbed` 등재 후 샤드 경계(하루)를 넘겨 흡수 실효를 수치 확인. TEST.md Run(예정) 참조.
...
## 20260731T0300-band-visual-fit

- [x] E.13 **F1 싱글턴 밴드 제거** — `nm:` 싱글턴을 `기타` 로 흡수. 예외 근거("2차 attach 로
      커질 수 있어")가 이미 끝난 단계라 성립하지 않았다. 라이브: 한 스키마에 `…integrateunion·1`
      `spget…·1` `spupdate…·1` 3개가 46-멤버 밴드와 같은 시각 무게로 서 있었고 패널에도 동일 노출.
- [x] E.14 **F2 루틴 칩 라벨 넘침** — `labelMaxWidth` 하드코딩 176 → `w - 10`. 칩 폭은
      `min(190, TW+rel*40)` 이라 rel 부스트 없으면 150 → 26px 초과. §45 가 테이블만 고치고 루틴을
      빠뜨린 자국. 라이브: 아이콘이 알약 왼쪽 밖, 말줄임표가 오른쪽 밖.
- [x] E.15 회귀 계약 4건(pytest 소스 계약 — 헤드리스 번들은 CI 미실행).


## 20260803T1730-visual-postdeploy

- [x] E.12 label-canon 전량 전환 측정 — 전역 루틴 밴드 **2,703 → 2,703(불변)** · 라벨 종수
...
- [x] EA.7 **POST-DEPLOY PB-0008 실 Windows 브라우저 시각검증**(배포 3d3adb4b, Chrome 150) — 애니메이션
      라이브 동작 실측(`move` items 12/moved 12/incident 5 · 씬교체 가드 발동) · 카메라 "보이면 이동 0"
      픽셀 대조(형제 225px 이동 중 선택 노드 좌표 불변) · 사용자 팬 우선 확인. **한계 2건 명시**:
      중간 프레임 시각 캡처와 "화면 밖 이탈 시 추종" 라이브 재현은 미확보(CDP relay 가 호출 사이
      rAF 를 재움 / 소규모 스키마에서 이탈 미발생). Run: `test-runs.d/20260813T1121-…md`.
- [x] EA.8 **POST-DEPLOY 정정 — 스키마 펼침 중앙 focus 복원**: 라이브에서 스키마 펼침이 "9개 펼침"
      인데 화면엔 카드만 남는 회귀를 확인(펼친 combo > 뷰포트 → 중심 판정이 이동 0). 1차 구현이 이
      경로의 기존 중앙 이동을 요청 범위 밖에서 바꾼 것이 원인 — 되돌리고 계약 테스트로 잠금.
