...
## REV-20260903T110000-ai-claude-corp-feature-0043-caps-finally-swallow [SKIPPED:tool-restricted:adversarial-subagent] — APPROVED

> **왜 SKIPPED 인가**: 본 세션 운영 지시가 사용자 명시 요청 없는 `Agent` 호출을 금지한다.
> 대체 검증은 적대 뮤테이션 3종(아래 렌즈 2)이며, 그중 1종이 실제로 헛통과를 잡아냈다.

### 렌즈 1 — 사용자 콘솔의 경고는 lint 가 아니다

이 결함은 사용자가 **연결 검토를 요청하며 붙인 출력**에 그대로 찍혀 있었다. 「경고일 뿐」로
넘기지 않은 이유는 두 가지다: (a) 사용자 화면에 노출되면 「내 설치가 잘못됐나」로 읽힌다,
(b) 그 자리가 **하필 사용자가 지금 겪는 실패가 나는 함수**였다. Python 3.14 가 이 형태를
경고하기 시작한 것은 정확히 이 위험(예외 삼킴) 때문이다.

### 렌즈 2 — F2 가 헛통과를 잡았다 (이 cycle 의 배움)

초판 단정은 「출력 **어딘가에** 예외 문자열이 있는가」였고, 사유 기록을 제거한 뮤테이션 F2 가
그것을 **통과했다**. 이유: 같은 수정에서 추가한 `caps.probe_crashed` 가 `exc=` 로 예외를
이미 싣기 때문이다. 즉 **내 수정의 한 절반이 다른 절반의 테스트를 헛통과시켰다.**

그래서 단정을 「사용자가 실제로 읽는 자리」(요약 줄 `<런타임>: 사유 —`)로 좁혔다. 교훈은
`REV-20260902T140000` 의 M1·M5 와 같은 축이다 — **넓은 단정은 방어가 성립하지 않아도 통과한다.**

### 렌즈 3 — 예외를 잡되 전파하지 않는 선택

`except` 로 잡고 삼키는 것은 일반적으로 나쁘다. 여기서 고른 이유: 이 함수는 **스레드 진입점**
이고, 예외가 전파되면 그 스레드가 죽어 협상이 「조용히 안 끝나는」 상태가 되며 한 플랫폼
사고가 나머지 플랫폼의 협상까지 죽인다. 삼키는 것과의 차이는 **사유를 두 곳(요약 줄·원장)에
남긴다**는 점이고, 테스트가 그 두 곳을 각각 잠근다. 「조용히」가 아니라 「기록하고 계속」이다.

### 렌즈 4 — 이 cycle 은 사용자 증상을 고치지 않는다 (명시)

같은 조사에서 답변 미수신의 실제 원인을 확정했고 **우리 코드가 아니다**: `claude.exe -p` 가
300초 무한 hang(출력 0바이트)이며, 디버그 로그 **전 세션**에 `oauth/token status=400`
(세션당 최대 26회 재시도)가 있다. 그 머신의 Claude Code 가 갱신 불가한 OAuth 상태다.
이 cycle 이 개선하는 것은 **그 사실이 사용자에게 보이는 속도**뿐이며, 그렇게 적는다 —
「고쳤다」로 쓰면 사용자는 재인증 없이 동작을 기대하게 된다.

### 렌즈 5 — 회귀

- 컨테이너 feature-0043: `main` 기준선과 FAILED 집합 **동일**(동일 이미지 대조) · 신규 5건 green.
- 빌드 산출물 `SyntaxWarning` 0건(컴파일로 단정) · `finally`-return 0건(AST 로 단정).

- **판정**: **APPROVED**. 잔여: 사용자 머신 `claude` 재인증(우리 범위 밖).
...
## REV-20260903T200000-ai-claude-corp-feature-0043-ai-unknown-not-ready [SKIPPED:tool-restricted:adversarial-subagent] — ACCEPTED

- Related TASK: TASK-20260903T200000 (feature-0043-external-llm-bridge · feature-0003-agent-web-ui)
- Timestamp: 2026-09-03T20:00:00+09:00 · Verdict: PASS · Human Approval Needed: no
- Reason: 본 세션은 `Agent` 서브에이전트 호출이 사용자 명시 요청 없이는 금지된다. 리뷰 패널
  대신 **적대적 뮤테이션 스윕 13종**을 대체 수단으로 돌렸다 — 결함을 되살리는 최소 편집마다
  테스트가 실제로 죽는지 확인하는 방식이라, 「방어를 넣었다」와 「방어가 성립한다」를 가른다.

### 스윕이 실제로 찾은 것 (자기검증이 통과 도장이 아니었다는 증거)

1R 에서 **M6 SURVIVED** — `note_ai_probing` 호출 두 곳 중 협상 경로 **하나만** 지워도 테스트가
통과했다. 원인은 제 테스트가 `"note_ai_probing(" in body` 라는 **소스 문자열 존재**만 봤다는
것이다. 호출부가 둘이니 하나가 남으면 초록이다. 이것은 이 저장소가 반복해 만든 「죽은 가드」
부류를 **제 테스트 쪽에서 재현**한 것이다.

조치: 협상 본체(`resolve_caps`)가 불린 **그 순간의 원장 값**을 관측하는 행위 단정으로 교체
(`test_renegotiation_marks_unknown_while_it_runs`). 구조 단정은 「읽는 사람을 위한 표지」로만
남기고, 그 한계를 docstring 에 명시했다.

1R 의 **M3 ANCHOR-MISS** 도 그대로 기록한다 — 뮤테이션 문자열의 들여쓰기가 실제 소스와 달라
**적용되지 않았다**. 그 상태의 「초록」은 KILL 이 아니라 **아무것도 검증하지 않은 것**이며,
스윕이 그것을 SURVIVED 로 세도록 만들어 둔 덕에 발각됐다. 앵커를 고쳐 재실행했다.

### 자기검증에서 스스로 잡은 것

- 제가 쓴 `test_the_fail_open_initial_value_is_gone_from_source` 가 **정상 코드를 결함으로
  신고**했다 — `note_ai_outcome` 안의 정당한 `_AI_READY = True`(성공 관측 반영)까지 걸렸다.
  들여쓰기를 지우고 비교한 탓이다. 모듈 최상위 대입만 보도록 좁혔다. 항진 검사축을 만드는
  부류라, 그 이유를 테스트 주석에 남겼다.
- 기존 테스트 4건이 **고친 결함을 그대로 잠그고 있었다**(`assert ai_health() == (True, "")`).
  약화가 아니라 **계약 변경**이므로 새 계약으로 재작성하고, 게이트 축 단정은
  `ai_blocked()` 로 옮겼다 — 표시 축에서 `True` 를 요구하던 것이 바로 그 거짓이었다.

### 검토한 위험 축

| 축 | 판단 |
|---|---|
| `None` 을 `False` 로 접어 **모든 첫 질문이 죽는** 회귀 | `ai_blocked()` 를 별도 함수로 분리해 구조적으로 차단 · M2 가 잡는다 |
| 구 러너 사용자 전원이 「확인 중」으로 굳는 회귀 | 「키 없음」은 종전대로 미기록(M8·M10) · 구 러너는 자기갱신으로 곧 벗어난다 |
| 「확인 중」이 **영구 상태**가 되는 새 결함 | 생존 확인 1회로 반드시 떨어뜨린다(M5·M7 + 캐시 경로 행위 테스트) |
| 「확인 중」이 관측된 불가를 **세탁**하는 방향 역전 | `note_ai_probing` 이 `False` 를 되돌리지 않는다(M3) |
| 매 기동 AI 호출 1회 추가(비용) | 구독형이며 한 줄 JSON 1건. 이 비용 없이 「정상」을 말하면 그것이 거짓이라는 판단 |
| `<=>` 는 MySQL 전용 문법 | 이 저장소의 웹 저장소는 MySQL 고정(형제 함수들도 같은 방언 사용) |

### 남긴 미검증 (숨기지 않는다)

- 질문을 **이미 보낸 뒤** 확인이 실패로 떨어지는 창(최대 30초)에서 그 질문은 즉시-정직-응답
  경로를 타지 못하고 정상 경로로 간다. 종단 보장은 P0-AK 소관이며 이 cycle 이 좁히지 않았다.
- 서버가 러너로부터 **아무 하트비트도 못 받는** 경우(러너 미기동)는 종전 `listening` 축이
  다루며, 이 cycle 범위 밖이다.
...
`REV-20260903T160000` 과 그 TASK note 는 「자동 갱신이 발동하지 않는다」를 별개 결함으로
적었다. 근거는 「감사 원장에 `stale_build`·self-update 사건 0건」이었다. **그 추론이 틀렸다** —
그 침묵은 설계된 것이고(self-update 가능 시 신호만 세우고 로그를 남기지 않는다, 2026-09-02
사용자 결정), 실제 갱신은 관측 13분 뒤에 정상 발동했다(`run.selfupdate` 실측).

### 왜 이 정정이 load-bearing 인가

틀린 결함 기록은 두 가지를 망친다: ① 다음 사람이 멀쩡한 기능을 고장으로 읽고 조사에 시간을
쓴다 ② **존재하지 않는 이월 항목**이 backlog 에 남아 우선순위를 왜곡한다. 그리고 사용자에게
「수동 재기동이 필요하다」고 잘못 안내하게 된다(실제로 그렇게 보고했고, 그것도 정정 대상이다).

### 반복된 실수의 형태

이 세션에서 **부재를 신호로 읽는** 판단을 두 번 했다. 한 번은 맞았고(`ai.fail` 의 `exit`
필드 부재 = `returncode is None` — 그 필드는 값이 있으면 반드시 기록되도록 설계돼 있었다),
한 번은 틀렸다(여기 — 그 사건은 **이 조건에서 기록되지 않도록** 설계돼 있었다).

규율: **부재를 근거로 쓰기 전에 「그 신호가 이 조건에서 기록되도록 설계돼 있는가」를 먼저
확인한다.** 두 사례의 차이는 정확히 그 확인을 했는지 여부였다. 아울러 **스냅샷으로 영구
상태를 단정하지 않는다** — 조건부 트리거(유휴·간격·협상 종료)는 관측 시점에 아직 조건이
맞지 않았을 뿐일 수 있다.

- **판정**: **APPROVED**. 정정 후 남는 이월은 「러너가 제출조차 못 하는 경우의 서버측 종결」
  하나이며(`TASK-20260903T140000` §7), 자동 갱신 축은 이월에서 **제거**한다.

## REV-20260904T040000-ai-claude-ux-contract [SKIPPED:계약 전제 변경] — ACCEPTED
- Timestamp: 2026-09-04T04:00:00+09:00
- `test_ux_parity.py` 3건이 **옛 주 경로(터미널)** 를 계약으로 잠그고 있어 깨졌다.
  주 경로가 연결 프로그램으로 바뀐 것은 사용자 결정이므로 **계약을 갱신**했다.
- ⚠ 갱신하면서 **보호하던 성질은 그대로 뒀다**: ① 대상을 밝힌다(주 경로는 「무엇을 누른다」,
  접힌 경로들은 여전히 「어디에 붙여넣는다」) ② 사람이 할 일의 끝을 약속한다(주어만 바뀜)
  ③ 동시에 고르게 하지 않는다(`== 1` → `<= 1`; 계약은 개수가 아니라 동시 노출이다).
- 전제가 바뀔 때 그 게이트를 그냥 지우면 결함이 계약이 된다. 그래서 지우지 않고 옮겼다.
...
## REV-20260904T190000-ai-claude-runner-wsl-merge [SKIPPED:merge-verification] — ACCEPTED
- Timestamp: 2026-09-04T19:00:00+09:00
- 자동 병합이 실패한 자리를 손으로 해소했으므로 **양측 부모 대비**로 확인했다 — 손 해소는
  한쪽을 통째로 고르기 쉬운 자리이고, 그러면 남의 기록이 조용히 사라진다.


## REV-20260904T200000-ai-claude-ssot-scan-without-git [SKIPPED:자기 적대 리뷰] — ACCEPTED
- Timestamp: 2026-09-04T20:00:00+09:00

**내가 낸 결함이고, 형태가 나쁘다.** 개명 주기에서 「저장소 전체를 조회한다」는 강한 게이트를
세웠는데 그 조회 수단이 **실행 환경에 없었다.** 게이트는 붉게 죽어 있었고, 붉은 것이 이미
있는 스위트에서는 한 건 더 붉어도 눈에 띄지 않는다 — 게이트를 세운 사람이 그것이 도는지
확인하지 않으면 이렇게 된다.

**skip 을 고르지 않았다.** 가장 쉬운 수선은 `pytest.skip("git 없음")` 이고 스위트는 즉시
초록이 된다. 그러면 이 게이트는 **CI 에서 영영 안 돈다** — 초록은 「검사했다」가 아니라
「검사하지 않았다」가 된다.

**두 수단의 드리프트를 인정하고 잠갔다.** 조회 수단이 둘이면 한쪽만 고쳐진다. 그래서 둘 다
되는 머신에서 **판정이 같은지** 대조한다. 실제로 이 대조가 두 번 울렸고(추적 안 된 새 파일 ·
빌드 산출물), 둘 다 진짜 어긋남이었다.

**남는 비대칭**: 걷기는 dotfile(`.github/…`)을 보지 않는다. 지금은 거기에 이 이름이 없어
판정이 같지만, 생기면 대조 테스트가 먼저 운다 — 놓친 채로 지나가지는 않는다.
...
## REV-20260907T060000-ai-claude-model-list-visible [CODEX:runner-caps] — ACCEPTED
- Timestamp: 2026-09-07T06:00:00+09:00

**진단이 이 주기의 절반이었다.** 「모델 목록이 안 나온다」는 서버·프론트 어디를 봐도
정상이었다 — `RunnerCapabilities` 가 `'[]'` 였고, 그 값을 만든 것은 러너도 서버도 아닌
**codex 자신**이었다. 열거 질의에 22.7초 만에 `{"models": [], "model_flag":
["--model","{model}"]}` 로 답한다. 플래그는 주면서 값은 없다는 것은 「지원하지 않는다」가
아니라 「지금 답하지 못한다」인데, 코드에는 그 둘을 가르는 자리가 **모델 축에만** 없었다
(추론등급 축에는 `_settle_effort_axis` 가 이미 있다 — 같은 결함 클래스의 절반만 고쳐져
있었던 셈이다, §16.7 G8).

**우회로를 세 번 시도했고 세 번 다 실패했다.** 가드 해제 200초 · 모델축만 좁게 240초 ·
서버 원장 확인질의 600/603초 — 전부 타임아웃. 「질문을 좁히면 답이 수렴한다」는 이
feature 의 기존 처방이 이 축에서는 통하지 않는다는 것이 실측으로 확인됐다. 여기서 멈추고
사용자에게 물은 것이 옳았다: 남은 선택지가 **2026-09-02 사용자 결정(「확인-후-표시」)을
뒤집는 것**이었기 때문이다(§18.12 미결정 제시 채널).

**사용자의 추가 한 줄이 설계를 바꿨다.** *"공식 웹사이트 등에서 해당 목록을 즉시 얻을 수
있다면"* — 웹사이트를 찾기 전에 `codex debug --help` 를 읽었더니 `models  Render the raw
model catalog as JSON` 이 있었다. **0.23초 · 토큰 0 · 네트워크 무관**이고, 값이 우리 표가
아니라 그 CLI 의 출력이라 「목록의 출처는 연결된 AI」 계약을 오히려 더 정확히 만족한다.
LLM 에게 자기소개를 시키던 축 전체가 이 한 줄로 대체됐다 — 웹 스크래핑은 검토할 필요조차
없었다(네트워크·파싱 취약성·드리프트를 모두 피한다).

**정직하게 남기는 것 셋.**

1. **등급을 교집합으로 좁혔다.** 카탈로그는 등급을 **모델마다** 신고하는데(실측
   `gpt-5.6-luna` 에는 `ultra` 가 없다) 신고 스키마는 런타임당 목록 하나다. 합집합이면
   「고를 수 있는데 그 모델에서는 거부되는」 조합이 생기므로 — 이 feature 가 P0-T 에서 지운
   바로 그 상태 — **덜 주되 항상 맞는** 쪽을 골랐다. 모델별 등급은 스키마 변경이라 별도 cycle.
2. **원장 폴백은 낡을 수 있다.** 실제로 지금 원장은 낡았다 — `gpt-5-codex`·`gpt-5.2`·`gpt-5.4`
   는 카탈로그에 더 이상 없고 `gpt-6-astra`·`gpt-5.4-mini` 가 새로 있다. 그래서 이 폴백은
   **맨 뒤**이고 캐시되지 않으며 재시도가 계속 돈다. 낡은 값을 골랐을 때의 backstop 은
   `handler.py` 의 「반영하지 못한 지정을 답변에 적는다」다.
3. **재시도는 비용이 있다.** 카탈로그가 없고 열거도 실패하는 런타임에서는 시간당 최대 4회
   질의가 돈다. 총 횟수 상한을 두지 않은 이유는 「한 시간 뒤에 CLI 로그인을 고친 사용자」가
   목록을 영영 못 받는 상태가 이 수정이 없애려는 상태와 **같기** 때문이다. 간격 사다리가
   비용을 억제한다.

**뒤집은 계약을 테스트에 명시했다.** `test_baseline_alone_is_not_reported` →
`test_baseline_is_reported_when_live_confirmation_never_lands`. 이름을 바꾸고 docstring 에
왜 뒤집혔는지를 적었다 — 계약이 조용히 반대가 되면 다음 사람이 그것을 회귀로 읽는다.
그리고 **남은 경계**(출처가 `baseline` 로 정직하게 표시된다 · 캐시에 남지 않는다)를 같은
테스트가 함께 잠근다. 완화가 곧 무규제가 아니다.

**첫 구현이 «존재하지만 실행되지 않는» 상태였다** (§16.7 G14-e). 카탈로그 읽기를
`_CAPS_PROBE_MAX_BYTES`(256KB)로 잘랐는데 실측 출력이 413,777바이트라 JSON 이 항상 중간에서
끊겼다 — 배선은 완성돼 있고 단위 테스트도 통과하는데(작은 표본을 썼으니) 라이브에서는 **한
번도 성공하지 않는다**. 종단 실행(`detect_runtimes` 를 실제 codex 로)을 돌리지 않았으면
그대로 나갔을 것이다. 회귀 테스트가 **종전 상한을 넘는 표본**을 쓰는 것이 그 잠금이다.


### codex 적대 리뷰 (`[CODEX:runner-caps]`) — P1 **0** · P2 4 (전건 조치·판정)

- Trigger: API/response shape(`_SANITIZE_SOURCE_ALLOW`·신고 스키마) + performance/캐싱 keyword
  matched → backend·qa 축. 이 저장소에는 도메인 subagent 정의가 `improve-fit-reviewer` 하나뿐
  이므로 §18.8.1 #2 의 **codex 채널**로 수행했다.
- **호출 형태가 결과를 갈랐다** (LRN 재확인): 1차는 「저장소를 직접 읽어라」로 던졌고 **40분
  타임아웃·산출 0**(exit 143)이었다. 2차는 **도구 금지 + diff 인라인(38KB) + 3줄 출력 형식**
  으로 바꾸자 6분 만에 정상 판정이 나왔다. 무산출은 모델이 아니라 호출 형태의 문제였다.

| # | 지적 | 판정 | 조치 |
|---|---|---|---|
| P2-1 | `_needs_retry` — 한 런타임이 성공하면 재시도가 멎어 **다른 런타임의 빈 목록이 영구화** | **타당** | 판정 모수를 신고 목록 → **직전 회차가 물어본 런타임 집합**(`asked_out`)으로 바꿨다. claude 성공 + codex 실패가 이 저장소의 실제 조합이라 실측 가능한 형태였다. 회귀 테스트 1건 |
| P2-2 | `_parse_codex_catalog` — `supported_reasoning_levels` 컨테이너 타입 미검사 → `TypeError` 가 **확인·열거·원장 폴백 3경로를 통째로 건너뛴다** | **타당** | 타입 가드 + 호출부 `try/except` 두 겹. 서버 `_capped_list` 와 같은 결함 클래스의 반대편이다 |
| P2-3 | `capture_output=True` 라 8MB 상한이 **메모리를 막지 못한다** | **타당(주석이 거짓이었다)** | 코드는 형제 헬퍼(`_ask_json`·`_cli_help_text`)와 같은 규약이라 유지하고, **거짓을 주장하던 주석을 정정**했다 — 상한은 «파싱 대상» 을 자르고 메모리 경계는 20초 timeout 이다. 스트리밍 전환은 세 곳을 함께 바꿔야 하는 별도 축 |
| P2-4 | 컴포저 안내 제거로 **복구 안내가 유실**된다 | **by-design (사용자 지시)** | 이 cycle 의 원 요청이 그 UI 의 제거다 — 인용은 `feature-0003-agent-web-ui/docs/TASK.md` §4. §18.8 「의도된 구성」 조항대로 리뷰 입력이 아니라 **요청 자체**가 근거이며, 대가는 REPORT.md·REVIEW.md 에 명시했다(남은 표면 = 연결 칩 + 연결 모달) |

**자기검토가 별도로 1건을 더 잡았다** — 카탈로그를 `ask` 에 무조건 넣으면서, 그 카탈로그가
실패하는 빌드(구 codex)에서 **이미 확정된 캐시가 축 재질의로 흘러** 매 기동마다 LLM 질의를
한 번씩 태우게 됐다. 캐시를 둔 이유를 정면으로 깨는 회귀라 `_caps_axis_unsettled` 로 막고
회귀 테스트를 붙였다. codex 는 이 건을 못 봤다 — diff 만 보면 `ask` 의 종전 불변식(«여기 오는
캐시는 반드시 미확정») 이 보이지 않기 때문이다. 두 축이 서로를 덮었다.

**수렴 계약(§18.8 v3.48.0)**: 마지막 라운드 P1 **0**. P1 수정이 없었으므로 확인 라운드 조건에
해당하지 않는다(P2 조치 4건은 라운드 재개시 대상이 아니다 — 종결 조건은 P1 0 이다).


## REV-20260907T063000-ai-claude-model-list-visible [SUBAGENT:backend,qa,security,ux,design] — ACCEPTED
- Timestamp: 2026-09-07T06:30:00+09:00
- Trigger: `API/endpoint/contract` + `UI/layout/screen` keyword matched → backend·security·qa·ux·design
- Channel: subagent 패널 3인 (backend/qa · security · ux/design). **codex 채널은 실패했다** —
  `codex exec` 리뷰가 40분 timeout 에 **산출 0** 으로 죽었다(이 저장소의 메모리에 이미 기록된
  실패 모드: 저장소 탐색을 요구하는 프롬프트는 무응답). 채널을 바꾼 것이 옳았다.

### 라운드 1 — P1 3건 · P2 18건

**두 리뷰어가 독립적으로 같은 P1 둘에 수렴했다.** 서로 다른 도메인(backend/qa · security)이
같은 두 줄을 지목한 것은 그 자체가 강한 신호다.

**P1-1 건강 축 오염 (`caps.py`)** — `if got: note_ai_outcome(True)` 가 출처를 보지 않아
`catalog`·`baseline` 도 「그 AI 는 지금 응답한다」로 확정했다. `codex debug models` 는 **로컬
조회라 OAuth 가 만료돼도 exit 0** 이고, `baseline` 은 서버 원장을 되받은 것이라 AI 가 개입조차
하지 않는다. 그 `True` 때문에 lifecycle 의 생존 확인(`if ai_health()[0] is None: …`)이 건너뛰어져
**답하지 못하는 AI 가 화면에 「대기 중」으로 나간다** — 이 feature 가 2026-09-03 에 닫은
「연결되지 않았는데 정상이라 말한다」의 정확한 재생산이다. 조치: `_LIVE_ANSWER_SOURCES =
{probe, verified}` 로 좁혔다. 모르면 `None` 으로 남겨 생존 확인이 돌게 한다.

**P1-2 원장 화석 영구화 (`shared/bridge_caps.py`)** — `baseline` 에코가 `else` 가지로 흘러
`probed_at`·`verify_streak` 은 얼렸지만 `last_used_at = stamp` 는 매번 찍었다. 그러면 만료의 두
축이 **둘 다** 죽는다: `prune_stale` 의 TTL(14일)은 그 값을 읽으므로 도달 불가, `verify_streak`
는 0에 멈춰 `BASELINE_MAX_VERIFY_STREAK` 차단도 불가. 러너 재시도 주기(≤900초)가 throttle
(3600초)을 매시간 넘기므로 실제로 매시간 밀린다 — **원장을 되받아 원장을 영구화**하는 닫힌
루프다. 리뷰어가 100일 병합을 실행해 재현했다. 조치: `baseline` 출처는 항목을 **건드리지 않고
지나간다**(`continue`). 내 주석은 이미 「아무것도 건드리지 않는다」라고 적고 있었는데 코드가
그것과 달랐다 — **주석이 코드를 앞질러 참을 주장한 형태**다.

**P1-3 무음 강등 (`composer.js`, ux)** — 카탈로그를 못 받으면(`/api/api-vault/options` 503·
타임아웃) 선택기가 사라지고 `model`·`reasoning_level` 이 전송에서 빠지는데, 그 사실을 말하던
유일한 표면이 이번에 제거한 안내 문단이었다. 같은 파일이 **동형의 무음 강등에 이미 토스트를
쓰고 「무음 금지」라고 적어 두었다**(`_modelSelectionSilentlyDropped`). 조치: 같은 채널로 옮겼다 —
레이아웃 비용 0 이라 사용자의 제거 지시와 충돌하지 않는다. 상태 전이마다 1회만 알린다.

### P2 조치 (택한 것 / 남긴 것)

| # | 지적 | 조치 |
|---|---|---|
| 등급 교집합 | 노출 모델 **하나**가 등급을 안 실으면 전 등급이 사라짐 | 모수를 «신고한 모델» 로 한정 + 절단을 로그로 남김(무음 절단 금지) |
| 사유 마스킹 | 카탈로그 사유가 `reasons[nm]=` 로 먼저 박혀 뒤 `setdefault` 를 가림 | 별 dict 로 분리해 **폴백**으로만 쓰게 |
| 캐시 삭제 | 카탈로그 승리가 `_detail` 에서 키를 빼 `save_conf` 가 기존 캐시를 지움 | 캐시 금지 출처면 **기존 캐시를 그대로 유지** |
| 재시도 문 술어 | 문은 `not runtimes`, 판정은 `_needs_retry()` — 갈려 있었음 | 문도 `_needs_retry()` 로 통일 |
| 회차 겹침 | 앞 회차 지각 스레드가 새 목록을 되덮고 `dict(caps)` 가 `RuntimeError` | 회차 토큰 + 게시 락 + 락 아래 스냅샷 |
| 재시도 비용 | 폴백이 서 있어도 15분마다 240초 질의를 무기한 태움 | 「보여줄 것이 있을 때」 천장을 1시간으로 분리 |
| 툴팁 상한 | `aiUnreadyReason` 이 여러 줄이면 「최대 3문단」이 깨짐 | 표시 직전 한 문단으로 접고 140자 상한 |
| aria-live | 제거된 문단의 `aria-live` 를 승계한 곳이 없음 | 칩에 `role="status" aria-live="polite"` 승계 |
| 과장 서술 | 「칩·게이트로 일원화」가 코드와 어긋남(게이트는 미연결일 때만 뜸) | 주석·FUNCTION.md 를 실제 커버리지로 **정정** |
| 잔여 | `.composer-gate`·`.timeout-extend-banner` 도 컴포저 높이를 흔듦 · `model_selector_reason` 소비처 0 | 이 cycle 밖 — `REPORT.md §8` 에 기록만 |

### 이 라운드가 드러낸 나 자신의 실패 패턴

**내가 쓴 테스트 셋이 내가 만든 P1 을 통과시켰다.** 리뷰어가 그것을 실행으로 증명했다 —
원장 테스트는 `probed_at`·`verify_streak` 만 보고 정작 만료를 정하는 `last_used_at` 을 안 봤고,
폴백 테스트는 출처·캐시만 보고 건강 축을 안 봤으며, 툴팁 예산 게이트는 **깨질 수 있는 유일한
입력을 주석으로 명시 제외**했다. 셋 다 「검사한 것처럼 보이는 자리」를 검사한 것이다.
세 테스트를 그 축까지 넓히고, TTL 종단 테스트(폴백만 28일 반복 → 실제로 걷히는가)를 더했다.
...
## REV-20260908T010000-ai-claude-live-record [SKIPPED:non-policy-doc] — ACCEPTED
- Timestamp: 2026-09-08T01:00:00+09:00

문서 전용 주기(코드 델타 0) — 패널 dispatch 대상이 아니다. 리뷰할 것은 **기록이 사실인가** 뿐이다.

**이번에야 「사용자 화면에서 봤다」를 말할 수 있다.** 앞선 실측은 검증 계정(`bootstrap_admin`)
으로 했고, 그것으로 「같은 코드니 사용자도 같을 것」을 미룰 수 있었다 — 이 저장소가 반복해서
겪은 간극이 정확히 거기다(세션 쿠키 결함도 그 간극에 숨어 있었다). 이번에는 사용자 계정의
앱 창에서 직접 봤고, 화면에 뜬 것이 그 계정의 러너가 신고한 값임을 DB·서버 응답·화면 세
지점에서 같은 값으로 대조했다.

**정직하게 남기는 것 셋.**

1. **사용자 러너는 스스로 살아나지 않았다.** 배포의 자기갱신이 그것을 죽였고(경합) 약 15시간
   끊겨 있었다. 이번 cycle 이 살린 것은 사람이(내가) 개입했기 때문이지 제품이 회복한 것이
   아니다 — 그 구분을 §8 에 명시했다.
2. **토큰을 다뤘다.** 사용자 계정의 연결 토큰을 발급해 러너에 넘겼다. 값은 임시 파일로만
   옮겨 프로세스 인자·로그·이 문서 어디에도 남기지 않았고, 측정 후 파일을 지웠다.
