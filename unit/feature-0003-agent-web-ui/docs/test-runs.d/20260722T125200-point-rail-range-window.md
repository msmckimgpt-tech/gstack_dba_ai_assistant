...
- **[POST-DEPLOY 튜닝 재검증 2026-07-22, 배포본 66d1e735] Environment: Windows-browser (AI 직접 — win-browser host-resolver relay, Chrome 150)**: `WINDOW_INITIAL_RENDER` 8→3 배포 후 라이브 실측. conv[2](14개·개별 메시지 큼) 초기 렌더 **3개**(dots 3, ratio 13.2→**7.7배** 개선)·최상단 스크롤 시 3→13→14 점진 창 확장(dots 동기)·A 막대 범위화 정상(allBars=true·높이 다양 14.4%/0.96%/10.9%/1.6%). 스크린샷 evidence/point-rail-windowing-live.png(초기 최근 3개·우측 rail 뱃지 3개로 밀집 완화). **사용자 후속 요구 "이전 대화가 너무 많이 불러와졌을 경우 일부만 로딩 + 최상단 상승 시 추가 로딩" 라이브 실증 완료**(14→3 초기·스크롤 확장).
