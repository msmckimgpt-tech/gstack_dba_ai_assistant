...
- [x] 구현·단위/통합 회귀·검증 패널
- [x] 최신 main 830f39fd 통합 및 집중 180건 회귀 재검증 PASS.
...
## TASK-20260814T183000-attach-change-signal-server-authority — 첨부 변경-인지 신호의 서버 권위 봉인 (Major §12.3)

`/_dqa:conversation_audit` 라이브 진단(`FR-attach-change-signal-client-only`). 사용자 호출
(2026-08-14): "`수정된 랭킹 쿼리 코드 재검토` — assistant 가 첨부파일의 변경사항을 제대로 인지하지
못하는 현상이 간헐적으로 나타난다. 특이사항: 추론 실행 중 해당 대화를 공유대화로 전환했다."

- [x] **라이브 실측 — 무엇이 실제로 비어 있었나**. 대화 `…f72f26ef`(P-119, owner A-10) 2차 turn:
      사용자가 16:35:26~27 에 파일 3개를 v2 로 재업로드 → 16:36:41 재리뷰 요청. 그 job 의
      `payload.new_attachment_ids` 가 **빈 배열**(1차 turn 은 4건 정상). 같은 시각
      `core_attachments` 에는 그 3건의 **v1→v2 unified diff 가 이미 계산·저장**돼 있었다
      (1013 / 1992 / 480자). 서버가 가진 사실을 클라이언트 플래그 부재를 이유로 감춘 것.
- [x] 근본 확정(RC-1) — 변경-인지 4축이 **단일 클라이언트 신호**에 걸려 있다.
      `agent_core._load_new_attachment_ids()` 가 반환하는 집합 하나로 ① `★신규`/`◆세션` 라벨
      (:1822) ② `## FILE UPDATES` diff 렌더(:2037) ③ ATTACHMENT SET 권위 사실(:2212 침묵 분기)
      ④ 리뷰어 digest version_diff(:4720) 가 동시에 결정된다. 그 신호는 브라우저 in-memory pill
      상태에서 나오므로 대화 전환·첨부 패널 조작·새로고침이면 서버 목록 재수화로 사라진다
      (`composer.js::_loadConversationAttachments` 가 전 항목을 `source:"session"` 으로 덮었다).
      **갱신 파일이 오히려 "이전 세션에서 첨부" 로 오라벨**되는 것이 이 결함의 실제 표현이다.
- [x] corroboration **structural** — 60일: 직전 ask 이후 사용자 업로드가 있는데 신호가 빈 job
      **14건 / 14 대화**, 버전 갱신 축 **5/37 = 13.5%**. 1:1 6.8% vs 그룹 15% 로 그룹이 2배지만
      **양쪽에서 발생** — 사용자가 지목한 "공유대화 전환" 은 판별자가 아니다(기각).
- [x] 거짓양성 기각(`refuted`) — ① **"업로드가 스코프에 안 들어갔다" → REFUTED**: 해당 15개 첨부
      전부 `payload.attachment_ids` 에 실려 있었다(라벨만 빠짐, 파일 자체는 보였다). ② **"공유대화
      전환이 원인" → REFUTED**(위 rate 대조). ③ **"08-05 봉인이 이미 덮는다" → REFUTED**: 그 봉인은
      신호가 **있을 때** 동작하며, 빈 신호에서는 설계상 침묵한다(그 침묵 자체는 옳다 — 부정 단정
      금지). 즉 이 건은 선행 봉인의 **미도달 구멍**이지 회귀가 아니다.
- [x] 정직 — affected set 의 최종 답변에서 **명시적 부재 단정은 0건**이다. 관측 대화에서도 모델이
      `read_attachment` 4회로 파일을 다시 읽어 복구했다(22분 소요, diff 를 스스로 재구성). 확정된
      오답이 아니라 **입력 저하 + 봉인 미도달**로 기록한다. 그 시각 클라이언트 wipe 트리거는 웹
      컨테이너 재시작(16:49)으로 액세스 로그가 소실돼 직접 관측하지 못했다(경로는 코드로 확정).
- [x] 사전 검증(구현 전, 60일 재생) — 서버 파생 규칙을 과거 job 에 적용: 복구 **6 job / 5 대화**,
      기존 합치 43, **침묵 유지 299**, 오탐 **0**(6건 전부 직전 턴 이후 실제 사용자 업로드).
      관측 turn 에서 정확히 `{1187,1188,1189}` 를 복원한다.
- [x] 봉인 구현 — 서버 권위 파생(`_derive_server_new_attachment_ids`) + 클라이언트 신호와 합집합.
      기준선은 직전 job 의 전달 스코프(`load_prev_turn_attachment_ids`), 판정은 **첨부 id 단조성**
      (시간축 비의존). 프론트는 재수화가 미전송 신규 표식을 보존(feature-0003 cross-ref).
- [x] 검증 — 신규/갱신 **24 PASS**(서버 17 + 프론트 구조 가드 7) · 뮤테이션 **5/5 KILLED**
      (합집합 제거 · id 단조 가드 제거 · 기준선 None↔[] 혼동 · 프론트 무조건 session · 강등 키에서
      effective 키 제거) · 전체 스위트 회귀 0(잔여 실패 1건은 pristine main 에서도 동일 재현되는
      환경 산물 `chattr` 부재) · ruff clean.
- [x] **§18.8 적대 검증 — codex 5+2 라운드**(`REV-20260814T183000-attach-change-signal-server-authority`).
      세션 지침상 Agent tool 미사용 → §18.8.2 제약-없는-채널 우선으로 `codex review --uncommitted`.
      **P1 5 · P2 7 적발 → 전건 처리, 최종 라운드 P1 0건**. 설계를 바꾼 지적:
      ① 현재 job 을 `run_id` 로 되찾으면 RO replica 지연에서 **봉인이 조용히 꺼진다** → 워커가 아는
      job id 직접 전달. ② `COALESCE` 가 unknown 을 known-empty 로 승격 → 제거. ③ 그룹에서 **발신자별
      스코프는 비교 불가** → 기준선을 같은 account 로 한정. ④ 완료되지 않은 턴은 기준선 자격 없음
      → `status='done'`. ⑤ 강등은 **전송 스냅샷 id** 로만(응답 대기 중 올린 파일 보호). ⑥ 큐 조회를
      히스토리 읽기 토글에서 **분리**(설정 하나로 봉인이 꺼지지 않게).
      **자기 회귀 1건 포함** — ④ 수정 때 넣은 `state.activeConversationId` 가 타 대화의 미전송 표식을
      지우는 역회귀라 codex 가 잡아 되돌렸다.
- [x] **PB-0008 라이브 시각검증**(머지 전, 라이브 무접촉 격리 컨테이너) —
      1턴 `new_attachment_ids=[1203]` → 완료 후 2턴 `[]`(강등) → 대화 전환 왕복 후 `[1204]`(보존).
...
- 사용자 승인 범위(AskUserQuestion 2026-08-14): **"서버 + 프론트 표식 보존"** — 서버 권위 파생을
  기본 봉인으로 두고 프론트 표식 보존까지 포함. 첫 턴 확대는 실익 낮아 미포함(전부 v1 이라 비교
  대상이 없다), fork 오라벨 위험도 회피.
...
- [x] 배포 — PR #1303 merge main `6cd45761` → `deploy-web` 전체 롤아웃(6서비스 healthy ·
      무중단 실측 blip 0 · 배포본 봉인 심볼/배선 실증 완료).
- [ ] 라이브 재현 실측 — **미수행**(자격증명 접근 차단으로 PB-0008 로그인 단계에서 중단).
      다음 audit 이 관측 지표 3종(봉인 A 발동 로그 · `enqpre-` 고착 대화 수 · 조기 종료 job 의
      KV terminal 도달률)으로 재측정 → `verified` 전이.
...
## 20260814T0800-attach-provenance-gate — 타 멤버 첨부 본문이 실린 턴의 쓰기 도구 차단 (Critical §12.3)

- **출처**: 사용자 결정(2026-08-14) — 남은 판단 ③ "쓰기·외부영향 도구만 게이트".
  SECURITY §47.4 가 **수용 위험**으로 남긴 confused-deputy 경로를 실행 단계에서 좁힌다.

### 배경 — 프롬프트 계약만으로는 보장이 아니다

공유 대화에서 타 멤버 첨부를 읽을 수 있게 되면서(§47), 그 파일 안의 지시문이 **호출자 권한으로**
도구를 움직일 여지가 생겼다. datamark + "데이터로만 취급" 계약은 확률적 완화이며, 도구 실행 시점에
"이 지시가 어느 파일에서 왔는가" 를 검사하는 provenance 게이트는 없었다.

### 조치

- [x] **단일 choke-point** — `tools.execute_tool()` 앞에 `_provenance_gate()`. 모든 도구가 이 함수를
      거치므로 새 도구가 추가돼도 자동으로 덮인다(핸들러마다 흩으면 하나가 조용히 빠진다).
- [x] **대상은 상태 변경만** — `scratch_sql`·`scratch_import`·`scratch_reset`·`update_attachment`.
      `execute_sql` 은 **의도적 제외**: `sql_guard` 가 단일 SELECT/CTE 만 허용하고 DDL/DML 을 전면
      차단하므로 성격이 조회다. 그것까지 막으면 "남의 파일을 보며 라이브 DB 와 대조" 하는 그룹
      대화의 핵심 작업이 죽는다(과차단).
- [x] **발동 조건은 본문 렌더** — `_UNTRUSTED_ATTACH_BODY_CTX` 는 타 멤버 파일의 **본문이 실제로
      프롬프트에 들어간** 순간에만 선다. 목록·파일명은 주입 벡터가 아니며, 그것까지 신호로 삼으면
      남의 파일이 대화에 있다는 이유만으로 쓰기가 막힌다.
- [x] **실패는 막는 쪽으로** — 신호를 못 읽으면(contextvar 깨짐·import 실패) 차단한다. 이 게이트가
      조용히 열리면 존재 이유가 없다.
- [x] **run 경계 초기화** — `compose_system_prompt` 진입 시 False 로 리셋(워커 스레드 재사용 시
      이전 run 의 상태가 남지 않게 — turn-facts 와 같은 규약).
- [x] **거부는 교정 정보를 싣는다** — 왜 막혔는지·무엇은 되는지(조회)·어떻게 푸는지(그 파일 없이
      재요청 / 본인 파일로 / 분석만 이어가기) + "이유를 숨기지 말 것". 이유 없는 거부는 모델이
      같은 호출을 반복하거나 사용자에게 "실패" 만 전하게 만든다.

### 왜 "사용자 확인" 이 아니라 "차단" 인가 (정직)

승인 문구는 "추가 확인을 요구" 였으나, 이 시스템은 **비동기 워커**라 턴 중간에 사용자 확인을 받을
수단이 없다. 모델 자신이 세우는 확인 플래그(`confirm_heavy` 류)는 여기서 방어가 되지 않는다 —
**주입된 지시가 그 플래그도 세우게 만들 수 있기 때문**이다. 그래서 "이번 턴 거부 + 다음 턴에
사용자가 선택" 으로 뒀다. 사용자는 거부 사유를 보고 1~3 중 하나를 고르면 된다.

### 검증

- [x] `tests/test_attach_provenance_gate.py` **신규 11 PASS** — 대상 4종 차단 · 조회 8종 무차단 ·
      `execute_sql` 제외가 의도임을 고정 · 게이트 목록 동결 · 무신호 시 무차단(무회귀) ·
      **목록만 실리면 플래그 안 섬**(과차단 방지) · 본문 실리면 섬 · run 경계 리셋 ·
      신호 조회 실패 시 차단 · helper 자체 fail-closed · 거부 문구 4요소 · dispatch 선행 배치.
- [x] 적대 패널 `[CODEX:adversarial-bypass]` — **[P1] 2 · [P2] 1 전건 반영**:
      ① **`read_attachment` 우회(재현됨)** — 이 도구는 프롬프트 조립 **이후** 본문을 끌어온다.
         인라인 경로에서만 신호를 세운 초판은 타 멤버 파일을 인라인 상한 밖에 두고 읽는 것만으로
         통째로 우회됐다(`flag_after_read=False` → `scratch_sql` 성공). → 본문을 돌려주는 자리에서
         소유자 기준으로 신호를 세운다(`_load_scoped_attachment_rows` 에 AccountId 추가 — 없어서
         판별 자체가 불가능했다).
      ② **AI 생성본 우회** — `created_by_role=="assistant"` 를 통째로 제외해 타 계정 소유 AI 파일이
         우회 경로가 됐다. → 라벨(사람 파일)과 소유 사실(AI 포함)을 분리.
      ③ **[P2] 과차단이 빠져나갈 수 없음** — 공유 대화는 매 턴 첨부를 인라인하므로 초판 대상은
         상시 차단이 됐고 거부 안내가 성립하지 않는 해결책이었다. → 대상을 scratch 3종으로 좁히고
...
- provenance 기반 도구 게이트(쓰기·외부영향 한정) — ✓.
- **범위 밖(정직)**: 도구 인자 수준의 출처 추적(예: 이 SQL 문자열이 어느 파일에서 유래했는가)은
  하지 않았다. 턴 단위 신호로 충분히 좁혀지며, 문자열 출처 추적은 오탐·누락이 모두 크다.

## 20260814T1000-vision-provenance — 이미지 첨부의 provenance 신호 (Critical §12.3)

- **출처**: 선행 cycle `20260814T0800-attach-provenance-gate` 의 §18.8 적대 리뷰가 **미해소로 남긴 축**
  (REPORT §잔여 · SECURITY §47.4 에 "vision 경로 미적용" 으로 명시했던 것). 사용자 지시 "우선순위에
  따라 진행" 에 따라 **1순위**로 착수 — 게이트를 만들어 놓고 한 경로가 비어 있으면 "막았다" 는 인식만
  주고 실제로는 뚫린다.

### 왜 이미지가 특히 위험한가

이미지는 **datamark 로 감쌀 수 없는** 콘텐츠다. 텍스트 첨부는 비신뢰 구획으로 묶고 "데이터로만
취급" 계약을 걸 수 있지만, 이미지는 프롬프트에 그대로 붙는다. 그 안에 심긴 지시문은 텍스트 축의
어떤 방어도 거치지 않으며, 종전에는 provenance 신호조차 서지 않아 도구 게이트가 통째로 비어 있었다.

### 조치

- [x] **소유자를 실어 나른다** — 이미지 조회 SELECT 에 `AccountId` 추가(**MySQL 폴백 + PG 미러 양쪽** —
      한쪽만 실으면 읽기 백엔드에 따라 방어가 사라진다) → inline JSON 에 `account_id` 동봉.
      종전 JSON 은 `filename`/`mime_type`/`base64_data` 뿐이라 판정 자체가 불가능했다.
- [x] **로더가 신호를 세운다** — `_load_attachment_inline_images()` 가 소유자와 호출자를 비교해
      다르면 `_UNTRUSTED_ATTACH_BODY_CTX` 를 세운다. **호출측에 의존하지 않는다**(로더 자신이 책임).
- [x] **순서 계약** — 이 로더는 `_call_llm` 안에서 돈다. `compose_system_prompt`(플래그 리셋) 이후,
      도구 실행 이전이다. 순서가 뒤집히면 신호가 지워진 채 도구가 실행된다.
- [x] **소유자 키 부재는 종전 동작** — 배포 혼합 창의 구 JSON 을 막는 쪽으로 두면 그 동안 **1:1
      사용자까지** 도구가 막힌다. web 이 먼저 롤링되므로 워커가 신코드일 때 JSON 도 신형식이며,
      이 경로는 짧은 과도기에만 존재한다.
- [x] **파싱 실패는 막는 쪽** — 소유자를 숫자로 못 읽었는데 이미지 본문은 이미 프롬프트로 간다.

### 검증

- [x] `tests/test_vision_provenance.py` **신규 9 PASS** — 타 멤버 이미지 발동 · 본인 이미지 미발동 ·
      혼재 시 발동 · 소유자 키 부재 무회귀 · 호출자 미상 미발동 · 빈 파일 no-op · 파싱 실패 차단 ·
      순서 계약 · **web 양 백엔드가 소유자를 싣는지**(파일 기반 — web import 는 순환).
- [x] 적대 패널 `[CODEX:adversarial-coverage]` — **[P1] 2 · [P2] 2**: 2건 반영 · 1건 **한계로 채택** ·
      1건 과도기 수용.
      ① **[P1] sandbox csv/xlsx 축 누락** — csv/xlsx 는 본문이 아니라 샘플 행으로 들어간다.
         공격 셀 → `scratch_*` 우회가 남았다. → 타 멤버 소유 csv/xlsx 면 신호.
      ② **[P2] 순서 테스트가 약함** — 문자열 존재만 봐서 순서 회귀를 못 잡았다. → 리셋 직후 상태에서
         **게이트 끝단 도달**까지 검증(+ 로더 미실행 시 열려 있는지 반대 축).
      ③ **[P1] 히스토리 replay — 한계로 채택**: 타 멤버 채팅·`read_attachment` 결과가 다음 턴에 다시
         들어오지만 신호를 세우지 않는다. 넓히면 **그룹 대화에서 scratch 상시 차단**이라 §47.4 가
         피하려 한 과차단과 같아진다. 게이트 정의를 "이번 턴에 타 멤버 첨부 본문이 **새로** 실렸는가"
         로 못 박고 히스토리 축은 프롬프트 계약에 맡긴다.
      ④ **[P2] owner 키 부재 과도기** — 구 web payload 를 신 worker 가 읽는 짧은 창의 fail-open.
...
- [x] **배포 완료·배포본 실증**(2026-08-26, PR #1348 → main `ded1812d`, 6서비스 `ded1812d`,
      caddy blip 0). 원 마찰 첨부 1239/1240 에 계보 라벨·진입점·금지 문구가 렌더됨을 실측.
...
실측까지 진행」. 계정에 AI 를 연결해 **실제 대화를 완주**시키자, 직전 두 cycle 이 만든 도달
경로가 라이브에서 **여전히 0 기여**임이 드러났다. 단위 테스트·핸들러 직접 호출로는 절대 안
나올 결함이다 — **AI 가 도구를 부를지 말지**에 달려 있었기 때문이다.

| 항목 | 내용 |
|---|---|
| 증상 | AI 가 "등록 메타데이터 어디에도 없습니다" — 번들에는 있는데 |
| 원인 | 러너 도구 목록에 `get_task_context` 부재 → AI 가 존재를 모름 |
| 처방 | 서버가 점유 응답에 근거를 실어 **자동 주입**(도구 호출 불요) |
| 집행 위치 | 서버(러너는 사용자 머신 설치본이라 배포가 안 닿음) |

### 완료 체크리스트

- [x] `claim_request` 가 KB 근거를 조립해 `kb_context`·`kb_notes` 로 응답
- [x] 원문(`question`)으로 매칭 — 가드 래퍼(`marked`) 아님
- [x] 러너가 받은 근거를 **질문보다 앞**에 배치 + 재조사 금지 안내
- [x] 근거 없으면 블록 생략(옛 서버 호환)
- [x] 상한 + 절단 고지 유지
- [x] 러너 두 사본 byte-동치
- [x] 뮤테이션 7/7 KILL (baseline green 확인 후) · `make test` rc=0 · 6,957 tests
