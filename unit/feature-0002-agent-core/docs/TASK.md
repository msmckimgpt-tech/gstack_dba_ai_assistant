...
## 1. Current Status
- State: **TASK-0247 (데이터플레인 연결 격리 — 불안정 datasource 의 blast radius 를 각 연결별로 차단) 구현 완료, 리뷰·배포 진행** — bounded connect timeout(`AGENT_DB_CONNECT_TIMEOUT_SEC`) + per-datasource circuit breaker(`scope_key` 기준 in-memory)로, 한 datasource 의 연결 불안정이 단일 ask-worker 를 점유해 정상 datasource 제품까지 지연시키던 직렬 starvation 을 끊음 (cycle: TASK-0247)
...
- Owner: AI (본 cycle: agent_core `db.py`(`_connect_dataplane`/circuit breaker/`_dataplane_connect_timeout`) + `config.py` env var)
- Priority: high
