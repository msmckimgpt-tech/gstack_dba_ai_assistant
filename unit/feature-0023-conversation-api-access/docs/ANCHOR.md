...
세 번째 의문(2026-08-12, **방향 분기**): "외부 AI가 `ask` 로 답변을 받아가는 대신, **자기 LLM으로
직접 추론**하면서 데이터소스·RAG에만 접근하게 하면 안 되나?" 답: 되지만 **이 feature의 축이
아니다.** 본 feature의 외부 AI는 *질문하는 사람* 역할이고 추론은 우리 LLM이 한다 — 비용·가용성·
답변 품질 책임이 전부 우리 것이라는 전제 위에 절대 denylist와 안전 기본 scope가 설계돼 있다.
추론 주체를 뒤집으면 신뢰경계(우리 LLM → 통제 밖 LLM)·비용 주체·품질 책임이 동시에 바뀌므로
같은 unit에 두면 그 설계가 두 목적 사이에서 흔들린다. 그 축은
**feature-0041-external-ai-tool-surface** 로 분리했다. 본 feature 는 `ask` 축으로 유지되며
두 표면은 용도로 구분해 병존한다(무회귀).
