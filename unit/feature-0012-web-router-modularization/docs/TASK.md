...
- [~] TASK-0012-7 DI seam Phase 2..7: 클러스터별 핸들러+테스트 동반 마이그(A 단순→B 인라인perm→C/E1 require_permission→F optional→D 동적→트랜잭션4+E2)
  ...
  - [ ] Phase 3 cat-B 잔여: 보류 3(suggestions fail-soft·admin_products_insight_coverage·admin_overview 재검토) + preauth/longpoll ~32(아키텍처 이연 — cat-A preauth 동일) → cat C(require_permission 정적)/D(동적)/E(헬퍼)/F(optional)/트랜잭션 → Final.
...
## 8. Completion Checklist (DI seam Phase 3 cat-B batch 5 = TASK-0012-7 testconv 4)
- [x] testconv 4 핸들러 전환(AO 3 + RP 1) — cat-B 잔여 workflow 확정 14 완결, 누적 cat-B 29
- [x] behavior-neutral — make test progress-chars 1238(baseline 동일)·0 fail, route-parity 179 불변, ruff clean, py_compile OK
- [x] §18.8 적대 패널 — REV-20260630-0007(REV-0006 확정분 재사용). RP vs AO 테스트 전환 차이 검증
- [x] 동반 테스트 전환 — AO 3은 직접호출에 actor|account/conn 명시 인자(concurrency 테스트 보존), RP 1은 TestClient+as_account(require_permission 검사 보존). 잔류 직접호출 0
- [x] TASK/REPORT/MODIFY/REVIEW 반영, BLOCKED 없음
- [ ] verify-completion PASS(9 checks) → Git 커밋 (Phase 3 cat-B batch 5)
