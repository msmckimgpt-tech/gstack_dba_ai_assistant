...
- **stale leftover**(2026-09-02 재실측): ① 등록 worktree 중 `ahead == 0` 방치분 **6건** — `ai/claude/feature-0007-llm-timeout-align`(1200 behind) · `ai/claude/feature-0041-external-ai-tool-surface`(414 behind) · `ai/claude/feature-0006-winbrowser-tab-survival`(242 behind) · `ai/claude/feature-0043-tool-permission-friction`(210 behind) · `ai/claude/feature-0043-ai-assisted-setup`(204 behind) · `ai/claude/conv-status-dot-wiring`(27 behind). 3~5번째는 **2026-08-28 창의 PR 착륙(#1404·#1417·#1421)으로 ahead 0 이 된** 방치분이고, `conv-status-dot-wiring` 은 **2026-09-01 창에 tip `0f89132e` 가 main 이력에 포함되며 새로 ahead 0 이 된 것**이다 → `bin/cycle-finalize.sh` 또는 `git worktree remove` 대상(본 run 의 `ai/claude/doc-sync-20260902-010304` 는 실행 중이라 제외). ② worktree 등록 없이 `.worktrees/` 에 디렉토리만 남은 잔재 7건(`base-check` · `datasource-accessible-db` · `feature-0003-attach-version-diff` · `feature-0003-attach-diff-colgroup` · `feature-0016-cluster-label-target` · `task0215-insight-toggle` · `task0216-rag-ds-aware`) → `git worktree prune` + 디렉토리 삭제 대상. 구 지목분(attach-cutover / conn-health / task0232)은 정리 완료(등록·디스크 모두 부재). 어느 쪽도 흡수 대상 아님.
