...
## 진행 요약 (2026-07-30, T0b) — 커넥션 축 게이트 + 콘솔 노출

T0 에서 정직하게 이연한 두 항목을 닫았다. **T1 접지의 전제** — T1 은 L0 증거 수집(운영 DB read)이고,
그 부하를 강제할 `ds` 게이트가 없으면 "적응형 부하 제어"가 강제 수단 없는 선언에 그친다.

### 들어간 것
- `ds`(소스 DB 동시 연결) **정확 게이트** 3지점 — 점유 수명 = 연결 수명(같은 finally 반납)
- `task`(동시 진행 백그라운드 작업) 진입 게이트 3지점 — 래퍼/`_*_inner` 분리로 **반납 누수 구조적 불가**
- 콘솔 노출 — web 이 **공유 볼륨의 워커 flush 파일**을 읽는다(PG 테이블·마이그레이션·신규 라우트 **0**).
  거절 발생 시 `attention` 부상, stale·손상·거대 파일은 degrade 강등

### 설계 판단
- **`AGENT_WORKER_PG_BUDGET` 을 만들지 않았다** — PG 동시 점유의 정확한 강제는 커넥션 수명·예산 수명
  결합(광범위 리팩터)이 필요하다. `task` 로 근사하고 이름을 그 의미로 유지한다. 게이트 없는 이름을
  쓰지 않는다는 ADR-0025-06 의 연장.
- **진입 게이트는 구조로 누수를 막는다**(ADR-0025-07) — 반납 누수는 "다음부터 아무 작업도 안 됨"이라는
  조용한 전역 고장이고 원인이 멀다. 수동 enter/exit 는 `ds` 처럼 수명을 정확히 맞춰야 할 때만 쓰고,
  그때도 획득·반납을 한 쌍의 헬퍼로 모은다.
- **콘솔은 파일 경유**(ADR-0025-08) — 관측 목적이 "상한을 조여도 되는가" 판정이라 현재 상태 스냅샷으로
  충분하다. 라이브=운영에서 마이그레이션 비용·위험을 지불할 이유가 아직 없다.

### 검증
신규 15건(예산 6 + 콘솔 9) + 기존 계약 확장, 관련 스위트 **168 passed / 0 failed**. 등재 자원 전부가
실제 게이트 지점을 가진다는 **소스 단정**으로 ADR-0025-06 을 기계적으로 고정했다.

### 잔여
- 배포 후 라이브 실증 — 콘솔 `worker_resources` 섹션 노출 · `ds_conns` 증분 관측
- 게이트 실전 동작(`rejected > 0`)은 상한을 현행 사용량 미만으로 내려야 관측 — 운영 설정 변경이라
  사용자 결정(T0 부터 이어진 잔여)
- **다음 트랙 = T1 접지**(L0 통계 전용 증거층, 원시 샘플값 배제 확정)
