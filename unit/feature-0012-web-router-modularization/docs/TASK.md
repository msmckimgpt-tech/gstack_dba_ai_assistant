...
- [~] TASK-0012-7 DI seam Phase 2..7: 클러스터별 핸들러+테스트 동반 마이그(A 단순→B 인라인perm→C/E1 require_permission→F optional→D 동적→트랜잭션4+E2)
  ...
  - [ ] Phase 3 cat-B batch 2+: 확정 잔여 9(본문 finally:conn.close() sole-in-block → try/finally 제거+dedent 필요: public_share_fork·upload/delete_product_icon·admin_llm_usage·admin_get_dashboard_prefs·verify_audit_chain·admin_health_attachment_grants·admin_list_datasources·admin_datasource_databases) + workflow 보류 8(동반 테스트 전환 누락 5: admin_test_datasource·admin_product_db_insights·admin_usage_conversations·admin_archived_conversations·admin_overview; action-분기 admin_get_system_prompt; fail-soft suggestions; admin_products_insight_coverage) + AND pre-auth gate/multi + OR·BRANCH 23 → cat C/D/E/F/트랜잭션.
...
## 8. Completion Checklist (DI seam Phase 3 cat-B batch 1 = TASK-0012-7 require+perm 6)
- [x] cat-B 6 핸들러 전환: RP 4(require_permission, 403 메시지 verbatim 보존) + AO 2(get_current_account, perm 본문 유지)
- [x] behavior-neutral — make test progress-chars 1238(baseline 동일)·0 fail, route-parity 179 불변, ruff clean, py_compile OK, account-anchor 53→47
- [x] §18.8 적대 패널 — REV-20260630-0003: cat-B workflow(67 agents) clean-AND 23 → 15 확정/8 보류. RP 메시지 byte-보존·perm-before-resource·401≺403 순서 적대검증 통과
- [x] 동반 테스트 — 6개 모두 직접호출/패치 없음(HTTP-레벨/route-snapshot/source-assertion). 잔류 직접호출 0 확인
- [x] sole-in-block conn.close() 사전 스캔으로 6 clean 만 적용(9 는 본문 finally try/finally 제거 필요로 batch2 분리)
- [x] TASK/REPORT/MODIFY/REVIEW 반영, BLOCKED 없음
- [ ] verify-completion PASS(9 checks) → Git 커밋 (Phase 3 cat-B batch 1)
