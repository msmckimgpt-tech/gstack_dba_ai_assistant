...
## REV-20260903T010309-doc-sync-0903 [SKIPPED:cross-cutting-doc-index-sync] — doc_sync 09-03 색인·미러 정합 66건 (META-0070)
- 범위: `docs/{ARCHITECTURE,CODEBASE_MAP,CODE_TASKS,DOC_REGISTRY,SECURITY,STATUS}.md` + `wiki/{Features,Architecture,Flows,concepts,overview,README,hot,Log}` 색인·미러. **정본(`unit/*/docs/*`·`docs/DECISIONS.md`) 변경 0 · 코드 변경 0 · 권한 변경 0.**
- 델타 창: `d2d43b1f`(META-0069, 09-02 02:00) → `5d984b75` = **151 커밋(74 non-merge / 77 merge)** · 착륙 2026-09-02 02:03~21:52. **신규 feature 0개**(45 불변 — `ls -d unit/feature-*` 45 = `wiki/Features/feature-*.md` 45). 무게중심 feature-0043(51 non-merge) · feature-0006(3) · feature-0016(2) · feature-0003(1) · feature-0002(1). 창이 wiki 에 남긴 것은 `Flows/` 신설 + `Index`/`Log`/`hot`/`Data-Flow` 배선뿐이라 **feature 카드 45개 · `Architecture/Overview` · `Features/_Index` · `Module-Map` 은 한 줄도 안 건드려** 전량 stale 이었다.
- reconcile-first(**커밋 직전 재측정 2026-09-03T01:55:08+09:00**): 서빙 `release-notes-data.js` md5 `d6876ca9…` = `origin/main` blob 동일 → **파리티 갭 0**. `repo/` main clean · HEAD == `origin/main` == `5d984b75` · `deploy-web.state` mtime 09-02 22:03 · 배포 실패 마커 0. run 시작(01:05) 값을 옮기지 않고 다시 쟀다. landing·배포 소유권은 cron wrapper 라 **detect-and-report**(스킬은 로컬 commit 까지).
- 정합 66건(META 65 + 릴리즈노트 1): `docs/ARCHITECTURE.md` §4 feature-0043 2건 + **0002·0003·0006·0016 4행**(검증조 MISS) · `docs/SECURITY.md` §49.3 **5행**(러너 자기 갱신 · 런처 CA 핀 · 콘솔 작업 위임 설정 · 「AI 작업」 노출 + **KB 근거 자동 주입**) · `docs/CODEBASE_MAP.md` 4건 · `docs/STATUS.md` passthrough 8행 · `docs/{CODE_TASKS,DOC_REGISTRY}.md` · wiki 는 0043·0003·0002·0006·0016 카드 · `Architecture/Overview` §2.3 5건 · `Features/_Index` MOC 5건 · `Module-Map` · `Data-Flow` · `Flows/{External-AI-Bridge,Feature-Operations}` · `concepts/{rag,insight-worker}` · `overview` · `README` 트리 · `hot.md` 4건 · `Log.md` append-only entry. 릴리즈노트는 별도 operational 커밋(feature-0003).
- **제1 발견 — 이 창의 최대 KB 반출 변경이 보안 색인에 0건이었다**: `7b296ade` 가 접지 지식의 도달 경로를 「외부 AI 가 도구를 부르면 받음」(`get_task_context`)에서 **점유 응답에 무조건 실림**(`claim_request` → `kb_context`/`kb_notes`)으로 바꿨는데 `docs/SECURITY.md` 전체에 그 문자열이 **0건**이었다. §49.3 은 「새로 생긴 표면과 그 경계」를 전수 열거한다고 주장하는 표라 최대 표면 누락은 기존 행까지 약화시킨다 — 정본 feature-0002 `TASK-20260902T110000-kb-prompt-grounding` 이 확정한 경계(product scope 는 그 task 의 `ProductId` · 값 없으면 블록 생략 · 층별 fail-soft)를 **행 1개로 색인 반영**했다. 정책 신설이 아니다.
- **제2 발견 — 폐기 명제 「`get_task_context` 가 유일한 경로」가 wiki 5표면에 산재**: 축 B 가 4표면(`Flows/External-AI-Bridge` §2.7 · `Architecture/Data-Flow` §2.2 · `concepts/rag` 2곳)을 잡았으나 두 검증 렌즈가 **독립적으로** 5번째(`Flows/Feature-Operations`:206)를 찾았다. 창이 **신설한** 문서가 신설 시점부터 폐기 명제를 들고 있던 사례 — 과거 문장을 지우지 않고 「주 경로」로 완화 + 09-02 정정을 덧붙였다.
- **제3 발견 — 이 run 의 편집 세트 자체가 새 비대칭을 만들 뻔했다**: 축 B 는 `wiki/Architecture/Overview` §2.3 을 창이 건드린 **5 feature 전건** 갱신하는데 축 C 는 짝 표면 `docs/ARCHITECTURE.md` §4 에서 **feature-0043 만** 고쳤다. 결합 적용하면 같은 커밋 안에서 0002·0003·0006·0016 네 행이 wiki=09-02 / docs=09-01(0016 은 08-13)로 갈린다. 두 렌즈가 동시에 MISS 로 잡아 4행을 같은 커밋에 포함시켰다.
- **적대검증**: ULTRACODE 워크플로 `wf_0c1fdc18-fd9` — 6 에이전트 / **1,208,678 토큰** / 395 tool-use / 38분. 4축 병렬 스윕(릴리즈노트·wiki·정책문서·정본교차) → 2조 교차검증(정본대조 렌즈 / 기계적용+거버넌스 렌즈, 각자 MISS 능동 탐색). **56 findings · 112 verdict**(정본대조 53/56 · 기계 47/56 CONFIRMED) · **신규 MISS 15건(중복 제거 후 10건 채택)** · 상호배타 치환 **4쌍 적발**.
- **기각 4건(중복 span)**: `DX-01`↔`C-13`(`CODEBASE_MAP` §4 러너 모듈 수, 바이트 동일 span) · `DX-03`↔`C-12`(§3 Shared Module Index 3행 append — 둘 다 적용 시 6행 중복) · `DX-07`↔`B-33`(`README` 트리 `Flows/` 블록 2회) · **`B-32`↔`DX-04`**. 앞 셋은 문면 차이뿐이라 하나씩 채택했고, 마지막은 **사실 판정으로 갈렸다** — 두 렌즈 권고가 정반대였고(기계 렌즈 B-32 / 정본 렌즈 DX-04) 제3 증거로 종합했다: `git log --diff-filter=A -- shared/bridge_consent.py` = `cc8e8571`(**2026-09-01**)이므로 B-32 의 「09-02 창 신설」 귀속이 사실오류다 → **DX-04 채택**.
- **교정 5건 수용**: `B-02`·`B-51`(feature-0043 카드·`hot.md` 가 러너 정본을 **현재형 18 모듈**로 단정 — 실측은 `ls src/agent/*.py` 20 − `__init__` = **19** = `__init__._EMIT_ORDER` 19항목. 같은 창 `016f3e0c` 의 `selfupdate.py` 때문이며, docs 축은 전부 19 를 쓰므로 그대로 두면 이 커밋이 wiki=18/docs=19 비대칭을 **새로 만든다**. 「분할 시 18 · 자기 갱신 추가로 현재 19」로 시점 분리) · `B-30`(미러가 정본의 유보절 「연결 순서 판정축 정정은 POST-DEPLOY 미완」을 **삭제** — 정본 `TASK.md`:2032 는 여전히 `- [ ]` 미체크라 SHA 만 전진시키고 유보절 복원) · `C-08`(STATUS 셀에 752자 서사 추가 → ADR-0031 §1 인덱스 모델 위반, 앵커는 남기고 포인터형으로 축약) · `B-60`(Log entry).
- **하네스 결함 1건(자기 귀속)**: `B-60` 교정본이 **최종 대체문이 아니라 「국소 치환 2건」을 서술한 지시문**이었는데 기계 적용이 그것을 그대로 `Log.md` 에 append 했다. `git status` 확인에서 적발해 원본 `newString`(4,122자)을 복구하고 지시대로 두 치환을 반영했다(5,013자). 교훈: `correctedNewString` 은 fence 추출 후에도 **대체문인지 지시문인지** 판별해야 한다.
- **검증자 주장 1건 반증**: 정본 렌즈가 MISS-01 의 보강 앵커로 제시한 `20260902T1230-console-jobs-apifetch` 는 `unit/feature-0003-agent-web-ui/docs/TASK.md` 에 **count=0** 이었다(나머지 4 앵커는 count=1 확인). 그 인용만 빼고 검증된 `20260901T1900-conv-status-dot-wiring` 으로 한정했다 — 검증자의 「실측」 주장도 재실측 대상이다.
- **verify 한계 + 대체 검증**: cross-cutting doc 정합은 단일 feature-id 가 없어 feature-mode verify 가 부적합하고 check #1·#5 는 v1.1 deferred 라 STATUS·wiki 정합을 보증하지 않는다. 대체로 타깃별 기계 검증을 직접 수행 — `node --check` PASS · `node tests/verify_release_notes.mjs` **34 passed / 0 failed**(편집 **전** 동일 스크립트로 34/0 을 측정했으므로 회귀 0) · `bin/gen-status.sh --check` 최신(frontmatter 20 · passthrough 25 · 신규 0) · `bin/ssot-lint.sh` **4건 = baseline**(선재 tracked `.env` 백업) · `bin/wiki-lint.sh` total **32 = baseline**(broken 1 선재 `Log.md → project_task_conv_entry_defaults`) · 렌더 행 `Features/_Index` **45** · `docs/STATUS.md` 표 **45** = ground truth **45** · glued `||` **0건** · `docs/SECURITY.md` §49.3 날짜 순열 08-28→09-02 **오름차순 유지**(표 마지막 행 anchor append) · 전 66 edit `oldString` count==1 기계 검증 후 적용.
- **report-only(정본·소유자 소관 — 미러만 고치면 다음 창에 재발)**: ⓐ feature-0043 `TASK.md` §1 이 `TASK-20260902T172500` 에 멈춰 뒤이은 2 cycle(`20260902T1900-caps-probe-fast`·`TASK-20260902T193000`)을 담지 않는다(직전 창 이월 ⓐ 의 **부분 해소 후 재발** — 문면만 전진). ⓑ 같은 파일이 「`insight_summary` 재확인」·「AC-3 lease 회수 관찰」을 두 절(:2703-2705, :2726)에서 중복 추적해 최신 판정을 문서만으로 결정 불가. ⓒ `docs/RELEASE_NOTES.md` `status: active` 인데 본문 최신 2026-06-30 · 마지막 touch 07-13, 그런데 `AGENTS.md:664-665` 가 여전히 산출 표면으로 지목(**4창 연속 이월** — 실 수리 지점은 정본 `AGENTS.md`). ⓓ `docs/ARCHITECTURE.md` `## 8.` 2개 · `docs/CONVENTIONS.md` `## 10.`/`## 12.` 각 2개 — §번호 인용 결정 불가(**4창 연속 이월**). ⓔ `docs/CODEBASE_MAP.md` §4 는 고유 44 번호 중 **22 커버 · 결손 22**(축 C 가 「45 중 20·결손 25」로 보고했으나 검증조 재실측이 축 D 수치를 채택했다). ⓕ `wiki/Features/feature-0016-metadata-graph.md` §2 단계 목록의 07-14~07-29 구간 미backfill(카드 자신이 명시). ⓖ `docs/improvements/onboarding-accessibility/{RESEARCH,ROADMAP}.md` 가 **삭제된 파일** `bridge_agent.py` 를 구현 대상으로 지목하고 「18 모듈·5,849행」 폐기값을 든다 — `improve_research`/`improve_listup` persona 산출물이라 doc_sync 미러 범위 밖으로 보고 손대지 않았다(다음 `improve_cycle` 이 stale 전제로 착수하지 않도록 명시). ⓗ `wiki/hot.md` 는 압축했으나(2,543→2,355자) 여전히 소유 규약 목표 ≤500자의 4.7배 — 압축은 다음 창 과제로 이월.
- **[SKIPPED] 사유**: cross-cutting **색인·미러 정합 전용**. 정책·구조 결정을 새로 쓰지 않고 정본과 어긋난 미러를 정본 쪽으로 수렴시킨 것뿐이다. 코드·스키마·권한 변경 0. 적대검증은 위 워크플로 6 에이전트가 수행했다. **Human Approval Needed**: 아니오.

## REV-20260903T130000-ai-claude-spike-02-native-client [SKIPPED:non-policy-doc] — SPIKE-02 판정 (conditional-go)
- Related TASK: _meta_ (ROADMAP SPIKE-02 · 산출물 `docs/improvements/onboarding-accessibility/SPIKE-02-native-client.md`)
- Timestamp: 2026-09-03T13:00:00+09:00
- 범위: **조사 문서 1건 신설 + ROADMAP/RESEARCH 갱신.** 코드 변경 **0** (SPIKE 계약대로 제품 코드 무접촉).
- **[SKIPPED] 사유**: §18.8.1 마지막 문단 — `docs/improvements/**` 는 정책 doc 목록 밖의 비정책 doc. 조사 산출물이라 적대 패널이 볼 코드 표면이 없다.
- **실측 4축 중 2축을 실제로 실행했다** (조사 문서의 가치는 근거 등급에 있다):
  - **항목1 GO** — `claude auth login/logout/status` · `codex login/logout/login status` 존재를 `--help` 로 확인하고, **격리 설정 디렉토리**로 미로그인 상태를 만들어 양방향 대조: 로그인됨 `exit 0`+JSON(`loggedIn:true`) / 미로그인 `exit 1`+`loggedIn:false`. codex 도 `exit 0/1` + 문자열. **로그아웃하지 않고** 측정했고 실행 후 현 세션 인증 무손상을 재확인했다.
  - **항목3 GO** — 러너 `src/agent/` **AST 전수 의존성 검사**: 최상위 import 19개 중 **서드파티 0** (stdlib 전용). 동봉에 플랫폼별 특수 처리가 불요하다는 결론의 근거.
- **항목4 — 「보고를 실측으로 승격」하려다 「보고가 틀렸음」을 찾았다**: `claude -p` 의 Agent SDK 크레딧 풀 분리는 2026-05-14 발표 후 **2026-06-15 취소**됐다. 현재도 구독 한도에서 차감된다. **앞선 세션 대화에서 이것을 사용자에게 주의사항으로 전달했으므로 정정 대상**이며, RESEARCH §9.0.1 의 해당 문구를 취소선 처리하고 근거를 달았다. SPIKE 를 두지 않았으면 잘못된 안내를 제품에 넣었을 것이다.
- **항목2 — ROADMAP 의 하드 게이트 문구를 완화 제안**: 초판은 「서명 없으면 경고가 터미널 벽을 그대로 대체」였는데, 조사 결과 **벽의 높이가 다르다**(터미널=무엇을 할지 모름 / SmartScreen=경고 읽고 2클릭). 다만 **macOS 는 경고가 아니라 차단**이라 지원 대상에 넣으면 Apple Developer(9/년)가 필수. → 「확보 불가 = 차단」을 **「확보 불가 = 범위 축소(Windows 우선)」** 로 재분류 제안. 채택은 사용자 결정이며 ROADMAP 게이트 문구에 그 선택지를 명시했다.
...
## REV-20260908T024925-META73-template-security [SUBAGENT:security-reviewer] — PASS
- Related TASK: _meta_
- Timestamp: 2026-09-08T02:48:39.716969+00:00
- Trigger: template v3.54.2 Codex agent-board delivery; upstream security review reused (no new consumer-specific reviewer invocation).
...
- Artifact: [security review](https://github.com/msmckimgpt-tech/ai_delegate_dev_template/blob/97131eb/_template_maintainer/reviews/20260908-codex-board-security.md)
...
## REV-20260908T024925-META73-template-qa [SUBAGENT:qa-reviewer] — PASS
- Related TASK: _meta_
- Timestamp: 2026-09-08T02:48:39.716969+00:00
- Trigger: template v3.54.2 Codex agent-board delivery; upstream qa review reused (no new consumer-specific reviewer invocation).
- Verdict: PASS — upstream implementation reviewed; canonical isolated --apply succeeded and delivered code/context bytes match source 97131eb.
- Artifact: [qa review](https://github.com/msmckimgpt-tech/ai_delegate_dev_template/blob/97131eb/_template_maintainer/reviews/20260908-codex-board-qa.md)
- Scope: preserved consumer customization outside the managed board section; project/token authorization unchanged. External PR/merge approval pending.

## REV-20260908T024925-META73-template-backend [SUBAGENT:backend-reviewer] — PASS
- Related TASK: _meta_
- Timestamp: 2026-09-08T02:48:39.716969+00:00
- Trigger: template v3.54.2 Codex agent-board delivery; upstream backend review reused (no new consumer-specific reviewer invocation).
- Verdict: PASS — upstream implementation reviewed; canonical isolated --apply succeeded and delivered code/context bytes match source 97131eb.
...
## REV-20260908T040415-delegation-idempotence [SUBAGENT:workflow_audit] — PASS
- Related TASK: TASK-20260908T020000-delegation-friction
- Trigger: live already-done completion warning and obsolete direct-init new-session instructions
