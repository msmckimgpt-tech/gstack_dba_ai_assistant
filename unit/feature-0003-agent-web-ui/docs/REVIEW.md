...
## REV-20260824T220000-tween-render-defer [CODEX:frontend-adversarial] — [P1] 2 · [P2] 2 전건 흡수

**Trigger**: `UI/렌더/애니메이션` keyword matched (§18.8), 제약 없는 채널(codex)로 수행(§18.8.2).

### 이 cycle 이 존재하는 이유 (정직)

직전 cycle 에서 codex 가 "트윈이 두 번째 렌더에 잘린다"([P1])를 지적했고, 나는 대화 이동의
즉시-렌더를 제거해 해결했다고 보고 **배포**했다. 그런데 배포 후 라이브 실측에서 여전히 잘렸다 —
이동 직후에는 읽음 처리·히스토리 로드 등 후속 갱신 렌더가 여럿 따라오고, 그 중 하나만 트윈 창에
겹쳐도 재구성이 진행 중인 transform 을 지운다. **한 경로를 막아 해결했다고 판단한 것이 성급했다.**
경로가 아니라 렌더 진입점 한 곳에서 막는 것이 옳았다.

### 지적과 처리

- **[P1] 보호 창이 commit 시각 기준.** 실제 transition 은 다음 rAF 에서 시작하므로 rAF 가 지연되면
  창이 먼저 닫힌다. **타당하다** — "언제 시작하는가" 와 "언제 시작한다고 가정했는가" 를 혼동했다.
  → `_playReorderMove` 가 실제 시작 기준으로 창을 연장(줄이지 않음).
- **[P1] 타이머 세대 경합.** IME 보류와 플래그를 공유해 같은 창에 복수 타이머가 생길 수 있고,
  오래된 타이머가 새 트윈의 창을 지운다. **타당하다** — 기존 메커니즘 재사용의 대가를 보지 못했다.
  → deadline 토큰 + 창 연장 확인(이중 방어).
- **[P2] 트윈 중 편집 진입 시 포커스 유실.** 편집 진입이 동기 렌더를 전제한다는 사실을 놓쳤다.
  → 편집 세션이 살아 있으면 보류하지 않는다. **연출보다 입력 반응성이 우선**이라는 판단.
- **[P2] 하네스가 문자열만 확인.** 세 번째 같은 지적이다(기대값 토톨로지 → 배선 문자열 → 보류
  문자열). → wrapper 를 가상 시간·가상 타이머로 **실제 구동**하는 테스트로 교체.

### 남은 한계

개별 가드(세대 토큰 / 창 연장 확인)는 이중 방어라 하나만 제거해도 동작이 유지된다 — 실행 테스트로는
개별 제거를 잡지 못해 **존재 계약**으로 보완했다. 둘 다 제거하면 실행 테스트가 red 다.
