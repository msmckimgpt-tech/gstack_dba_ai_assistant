...
- **blocked_reason**: ~~ITEM-14 미완~~ **해소(2026-08-14)**. 남은 차단 사유는 **검증 표본 부족** 하나다 —
  ...
  → **선행 확정(2026-08-14 사용자 위임 판단)**: 프롬프트 **출력 계약 회귀 테스트 신설**을 채택한다.
  대안이던 "ITEM-11 플래너로 증거 커버리지 확대"는 워커가 시간을 두고 채우는 축이라 착수 시점을
  우리가 정할 수 없고(현재 1.3%), verdict 표본이 유의미해질 때까지 ITEM-15 가 무기한 대기한다.
  회귀 테스트는 지금 만들 수 있고 **결정론적**이다 — 프롬프트 변경이 출력 계약(4필드 존재 ·
  `caveats` 빈값 규칙 · `role` enum · `suggested_links` 끝점 실재)을 깨는지를 LLM 없이 판정한다.
  커버리지 확대는 이 축과 배타적이지 않으므로 ITEM-11 진행분이 쌓이면 품질 축 검정력이 함께 오른다.
- **effort**: 小(편집) / 中(검증)
